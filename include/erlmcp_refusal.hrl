...
-define(REFUSAL_METADATA,
        [% Queue and Backpressure (1001-1005)
         {1001,
          ...
          <<"Reduce message rate or increase queue_limits.max_messages in config">>,
          error},
         {1002,
          ?HTTP_429_TOO_MANY_REQUESTS,
          <<"Byte capacity exceeded">>,
          <<"Message size too large or queue full; reduce message size or wait">>,
          error},
         {1003,
          ?HTTP_429_TOO_MANY_REQUESTS,
          <<"Tenant quota exceeded">>,
          <<"Tenant aggregate limit reached; contact administrator">>,
          critical},
         {1004,
          ?HTTP_503_SERVICE_UNAVAILABLE,
          <<"Buffer overflow">>,
          <<"Internal buffer exhausted; reduce load or increase buffer_pool size">>,
          critical},
         {1005,
          ?HTTP_503_SERVICE_UNAVAILABLE,
          <<"Backpressure active">>,
          <<"Server under load; retry after delay or reduce request rate">>,
          warn},
         % Authentication (1011-1016)
         {1011,
          ?HTTP_401_UNAUTHORIZED,
          <<"Authentication failed">>,
          <<"Provide valid credentials or check token format">>,
          error},
         {1012,
          ?HTTP_401_UNAUTHORIZED,
         ...
         {1014,
          ...
          <<"You don't have permission for this resource; contact administrator">>,
          error},
         {1015,
          ?HTTP_401_UNAUTHORIZED,
          <<"Missing authentication">>,
          <<"Provide credentials via Authorization header or session ID">>,
          error},
         {1016,
          ?HTTP_401_UNAUTHORIZED,
          <<"Invalid session ID">>,
          <<"Provide valid MCP-Session-Id header (32+ hex chars)">>,
          error},
         % Parameters & Validation (1021-1029)
         {1021,
          ?HTTP_400_BAD_REQUEST,
          <<"Invalid parameters">>,
          <<"Check parameter names and types match API specification">>,
          error},
         {1022,
          ?HTTP_400_BAD_REQUEST,
          <<"JSON schema validation failed">>,
          <<"Review error details; structure must match schema">>,
          error},
         {1023,
          ?HTTP_400_BAD_REQUEST,
          <<"Invalid URI format">>,
          <<"Use valid absolute or relative URI (e.g., https://example.com/path)">>,
          error},
         {1024,
          ?HTTP_415_UNSUPPORTED_MEDIA_TYPE,
          <<"Invalid Content-Type">>,
          <<"Use application/json, text/plain, or application/octet-stream">>,
          error},
         {1025,
          ?HTTP_400_BAD_REQUEST,
          <<"Required header missing or invalid">>,
          <<"Include MCP-Protocol-Version and other required headers">>,
          error},
         {1026,
          ?HTTP_400_BAD_REQUEST,
          <<"Session ID invalid">>,
          <<"Session ID must be 32+ hexadecimal characters">>,
          error},
         {1027,
          ?HTTP_400_BAD_REQUEST,
          <<"Protocol version not supported">>,
          <<"Use protocol version 2025-11-25 or 2024-11-05">>,
          error},
         {1028,
          ?HTTP_400_BAD_REQUEST,
          <<"Required field missing">>,
          <<"Check request includes all mandatory fields per API spec">>,
          error},
         {1029,
          ?HTTP_400_BAD_REQUEST,
          <<"Field type mismatch">>,
          <<"Field type doesn't match schema; check API documentation">>,
          error},
         % Path & URI Security (1036-1040)
         {1036,
          ?HTTP_400_BAD_REQUEST,
          <<"Path traversal detected">>,
          <<"Remove .. and % sequences; use canonical paths">>,
          critical},
         {1037,
          ?HTTP_400_BAD_REQUEST,
          <<"Invalid path format">>,
         ...
         {1038,
          ...
          <<"Symlinks not allowed; use direct file paths">>,
          critical},
         {1039,
          ?HTTP_400_BAD_REQUEST,
          <<"URI out of bounds">>,
          <<"URI must resolve within configured roots">>,
          error},
         {1040,
          ?HTTP_400_BAD_REQUEST,
          <<"Canonical path violation">>,
         ...
         {1046,
          ?HTTP_404_NOT_FOUND,
          <<"Resource not found">>,
          <<"Check resource URI and ensure it exists">>,
          warn},
         {1047,
          ?HTTP_409_CONFLICT,
          <<"Resource already exists">>,
          <<"Use different URI or update existing resource">>,
          warn},
         {1048,
          ?HTTP_404_NOT_FOUND,
          <<"Tool not found">>,
          <<"Check tool name in tools/list or register new tool">>,
          warn},
         {1049,
          ?HTTP_409_CONFLICT,
          <<"Tool already registered">>,
          <<"Tool exists; use different name or unregister first">>,
          warn},
         {1050,
          ?HTTP_404_NOT_FOUND,
          <<"Prompt not found">>,
          <<"Check prompt name in prompts/list or create new prompt">>,
          warn},
         {1051,
          ?HTTP_409_CONFLICT,
          <<"Prompt already exists">>,
          <<"Prompt exists; use different name or update existing">>,
          warn},
         {1052,
          ?HTTP_409_CONFLICT,
          <<"Entity already exists">>,
          <<"Use different identifier or update via appropriate endpoint">>,
          warn},
         % Rate Limiting (1056-1060)
         {1056,
          ?HTTP_429_TOO_MANY_REQUESTS,
          <<"Rate limit exceeded">>,
          <<"Slow down request rate or implement exponential backoff">>,
          error},
         {1057,
          ?HTTP_429_TOO_MANY_REQUESTS,
          <<"Per-second rate limit exceeded">>,
          <<"Max 100 requests/sec; wait before retrying">>,
          error},
         {1058,
          ?HTTP_429_TOO_MANY_REQUESTS,
          <<"Per-minute rate limit exceeded">>,
          <<"Max 5000 requests/min; wait or increase plan">>,
          error},
         {1059,
          ?HTTP_429_TOO_MANY_REQUESTS,
          <<"Quota exceeded">>,
          <<"Monthly quota used; upgrade plan or wait for reset">>,
          error},
         {1060,
          ?HTTP_429_TOO_MANY_REQUESTS,
          <<"Concurrent connection limit exceeded">>,
          <<"Max 100 concurrent connections; close unused connections">>,
          error},
         % Protocol & Transport (1066-1070)
         {1066,
          ?HTTP_400_BAD_REQUEST,
          <<"Protocol error">>,
          <<"Review message format and protocol compliance">>,
          error},
         {1067,
          ?HTTP_503_SERVICE_UNAVAILABLE,
          <<"Transport error">>,
          <<"Network error detected; check connection and retry">>,
          error},
         {1068,
          ?HTTP_413_PAYLOAD_TOO_LARGE,
          <<"Message too large">>,
          <<"Max 1MB per message; compress or split data">>,
          error},
         {1069,
          ?HTTP_503_SERVICE_UNAVAILABLE,
          <<"Operation timeout">>,
          <<"Increase timeout or reduce server load">>,
          warn},
         {1070,
          ?HTTP_415_UNSUPPORTED_MEDIA_TYPE,
          <<"Encoding not supported">>,
          <<"Use UTF-8 or other supported encoding">>,
          error},
         % Server State (1076-1080)
         {1076,
          ?HTTP_503_SERVICE_UNAVAILABLE,
          <<"Server not initialized">>,
          <<"Call initialize before other RPC methods">>,
          error},
         {1077,
          ?HTTP_503_SERVICE_UNAVAILABLE,
          <<"Server shutting down">>,
          <<"Server is closing; reconnect after restart">>,
          warn},
         {1078,
          ?HTTP_503_SERVICE_UNAVAILABLE,
          <<"Service unavailable">>,
          <<"Server temporarily unavailable; retry later">>,
          warn},
         {1079,
          ?HTTP_503_SERVICE_UNAVAILABLE,
          <<"Internal error">>,
          <<"Unexpected error; check server logs and contact support">>,
          critical},
         {1080,
          ?HTTP_503_SERVICE_UNAVAILABLE,
          <<"Dependency unavailable">>,
          <<"External service down; wait or contact support">>,
          warn},
         ...
         {1087,
          ?HTTP_503_SERVICE_UNAVAILABLE,
          <<"Health check failed">>,
          <<"Server unhealthy; may be degraded or restarting">>,
          warn},
         {1088,
          ?HTTP_503_SERVICE_UNAVAILABLE,
          <<"Service degraded">>,
          <<"Some features unavailable; functionality reduced">>,
          warn},
         {1089,
          ?HTTP_503_SERVICE_UNAVAILABLE,
          <<"Resource exhausted">>,
          <<"Server out of resources (memory, connections, etc.)">>,
          critical}]).
