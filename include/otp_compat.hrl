...
%%-----------------------------------------------------------------------------
-define(HAVE_PROCESS_NEXT, erlang:function_exported(erlang, process_next, 1)).
%%-----------------------------------------------------------------------------
...
-define(SET_PRIORITY_HIGH(), process_flag(priority, high)).
...
-define(SEND_PRIORITY(Pid, Msg), erlang:send(Pid, Msg, [nosuspend, {priority, high}])).

- else .

-define(SEND_PRIORITY(Pid, Msg), erlang:send(Pid, Msg, [nosuspend])).
...
-define(SEND_ALIAS(Alias, Msg), erlang:send(Alias, Msg, [nosuspend])).
...
%% Returns: <<"27">>, <<"28">>, etc.
%%-----------------------------------------------------------------------------
-define(OTP_VERSION(), list_to_binary(erlang:system_info(otp_release))).
%%-----------------------------------------------------------------------------
%% Check if OTP 28+
%%-----------------------------------------------------------------------------
...
%%-----------------------------------------------------------------------------
-define(IS_OTP_28_PLUS(), ?HAVE_PRIORITY_MESSAGES).
%%-----------------------------------------------------------------------------
%% Check if OTP 27+
%%-----------------------------------------------------------------------------
...
%% Runtime version check.
%%-----------------------------------------------------------------------------
-define(IS_OTP_27_PLUS(), ?HAVE_NATIVE_JSON).
...
%% Note: OTP 26 or earlier is not supported - minimum is OTP 27
