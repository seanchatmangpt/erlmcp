...
-export([basic_startup_test/1, basic_send_test/1, basic_close_test/1, get_state_test/1,
         transport_behavior_send/1, transport_behavior_close/1, transport_behavior_get_info/1,
         transport_behavior_handle_transport_call/1, registry_registration/1,
         registry_unregistration/1, registry_message_routing/1, registry_response_handling/1,
         config_validation/1, config_defaults/1, test_mode_startup/1, server_binding/1,
         error_handling_no_server/1, error_handling_routing_failure/1,
         error_handling_reader_crash/1, error_handling_invalid_messages/1, message_parsing/1,
         message_buffering/1, line_trimming/1, empty_line_handling/1, supervisor_integration/1,
         graceful_shutdown/1, abnormal_termination/1, high_volume_messages/1, memory_usage/1,
         latency_measurement/1, end_to_end_communication/1, concurrent_operations/1,
         stress_test/1]).    % Basic functionality
...
groups() ->
    [{basic_functionality,
      [parallel],
      [basic_startup_test, basic_send_test, basic_close_test, get_state_test]},
     {transport_behavior,
      [sequential],
     ...
     {configuration,
      [parallel],
      [config_validation, config_defaults, test_mode_startup, server_binding]},
     {error_handling,
      [sequential],
     ...
     {message_processing,
      [parallel],
      [message_parsing, message_buffering, line_trimming, empty_line_handling]},
     {lifecycle,
      [sequential],
      [supervisor_integration, graceful_shutdown, abnormal_termination]},
     {performance, [sequential], [high_volume_messages, memory_usage, latency_measurement]},
     {integration,
      [sequential],
      [end_to_end_communication, concurrent_operations, stress_test]}].
...
transport_behavior_send(Config) ->
    ...
    TestCases =
        [<<"binary data">>, "string data", [<<"list">>, " ", <<"of">>, " ", <<"iodata">>]],

    lists:foreach(fun(Data) -> ?assertEqual(ok, erlmcp_transport_stdio_new:send(State, Data))
                  end,
                  TestCases),
...
config_validation(Config) ->
    ...
    lists:foreach(fun(TestConfig) ->
                     {ok, Pid} =
                         erlmcp_transport_stdio_new:start_link(test_transport_config, TestConfig),
                     ?assert(is_process_alive(Pid)),
                     ok = gen_server:stop(Pid)
                  end,
                  ValidConfigs),
...
error_handling_invalid_messages(Config) ->
    ...
    lists:foreach(fun(Msg) -> gen_server:call(Pid, {simulate_input, Msg}) end,
                  InvalidMessages),
...
empty_line_handling(Config) ->
    ...
    lists:foreach(fun(Line) -> gen_server:call(Pid, {simulate_input, Line}) end, EmptyLines),
...
latency_measurement(Config) ->
    ...
    ct:pal("Latency stats - Avg: ~.2f Î¼s, Min: ~p Î¼s, Max: ~p Î¼s",
...
concurrent_operations(Config) ->
    ...
    Processes =
        [spawn(fun() ->
                  lists:foreach(fun(N) ->
                                   ...
                                   {reply, {ok, _}, _} =
                                       gen_server:call(Pid, {transport_call, get_buffer})
                                end,
                                lists:seq(1, OperationsPerProcess)),
                  Parent ! {done, self()}
               end)
    ...
    lists:foreach(fun(ProcessPid) ->
                     receive
                         {done, ProcessPid} -> ok
                     after 5000 -> ?assert(false, "Concurrent operation timed out")
                     end
                  end,
                  Processes),
...
stress_test(Config) ->
    ...
    Clients =
        [spawn(fun() ->
                  lists:foreach(fun(N) ->
                                   ...
                                   case N rem 3 of
                                       0 -> {ok, _} = gen_server:call(Pid, get_state);
                                       1 ->
                                           gen_server:call(Pid,
                                                           {simulate_input,
                                                            iolist_to_binary([<<"stress_">>,
                                                                              integer_to_list(N)])});
                                       2 ->
                                           {reply, {ok, _}, _} =
                                               gen_server:call(Pid, {transport_call, get_test_mode})
                                   end
                                end,
    ...
    lists:foreach(fun(ClientPid) ->
                     ...
                     after 10000 -> ?assert(false, "Stress test client timed out")
                     end
                  end,
                  Clients),
...
cleanup_processes() ->
    ...
    lists:foreach(fun(Name) ->
                  ...
                  [test_server_latency, test_server_e2e]).
