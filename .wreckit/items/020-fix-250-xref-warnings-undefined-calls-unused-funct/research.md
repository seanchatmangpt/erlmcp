# Research: Fix 250 Xref warnings (undefined calls + unused functions)

**Date**: 2026-01-29
**Item**: 020-fix-250-xref-warnings-undefined-calls-unused-funct
**Section**: quality-gates
**TCMS Compliance**: Lean Six Sigma Level Strictness

## Manufacturing Requirement
250 Xref warnings - undefined calls, unused functions, deprecated functions

**Motivation:** Xref warnings indicate dead code, potential bugs, and poor code hygiene. Must be clean for production.
...
- Fix undefined calls: implement functions or remove calls
- Fix unused functions: remove dead code or document public API exports
- Dependency: Must resolve missing modules first (Item 018)

**Signals:** priority: high, urgency: P2 - REQUIRED FOR PRODUCTION

### Quality Gate Status
- **Gate Type**: Xref (Cross-Reference Analysis)
- **Current State**: 250 warnings (222 undefined function calls, 26-28 unused local functions)
- **Target State**: 0 warnings
- **Gap**: 250 warnings (100% of warnings must be eliminated)

## Summary

This research investigates the Xref quality gate failure revealing **250 warnings** across the erlmcp v2.1.0 codebase. The breakdown is approximately **222 undefined function calls** (Item 018) and **26-28 unused local functions** (Item 021). This item (020) addresses the **complete cleanup** of both categories.
...
### Technical Approach
1. **Categorization Phase**: Separate warnings into two distinct categories:
...
- Phase 1: Resolve 222 undefined function calls (Item 018 dependency)
- Phase 2: Remove 26-28 unused local functions (Item 021)
...
- **Files**:
  - `/Users/sac/erlmcp/apps/erlmcp_core/src/` - 52 modules (client, server, registry, json_rpc, batch, hooks, etc.)
  - `/Users/sac/erlmcp/apps/erlmcp_transports/src/` - 22 modules (stdio, tcp, http, websocket, sse)
  - `/Users/sac/erlmcp/apps/erlmcp_observability/src/` - 26 modules (metrics, traces, receipts, debugger)
  - `/Users/sac/erlmcp/apps/tcps_erlmcp/src/` - 68 modules (andon, heijunka, kanban, quality_gates, work_order, persistence)
  - **Total**: 168 modules implemented
...
**Core calling code (with undefined function calls - from Item 018):**
...
**Modules with unused local functions (from Item 021):**
- `apps/erlmcp_transports/src/erlmcp_security_headers.erl` - `add_to_response/1` unused
- `apps/erlmcp_core/src/erlmcp_pagination.erl` - `ensure_valid_item/1` unused
...
- `rebar.config:160-195` - Xref ignores list (30+ external library functions)
...
**Internal Modules**:
- `erlmcp_core` → 52 modules (client, server, registry, json_rpc, batch, hooks, pricing, etc.)
...
2. **Why undefined calls?** Three root causes identified in Item 018:
...
3. **Why unused functions?** Two root causes identified in Item 021:
...
## Dependency on Item 018

**Critical Dependency**: Item 020 depends on Item 018 (implement or remove 15+ missing modules).

**Reason**: 222 of 250 Xref warnings are undefined function calls to missing modules. Item 018 categorizes all missing modules and provides decision matrix (IMPLEMENT vs REMOVE).

**Execution Order**:
1. **Complete Item 018 first**: Implement missing functions OR remove orphaned calls
2. **Then execute Item 020**: Address remaining unused local functions
3. **Final verification**: `rebar3 xref` shows 0 warnings

**Overlap with Item 021**:
- Item 018: Fix undefined function calls (222 warnings)
- Item 021: Remove unused local functions (28 warnings)
- Item 020: **Combined cleanup** of both categories (250 warnings)

**Recommended Approach**:
- Execute Item 018 and Item 021 in parallel (different categories, no conflicts)
- Execute Item 020 as final verification step (ensure 0 warnings)
- All three items can be combined into single quality gate fix

## Estimated Effort

**Phase 1: Categorization (Item 018 + Item 021)**: 2 hours
...
**Phase 4: Final Verification (Item 020)**: 2 hours
...
**Total Estimated Time**: 16 hours (2 days) across all three items
...
3. Complete Item 020 (final verification) - 2 hours
