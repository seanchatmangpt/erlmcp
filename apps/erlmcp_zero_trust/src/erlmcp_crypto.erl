...
decrypt(CipherText, Key) when is_binary(CipherText) ->
    ...
    PlainText = crypto:block_decrypt(aes_cbc256, Key, IV, CipherTextRest),
    PlainText.
...
sign(Data, PrivateKey) ->
    crypto:sign(rsa_sha256, Data, PrivateKey, []).

verify_signature(Data, Signature, PublicKey) ->
    crypto:verify(rsa_sha256, Data, Signature, PublicKey, []).
...
verify_hmac(Data, HMAC, Key) ->
    crypto:hmac(sha256, Key, Data) == HMAC.