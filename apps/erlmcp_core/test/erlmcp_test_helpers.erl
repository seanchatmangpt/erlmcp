...
-type server_config() ::
    #{capabilities => #mcp_server_capabilities{},
      ...
      timeout => timeout()}.
-type client_config() ::
    #{transport => {stdio, list()} | {tcp, map()} | {http, map()}, timeout => timeout()}.
-type cache_config() ::
...
%% @end
%%--------------------------------------------------------------------
-spec with_test_server(erlmcp_registry:server_id(),
...
%% @end
%%--------------------------------------------------------------------
-spec with_test_client(client_config(), fun((pid()) -> test_result())) -> test_result().
