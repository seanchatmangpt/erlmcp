...
all_workloads() ->
    [
        ...
        #{id => <<"websocket_100_1kb">>, category => websocket, messages => 100, size => 1024},
        #{id => <<"websocket_1k_1kb">>, category => websocket, messages => 1000, size => 1024},
        #{id => <<"websocket_1k_10kb">>, category => websocket, messages => 1000, size => 10240},
        #{id => <<"websocket_10k_1kb">>, category => websocket, messages => 10000, size => 1024},
        #{id => <<"websocket_10k_10kb">>, category => websocket, messages => 10000, size => 10240},
        #{id => <<"websocket_1k_100kb">>, category => websocket, messages => 1000, size => 102400},

        %% SSE Streaming Benchmarks
        #{id => <<"sse_streaming_100_1kb">>, category => sse, events => 100, size => 1024},
        #{id => <<"sse_streaming_1k_1kb">>, category => sse, events => 1000, size => 1024},
        #{id => <<"sse_streaming_1k_10kb">>, category => sse, events => 1000, size => 10240},
        #{id => <<"sse_streaming_10k_1kb">>, category => sse, events => 10000, size => 1024},
        #{id => <<"sse_streaming_10k_10kb">>, category => sse, events => 10000, size => 10240},

        %% TLS Overhead Benchmarks
        #{id => <<"tls_handshake_100">>, category => tls, connections => 100, with_tls => true},
        #{id => <<"tls_vs_plain_1k_msg">>, category => tls, messages => 1000, size => 1024, compare => true},
        #{id => <<"tls_pooling_100_conn">>, category => tls, pool_size => 100, reuse => 10}
    ].
...
run_all() ->
    ...
    lists:foreach(fun(Workload) ->
        ...
        try
            ...
            Class:Reason:Stack ->
                io:format("ERROR in ~s: ~p:~p~n~p~n", [WorkloadId, Class, Reason, Stack])
        end,
...
run_workload(#{id := WorkloadId, category := Category} = Workload) ->
    ...
    Result = case Category of
        websocket -> benchmark_websocket(Workload);
        sse -> benchmark_sse(Workload);
        tls -> benchmark_tls(Workload)
    end,
...
benchmark_websocket(#{id := WorkloadId, messages := NumMessages, size := PayloadSize}) ->
...
benchmark_sse(#{id := WorkloadId, events := NumEvents, size := EventSize}) ->
...
benchmark_tls(#{id := WorkloadId, connections := NumConnections, with_tls := true}) ->
    ...
    HandshakeTimes = lists:map(fun(_) ->
        StartTime = erlang:monotonic_time(microsecond),
...
benchmark_tls(#{id := WorkloadId, messages := NumMessages, size := PayloadSize, compare := true}) ->
    %% TLS vs plain TCP throughput comparison
    io:format("  [TLS] Throughput comparison: ~p messages x ~p bytes~n", [NumMessages, PayloadSize]),
...
benchmark_tls(#{id := WorkloadId, pool_size := PoolSize, reuse := ReuseCount}) ->
...
benchmark_tcp_throughput(NumMessages, PayloadSize, UseTls) ->
    %% Start server
    {ok, ServerPid} = if
        UseTls -> start_tls_server();
        true -> start_plain_tcp_server()
    end,
    timer:sleep(?SERVER_START_DELAY),
    ...
    %% Connect client
    {ok, Socket} = if
        UseTls -> connect_tls_client();
        true -> connect_plain_tcp_client()
    end,

    %% Send messages
    StartTime = erlang:monotonic_time(microsecond),
    lists:foreach(fun(_) ->
        send_tcp(Socket, Payload, UseTls)
    end, lists:seq(1, NumMessages)),
    EndTime = erlang:monotonic_time(microsecond),
    ...
    if
        UseTls -> stop_tls_server(ServerPid);
        true -> stop_plain_tcp_server(ServerPid)
    end,
...
connect_plain_tcp_client() ->
    gen_tcp:connect("localhost", ?TCP_PORT, [binary, {packet, line}, {active, false}], ?CONNECT_TIMEOUT).
