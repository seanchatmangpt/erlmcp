...
test_cancel_completed() ->
    ...
    OperationPid = spawn(fun() -> receive after 100 -> ok end end),
    Token = erlmcp_cancellation:register(self(), OperationPid, <<"tools/call">>),
...
test_multiple_cancellations() ->
    ...
    Ops = [spawn(fun() -> receive after 5000 -> ok end end) || _ <- lists:seq(1, 5)],
...
test_cancel_during_tool() ->
    ...
    spawn(fun() ->
        erlmcp_server:call_tool(Server, ToolName, #{})
    end),
...
resource_cancellation_test_() ->
    {setup,
     ...
     fun(_) ->
         [
          {"Cancel during resource read", fun test_cancel_resource_read/0}
         ]
     end}.
...
test_cancel_resource_read() ->
    ...
    %% Start resource read
    spawn(fun() ->
        erlmcp_server:read_resource(Server, Uri)
    end),
...
progress_cancellation_test_() ->
    {setup,
     ...
     fun(_) ->
         [
          {"Cancel operation with progress", fun test_cancel_with_progress/0}
         ]
     end}.
...
test_cancel_with_progress() ->
    ...
    Handler = fun(_Args) ->
        OperationPid = spawn(fun() ->
            Parent ! {started, self()},
            lists:foreach(fun(N) ->
                erlmcp_server:report_progress(Server, <<"token1">>, float(N), 10.0),
                timer:sleep(100)
            end, lists:seq(1, 10)),
            #{result => <<"done">>}
        end),
