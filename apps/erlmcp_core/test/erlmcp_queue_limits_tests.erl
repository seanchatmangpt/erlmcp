...
test_statistics_collection(_State) ->
    {"Collect and report statistics",
     fun() ->
         TestPid = spawn(fun() -> timer:sleep(infinity) end),

         %% Generate some activity
         erlmcp_queue_limits:record_enqueue(task_worker, TestPid),
         erlmcp_queue_limits:record_enqueue(task_worker, TestPid),
         erlmcp_queue_limits:record_dequeue(task_worker, TestPid),

         timer:sleep(10),

         %% Get stats
         Stats = erlmcp_queue_limits:get_role_stats(task_worker),
         ?assertMatch(#{role := task_worker}, Stats),
         ?assertMatch(#{total_enqueued := E} when E >= 2, Stats),
         ?assertMatch(#{total_dequeued := D} when D >= 1, Stats),

         %% Reset stats
         ok = erlmcp_queue_limits:reset_stats(task_worker),
         NewStats = erlmcp_queue_limits:get_role_stats(task_worker),
         ?assertMatch(#{total_enqueued := 0}, NewStats),
...
test_backpressure_shedding(_State) ->
    {"Return 429 when capacity reached",
     fun() ->
         TestPid = spawn(fun() -> timer:sleep(infinity) end),

         %% Set very low limit
         ok = erlmcp_queue_limits:set_limit(shed_test, 2),

         %% Fill to capacity
         ok = erlmcp_queue_limits:check_capacity(shed_test, TestPid),
         erlmcp_queue_limits:record_enqueue(shed_test, TestPid),
         ok = erlmcp_queue_limits:check_capacity(shed_test, TestPid),
         erlmcp_queue_limits:record_enqueue(shed_test, TestPid),
         ...
         Result = erlmcp_queue_limits:check_capacity(shed_test, TestPid),
         ?assertMatch({error, {capacity_exceeded, #{total_rejected := R}}} when R > 0, Result),

         %% Verify rejection counted in stats
         Stats = erlmcp_queue_limits:get_role_stats(shed_test),
         ?assertMatch(#{capacity_exceeded_count := C} when C > 0, Stats),
