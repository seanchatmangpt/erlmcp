...
echo_executor(Requests) ->
    [{ok, #{request_id => ReqId, method => Method, params => Params}} ||
...
failing_executor(N) ->
    fun(Requests) ->
        lists:map(fun({Idx, Req}) ->
            case Idx rem N of
                0 -> {error, simulated_failure};
                _ -> {ok, Req}
            end
        end, lists:zip(lists:seq(1, length(Requests)), Requests))
    end.
...
test_all_failures() ->
    ...
    Executor = fun(_Requests) ->
        [{error, always_fails} || _ <- _Requests]
    end,
