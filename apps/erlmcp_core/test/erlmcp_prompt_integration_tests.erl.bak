...
test_complete_template_workflow() ->
    ...
    Template =
        <<"Hello {{name}}, you have {{count}} new messages.{{#unread}} ({{unread}} unread){{/unread}}">>,
    Variables =
        #{<<"name">> => <<"Alice">>,
          <<"count">> => 5,
          <<"unread">> => 2},
...
test_template_with_validation_pipeline() ->
    ...
    ?assertMatch({error, {invalid_template_syntax, _}},
                 erlmcp_prompt_template:validate(InvalidTemplate)),
...
test_complex_message_template_workflow() ->
    ...
    Variables =
        #{<<"from_name">> => <<"Bob Smith">>,
          ...
          <<"message">> =>
              <<"The project is progressing nicely. We have completed the first phase.">>,
          <<"attachments">> =>
...
test_api_response_template() ->
    ...
    %% Success case
    SuccessVars = #{<<"success">> => true, <<"message">> => <<"Resource created">>},
...
test_user_notification_template() ->
    ...
    %% Without updates
    NoUpdates = #{<<"username">> => <<"Bob">>, <<"updates">> => []},
