...
worker_execute(State, OpsPerSecond, ParentPid) ->
    ...
    after 0 ->
        ...
        case SleepMicros > 0 of
            true -> timer:sleep(SleepMicros div 1000);
            false -> ok
        end,
        ...
        false ->
            ...
            case check_termination_conditions(Sample) of
                ...
                continue ->
                    ...
                    NewState = State#stress_state{
                        samples = [Sample | State#stress_state.samples],
...
collect_sample(State, WorkerPids, ElapsedMs) ->
    ...
    Throughput = case TimeSec > 0 of true -> TotalOps / TimeSec; false -> 0.0 end,

    %% Calculate latency percentiles from recent samples
    RecentLatencies = lists:sublist(State#stress_state.latencies, 1000),
    P99 =
        case RecentLatencies of
            [] -> 0;
            _ -> percentile(RecentLatencies, 99)
        end,
...
collect_worker_stats(Pid) ->
    ...
    receive
        {worker_stats, OpsCompleted} -> OpsCompleted
    after 100 -> 0
    end.
...
collect_latencies(Acc) ->
    receive
        {latency, Latency} -> collect_latencies([Latency | Acc])
    after 0 -> Acc
    end.
...
check_termination_conditions(Sample) ->
    ...
    case MemoryMib > MaxMemoryMib of
        true -> throw({terminate, memory_limit});
        false -> ok
    end,

    %% CPU limit: 95% sustained
    case CpuPercent > 95.0 of
        true -> {terminate, cpu_limit};
        false -> continue
    end.
...
get_cpu_usage_percent() ->
    ...
    case erlang:statistics(scheduler_wall_time) of
        ...
        SchedulerTimes ->
            ...
            case TotalWall > 0 of
                true -> (TotalActive / TotalWall) * 100;
                false -> 0.0
            end
    end.
