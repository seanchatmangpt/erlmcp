...
test_health_check_priority(_State) ->
    {"Health check messages handled with priority",
     fun() ->
         %% Register handler
         Handler = fun({control, health_check, Data}) ->
...
test_drain_session_priority(_State) ->
    {"Drain session messages preempt data traffic",
     fun() ->
         %% Register handler
         Handler = fun({control, drain_session, Data}) ->
...
test_cancel_task_priority(_State) ->
    {"Cancel task messages bypass queue depth",
     fun() ->
         %% Register handler
         Handler = fun({control, cancel_task, Data}) ->
...
test_circuit_breaker_priority(_State) ->
    {"Circuit breaker trips are immediate",
     fun() ->
         %% Register handler
         Handler = fun({control, circuit_breaker, Data}) ->
...
test_latency_tracking(_State) ->
    {"Latency tracking for priority messages",
     fun() ->
         ...
         erlmcp_control_plane:register_component(latency_test, Handler),

         %% Send multiple health checks
         lists:foreach(
             fun(_) ->
                 {ok, _LatencyUs} = erlmcp_control_plane:send_health_check(latency_test, liveness)
             end,
             lists:seq(1, 10)
         ),
         ...
         Stats = erlmcp_control_plane:get_stats(),
         ?assertMatch(#{latency_p50_us := P50} when P50 > 0, Stats),
         ?assertMatch(#{latency_p95_us := P95} when P95 > 0, Stats),
         ?assertMatch(#{latency_p99_us := P99} when P99 > 0, Stats),
         ?assertMatch(#{max_latency_us := Max} when Max > 0, Stats)
     end}.

test_slo_violations(_State) ->
    {"SLO violations tracked for health checks >100ms",
     fun() ->
         %% Register handler with excessive delay
         Handler = fun({control, health_check, _Data}) ->
             timer:sleep(150),  % Violate 100ms SLO
             ok
         end,
         erlmcp_control_plane:register_component(slo_test, Handler),

         %% Send health check that violates SLO
         {ok, LatencyUs} = erlmcp_control_plane:send_health_check(slo_test, liveness),
         ?assert(LatencyUs > 100000, "Should exceed 100ms SLO"),

         %% Verify SLO violation recorded
         Stats = erlmcp_control_plane:get_stats(),
         ?assertMatch(#{slo_violations := V} when V > 0, Stats)
     end}.
