...
prop_transport_registration_idempotent() ->
    ?FORALL({TransportId, _Config}, {transport_id(), transport_config()},
        begin
            ...
            %% Register twice
            Result1 = erlmcp_registry:register_transport(TransportIdBin, self(), TransportConfig),
            Result2 = erlmcp_registry:register_transport(TransportIdBin, self(), TransportConfig),
...
prop_transport_lookup_consistency() ->
    ?FORALL(TransportId, transport_id(),
        begin
            %% Ensure registry is running
            {ok, _RegPid} = ensure_registry(),
...
prop_transport_death_auto_unregisters() ->
    ?FORALL(TransportId, transport_id(),
        begin
            ...
            TransportPid = spawn(fun() -> receive after 5000 -> ok end end),

            %% Register transport
            ok = erlmcp_registry:register_transport(TransportIdBin, TransportPid, TransportConfig),
...
prop_list_transports_consistency() ->
    ?FORALL(Count, proper_types:range(1, 10),
        begin
            ...
            TransportIds = [list_to_binary("transport_" ++ integer_to_list(N)) || N <- lists:seq(1, Count)],
            TransportConfig = #{type => stdio},
