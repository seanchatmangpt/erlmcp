...
test_multiple_operations() ->
    ...
    OpPids = [spawn(fun() -> receive after 5000 -> ok end end) || _ <- lists:seq(1, 5)],
...
test_register_generates_unique_tokens() ->
    ...
    Tokens = [begin
        Pid = spawn(fun() -> receive after 5000 -> ok end end),
...
%% @doc Test cancelling non-existent operation returns ok (cast)
test_cancel_nonexistent_returns_error() ->
    %% Cancel non-existent token (cast always returns ok)
    FakeToken = make_ref(),
    ?assertEqual(ok, erlmcp_cancellation:cancel(FakeToken)),
...
test_list_operations() ->
    ...
    OpPids = [spawn(fun() -> receive after 5000 -> ok end end) || _ <- lists:seq(1, 3)],
    _Tokens = [erlmcp_cancellation:register(ClientPid, Pid, <<"test_op">>) || Pid <- OpPids],

    %% List operations
    Operations = erlmcp_cancellation:list_operations(),

    %% Verify count (at least our 3 operations)
    ?assert(length(Operations) >= 3),

    %% Verify structure for our operations (they'll be first or among the list)
...
test_set_cleanup_handler() ->
    ?assertEqual(ok, erlmcp_cancellation:set_cleanup_handler(<<"tools/call">>, erlmcp_test_cleanup_handler)).

%% @doc Test cleanup handler is called on cancellation
test_cleanup_handler_called_on_cancel() ->
    %% Start the cleanup handler server
    {ok, _HandlerPid} = erlmcp_test_cleanup_handler:start_link(),

    %% Create a tracker process to receive cleanup notifications
    TrackerPid = spawn(fun() ->
        receive
            {cleanup, Token, Reason} ->
                %% Send to parent for verification
                self() ! {cleanup_verified, Token, Reason}
        end
    end),

    %% Set the tracker (use self() as the parent that will receive verification)
    ParentPid = self(),
    RealTracker = spawn(fun() ->
        TrackerPid,
        receive
            {cleanup, Token, Reason} ->
                ParentPid ! {cleanup_verified, Token, Reason}
        end
    end),

    erlmcp_test_cleanup_handler:set_tracker(RealTracker),
    ...
    receive
        {cleanup_verified, Token, _Reason} ->
            ?assert(true)
    after 500 ->
        ?assert(false, "Cleanup handler was not called")
    end,

    %% Cleanup
    gen_server:stop(erlmcp_test_cleanup_handler),
    exit(OperationPid, kill),
    exit(RealTracker, kill).
...
test_cleanup_handler_failure_handled() ->
    ...
    erlmcp_cancellation:set_cleanup_handler(<<"failing_op">>, erlmcp_test_failing_cleanup_handler),
...
test_concurrent_registrations() ->
    ...
    PidsRefs = lists:map(fun(_) ->
        Parent = self(),
        spawn(fun() ->
            OpPid = spawn(fun() -> receive after 5000 -> ok end end),
