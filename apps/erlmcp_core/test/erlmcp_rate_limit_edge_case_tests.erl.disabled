...
test_consecutive_bursts() ->
    ...
    % First burst
    _ = [erlmcp_rate_limiter:check_message_rate(ClientId, TimeNowMs)
         || _ <- lists:seq(1, 10)],
    ...
    Results = [erlmcp_rate_limiter:check_message_rate(ClientId, TimeAfterMs)
               || _ <- lists:seq(1, 5)],
...
test_priority_switching() ->
    ...
    % Switch to normal priority - should be limited
    Results = [erlmcp_rate_limiter:check_message_rate(ClientId, TimeNowMs, normal)
               || _ <- lists:seq(1, 15)],
...
test_retry_after_rounding() ->
    ClientId = rounding_client1,
    TimeNowMs = erlang:system_time(millisecond),
    ...
    ?assertMatch({error, rate_limited, RetryAfter} when RetryAfter > 0, Result),
...
test_global_limit_precision() ->
    ...
    Results = [erlmcp_rate_limiter:check_global_rate(TimeNowMs)
               || _ <- lists:seq(1, 100)],
...
test_global_limit_retry_after() ->
    ...
    _ = [erlmcp_rate_limiter:check_global_rate(TimeNowMs)
         || _ <- lists:seq(1, 100)],

    % Should get rate limited with proper retry-after
    Result = erlmcp_rate_limiter:check_global_rate(TimeNowMs),
    ?assertMatch({error, global_rate_limited, RetryAfter} when RetryAfter > 0, Result),
    {error, global_rate_limited, RetryAfter} = Result,
    ?assert(RetryAfter > 0),
    ?assert(is_integer(RetryAfter)).
...
test_global_multiple_clients() ->
    ...
    Client1Results = [erlmcp_rate_limiter:check_message_rate(client_global1, TimeNowMs)
                      || _ <- lists:seq(1, 50)],
    Client2Results = [erlmcp_rate_limiter:check_message_rate(client_global2, TimeNowMs)
                      || _ <- lists:seq(1, 30)],
...
test_time_skew_handling() ->
    ...
    Results = [erlmcp_rate_limiter:check_message_rate(ClientId, BaseTime - N * 10)
               || N <- lists:seq(0, 9)],

    % Should handle gracefully (may all succeed due to time skew)
    ?assert(lists:all(fun({ok, _}) -> true; ({error, _}) -> true end, Results)).
