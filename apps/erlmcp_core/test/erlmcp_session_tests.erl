...
concurrent_access_test_() ->
    {setup,
     ...
     fun cleanup_session_manager/1,
     fun(_) -> [
         ?_test(test_concurrent_session_creation()),
         ?_test(test_concurrent_session_updates())
     ] end}.
...
test_concurrent_session_creation() ->
    ...
    SessionIds = [receive {created, Id} -> Id end || _ <- Pids],
    ...
    lists:foreach(fun(Id) ->
        ?assertMatch({ok, _}, erlmcp_session:retrieve(Id))
    end, SessionIds).
...
test_concurrent_session_updates() ->
    ...
    _ = [receive updated -> ok end || _ <- Pids],
...
session_metadata_test_() ->
    {setup,
     ...
     fun cleanup_session_manager/1,
     fun(_) -> [
         ?_test(test_metadata_persistence()),
