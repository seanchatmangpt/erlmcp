...
test_verify_tampered_header() ->
    ...
    TamperedHeader = base64:encode(jsx:encode(#{<<"alg">> => <<"HS256">>})),
...
test_verify_tampered_payload() ->
    ...
    TamperedPayload = base64:encode(jsx:encode(#{<<"sub">> => <<"hacker">>})),
...
test_verify_signature_algorithm_mismatch() ->
    ...
    [_Header, Payload, Signature] = binary:split(Token, <<".">>, [global]),
    FakeHeader = base64:encode(jsx:encode(Header)),
...
test_key_multiple_concurrent() ->
    %% Add multiple keys
    Keys = [{<<"key-1">>, generate_rsa_keypair()},
            {<<"key-2">>, generate_rsa_keypair()},
            {<<"key-3">>, generate_rsa_keypair()}],

    lists:foreach(fun({Kid, {PubKey, _PrivKey}}) ->
        ok = erlmcp_auth:rotate_public_key(Kid, PubKey)
    end, Keys),
...
test_revoke_multiple_tokens() ->
    ...
    Tokens = [create_jwt(
        #{<<"sub">> => <<"user", (integer_to_binary(I))/binary>>,
          <<"exp">> => erlang:system_time(second) + 3600},
        PrivKey,
        <<"test-key-1">>
    ) || I <- lists:seq(1, 5)],
...
test_error_deeply_nested_claims() ->
    ...
    DeepStruct = lists:foldl(fun(I, Acc) ->
        #{integer_to_binary(I) => Acc}
    end, #{<<"value">> => <<"deep">>}, lists:seq(1, 50)),
...
test_integration_full_auth_flow() ->
    ...
    %% Create session
    {ok, SessionId} = erlmcp_auth:create_session(<<"user123">>, #{
        auth_method => jwt,
        token => Token
    }),
...
test_integration_multi_user_scenario() ->
    ...
    Tokens = [create_jwt(
        #{<<"sub">> => User, <<"exp">> => erlang:system_time(second) + 3600},
        PrivKey,
        <<"test-key-1">>
    ) || User <- Users],
