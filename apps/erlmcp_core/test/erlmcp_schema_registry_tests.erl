...
test_schema_registration() ->
    ...
    % Register schema
    ?assertEqual(ok, erlmcp_schema_registry:register(
        <<"user">>, {1, 0, 0}, tool, Schema)),
...
test_schema_retrieval() ->
    ...
    ?assertEqual({error, not_found},
                 erlmcp_schema_registry:get(<<"nonexistent">>, {1, 0, 0})).
...
test_schema_versioning() ->
    ...
    V110Schema = maps:put(<<"properties">>,
        maps:merge(maps:get(<<"properties">>, BaseSchema),
                   #{<<"email">> => #{<<"type">> => <<"string">>}}),
        BaseSchema),
...
test_schema_validation() ->
    ...
    ?assertEqual(ok, erlmcp_schema_registry:validate(
        <<"user_validation">>, {1, 0, 0}, ValidData)),

    % Missing required field
    InvalidData1 = #{<<"age">> => 30},
    Result1 = erlmcp_schema_registry:validate(
        <<"user_validation">>, {1, 0, 0}, InvalidData1),
    ?assertMatch({error, _}, Result1),

    % Invalid type
    InvalidData2 = #{<<"name">> => <<"Jane">>, <<"age">> => <<"thirty">>},
    Result2 = erlmcp_schema_registry:validate(
        <<"user_validation">>, {1, 0, 0}, InvalidData2),
    ?assertMatch({error, _}, Result2),

    % Out of range
    InvalidData3 = #{<<"name">> => <<"Bob">>, <<"age">> => 150},
    Result3 = erlmcp_schema_registry:validate(
        <<"user_validation">>, {1, 0, 0}, InvalidData3),
...
test_compatibility_checking() ->
    ...
    V2Schema = maps:put(<<"properties">>,
        maps:merge(maps:get(<<"properties">>, V1Schema),
                   #{<<"email">> => #{<<"type">> => <<"string">>}}),
        V1Schema),
...
test_schema_deletion() ->
    ...
    % Verify it's gone
    ?assertEqual({error, not_found},
                 erlmcp_schema_registry:get(<<"deletable">>, {1, 0, 0})),

    % Delete non-existent schema
    ?assertEqual({error, not_found},
                 erlmcp_schema_registry:delete(<<"nonexistent">>, {1, 0, 0})).
