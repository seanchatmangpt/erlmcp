...
test_subscribe_multiple_same_id() ->
    ...
    Subscriber1 = self(),
    Subscriber2 = spawn(fun() -> receive after 5000 -> ok end end),
    Subscriber3 = spawn(fun() -> receive after 5000 -> ok end end),
...
test_subscribe_multiple_ids() ->
    ...
    lists:foreach(fun(Id) ->
        ?assertEqual(ok, erlmcp_subscription:subscribe(Id, Subscriber))
    end, Ids),
...
test_list_subscribers_with_data() ->
    ...
    Subs = [spawn(fun() -> receive after 5000 -> ok end end) || _ <- lists:seq(1, 5)],

    lists:foreach(fun(Sub) ->
        erlmcp_subscription:subscribe(SubscriptionId, Sub)
    end, Subs),
...
test_get_subscription_count() ->
    ...
    Id1 = <<"count1">>,
    Id2 = <<"count2">>,
    Sub1 = spawn(fun() -> receive after 5000 -> ok end end),
    Sub2 = spawn(fun() -> receive after 5000 -> ok end end),
...
test_get_subscriber_count() ->
    ...
    Subs = [spawn(fun() -> receive after 5000 -> ok end end) || _ <- lists:seq(1, 7)],
...
test_notify_multiple() ->
    ...
    %% Verify all received
    ReceivedCount = lists:foldl(fun(_, Acc) ->
        receive
            {_, received, _} -> Acc + 1
        after 1000 -> Acc
        end
...
test_subscriber_death() ->
    ...
    %% Create subscriber
    Subscriber = spawn(fun() -> receive after 5000 -> ok end end),
...
test_multi_sub_death() ->
    ...
    lists:foreach(fun(Id) ->
        erlmcp_subscription:subscribe(Id, Subscriber)
    end, Ids),

    %% Verify all subscriptions exist
    lists:foreach(fun(Id) ->
        ?assertEqual(1, erlmcp_subscription:get_subscriber_count(Id))
    end, Ids),
    ...
    lists:foreach(fun(Id) ->
        ?assertEqual(0, erlmcp_subscription:get_subscriber_count(Id))
    end, Ids).
...
test_concurrent_notify() ->
    ...
    [spawn(fun() ->
        erlmcp_subscription:notify(SubscriptionId, #{<<"seq">> => N})
    end) || N <- lists:seq(1, 10)],

    %% Wait and count responses
    timer:sleep(500),
    ReceivedCount = length([ok || _ <- lists:seq(1, 20), receive {_, got_it} -> ok after 0 -> timeout end =/= timeout]),
