...
connection_initialization_test_() ->
    {setup, fun setup_application/0, fun cleanup_application/1, fun(_) -> [
...
test_stdio_connection() ->
    case erlmcp_client:start_link({stdio, #{test_mode => true}}) of
        {ok, Client} ->
            ...
            erlmcp_client:stop(Client), timer:sleep(50), ?assertNot(erlang:is_process_alive(Client));
        {error, Reason} -> logger:info("Stdio not available: ~p", [Reason]), ?assert(true)
    end.
...
test_stdio_connection_with_opts() ->
    ...
    case erlmcp_client:start_link({stdio, #{test_mode => true}}, Opts) of
        {ok, Client} -> ?assert(is_pid(Client)), erlmcp_client:stop(Client);
        {error, _} -> ?assert(true)
    end.
...
capability_negotiation_test_() ->
    {setup, fun setup_client/0, fun cleanup_client/1, fun(Client) -> case Client of
        undefined -> []; _ -> [
            {"Capability negotiation success", ?_test(test_capability_negotiation_success(Client))},
            {"Capability encoding formats", ?_test(test_capability_encoding_formats(Client))},
            {"Server capability extraction", ?_test(test_server_capability_extraction(Client))}
        ] end end}.

test_capability_negotiation_success(Client) ->
    Caps = #mcp_client_capabilities{roots = #mcp_capability{enabled = true}},
    try ?assertMatch({error, _}, erlmcp_client:initialize(Client, Caps)) catch error:badarg -> ok end.
...
test_server_capability_extraction(_) ->
    ServerResp = #{<<"protocolVersion">> => ?MCP_VERSION, <<"capabilities">> => #{resources => #{}}},
...
protocol_phases_test_() ->
    {setup, fun setup_client/0, fun cleanup_client/1, fun(Client) -> case Client of
        undefined -> []; _ -> [
            ...
            {"Initialization phase transition", ?_test(test_initialization_phase_transition(Client))},
            {"Initialized phase enforcement", ?_test(test_initialized_phase_enforcement(Client))},
            {"Error phase handling", ?_test(test_error_phase_handling(Client))},
            {"Phase transition notifications", ?_test(test_phase_transition_notifications(Client))}
        ] end end}.
...
test_initialization_phase_transition(Client) ->
    case erlmcp_client:initialize(Client, #mcp_client_capabilities{}) of
        {error, Reason} -> logger:info("Init failed: ~p", [Reason]); {ok, _} -> ok end.

test_initialized_phase_enforcement(Client) ->
    ?assertMatch({error, {not_initialized, pre_initialization, _}}, erlmcp_client:list_tools(Client)),
    case erlmcp_client:initialize(Client, #mcp_client_capabilities{}) of {error, _} -> ok; {ok, _} -> ok end.

test_error_phase_handling(Client) ->
    case erlmcp_client:initialize(Client, #mcp_client_capabilities{}) of
        {error, _} -> ?assertMatch({error, {not_initialized, pre_initialization, _}}, erlmcp_client:list_tools(Client)); _ -> ok
    end.
...
test_client_start_stop() ->
    case erlmcp_client:start_link({stdio, #{test_mode => true}}) of
        {ok, Client} ->
            ?assert(erlang:is_process_alive(Client)), ?assertMatch(ok, erlmcp_client:stop(Client)),
            timer:sleep(100), ?assertNot(erlang:is_process_alive(Client));
        {error, _} -> ?assert(true)
    end.
...
test_client_restart() ->
    case erlmcp_client:start_link({stdio, #{test_mode => true}}) of
        {ok, Client1} ->
            ok = erlmcp_client:stop(Client1), timer:sleep(50),
            case erlmcp_client:start_link({stdio, #{test_mode => true}}) of
                {ok, Client2} -> ?assert(Client1 =/= Client2), ok = erlmcp_client:stop(Client2); {error, _} -> ok
            end;
        {error, _} -> ?assert(true)
    end.
...
test_client_multiple_instances() ->
    case {erlmcp_client:start_link({stdio, #{test_mode => true, i => 1}}),
          erlmcp_client:start_link({stdio, #{test_mode => true, i => 2}})} of
        {{ok, C1}, {ok, C2}} -> ?assert(C1 =/= C2), ok = erlmcp_client:stop(C1), ok = erlmcp_client:stop(C2);
        {{ok, C1}, _} -> ok = erlmcp_client:stop(C1); {error, _} -> ?assert(true)
    end.

test_client_shutdown_timeout() ->
    case erlmcp_client:start_link({stdio, #{test_mode => true}}) of
        {ok, Client} ->
            spawn_link(fun() -> timer:sleep(5000), erlmcp_client:list_resources(Client) end),
...
test_client_state_cleanup() ->
    case erlmcp_client:start_link({stdio, #{test_mode => true}}) of
        {ok, Client} ->
            ...
            ok = erlmcp_client:stop(Client), timer:sleep(100), ?assertNot(erlang:is_process_alive(Client));
        {error, _} -> ?assert(true)
    end.
...
test_batch_timeout(Client) ->
    case erlmcp_client:start_link({stdio, #{test_mode => true}}, #{timeout => 200}) of
        {ok, C} ->
            Start = erlang:monotonic_time(millisecond),
            ?assertMatch({error, _}, erlmcp_client:with_batch(C, fun(B) -> erlmcp_client:list_tools(B) end)),
            ?assert(erlang:monotonic_time(millisecond) - Start < 1000),
            erlmcp_client:stop(C);
        {error, _} -> ?assert(true)
    end.
...
setup_client() ->
    setup_application(),
    case erlmcp_client:start_link({stdio, #{test_mode => true}}) of
        {ok, Client} -> Client; {error, _} -> undefined
    end.
...
get_free_port() ->
    {ok, S} = gen_tcp:listen(0, []), {ok, P} = inet:port(S), gen_tcp:close(S), P.
