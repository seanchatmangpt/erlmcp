...
-include("erlmcp.hrl").
-include("erlmcp_test_constants.hrl").

%% Suite callbacks
-export([all/0, groups/0, init_per_suite/1, end_per_suite/1, init_per_group/2, end_per_group/2,
         init_per_testcase/2, end_per_testcase/2]).
%% Test cases
-export([test_system_startup_shutdown/1, test_complete_message_flow/1,
         test_multi_transport_coordination/1, test_server_registry_coordination/1,
         test_configuration_loading/1, test_configuration_hot_reload/1,
         test_transport_config_validation/1, test_failure_recovery_integration/1,
         test_transport_failure_recovery/1, test_server_crash_recovery/1,
         test_registry_failure_handling/1, test_concurrent_connections/1,
         test_high_message_throughput/1, test_resource_management_under_load/1,
         test_real_mcp_client_interaction/1, test_tool_execution_end_to_end/1,
         test_resource_access_end_to_end/1, test_prompt_handling_integration/1,
         test_monitoring_integration/1, test_metrics_collection_integration/1,
...
test_complete_message_flow(Config) ->
    ...
    TestResource =
        fun(_Uri) -> #{content => <<"Test resource content">>, mimeType => <<"text/plain">>} end,
          ...
          ?JSONRPC_FIELD_PARAMS =>
              #{?MCP_FIELD_PROTOCOL_VERSION => ?MCP_VERSION,
                ...
                ?MCP_FIELD_CLIENT_INFO =>
                    #{?MCP_INFO_NAME => <<"integration_test">>, ?MCP_INFO_VERSION => <<"1.0.0">>}}},
...
test_multi_transport_coordination(Config) ->
    ...
    Transports =
        [{make_test_transport_id(3, stdio), stdio, #{server_id => ServerId}},
         {make_test_transport_id(3, tcp),
          tcp,
          #{server_id => ServerId,
            host => "localhost",
            port => 8080}},
         {make_test_transport_id(3, http),
          http,
          #{server_id => ServerId, url => "http://localhost:8000"}}],

    TransportPids =
        lists:map(fun({TransportId, Type, Config}) ->
    ...
    case Bindings of
        {error, _} ->
            ct:pal("Transport bindings not available");
...
test_server_registry_coordination(Config) ->
    ...
    lists:foreach(fun({ServerId, _Pid}) -> ok = erlmcp:stop_server(ServerId) end, ServerPids),

    %% Verify all servers are unregistered
    timer:sleep(100),
    FinalServers = erlmcp:list_servers(),
    RegisteredIds = [Id || {Id, _} <- FinalServers],
    lists:foreach(fun(ServerId) -> ?assertNot(lists:member(ServerId, RegisteredIds)) end,
                  ServerIds),
...
test_concurrent_connections(Config) ->
    ...
    ClientPids =
        lists:map(fun(ClientNum) ->
                     spawn(fun() ->
                              client_simulation_worker(ServerId,
                                                       ClientNum,
                                                       10) % 10 messages per client
                           end)
                  end,
                  lists:seq(1, ?TEST_CONCURRENT_CLIENTS)),
...
test_high_message_throughput(Config) ->
    ...
    MessageResults =
        lists:map(fun(MsgNum) ->
                     ToolName =
                         lists:nth(MsgNum rem length(Tools) + 1, [Name || {Name, _} <- Tools]),
                  ...
                  lists:seq(1, ?TEST_MESSAGE_BURST_SIZE)),
...
test_resource_management_under_load(Config) ->
        ...
        lists:map(fun(ServerId) ->
                     ...
                     lists:foreach(fun(ResourceNum) ->
                                      ...
                                      Handler =
                                          fun(_) ->
                                             #{content =>
                                                   iolist_to_binary(io_lib:format("Resource ~p content",
                                                                                  [ResourceNum])),
    ...
    LoadTestPids =
        lists:map(fun({ServerId, _}) ->
                     spawn(fun() ->
                              lists:foreach(fun(OpNum) ->
                                               try
                                                   %% Test resource operations
                                                   Uri =
                                                       iolist_to_binary(io_lib:format("test://~p/resource_~p",
                                                                                      [ServerId,
                                                                                       OpNum rem 10
                                                                                       + 1])),
    ...
    %% Cleanup all servers
    lists:foreach(fun({ServerId, _}) -> ok = erlmcp:stop_server(ServerId) end, ServerPids),
...
test_real_mcp_client_interaction(Config) ->
    ...
    ServerConfig =
        #{capabilities =>
              #mcp_server_capabilities{resources = #mcp_capability{enabled = true},
                                       tools = #mcp_capability{enabled = true},
                                       prompts = #mcp_capability{enabled = true}},
          server_info => #{name => <<"ErlMCP Integration Test Server">>, version => <<"1.0.0">>}},
    ...
    ClientInteraction =
        [%% 1. Initialize
         ...
         %% 3. List resources
         #{id => 3, method => ?MCP_METHOD_RESOURCES_LIST},
         %% 4. List prompts
         #{id => 4, method => ?MCP_METHOD_PROMPTS_LIST},
         %% 5. Call a tool
         #{id => 5,
         ...
         #{id => 7,
           ...
           params =>
               #{<<"name">> => <<"code_review">>,
                 <<"arguments">> =>
                     #{<<"language">> => <<"erlang">>, <<"code">> => <<"test_code() -> ok.">>}}}],
...
test_tool_execution_end_to_end(Config) ->
    ...
    lists:foreach(fun({Name, Handler}) -> ok = erlmcp:add_tool(ServerId, Name, Handler) end,
                  TestTools),
...
test_resource_access_end_to_end(Config) ->
    ...
    %% Add all resources
    lists:foreach(fun({Uri, Handler}) -> ok = erlmcp:add_resource(ServerId, Uri, Handler) end,
                  TestResources),
    ...
    lists:foreach(fun(ResourceUri) ->
                     ReadMessage =
                         #{?JSONRPC_FIELD_JSONRPC => ?JSONRPC_VERSION,
                           ...
                           ?JSONRPC_FIELD_PARAMS => #{<<"uri">> => ResourceUri}},
...
test_prompt_handling_integration(Config) ->
          ...
          fun(Args) ->
             ...
             #{messages =>
                   [#{role => <<"system">>,
                    ...
                    #{role => <<"user">>,
                      content =>
                          <<"Please review this ", Language/binary, " code: ", Code/binary>>}],
               model => <<"gpt-4">>,
               temperature => 0.1}
          end},
    ...
    %% Add all prompts
    lists:foreach(fun({Name, Handler}) -> ok = erlmcp:add_prompt(ServerId, Name, Handler) end,
                  TestPrompts),
    ...
    PromptTests =
        [{<<"code_review">>,
         ...
         {<<"documentation_helper">>,
          #{<<"topic">> => <<"ErlMCP Integration Tests">>, <<"detail_level">> => <<"detailed">>}},
         {<<"test_generator">>,
          #{<<"module">> => <<"math_utils">>,
...
test_metrics_collection_integration(Config) ->
    ...
    case MetricsAvailable of
        true ->
            ...
            erlmcp_metrics:record_server_operation(ServerId,
                                                   <<"initialize">>,
                                                   200,
                                                   #{success => true}),
...
test_tracing_integration(Config) ->
    ...
    try
        ...
        Attributes = #{<<"test_type">> => <<"integration">>, <<"component">> => <<"tracing">>},
...
make_test_server_id(TestNum) ->
    list_to_atom(lists:flatten(
                     io_lib:format("~s_~p", [?TEST_SERVER_PREFIX, TestNum]))).

make_test_server_id(TestNum, SubNum) ->
    list_to_atom(lists:flatten(
                     io_lib:format("~s_~p_~p", [?TEST_SERVER_PREFIX, TestNum, SubNum]))).

%% Create unique test transport ID
make_test_transport_id(TestNum) ->
    list_to_atom(lists:flatten(
                     io_lib:format("~s_~p", [?TEST_TRANSPORT_PREFIX, TestNum]))).

make_test_transport_id(TestNum, Type) ->
    list_to_atom(lists:flatten(
                     io_lib:format("~s_~p_~p", [?TEST_TRANSPORT_PREFIX, TestNum, Type]))).
                                     ...
                                     #{?JSONRPC_FIELD_JSONRPC => ?JSONRPC_VERSION,
                                       ...
                                       ?JSONRPC_FIELD_PARAMS =>
                                           #{<<"name">> => <<"concurrent_test">>,
                                             <<"arguments">> =>
                                                 #{<<"client_id">> =>
                                                       iolist_to_binary(io_lib:format("client_~p",
                                                                                      [ClientNum])),
                                                   <<"message_id">> => MsgNum}}},
...
setup_comprehensive_mcp_server(ServerId) ->
    ...
    ok =
        erlmcp:add_resource(ServerId,
                            ...
                            fun(_Uri) ->
                               #{content =>
                                     <<"This is a test file content for integration testing">>,
                                 mimeType => <<"text/plain">>,
                                 size => 54}
                            end),
    ...
    ok =
        erlmcp:add_resource(ServerId,
                            ...
                            fun(_Uri) ->
                               ...
                               #{content => jsx:encode(Config), mimeType => <<"application/json">>}
                            end),

    %% Add prompts
    ok =
        erlmcp:add_prompt(ServerId,
                          <<"code_review">>,
                          fun(Args) ->
