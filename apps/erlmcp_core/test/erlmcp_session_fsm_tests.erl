...
cleanup(#{pid := Pid}) ->
    case is_process_alive(Pid) of
        true -> erlmcp_session_fsm:stop(Pid);
        false -> ok
    end.
...
test_suspend_resume_cycle(#{pid := Pid}) ->
    ...
    [
        ?_assertEqual(suspended, State1),
        ?_assertEqual(active, State2)
    ].
...
test_connection_management(#{pid := Pid}) ->
    ...
    [
        ?_assert(HasConnection),
        ?_assert(NoConnection)
    ].
...
test_connection_death_handling(#{pid := Pid}) ->
    ...
    [
        ?_assertEqual(0, maps:size(Connections)),
        ?_assertEqual(suspended, State)
    ].
...
test_state_name_query(#{pid := Pid}) ->
    ...
    State2 = erlmcp_session_fsm:state_name(Pid),
    [
        ?_assertEqual(negotiation, State1),
        ?_assertEqual(active, State2)
    ].
