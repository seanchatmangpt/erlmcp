...
-export([
    ...
    test_tool_completion_workflow/1,
    test_tool_elicitation_workflow/1,
    test_completion_cache_integration/1,
    test_elicitation_with_secrets/1,

    % Resource subscriptions
    test_resource_subscription_lifecycle/1,
    test_resource_subscription_notifications/1,
    test_subscription_cleanup_on_client_crash/1,
    test_subscription_rate_limiting/1,

    % Request cancellation
    test_tool_call_cancellation/1,
    test_resource_read_cancellation/1,
    test_cancellation_cleanup/1,
    test_concurrent_cancellations/1,

    % Secrets integration
    test_secrets_vault_integration/1,
    test_secrets_aws_integration/1,
    test_secrets_in_tool_execution/1,
    test_secrets_rotation/1,

    % Multi-client scenarios
    test_multi_client_subscriptions/1,
    test_multi_client_fairness/1,
    test_subscription_per_client_rate_limit/1,

    % Error recovery
    test_completion_handler_crash_recovery/1,
    test_subscription_failover/1,
    test_secrets_backend_failover/1,
    test_cancellation_after_completion/1,

    % End-to-end workflows
    test_full_mcp_workflow_with_all_features/1,
    test_streaming_with_cancellation/1,
    test_elicitation_timeout_recovery/1
]).
...
groups() ->
    [
        {completion_elicitation, [sequence], [
        ...
        {resource_subscriptions, [parallel], [
...
init_per_suite(Config) ->
    ...
    lists:foreach(fun(App) ->
        case application:start(App) of
            ...
            {error, _} -> application:start(App, temporary)
        end
    end, Apps),
...
test_tool_completion_workflow(Config) ->
    ...
    CompletionHandler = fun(_Ref, Arg, _Context) ->
    ...
    ToolHandler = fun(Args) ->
        ...
        #{
            result => <<"Processing file: ", FilePath/binary>>,
            status => <<"success">>
        }
    end,
    ...
    {ok, CompletionResult} = erlmcp_completion:complete(
        whereis(erlmcp_completion),
        CompletionRef,
    ...
    ?assertMatch(#{completions := Completions, hasMore := _, total := _}, CompletionResult),
    ...
    {ok, CachedResult} = erlmcp_completion:complete(
        whereis(erlmcp_completion),
        CompletionRef,
        #{name => <<"file_path">>, value => <<"/home">>},
        #{type => <<"tool">>, arguments => #{}}
...
test_tool_elicitation_workflow(Config) ->
    ...
    ToolWithElicitation = fun(Args) ->
        case maps:get(<<"api_key">>, Args, undefined) of
            undefined ->
                ...
                {ok, ElicitationId, ElicitationData} = erlmcp_elicitation:create_elicitation(
                    ElicitationConfig,
                    self()
                ),
    ...
    ?assertMatch(#{elicitation := #{id := _, prompt := _, mode := inline}}, Result1),
...
test_elicitation_with_secrets(Config) ->
    ...
    {ok, ElicitationId, _ElicitationData} = erlmcp_elicitation:create_elicitation(
        ElicitationConfig,
        self()
    ),
...
test_resource_subscription_notifications(Config) ->
    ...
    lists:foreach(fun(Pid) ->
        ok = erlmcp_subscription:subscribe(ResourceUri, Pid)
    end, ClientPids),
...
test_subscription_rate_limiting(Config) ->
    ...
    lists:foreach(fun(N) ->
        erlmcp_subscription:notify(ResourceUri, #{seq => N})
    end, lists:seq(1, 100)),
...
test_cancellation_cleanup(Config) ->
    ...
    Tokens = lists:map(fun(N) ->
        ...
        erlmcp_cancellation:register(self(), Pid, <<"test_op_", (integer_to_binary(N))/binary>>)
...
test_concurrent_cancellations(Config) ->
    ...
    _CancelPids = lists:map(fun({Token, _Pid}) ->
...
test_multi_client_fairness(Config) ->
    ...
    %% Send notifications
    lists:foreach(fun(N) ->
        erlmcp_subscription:notify(ResourceUri, {msg, N})
    end, lists:seq(1, NumNotifications)),
    ...
    lists:foreach(fun(Count) ->
        ...
        ?assert(Variance < 0.2, io_lib:format("Client received ~p notifications, expected ~p (variance: ~p)",
...
test_completion_handler_crash_recovery(Config) ->
    ...
    CrashingHandler = fun(_Ref, _Arg, _Context) ->
    ...
    Result = erlmcp_completion:complete(
        whereis(erlmcp_completion),
        CrashingRef,
    ...
    ?assert(is_process_alive(whereis(erlmcp_completion)), "Completion server should still be alive"),
...
test_full_mcp_workflow_with_all_features(Config) ->
    ...
    SubscriberPid = spawn(fun() ->
        receive {update, _} -> ok after 5000 -> ok end
    end),
    ...
    %% 4a. Get completion
    {ok, CompResult} = erlmcp_completion:complete(
        whereis(erlmcp_completion),
        CompletionRef,
        #{name => <<"table">>, value => <<"u">>},
        #{}
...
test_streaming_with_cancellation(Config) ->
    ...
    StreamHandler = fun(_Ref, _Arg, _Context) ->
        %% Large result set
        Items = [#{value => integer_to_binary(N), label => <<"Item ", (integer_to_binary(N))/binary>>}
                 || N <- lists:seq(1, 1000)],
        {ok, Items}
    end,
    ...
    %% Start streaming
    {ok, CompletionId, _StreamPid} = erlmcp_completion:stream_completion(
        whereis(erlmcp_completion),
        StreamRef,
        #{name => <<"test">>, value => <<"">>},
        #{}
...
receive_notifications(Count) ->
    receive
        stop -> Count;
        {msg, _} -> receive_notifications(Count + 1)
    after 100 ->
        receive_notifications(Count)
    end.
