...
cleanup(Pid) ->
    case is_process_alive(Pid) of
        true -> gen_server:stop(erlmcp_notification_handler_sup);
        false -> ok
    end,
    ok.
...
multiple_handlers_test_() ->
    {setup,
     ...
     fun(_SupPid) ->
         [
          ?_test(test_concurrent_handler_start()),
          ?_test(test_handler_isolation())
         ]
     end}.
...
test_concurrent_handler_start() ->
    ...
    HandlerPids = lists:map(fun(N) ->
        Method = <<"test/concurrent_", (integer_to_binary(N))/binary>>,
        Handler = fun(_M, _P) ->
            Self ! {handler, N},
            ok
        end,
        {ok, Pid} = erlmcp_notification_handler_sup:start_handler(Method, Handler, #{}),
        Pid
...
test_handler_isolation() ->
    ...
    HandlerPids = lists:map(fun(Method) ->
        Handler = fun(M, _P) ->
            Self ! {handler, M},
            ok
        end,
...
restart_strategy_test_() ->
    {setup,
     ...
     fun(_SupPid) ->
         [
          ?_test(test_transient_restart_strategy())
         ]
     end}.
...
supervisor_properties_test_() ->
    {setup,
     ...
     fun(_SupPid) ->
         [
          ?_test(test_supervisor_strategy()),
          ?_test(test_supervisor_intensity())
         ]
     end}.
...
integration_test_() ->
    {setup,
     ...
     fun(_SupPid) ->
         [
          ?_test(test_integration_with_notification_handler())
         ]
     end}.
