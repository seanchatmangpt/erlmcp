...
test_register_find_transport() ->
    ...
    %% Register transport
    ?assertEqual(ok, erlmcp_registry_distributed:register(transport, test_transport_1, TestPid, Config)),

    %% Find transport
    {ok, {Node, Pid, ReturnedConfig}} = erlmcp_registry_distributed:whereis(transport, test_transport_1),
    ?assertEqual(node(), Node),
    ?assertEqual(TestPid, Pid),
    ?assertEqual(Config, ReturnedConfig),
...
test_duplicate_registration() ->
    %% Create a test server process
    TestPid1 = spawn(fun() -> receive stop -> ok end end),
    TestPid2 = spawn(fun() -> receive stop -> ok end end),
    Config = #{capabilities => #mcp_server_capabilities{}},
...
test_unregister_transport() ->
    ...
    TestPid = spawn(fun() -> receive stop -> ok end end),
    Config = #{type => tcp},
    ?assertEqual(ok, erlmcp_registry_distributed:register(transport, unreg_transport, TestPid, Config)),
    ...
    %% Verify it's gone
    ?assertEqual({error, not_found}, erlmcp_registry_distributed:whereis(transport, unreg_transport)),
...
test_list_servers() ->
    ...
    lists:foreach(fun({Id, _}) ->
        ?assert(lists:member(Id, ServerIds))
    end, Pids),
...
test_list_transports() ->
    ...
    lists:foreach(fun({Id, _}) ->
        ?assert(lists:member(Id, TransportIds))
    end, Pids),
...
test_multiple_servers() ->
    ...
    lists:foreach(fun({Id, Pid}) ->
        {ok, {_Node, FoundPid, _Config}} = erlmcp_registry_distributed:whereis(server, Id),
...
test_multiple_transports() ->
    ...
    %% Verify all can be found
    lists:foreach(fun({Id, Pid}) ->
        {ok, {_Node, FoundPid, _Config}} = erlmcp_registry_distributed:whereis(transport, Id),
        ?assertEqual(Pid, FoundPid)
    end, Pids),
