    ...
    case erlmcp_json_rpc:decode_batch(Json) of
        {ok, Responses} ->
            ...
            case lists:nth(1, Responses) of
                #json_rpc_response{id = 1, result = <<"ok">>} ->
                    ok;
                _ ->
                    ?assert(false)
            end,
            case lists:nth(2, Responses) of
                #json_rpc_response{id = 2, error = #{<<"code">> := -32601}} ->
                    ok;
                _ ->
                    ?assert(false)
            end;
...
test_unicode_roundtrip() ->
    Original = #{<<"text">> => <<"Hello 世界 🌍 Привет мир">>},
