...
setup() ->
    ...
    #{
        queue_limits_pid => QLPid,
        overload_monitor_pid => OMPid
    }.
...
test_alert_generation(_State) ->
    {"Generate alerts when thresholds exceeded",
     fun() ->
         ...
         lists:foreach(
             fun(_) ->
                 erlmcp_queue_limits:record_enqueue(alert_test, TestPid)
             end,
             lists:seq(1, 9)
         ),
...
test_overload_detection(_State) ->
    {"Detect overloaded processes",
     fun() ->
         ...
         lists:foreach(
             fun(_) ->
                 erlmcp_queue_limits:record_enqueue(overload_test, TestPid)
             end,
             lists:seq(1, 5)
         ),
...
test_alert_thresholds(_State) ->
    {"Different alert levels for different utilization",
     fun() ->
         ...
         lists:foreach(
             fun(_) -> erlmcp_queue_limits:record_enqueue(warning_test, TestPid1) end,
             lists:seq(1, 85)
         ),
...
test_alert_history(_State) ->
    {"Track alert history",
     fun() ->
         ...
         lists:foreach(
             fun(_) -> erlmcp_queue_limits:record_enqueue(history_test, TestPid) end,
             lists:seq(1, 9)
         ),
...
test_health_check_integration(_State) ->
    {"Integration with health monitor",
     fun() ->
         ...
         lists:foreach(
             fun(_) -> erlmcp_queue_limits:record_enqueue(health_test, TestPid) end,
             lists:seq(1, 5)
         ),
...
test_circuit_breaker_integration(_State) ->
    {"Integration with circuit breaker on overload",
     fun() ->
         ...
         lists:foreach(
             fun(_) -> erlmcp_queue_limits:record_enqueue(breaker_test, TestPid) end,
             lists:seq(1, 10)
         ),
...
test_periodic_checks(_State) ->
    {"Periodic health checks run automatically",
     fun() ->
         ...
         lists:foreach(
             fun(_) -> erlmcp_queue_limits:record_enqueue(periodic_test, TestPid) end,
             lists:seq(1, 9)
         ),
