...
-export([
    ...
    run_scenario/2,
    scenarios/0,
    validate_bounded_refusal/2,
    generate_chaos_report/1
]).

%% Scenario functions
-export([
    chaos_process_crash/1,
    chaos_network_partition/1,
    chaos_memory_exhaustion/1,
    chaos_message_flood/1,
    chaos_invalid_payload/1,
    chaos_connection_leak/1,
    chaos_slow_consumer/1,
    chaos_supervisor_cascade/1,
    chaos_disk_full/1,
    chaos_cpu_saturation/1,
    chaos_large_payload/1
]).
...
validate_bounded_refusal(Result, Scenario) ->
    ...
    CodeMatches = case ExpectedCode of
        undefined -> true;
...
generate_chaos_report(Results) ->
    ...
    AvgRecoveryTime = case Results of
        [] -> 0.0;
        _ ->
    ...
    AvgDetectionTime = case Results of
        [] -> 0.0;
        _ ->
    ...
    #{
        ...
        <<"results">> => Results,
        <<"overall_status">> => case FailedScenarios of
            0 -> <<"PASS">>;
            _ -> <<"FAIL">>
        end
    }.
            ...
            receive
                {'DOWN', MonitorRef, process, Pid, _Reason} ->
                    ...
                    #{
                        ...
                        <<"actual_behavior">> => case Recovered andalso RecoveryTime < 1000.0 of
                            true -> <<"recovered">>;
                            false -> <<"failed_to_recover">>
                        end,
...
execute_scenario(Scenario, Config) ->
    #{id := ScenarioId, inject := InjectFun, timeout_ms := Timeout} = Scenario,
        ...
        {Ref, Result} ->
            ...
            case Result of
                {ok, ChaosResult} ->
                    ...
                    UpdatedResult = ChaosResult#{
                        <<"bounded_refusal_validated">> => BoundedOk
                    },
                    {ok, UpdatedResult};
                Error ->
                    Error
