...
subscribe_multiple_subscribers() ->
    ...
    Subscriber1 = self(),
    Subscriber2 = spawn(fun() -> receive after infinity -> ok end end),
...
notify_multiple_subscribers() ->
    ...
    Subscriber1 = spawn(fun() ->
        receive
            {'$mcp_subscription', Msg} -> Parent ! {sub1, Msg}
        end
    end),
    Subscriber2 = spawn(fun() ->
        receive
            {'$mcp_subscription', Msg} -> Parent ! {sub2, Msg}
        end
    end),
...
bulk_notify_multiple_subscriptions() ->
    ...
    Subscriber1 = spawn(fun() ->
        receive
            {'$mcp_subscription', Msg} -> Parent ! {bulk1, Msg}
        end
    end),
    Subscriber2 = spawn(fun() ->
        receive
            {'$mcp_subscription', Msg} -> Parent ! {bulk2, Msg}
        end
    end),
