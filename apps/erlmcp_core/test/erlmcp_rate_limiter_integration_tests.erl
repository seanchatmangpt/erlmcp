...
test_graceful_degradation_message_rate() ->
    TimeNowMs = erlang:system_time(millisecond),
    Results1 = [erlmcp_rate_limiter:check_message_rate(client_degrad1, TimeNowMs) || _ <- lists:seq(1, 10)],
    Successes1 = length([ok || {ok, _} <- Results1]),
    ?assertEqual(10, Successes1),
    Results2 = [erlmcp_rate_limiter:check_message_rate(client_degrad1, TimeNowMs) || _ <- lists:seq(1, 10)],
    Successes2 = length([ok || {ok, _} <- Results2]),
    ?assert(Successes2 < 10).

test_graceful_degradation_connection_rate() ->
    TimeNowMs = erlang:system_time(millisecond),
    _ = [erlmcp_rate_limiter:check_connection_rate(client_degrad2, TimeNowMs) || _ <- lists:seq(1, 5)],
