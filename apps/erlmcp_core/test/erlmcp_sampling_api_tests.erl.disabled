...
state_management_test_() ->
    {setup,
     ...
     fun(_) ->
         [
             fun request_count_increments/0,
             fun concurrent_requests/0
         ]
     end
    }.
...
request_count_increments() ->
    fun() ->
        ...
        ok = case erlmcp_sampling:create_message(Messages, Params) of
            {ok, _} -> ok;
    ...
    fun() ->
        ...
        [begin
            case is_process_alive(P) of
                true -> ?assert(false, "Process did not complete");
                false -> ok
            end
        end || P <- Pids]
    end.
