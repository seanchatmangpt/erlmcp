...
-export([
    benchmark_all/0,
    benchmark_workflow/1,
    run_single_workflow/1
]).
...
-type workflow_step() :: initialize | list_tools | list_prompts | list_resources |
                         call_tool | get_prompt | read_resource | shutdown.
-type workflow_def() :: #{
    id := workflow_id(),
    steps := [workflow_step()],
...
benchmark_workflow(#{id := Id, steps := Steps, iterations := Iterations} = Workflow) ->
    ...
    _ = lists:foreach(fun(_) -> run_single_workflow(Workflow) end, lists:seq(1, min(10, Iterations))),
    ...
    ok = erlmcp_server:add_tool_with_schema(
        ...
        fun(#{<<"a">> := A, <<"b">> := B}) ->
            ...
            #{
                <<"content">> => [#{
                    <<"type">> => <<"text">>,
                    <<"text">> => integer_to_binary(trunc(Result))
                }]
            }
        end,
        #{
...
execute_step(get_prompt, _ServerPid) ->
    try
        Params = #{<<"name">> => <<"test_prompt">>, <<"arguments">> => #{<<"topic">> => <<"bench">>}},
...
export_results(Results) ->
    ...
    Filename = lists:flatten(io_lib:format("bench/results/integration_~p.json", [Timestamp])),
