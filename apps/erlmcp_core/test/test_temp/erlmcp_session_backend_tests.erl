...
test_ets_store_and_fetch() ->
    ...
    ?assertEqual(SessionId,
                 maps:get(id, element(2, element(2, erlmcp_session_ets:fetch(SessionId, State))))),
    cleanup_ets(State).
...
test_ets_list() ->
    ...
    Sessions =
        [{<<"session_ets_list_1">>,
          #{id => <<"session_ets_list_1">>,
            created_at => 1,
            last_accessed => 1,
            timeout_ms => infinity,
            metadata => #{}}},
         {<<"session_ets_list_2">>,
          #{id => <<"session_ets_list_2">>,
            created_at => 1,
            last_accessed => 1,
            timeout_ms => infinity,
            metadata => #{}}},
         {<<"session_ets_list_3">>,
          #{id => <<"session_ets_list_3">>,
            created_at => 1,
            last_accessed => 1,
            timeout_ms => infinity,
            metadata => #{}}}],
    State1 =
        lists:foldl(fun({Id, Sess}, Acc) ->
...
setup_dets() ->
    {ok, State} =
        erlmcp_session_dets:init(#{table_name => erlmcp_sessions_dets_test,
...
test_dets_cleanup_expired() ->
    ...
    ?assertEqual(1, Count),
    ?assertEqual({error, not_found, State1},
                 erlmcp_session_dets:fetch(<<"expired_dets_1">>, State1)),
...
setup_mnesia() ->
    ...
    {ok, State} =
        erlmcp_session_mnesia:init(#{table_name => erlmcp_session_mnesia_test,
                                     disc_copies => false}),
    State.
...
backend_comparison_test_() ->
    {setup,
     ...
     fun(_) -> [?_test(test_all_backends_basic_ops()), ?_test(test_all_backends_ttl_expiration())]
     end}.
...
test_all_backends_basic_ops() ->
    ...
    lists:foreach(fun({BackendName, State}) ->
                     ...
                     StoreFun =
                         case BackendName of
                             ets ->
                                 fun erlmcp_session_ets:store/3;
                             dets ->
                                 fun erlmcp_session_dets:store/3;
                             mnesia ->
                                 fun erlmcp_session_mnesia:store/3
                         end,

                     FetchFun =
                         case BackendName of
                             ets ->
                                 fun erlmcp_session_ets:fetch/2;
                             dets ->
                                 fun erlmcp_session_dets:fetch/2;
                             mnesia ->
                                 fun erlmcp_session_mnesia:fetch/2
                         end,
...
test_all_backends_ttl_expiration() ->
    ...
    lists:foreach(fun({BackendName, State}) ->
                     ...
                     CleanupFun =
                         case BackendName of
                             ets ->
                                 fun erlmcp_session_ets:cleanup_expired/1;
                             dets ->
                                 fun erlmcp_session_dets:cleanup_expired/1;
                             mnesia ->
                                 fun erlmcp_session_mnesia:cleanup_expired/1
                         end,
