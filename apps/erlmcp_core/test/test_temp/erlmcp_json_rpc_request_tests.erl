...
test_decode_request_with_string_id() ->
    ...
    Result = erlmcp_json_rpc:decode_message(Json),
    case Result of
        {ok, #json_rpc_request{id = <<"req-abc-123">>, method = <<"test">>}} ->
            ok;
        _ ->
            ?assert(false)
    end.
...
test_decode_request_missing_method() ->
    Json = jsx:encode(#{<<"jsonrpc">> => <<"2.0">>, <<"id">> => 1}),
    Result = erlmcp_json_rpc:decode_message(Json),
    ?assertMatch({error, _}, Result).

test_decode_request_missing_id() ->
    Json = jsx:encode(#{<<"jsonrpc">> => <<"2.0">>, <<"method">> => <<"test">>}),
...
test_method_empty_string() ->
    ...
    case Result of
        {ok, _} ->
            ok;  % Parser accepts it
        {error, _} ->
            ok  % Or rejects it - both are acceptable
    end.
