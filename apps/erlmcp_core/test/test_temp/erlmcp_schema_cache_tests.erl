...
test_validate_object(_Pid) ->
    ?_test(begin
               ...
               ?assertMatch({error, {type_mismatch, expected_object}},
...
test_validate_array(_Pid) ->
    ?_test(begin
               ...
               ?assertMatch({error, {type_mismatch, expected_array}},
...
test_validate_primitives(_Pid) ->
    ?_test(begin
               ...
               ?assertMatch({error, {type_mismatch, expected_string}},
               ...
               ?assertMatch({error, {type_mismatch, expected_integer}},
...
test_invalid_schema_structure(_Pid) ->
    ?_test(begin
               ...
               ?assertMatch({error, missing_required_fields},
...
test_zero_copy_schema_access(_Pid) ->
    ?_test(begin
               ...
               LargeSchema =
                   #{<<"type">> => <<"object">>,
                     <<"properties">> =>
                         maps:from_list([{list_to_binary("field_" ++ integer_to_list(N)),
                                          #{<<"type">> => <<"string">>}}
                                         || N <- lists:seq(1, 1000)])},
