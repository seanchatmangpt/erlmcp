...
run_all() ->
    ...
    lists:foreach(fun(Workload) -> run_workload(Workload) end, workloads()),
...
run_workload(#{id := WorkloadId, category := Category} = Workload) ->
    ...
    Result =
        case Category of
            tool_calls ->
                benchmark_tool_calls(Workload);
            subscriptions ->
                benchmark_subscriptions(Workload);
            prompts ->
                benchmark_prompt_rendering(Workload);
            sampling ->
                benchmark_sampling(Workload)
        end,
    ...
    case validate_report(Report) of
        ok ->
            Timestamp = erlang:system_time(second),
            Filename =
                io_lib:format("bench/results/mcp_features_~s_~p.json", [WorkloadId, Timestamp]),
...
benchmark_tool_calls(#{operations := Ops, complexity := Complexity}) ->
    ...
    Latencies =
        lists:map(fun(_) ->
                     ...
                     _Request =
                         erlmcp_json_rpc:encode_request(RequestId, ?MCP_METHOD_TOOLS_CALL, Params),
    ...
    erlmcp_server:add_tool_with_schema(ServerPid,
                                       ...
                                       fun(Args) ->
                                          ...
                                          Results =
                                              lists:map(fun(I) ->
                                                           Key =
                                                               iolist_to_binary(io_lib:format("param~p",
                                                                                              [I])),
...
add_tool_by_complexity(ServerPid, complex) ->
    ...
    Properties =
        maps:from_list([{iolist_to_binary(io_lib:format("param~p", [I])),
                         #{<<"type">> => <<"string">>}}
                        || I <- lists:seq(1, 20)]),
    erlmcp_server:add_tool_with_schema(ServerPid,
                                       ToolName,
...
generate_tool_args(medium) ->
    maps:from_list([{iolist_to_binary(io_lib:format("param~p", [I])), <<"value">>}
                    || I <- lists:seq(1, 5)]);
...
benchmark_subscriptions(#{subscribers := NumSubs,
                          notification_rate := RateHz,
    ...
    Subscribers =
        lists:map(fun(_) -> spawn_link(fun() -> subscriber_loop([]) end) end,
                  lists:seq(1, NumSubs)),
    ...
    NotificationLatencies =
        lists:map(fun(_) ->
                  ...
                  lists:seq(1, TotalNotifications)),
    ...
    %% Cleanup
    lists:foreach(fun(SubPid) -> exit(SubPid, normal) end, Subscribers),
...
benchmark_prompt_rendering(#{operations := Ops, complexity := Complexity}) ->
    io:format("  [Prompt Rendering] Running ~p operations with ~p complexity...~n",
              [Ops, Complexity]),
...
generate_prompt_template(complex) ->
    ...
    Variables =
        maps:from_list([{iolist_to_binary(io_lib:format("var~p", [I])), <<"value">>}
                        || I <- lists:seq(1, 50)]),
    {Template, Variables}.
...
generate_sampling_params(temperature) ->
    #{<<"modelPreferences">> =>
          #{<<"hints">> => [#{<<"name">> => <<"mock-model">>}], <<"temperature">> => 0.7},
      <<"maxTokens">> => 100};
generate_sampling_params(top_k) ->
    #{<<"modelPreferences">> =>
          #{<<"hints">> => [#{<<"name">> => <<"mock-model">>}], <<"topK">> => 50},
      <<"maxTokens">> => 100}.
        ...
        true ->
            ...
            case lists:all(fun(Field) -> maps:is_key(Field, Env) end, EnvFields) of
                true ->
                    ok;
                false ->
                    {error, {missing_environment_fields, EnvFields}}
            end;
        false ->
