...
test_create_url_elicitation() ->
    ...
    Config = #{<<"mode">> => <<"url">>, <<"url">> => <<"https://example.com/elicitation">>},
...
test_create_elicitation_with_custom_timeout() ->
    Config = #{<<"mode">> => <<"inline">>, <<"timeout">> => 5000},
...
test_url_mode_blocks_localhost() ->
    Config = #{<<"mode">> => <<"url">>, <<"url">> => <<"http://localhost:8080/elicitation">>},
    Result = erlmcp_elicitation:create_elicitation(Config, self()),
    ?assertMatch({error, {unsafe_url, _}}, Result).

test_url_mode_blocks_private_ipv4() ->
    Config = #{<<"mode">> => <<"url">>, <<"url">> => <<"http://10.0.0.1/elicitation">>},
    Result = erlmcp_elicitation:create_elicitation(Config, self()),
    ?assertMatch({error, {unsafe_url, _}}, Result).

test_url_mode_blocks_private_ipv4_class_b() ->
    Config = #{<<"mode">> => <<"url">>, <<"url">> => <<"http://172.16.0.1/elicitation">>},
    Result = erlmcp_elicitation:create_elicitation(Config, self()),
    ?assertMatch({error, {unsafe_url, _}}, Result).

test_url_mode_blocks_private_ipv4_class_c() ->
    Config = #{<<"mode">> => <<"url">>, <<"url">> => <<"http://192.168.1.1/elicitation">>},
    Result = erlmcp_elicitation:create_elicitation(Config, self()),
    ?assertMatch({error, {unsafe_url, _}}, Result).
...
test_url_mode_blocks_invalid_url() ->
    Config = #{<<"mode">> => <<"url">>, <<"url">> => <<"not-a-valid-url">>},
    Result = erlmcp_elicitation:create_elicitation(Config, self()),
...
test_rate_limit_allows_within_threshold() ->
    ...
    Config = #{<<"mode">> => <<"inline">>},
    lists:foreach(fun(_) -> {ok, _, _} = erlmcp_elicitation:create_elicitation(Config, ClientPid)
                  end,
                  lists:seq(1, 10)),
    ?assert(true).
...
test_complete_elicitation_within_size_limit() ->
    Config = #{<<"mode">> => <<"inline">>, <<"size_limit">> => 1024},
...
test_complete_elicitation_exceeds_size_limit() ->
    Config = #{<<"mode">> => <<"inline">>, <<"size_limit">> => 100},
    {ok, ElicitationId, _} = erlmcp_elicitation:create_elicitation(Config, self()),
...
test_complete_timed_out_elicitation_fails() ->
    Config = #{<<"mode">> => <<"inline">>, <<"timeout">> => 100},
    {ok, ElicitationId, _} = erlmcp_elicitation:create_elicitation(Config, self()),
...
concurrent_limit_test_() ->
    {setup,
     fun setup/0,
     fun cleanup/1,
     fun(_) -> [?_test(test_concurrent_limit_enforcement())] end}.
...
client_cleanup_test_() ->
    {setup, fun setup/0, fun cleanup/1, fun(_) -> [?_test(test_client_cleanup_on_exit())] end}.
