...
-record(registry_state, {server_transport_map = #{} :: #{atom() => atom()}}).
        ...
        [% Register server
         ?_assertEqual(ok,
                       gen_server:call(Registry,
                                       {register_server, test_server_1, TestServer, ServerConfig})),
         % Verify registration
...
test_server_transport_binding(#{registry := Registry} = State) ->
    ...
    ok =
        gen_server:call(Registry,
                        {register_transport, test_transport_bind, TestTransport, #{type => stdio}}),
    ...
    Tests =
        [% Test binding (both server and transport must be registered first)
         fun() ->
            Result =
                gen_server:call(Registry,
                                {bind_transport_to_server, test_transport_bind, test_server_bind}),
    ...
    Tests2 =
        [fun() ->
            ...
            Result =
                gen_server:call(Registry,
                                {bind_transport_to_server, test_transport_bind2, nonexistent}),
         ...
         fun() ->
            ...
            Result =
                gen_server:call(Registry,
                                {bind_transport_to_server, nonexistent, test_server_bind2}),
    ...
    maps:put(test_pids,
             [TestServer, TestTransport, TestServer2, TestTransport2 | maps:get(test_pids,
                                                                                State,
                                                                                [])],
             State),
    Tests ++ Tests2.
...
test_message_routing_to_server(#{registry := Registry} = State) ->
    ...
    % Register and bind
    ?assertEqual(ok,
                 gen_server:call(Registry, {register_server, test_server_route, TestServer, #{}})),
    ?assertEqual(ok,
                 gen_server:call(Registry,
                                 {register_transport,
    ...
    gen_server:cast(Registry,
                    {route_to_server, test_server_route, test_transport_route, TestMessage}),
    ...
    Test =
        ?_assert(begin
                     ...
                     receive
                         {messages, Messages} ->
                             lists:any(fun ({server_received, test_transport_route, Msg})
                                               when Msg =:= TestMessage ->
                                               true;
                                           (_) ->
                                               false
                                       end,
    ...
    maps:put(test_pids,
             [Collector, TestServer, TestTransport | maps:get(test_pids, State, [])],
             State),
    [Test].
...
test_message_routing_to_transport(#{registry := Registry} = State) ->
    ...
    % Register and bind
    ?assertEqual(ok,
                 gen_server:call(Registry, {register_server, test_server_route2, TestServer, #{}})),
    ?assertEqual(ok,
                 gen_server:call(Registry,
                                 {register_transport,
                                  test_transport_route2,
                                  TestTransport,
                                  #{type => stdio}})),
    ?assertEqual(ok,
                 gen_server:call(Registry,
    ...
    gen_server:cast(Registry,
                    {route_to_transport, test_transport_route2, test_server_route2, TestResponse}),
    ...
    Test =
        ?_assert(begin
                     ...
                     receive
                         {messages, Messages} ->
                             lists:any(fun ({transport_received, test_server_route2, Msg})
                                               when Msg =:= TestResponse ->
                                               true;
                                           (_) ->
                                               false
                                       end,
...
test_list_operations(#{registry := Registry} = State) ->
    ...
    Tests = [?_assertEqual(5, length(ServerList)), ?_assertEqual(5, length(TransportList))],
...
test_concurrent_registration(#{registry := Registry} = State) ->
    ...
    Pids =
        lists:map(fun(_) ->
                     spawn_link(fun() ->
                                   ...
                                   Result =
                                       gen_server:call(Registry,
                                                       {register_server, ServerId, TestServer, #{}},
                                                       2000),
    ...
    SuccessCount =
        lists:foldl(fun({_Pid, Result}, Acc) ->
                       case Result of
                           ok ->
                               Acc + 1;
     ...
     fun(#{registry := Registry} = State) ->
        ...
        [?_test(begin
         ...
         ?_test(begin
                    ...
                    Result =
                        gen_server:call(Registry,
                                        {register_server, ServerId, TestServer2, ServerConfig}),
     ...
     fun(#{registry := Registry} = State) ->
        ...
        [?_test(begin
         ...
         ?_test(begin
                    ...
                    ?assertMatch({ok, ServerId},
                                 gen_server:call(Registry, {get_server_for_transport, TransportId}))
                end)]
     end}.
...
test_broadcast_routing_test_() ->
    {setup,
     ...
     fun(#{registry := Registry} = State) ->
        ...
        ok =
            gen_server:call(Registry,
                            {register_transport, Transport1Id, TestTransport1, #{type => stdio}}),
        ok =
            gen_server:call(Registry,
                            {register_transport, Transport2Id, TestTransport2, #{type => stdio}}),
        ok =
            gen_server:call(Registry,
                            {register_transport, Transport3Id, TestTransport3, #{type => stdio}}),
        ...
        [?_test(begin
                    gen_server:cast(Registry,
                                    {route_to_transport, broadcast, ServerId, TestMessage}),
...
test_transport_process_death_cleanup_test_() ->
    {setup,
     ...
     fun(#{registry := Registry} = State) ->
        ...
        ok =
            gen_server:call(Registry,
                            {register_transport, TransportId, TestTransport, #{type => stdio}}),
        ...
        [?_test(begin
         ...
         ?_test(begin
                    ...
                    ?assertMatch({error, not_found},
                                 gen_server:call(Registry, {find_transport, TransportId}))
                end),
         ?_test(begin
    ...
    {setup,
     ...
     fun(#{registry := Registry} = State) ->
        ...
        [?_test(begin
                    ...
                    ?assertMatch({ok, ServerId},
                                 gen_server:call(Registry,
                                                 {get_server_for_transport, Transport1Id})),
                    ?assertMatch({ok, ServerId},
                                 gen_server:call(Registry,
                                                 {get_server_for_transport, Transport2Id}))
                end),
         ?_test(begin
                    % Kill server process
                    unlink(TestServer),
    ...
    {setup,
     ...
     fun(#{registry := Registry}) ->
        [?_test(begin
                    ...
                    ?assertEqual(ok,
                                 gen_server:call(Registry,
                                                 {unbind_transport, nonexistent_transport}))
                end)]
     end}.
     ...
     fun(#{registry := Registry} = State) ->
        ...
        [?_test(begin
         ...
         ?_test(begin
                    % Register and bind
                    ok =
                        gen_server:call(Registry, {register_server, state_server, TestServer, #{}}),
                    ok =
                        gen_server:call(Registry,
                                        {register_transport,
                                         state_transport,
                    ...
                    ok =
                        gen_server:call(Registry,
                                        {bind_transport_to_server, state_transport, state_server}),
     ...
     fun(#{registry := Registry}) ->
        ...
        [?_test(begin
         ...
         ?_test(begin
                    ...
                    ?assertMatch({error, invalid_state},
                                 gen_server:call(Registry, {restore_state, InvalidState}))
                end)]
     end}.
     ...
     fun(#{registry := Registry}) ->
        [?_test(begin
         ...
         ?_test(begin
                    ...
                    lists:foreach(fun(_) -> gen_server:cast(Registry, {test_msg, make_ref()}) end,
                                  lists:seq(1, 10)),
                    timer:sleep(50),
                    Depth = erlmcp_registry:get_queue_depth(),
                    ?assert(is_integer(Depth))
     ...
     fun(_) ->
        ...
        [?_test(begin
                    ?assertMatch({error, server_not_found},
                                 erlmcp_registry:route_message({server, nonexistent_server},
                                                               TestMessage))
                end)]
     end}.
    ...
    {setup,
     ...
     fun(#{registry := Registry} = State) ->
        ...
        [?_test(begin
                    ...
                    ?assertEqual(ok,
                                 erlmcp_registry:route_message({transport, TransportId},
                                                               TestMessage))
                end)]
     end}.
     ...
     fun(_) ->
        ...
        [?_test(begin
                    ?assertMatch({error, transport_not_found},
                                 erlmcp_registry:route_message({transport, nonexistent_transport},
                                                               TestMessage))
                end)]
     end}.
     ...
     fun(#{registry := Registry} = State) ->
        ...
        {Transports, TransportIds} =
            lists:unzip(
                lists:map(fun(N) ->
                             TransportId =
                                 list_to_atom("concurrent_transport_" ++ integer_to_list(N)),
                             TestTransport =
                                 spawn_link(fun() ->
                                               receive
                                                   stop ->
    ...
    {setup,
     ...
     fun(#{registry := Registry}) ->
        [?_test(begin
                    ...
                    ?assertMatch({error, unknown_request},
                                 gen_server:call(Registry, unknown_request))
                end)]
     end}.
