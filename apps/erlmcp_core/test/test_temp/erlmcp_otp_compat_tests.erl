...
json_macro_encode_test() ->
    ...
    ?assertMatch(#{"test" := <<"macro">>}, jsx:decode(Encoded, [return_maps])),
    ?assertMatch(#{"test" := <<"macro">>}, jsx:decode(EncodedSafe, [return_maps])).
...
safe_processes_test() ->
    ...
    lists:foreach(fun(Pid) -> ?assert(is_pid(Pid)) end, PidList),
...
large_json_test() ->
    ...
    Large =
        lists:foldl(fun(N, Acc) -> Acc#{<<"key_", (integer_to_binary(N))/binary>> => N} end,
                    #{},
                    lists:seq(1, 100)),
