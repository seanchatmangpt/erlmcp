...
metadata_key() ->
    proper_types:oneof([proper_types:atom(), proper_types:binary()]).
...
metadata_value() ->
    proper_types:oneof([proper_types:binary(),
                        ...
                        proper_types:list(
                            proper_types:int()),
                        proper_types:map(
                            proper_types:binary(), proper_types:int())]).
...
prop_metadata_set_get_idempotent() ->
    ?FORALL({Metadata, Key, Value},
            {metadata_map(), metadata_key(), metadata_value()},
            begin
    ...
    ?FORALL({Metadata, Key1, Key2, Value1, Value2},
            ...
            ?IMPLIES(Key1 =/= Key2,
                     begin
                         ...
                         erlmcp_session:get_metadata(Session1, Key1)
                         =:= erlmcp_session:get_metadata(Session2, Key1)
                         andalso erlmcp_session:get_metadata(Session1, Key2)
                                 =:= erlmcp_session:get_metadata(Session2, Key2)
                     end)).
...
proper_test_() ->
    [{"Session IDs are unique",
      ?_assertEqual(true, proper:quickcheck(prop_session_ids_are_unique(), 50))},
     {"Session ID format is valid",
      ?_assertEqual(true, proper:quickcheck(prop_session_id_format(), 100))},
     {"Session creation timestamps",
      ?_assertEqual(true, proper:quickcheck(prop_session_creation_timestamp(), 10))},
     {"Session empty metadata",
      ?_assertEqual(true, proper:quickcheck(prop_session_empty_metadata(), 50))},
     {"Session preserves metadata",
      ?_assertEqual(true, proper:quickcheck(prop_session_preserves_metadata(), 100))},
     {"Metadata set/get idempotent",
      ?_assertEqual(true, proper:quickcheck(prop_metadata_set_get_idempotent(), 100))},
     {"Metadata set overwrites",
      ?_assertEqual(true, proper:quickcheck(prop_metadata_set_overwrites(), 50))},
     {"Metadata missing returns undefined",
      ?_assertEqual(true, proper:quickcheck(prop_metadata_missing_returns_undefined(), 50))},
     {"Metadata operations commutative",
      ?_assertEqual(true, proper:quickcheck(prop_metadata_commutative(), 50))},
     {"Metadata preserves session ID",
      ?_assertEqual(true, proper:quickcheck(prop_metadata_preserves_session_id(), 100))},
     {"Metadata preserves created_at",
      ?_assertEqual(true, proper:quickcheck(prop_metadata_preserves_created_at(), 100))},
     {"Multiple sessions independent",
      ?_assertEqual(true, proper:quickcheck(prop_multiple_sessions_independent(), 50))},
     {"Session ID independent of metadata",
      ?_assertEqual(true, proper:quickcheck(prop_session_id_independent_of_metadata(), 50))},
     {"List sessions returns list",
      ?_assertEqual(true, proper:quickcheck(prop_list_sessions_returns_list(), 20))}].
