...
test_replay_from_disconnect() ->
    ...
    {ok, MissedEvents} =
        erlmcp_sse_event_store:get_events_since(SessionId, <<"session_replay_disconnect_3">>),
...
test_replay_after_multiple_disconnects() ->
    ...
    {ok, Batch2} =
        erlmcp_sse_event_store:get_events_since(SessionId, <<"session_replay_multiple_2">>),
    ...
    {ok, Batch3} =
        erlmcp_sse_event_store:get_events_since(SessionId, <<"session_replay_multiple_4">>),
...
test_replay_from_beginning() ->
    ...
    ok = add_events(SessionId, [{1, <<"first">>}, {2, <<"second">>}, {3, <<"third">>}]),
...
test_replay_from_middle() ->
    ...
    %% Replay from event 3
    {ok, Events} =
        erlmcp_sse_event_store:get_events_since(SessionId, <<"session_last_id_middle_3">>),
...
test_replay_from_end() ->
    ...
    {ok, Events} = erlmcp_sse_event_store:get_events_since(SessionId, <<"session_last_id_end_3">>),
...
test_replay_beyond_end() ->
    ...
    {ok, Events} =
        erlmcp_sse_event_store:get_events_since(SessionId, <<"session_last_id_beyond_999">>),
...
test_client_reconnect_pattern() ->
    ...
    ok = add_events(SessionId, [{1, <<"initial1">>}, {2, <<"initial2">>}, {3, <<"initial3">>}]),

    %% 2. Client disconnects, server adds events 4-6
    ok = add_events(SessionId, [{4, <<"missed1">>}, {5, <<"missed2">>}, {6, <<"missed3">>}]),

    %% 3. Client reconnects with Last-Event-ID = 3
    {ok, MissedEvents} =
        erlmcp_sse_event_store:get_events_since(SessionId, <<"session_reconnect_pattern_3">>),
...
test_missed_events_recovery() ->
    ...
    ok =
        add_events(SessionId,
                   lists:map(fun(N) -> {N, <<"e", (integer_to_binary(N))/binary>>} end,
                             lists:seq(1, 10))),
    ...
    {ok, RecoveredEvents} =
        erlmcp_sse_event_store:get_events_since(SessionId, <<"session_missed_recovery_10">>),
...
test_incremental_replay() ->
    ...
    ok =
        add_events(SessionId,
                   lists:map(fun(N) -> {N, <<"event_", (integer_to_binary(N))/binary>>} end,
                             lists:seq(1, 100))),

    %% Client reconnects multiple times incrementally
    {ok, Batch1} = erlmcp_sse_event_store:get_events_since(SessionId, undefined),
    ?assertEqual(100, length(Batch1)),

    %% Simulate client processes first 25, reconnects
    {ok, Batch2} =
        erlmcp_sse_event_store:get_events_since(SessionId, <<"session_incremental_replay_25">>),
    ?assertEqual(75, length(Batch2)),

    %% Client processes next 25, reconnects again
    {ok, Batch3} =
        erlmcp_sse_event_store:get_events_since(SessionId, <<"session_incremental_replay_50">>),
