...
stress_test_() ->
    {setup,
     fun setup/0,
     fun cleanup/1,
     fun(_Pid) -> [?_test(test_high_concurrency_load()), ?_test(test_rapid_fire_operations())] end}.
...
test_rapid_fire_operations() ->
    ...
    NumOps = 1000,
    lists:foreach(fun(N) -> erlmcp_sse_event_store:add_event(SessionId, N, <<"data">>) end,
                  lists:seq(1, NumOps)),
...
race_condition_test_() ->
    {setup,
     ...
     fun cleanup/1,
     fun(_Pid) ->
        [?_test(test_concurrent_clear_and_add()), ?_test(test_concurrent_session_access())]
     end}.
