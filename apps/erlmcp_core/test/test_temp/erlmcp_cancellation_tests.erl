...
test_multiple_operations() ->
    ...
    lists:foreach(fun(Token) -> ?assertEqual(ok, erlmcp_cancellation:check(Token)) end, Tokens),
...
test_set_cleanup_handler() ->
    ?assertEqual(ok,
                 erlmcp_cancellation:set_cleanup_handler(<<"tools/call">>,
                                                         erlmcp_test_cleanup_handler)).
...
cancellation_shutdown_test_() ->
    {setup,
     ...
     fun(Pid) -> [?_test(test_shutdown_cancels_all_operations(Pid))] end}.
...
test_concurrent_cancellations() ->
    ...
    lists:foreach(fun(Token) -> spawn(fun() -> erlmcp_cancellation:cancel(Token) end) end, Tokens),
