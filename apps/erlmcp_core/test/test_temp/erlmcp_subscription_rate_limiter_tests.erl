...
%% Test state record
-record(test_state,
        {subscription_pid :: pid(), subscriber_pid :: pid(), subscription_id :: binary()}).
...
setup() ->
    ...
    #{subscription_pid => SubscriptionPid, rate_limiter_pid => RateLimiterPid}.
...
token_bucket_creation_test() ->
    ...
    after
        cleanup(#{subscription_pid => whereis(erlmcp_subscription),
                  rate_limiter_pid => whereis(erlmcp_rate_limiter)})
    end.
...
test_allow_within_limit() ->
    ...
    lists:foreach(fun(I) -> ?assertEqual(ok, erlmcp_subscription:notify(SubscriptionId, {msg, I}))
                  end,
                  lists:seq(1, 5)),
...
test_block_exceeds_limit() ->
    ...
    lists:foreach(fun(I) -> erlmcp_subscription:notify(SubscriptionId, {msg, I}) end,
                  lists:seq(1, 20)),
...
test_sliding_window_sliding() ->
    ...
    % 6th request should fail
    ?assertEqual({error, exceeded}, erlmcp_rate_limiter:check_sliding_window(Window5, 5, 50)),
