...
test_call_tool_uninitialized(Client) ->
    ?assertMatch({error, {not_initialized, pre_initialization, _}},
                 erlmcp_client:call_tool(Client, <<"t">>, #{})).
        ...
        {ok, _} ->
            Args =
                #{str => <<"s">>,
                  ...
                  nested => #{k => v},
                  list => [1, 2, 3]},
            case erlmcp_client:call_tool(Client, <<"complex">>, Args) of
                {ok, R} ->
                    ?assert(is_map(R));
                {error, _} ->
                    ?assert(true)
...
test_call_tool_empty_args(Client) ->
    case erlmcp_client:initialize(Client, #mcp_client_capabilities{}) of
        {ok, _} ->
            case erlmcp_client:call_tool(Client, <<"noargs">>, #{}) of
                {ok, R} ->
                    ?assert(is_map(R));
                {error, _} ->
                    ?assert(true)
...
test_call_nonexistent_tool(Client) ->
    case erlmcp_client:initialize(Client, #mcp_client_capabilities{}) of
        {ok, _} ->
            ?assertMatch({error, _}, erlmcp_client:call_tool(Client, <<"nonexistent_xyz">>, #{}));
        {error, _} ->
            ?assert(true)
    end.
...
test_list_resource_templates(Client) ->
    case erlmcp_client:initialize(Client, #mcp_client_capabilities{}) of
        {ok, _} ->
            case erlmcp_client:list_resource_templates(Client) of
                {ok, T} ->
                    ?assert(is_list(T));
                {error, _} ->
                    ?assert(true)
...
test_read_resource(Client) ->
    case erlmcp_client:initialize(Client, #mcp_client_capabilities{}) of
        {ok, _} ->
            case erlmcp_client:read_resource(Client, <<"file:///t.txt">>) of
                {ok, C} ->
                    ?assert(is_map(C));
                {error, _} ->
                    ?assert(true)
...
test_subscribe_resource(Client) ->
    case erlmcp_client:initialize(Client, #mcp_client_capabilities{}) of
        {ok, _} ->
            case erlmcp_client:subscribe_to_resource(Client, <<"file:///t.txt">>) of
                ok ->
                    ?assert(true);
                {error, _} ->
                    ?assert(true)
...
test_unsubscribe_resource(Client) ->
    case erlmcp_client:initialize(Client, #mcp_client_capabilities{}) of
        {ok, _} ->
            case erlmcp_client:unsubscribe_from_resource(Client, <<"file:///t.txt">>) of
                ok ->
                    ?assert(true);
                {error, _} ->
                    ?assert(true)
...
test_list_prompts(Client) ->
    case erlmcp_client:initialize(Client, #mcp_client_capabilities{}) of
        {ok, _} ->
            case erlmcp_client:list_prompts(Client) of
                {ok, P} ->
                    ?assert(is_list(P));
                {error, _} ->
                    ?assert(true)
...
test_get_prompt(Client) ->
    case erlmcp_client:initialize(Client, #mcp_client_capabilities{}) of
        {ok, _} ->
            case erlmcp_client:get_prompt(Client, <<"test_prompt">>) of
                {ok, P} ->
                    ?assert(is_map(P));
                {error, _} ->
                    ?assert(true)
...
test_complete_default_timeout(Client) ->
    case erlmcp_client:initialize(Client, #mcp_client_capabilities{}) of
        {ok, _} ->
            case erlmcp_client:complete(Client, <<"ref">>, <<"partial">>) of
                {ok, C} ->
                    ?assert(is_map(C));
                {error, _} ->
                    ?assert(true)
...
test_complete_custom_timeout(Client) ->
    case erlmcp_client:initialize(Client, #mcp_client_capabilities{}) of
        {ok, _} ->
            case erlmcp_client:complete(Client, <<"ref">>, <<"partial">>, 10000) of
                {ok, C} ->
                    ?assert(is_map(C));
                {error, _} ->
                    ?assert(true)
...
test_complete_invalid_ref(Client) ->
    case erlmcp_client:initialize(Client, #mcp_client_capabilities{}) of
        {ok, _} ->
            ?assertMatch({error, _}, erlmcp_client:complete(Client, <<>>, <<"p">>));
        {error, _} ->
            ?assert(true)
    end.
...
test_batch_request_execution(Client) ->
    try
        ?assertMatch({ok, 3},
                     erlmcp_client:with_batch(Client,
                                              fun(Bc) ->
                                                 erlmcp_client:send_batch_request(Bc,
                                                                                  <<"b1">>,
                                                                                  <<"tools/list">>,
                                                                                  #{}),
                                                 erlmcp_client:send_batch_request(Bc,
...
setup_with_server() ->
    ...
    Server =
        case erlmcp_server:start_link(<<"test_srv_tools">>, ServerCaps) of
            {ok, SP} ->
                SP;
            {error, _} ->
                undefined
...
cleanup_with_server({undefined, undefined}) ->
    cleanup_application(ok);
cleanup_with_server({Client, undefined}) ->
    erlmcp_client:stop(Client),
    cleanup_application(ok);
cleanup_with_server({undefined, Server}) ->
    erlmcp_server:stop(Server),
    cleanup_application(ok);
cleanup_with_server({Client, Server}) ->
    erlmcp_client:stop(Client),
    erlmcp_server:stop(Server),
    cleanup_application(ok).
