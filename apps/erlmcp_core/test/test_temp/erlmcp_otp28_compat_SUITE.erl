...
-export([all/0, groups/0, init_per_suite/1, end_per_suite/1, init_per_testcase/2,
         end_per_testcase/2]).
%% Test cases
-export([test_otp_version_detection/1, test_platform_defines/1, test_json_module_available/1,
         test_backward_compatibility/1, test_dirty_scheduler_suspend/1,
...
all() ->
    [{group, version_detection}, {group, feature_availability}, {group, backward_compat}].

groups() ->
    [{version_detection, [sequence], [test_otp_version_detection, test_platform_defines]},
     {feature_availability,
      [sequence],
      [test_json_module_available, test_dirty_scheduler_suspend, test_feature_flags_loaded]},
     {backward_compat, [sequence], [test_backward_compatibility]}].
...
init_per_suite(Config) ->
    ...
    [{otp_release, OTPRelease}, {otp_version, OTPVersion}, {features, Features} | Config].
...
test_feature_flags_loaded(Config) ->
    ...
    ExpectedFeatures = [json_module, priority_messages, process_iterator, dirty_scheduler_suspend],

    lists:foreach(fun(Feature) -> ?assert(proplists:is_defined(Feature, Features)) end,
                  ExpectedFeatures).
...
has_process_iterator() ->
    try
        ...
        error:undef ->
            false;
        _:_ ->
            true  % Might fail for other reasons, but API exists
    end.
