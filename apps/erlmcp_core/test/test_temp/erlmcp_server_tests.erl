...
tool_tests() ->
    ...
    ok =
        erlmcp_server:add_tool_full(Server,
                                    <<"tool_5">>,
                                    <<"Minimal">>,
                                    fun(_) -> <<"ok">> end,
                                    #{}),
...
prompt_tests() ->
    ...
    Args2 =
        [#mcp_prompt_argument{name = <<"topic">>,
                              description = <<"Topic to discuss">>,
                              required = true},
         #mcp_prompt_argument{name = <<"length">>,
    ...
    Handler2 =
        fun(Args) ->
           [#{role => <<"system">>,
              content =>
                  #{type => <<"text">>,
                    text => <<"Topic: ", (maps:get(<<"topic">>, Args))/binary>>}}]
        end,
    ...
    Args3 =
        [#mcp_prompt_argument{name = <<"query">>,
    ...
    Handler3 =
        fun(_Args) ->
           [#{role => <<"user">>, content => #{type => <<"text">>, text => <<"Schema validated">>}}]
        end,
    ok =
        erlmcp_server:add_prompt_with_args_and_schema(Server,
                                                      PromptName3,
                                                      Handler3,
                                                      Args3,
                                                      InputSchema),
    ...
    [begin
         ...
         H = fun(_) -> [#{role => <<"user">>, content => #{type => <<"text">>, text => <<"ok">>}}]
             end,
...
notification_tests() ->
    ...
    [begin ?assertEqual(ok, erlmcp_server:notify_resources_changed(Server)) end
     || _ <- lists:seq(1, 5)],
...
delete_tests() ->
    ...
    Handler1 =
        fun(_) -> [#{role => <<"user">>, content => #{type => <<"text">>, text => <<"ok">>}}] end,
    ok = erlmcp_server:add_prompt(Server, PromptName1, Handler1),
...
resource_link_tests() ->
    ...
    try
        ...
        _:_ ->
            ok  %% Expected to possibly fail validation
    end,
...
capability_tests() ->
    ...
    ServerAll =
        start_server_with_caps(#mcp_server_capabilities{resources = #mcp_capability{enabled = true},
                                                        tools = #mcp_capability{enabled = true},
                                                        prompts = #mcp_capability{enabled = true}}),
    ...
    ServerTools =
        start_server_with_caps(#mcp_server_capabilities{resources =
                                                            #mcp_capability{enabled = false},
                                                        tools = #mcp_capability{enabled = true},
                                                        prompts =
                                                            #mcp_capability{enabled = false}}),
    ...
    {setup,
     ...
     fun(Server) ->
        [?_test(begin
                    ...
                    ?assertEqual(ok,
                                 erlmcp_server:add_tool_with_description(Server,
                                                                         <<"tool_normal">>,
                                                                         NormalDesc,
                                                                         fun(_) -> ok end))
                end),
...
handler_cleanup_on_shutdown_test_() ->
    {timeout,
     ...
     fun() ->
        ...
        %% Register handlers
        ?assertEqual(ok,
                     erlmcp_server:register_notification_handler(Server, <<"method1">>, self())),
        ?assertEqual(ok,
                     erlmcp_server:register_notification_handler(Server, <<"method2">>, self())),
...
subscription_integration_test_() ->
    {timeout,
     ...
     fun() ->
        ...
        {ok, Server} =
            erlmcp_server:start_link(ServerId,
                                     #mcp_server_capabilities{resources =
                                                                  #mcp_capability{enabled = true}}),
        ...
        try
            ...
            ?assertEqual(ok,
                         erlmcp_server:notify_resource_updated(Server, Uri, #{<<"test">> => true})),
...
authorization_tests() ->
    ...
    {timeout,
     ...
     fun() ->
        ...
        ?assertEqual([{error, unauthorized}, {error, unauthorized}, {error, unauthorized}],
                     Results),
