...
setup() ->
    ...
    {ok, Pid} =
        erlmcp_completion:start_link(#{cache_ttl => 60,
                                       cache_max_size => 100,
                                       max_results => 10,
                                       rate_limit => 10,
...
test_add_handler(Pid) ->
    ...
    Handler =
        fun(_, Argument, _) ->
           ...
           Filtered =
               lists:filter(fun(#{value := V}) -> binary:match(V, Value) =/= nomatch end,
                            Completions),
           {ok, Filtered}
        end,
...
test_cache_hit(Pid) ->
    ...
    receive
        {invoked, _} ->
            ?assert(false, "Handler invoked twice - cache failed")
    after 100 ->
        ok
    end.
...
test_max_results(Pid) ->
    ...
    Handler =
        fun(_, _, _) ->
           %% Return 20 items
           {ok,
            [#{value => <<"item_", (integer_to_binary(N))/binary>>,
...
completion_rate_limiting_test_() ->
    {setup,
     fun setup/0,
     fun cleanup/1,
     fun(Pid) -> [{"Rate limit enforcement", fun() -> test_rate_limit(Pid) end}] end}.
