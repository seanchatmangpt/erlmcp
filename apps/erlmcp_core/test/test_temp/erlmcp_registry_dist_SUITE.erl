...
%% Test cases
-export([single_node_disabled/1, single_node_enabled/1, multi_node_registration/1,
         multi_node_failover/1, split_brain_detection/1, global_name_conflict/1,
         node_reconnection/1]).
...
all() ->
    [{group, single_node}, {group, multi_node}].

groups() ->
    [{single_node, [sequence], [single_node_disabled, single_node_enabled]},
     {multi_node,
      [sequence],
...
multi_node_registration(Config) ->
    ...
    {ok, {Node1, Pid1, _}} =
        rpc:call(Node2, erlmcp_registry_dist, whereis_global, [{server, multi_server}]),
...
start_slave_nodes(Count, N, Acc, Config) ->
    ...
    case ct_slave:start(NodeName,
                        [{boot_timeout, 60},
                         ...
                         {kill_if_fail, true},
                         {erl_flags,
                          "-setcookie erlmcp_test_cookie -kernel dist_auto_connect once"}])
    of
        {ok, Node} ->
...
setup_cluster_on_nodes(Nodes) ->
    lists:foreach(fun(Node) ->
                     ...
                     rpc:call(Node,
                              application,
                              set_env,
                              [erlmcp_core, cluster_nodes, Nodes -- [Node]]),
                     rpc:call(Node,
                              application,
                              set_env,
                              [erlmcp_core, cluster_cookie, erlmcp_test_cookie])
                  end,
                  Nodes).
