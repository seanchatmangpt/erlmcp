...
tool_execution_test() ->
    ...
    {ok, Result1} =
        erlmcp_consensus_poc:execute_tool(Node1, <<"add">>, #{<<"a">> => 5, <<"b">> => 3}),
    {ok, Result2} =
        erlmcp_consensus_poc:execute_tool(Node2, <<"multiply">>, #{<<"x">> => 4, <<"y">> => 2}),
...
leader_failover_test() ->
    ...
    {ok, Result} =
        erlmcp_consensus_poc:execute_tool(FirstRemaining,
                                          <<"add">>,
                                          #{<<"a">> => 10, <<"b">> => 20}),
...
exactly_once_semantics_test() ->
    ...
    Nodes = [Node1, Node2, Node3],
    [erlmcp_consensus_poc:execute_tool(Node1, <<"add">>, #{<<"a">> => I, <<"b">> => I})
     || I <- lists:seq(1, 5)],
    [erlmcp_consensus_poc:execute_tool(Node2, <<"multiply">>, #{<<"x">> => I, <<"y">> => I})
     || I <- lists:seq(1, 3)],
    [erlmcp_consensus_poc:execute_tool(Node3, <<"subtract">>, #{<<"a">> => 10, <<"b">> => I})
     || I <- lists:seq(1, 4)],
    timer:sleep(100),
