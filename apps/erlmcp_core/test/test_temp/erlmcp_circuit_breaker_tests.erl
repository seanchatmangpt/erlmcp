...
setup() ->
    ...
    lists:foreach(fun(Name) -> catch erlmcp_circuit_breaker:unregister_breaker(Name) end,
                  [test_breaker, breaker1, breaker2, breaker3]),
...
fallback_on_open_test() ->
    ...
    % Call with fallback
    Result =
        erlmcp_circuit_breaker:call_with_fallback(test_breaker,
                                                  fun() -> {ok, primary} end,
                                                  fun() -> {ok, fallback} end),
...
call_timeout_test() ->
    ...
    Result =
        try
            erlmcp_circuit_breaker:call(test_breaker,
                                        fun() ->
                                           timer:sleep(1000),
                                           {ok, slow}
                                        end,
