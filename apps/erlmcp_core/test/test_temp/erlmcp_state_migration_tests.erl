...
rate_limiter_legacy_state_test() ->
    ...
    %% Cleanup
    ets:delete(
        maps:get(clients, LegacyStateMap)),
    ets:delete(
        maps:get(violations, LegacyStateMap)),
    ok.
...
rate_limiter_downgrade_test() ->
    ...
    DowngradedState = erlmcp_rate_limiter:migrate_rate_limiter_state({down, v2}, State1, []),
...
version_field_present_test() ->
    ...
    case whereis(erlmcp_rate_limiter) of
        undefined ->
            ok;
        OldPid1 when is_pid(OldPid1) ->
            gen_server:stop(OldPid1)
    end,
    case whereis(erlmcp_session_manager) of
        undefined ->
            ok;
        OldPid2 when is_pid(OldPid2) ->
            gen_server:stop(OldPid2)
    end,
    case whereis(erlmcp_cache) of
        undefined ->
            ok;
        OldPid3 when is_pid(OldPid3) ->
            gen_server:stop(OldPid3)
    end,
