...
test_duplicate_registration_different_pid(#{}) ->
    ...
    erlmcp_test_helpers:with_test_server(<<"dup_server_1">>,
                                         fun(ServerPid1) ->
                                            erlmcp_test_helpers:with_test_server(<<"dup_server_2">>,
                                                                                 fun(ServerPid2) ->
                                                                                    [?_test(begin
...
test_route_to_nonexistent_server(#{}) ->
    ...
    [?_test(begin
                ...
                timer:sleep(100)            % If we got here without crash, test passed
            end)].
...
test_server_death_cleans_up_bindings(#{}) ->
    ...
    [?_test(begin
     ...
     ?_test(begin
                ...
                ?assertMatch({error, not_found},
                             erlmcp_registry:get_server_for_transport(Transport1Id)),
                ?assertMatch({error, not_found},
                             erlmcp_registry:get_server_for_transport(Transport2Id))
            end)].
...
test_unknown_handle_call(#{registry_pid := RegistryPid}) ->
    [?_test(begin
                ...
                ?assertMatch({error, unknown_request},
                             gen_server:call(RegistryPid, unknown_request))
            end)].
