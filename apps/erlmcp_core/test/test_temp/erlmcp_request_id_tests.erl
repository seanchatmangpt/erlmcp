...
safe_increment_negative_test_() ->
    ?_assertError(function_clause, begin erlmcp_request_id:safe_increment(-1) end).
...
realistic_workflow_test() ->
    ...
    % Increment to 100
    {ok, Id100} =
        lists:foldl(fun(_, {ok, AccId}) -> erlmcp_request_id:safe_increment(AccId) end,
                    {ok, 2},
                    lists:seq(1, 98)),
    ...
    proper:forall(
        ...
        fun(Id) ->
           case erlmcp_request_id:safe_increment(Id) of
               {ok, NextId} when NextId > Id ->
                   true;
               {error, overflow} when Id == ?MAX_SAFE_REQUEST_ID ->
                   true;
               _ ->
                   false
           end
...
prop_increment_threshold_consistency() ->
    proper:forall(
        proper_types:range(1, ?MAX_SAFE_REQUEST_ID - 1),
        fun(Id) ->
...
prop_check_thresholds_valid_levels() ->
    proper:forall(
        proper_types:range(0, ?MAX_SAFE_REQUEST_ID),
        fun(Id) ->
