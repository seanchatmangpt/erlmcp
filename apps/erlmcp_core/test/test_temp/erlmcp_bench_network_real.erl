...
-export([run_all/0, run_workload/1, run_workload/2, list_workloads/0, validate_metrics/1,
         metrics_to_json/1]).
%% Internal exports
...
-record(metrics,
        {workload_id :: binary(), benchmark = <<"network_real">> :: binary(), transport :: binary(),
         connections :: pos_integer(), duration_s :: pos_integer(),
         messages_sent :: non_neg_integer(), throughput_msg_per_s :: float(),
         bandwidth_mib_per_s :: float(), latency_p50_us :: float(), latency_p95_us :: float(),
         latency_p99_us :: float(), connection_setup_avg_ms :: float(),
         connection_failures :: non_neg_integer(), timeout_count :: non_neg_integer(),
         memory_rss_mib_per_node :: float(), memory_heap_mib_per_conn :: float(),
         cpu_percent_per_node :: float(), scope = <<"per_node">> :: binary(),
         tls_enabled :: boolean(), precision = <<"microsecond">> :: binary(),
         environment :: binary(), timestamp :: integer(), actual_duration_s :: float(),
...
workloads() ->
    #{%% TCP workloads - High performance, low overhead
      tcp_burst_100_1kib =>
          #workload{id = <<"tcp_burst_100_1kib">>,
                    transport = tcp,
                    connections = 100,
                    duration_s = 60,
      ...
      tcp_sustained_10k_1kib =>
          #workload{id = <<"tcp_sustained_10k_1kib">>,
                    transport = tcp,
                    connections = 10000,
                    duration_s = 300,
                    payload_size_bytes = 1024,
                    ...
                    messages_per_conn = undefined},
      tcp_sustained_10k_100kib =>
          #workload{id = <<"tcp_sustained_10k_100kib">>,
                    transport = tcp,
                    connections = 10000,
                    duration_s = 300,
      ...
      http_burst_100_1kib =>
          #workload{id = <<"http_burst_100_1kib">>,
                    transport = http,
                    connections = 100,
                    duration_s = 60,
                    payload_size_bytes = 1024,
      ...
      http_sustained_5k_1kib =>
          #workload{id = <<"http_sustained_5k_1kib">>,
                    transport = http,
                    connections = 5000,
                    duration_s = 300,
                    payload_size_bytes = 1024,
      ...
      http1_sustained_2k_512b =>
          #workload{id = <<"http1_sustained_2k_512b">>,
                    transport = http,
                    connections = 2000,
                    duration_s = 300,
...
run_all() ->
    ...
    %% Run workloads
    Results =
        lists:map(fun(WorkloadName) ->
                     try
                         run_workload(WorkloadName)
                     catch
                         Class:Reason:Stack ->
                             io:format("ERROR in ~p: ~p:~p~n~p~n",
                                       [WorkloadName, Class, Reason, Stack]),
...
run_workload(WorkloadName, Opts) ->
    ...
    case maps:get(WorkloadName, Definitions, undefined) of
        ...
        Workload ->
            ...
            Result =
                case FinalWorkload#workload.transport of
                    tcp ->
...
benchmark_tcp(Workload) ->
    ...
    io:format("~B clients connected (avg setup: ~.2f ms)~n", [length(ClientPids), AvgConnSetupMs]),
    timer:sleep(1000),  % Let connections stabilize

    %% Start metrics collection
    MetricsCollectorPid = spawn_link(?MODULE, tcp_server_stats_loop, [self(), WallClockStart, []]),
    ...
    FinalMetrics =
        aggregate_tcp_metrics(Workload,
                              ClientResults,
                              ActualDurationMs,
                              InitialMemory,
                              FinalMemory,
...
benchmark_http(Workload) ->
    ...
    {TlsHandshakeUs, _} =
        timer:tc(fun() -> establish_sample_connection(Workload#workload.protocol, ServerPort) end),
    ...
    Workers =
        [spawn_link(?MODULE,
                    ...
                    [Parent,
                     I,
                     Workload#workload.messages_per_conn,
    ...
    io:format("~B HTTP workers started (avg setup: ~.2f ms)~n", [length(Workers), AvgConnSetupMs]),
    ...
    %% Build metrics record
    FinalMetrics =
        aggregate_http_metrics(Workload,
                               WorkerResults,
                               ActualDurationMs,
                               InitialMemory,
                               FinalMemory,
        ...
        false ->
            ...
            case gen_tcp:send(Socket, [Payload, <<"\n">>]) of
                ok ->
                    ...
                    NewLatencies =
                        case length(Latencies) < 10000 of
                            true ->
                                [LatencyUs | Latencies];  % Sample first 10K
                            false ->
                                Latencies
                        end,

                    tcp_client_send_loop(ParentPid,
                                         Socket,
                                         Payload,
                                         EndTime,
                                         NewLatencies,
...
send_http_messages(GunPid, N, MsgSize, EndTime, Acc) ->
    ...
    case erlang:monotonic_time(millisecond) >= EndTime of
        ...
        false ->
            ...
            StartUs = erlang:monotonic_time(microsecond),
            StreamRef =
                gun:post(GunPid, "/mcp", [{<<"content-type">>, <<"application/json">>}], Message),
...
start_http_server(Port) ->
    ...
    Dispatch = cowboy_router:compile([{'_', [{"/mcp", http_bench_handler, []}]}]),

    {ok, _} =
        cowboy:start_clear(http_bench_server, [{port, Port}], #{env => #{dispatch => Dispatch}}),
...
aggregate_tcp_metrics(Workload,
    ...
    case SortedLatencies of
        [] ->
            #metrics{workload_id = Workload#workload.id,
                     transport = <<"tcp">>,
...
aggregate_http_metrics(Workload,
    ...
    #metrics{workload_id = Workload#workload.id,
             transport =
                 case Workload#workload.protocol of
                     http2 ->
                         <<"http_2.0">>;
                     http ->
                         <<"http_1.1">>
...
validate_and_save(Metrics, WorkloadName) ->
    ...
    case validate_metrics(JsonMap) of
        ok ->
            ...
            Filename =
                lists:flatten(
                    io_lib:format("bench/results/network_real_~s_~s.json",
...
metrics_to_json(Metrics) ->
    BaseMap =
        #{<<"workload_id">> => Metrics#metrics.workload_id,
          ...
          <<"connection_setup_avg_ms">> =>
              round(Metrics#metrics.connection_setup_avg_ms * 100) / 100,
          <<"connection_failures">> => Metrics#metrics.connection_failures,
          <<"timeout_count">> => Metrics#metrics.timeout_count,
          <<"memory_rss_mib_per_node">> =>
              round(Metrics#metrics.memory_rss_mib_per_node * 100) / 100,
          <<"memory_heap_mib_per_conn">> =>
              round(Metrics#metrics.memory_heap_mib_per_conn * 1000000) / 1000000,
    ...
    case Metrics#metrics.protocol of
        undefined ->
            BaseMap;
        Protocol ->
            BaseMap#{<<"protocol">> => Protocol,
                     <<"tls_handshake_avg_us">> =>
                         round(Metrics#metrics.tls_handshake_avg_us * 100) / 100,
...
create_json_rpc_payload(TargetBytes) ->
    ...
    BaseMsg =
        #{<<"jsonrpc">> => <<"2.0">>,
          ...
          <<"params">> => #{<<"name">> => <<"benchmark_tool">>, <<"arguments">> => #{}}},
...
apply_options(Workload, Opts) ->
    Workload#workload{connections = maps:get(connections, Opts, Workload#workload.connections),
                      duration_s = maps:get(duration_s, Opts, Workload#workload.duration_s),
                      payload_size_bytes =
                          maps:get(payload_size_bytes, Opts, Workload#workload.payload_size_bytes)}.

calculate_cpu_usage(InitialStats, FinalStats) ->
    TotalActive =
        lists:sum([ActiveF - ActiveI
                   || {{_, ActiveI, _}, {_, ActiveF, _}} <- lists:zip(InitialStats, FinalStats)]),
    TotalTime =
        lists:sum([TotalF - TotalI
                   || {{_, _, TotalI}, {_, _, TotalF}} <- lists:zip(InitialStats, FinalStats)]),
...
filter_runnable_workloads(Workloads, TcpAvailable, HttpAvailable) ->
    lists:filter(fun(Name) ->
                    case maps:get(Name, Workloads) of
                        #workload{transport = tcp} ->
                            TcpAvailable;
                        #workload{transport = http} ->
                            HttpAvailable
                    end
                 end,
                 maps:keys(Workloads)).
...
print_summary(Results) ->
    ...
    lists:foreach(fun ({ok, Metrics}) ->
                          Status =
                              case Metrics#metrics.throughput_msg_per_s
                                   >= get_target(Metrics#metrics.workload_id)
                              of
                                  true ->
                                      "PASS";
                                  false ->
...
get_target(WorkloadId) ->
    ...
    case [W || {_, W} <- maps:to_list(Definitions), W#workload.id =:= WorkloadId] of
        [Workload | _] ->
            Workload#workload.target_msg_per_s;
        [] ->
            0
    end.
