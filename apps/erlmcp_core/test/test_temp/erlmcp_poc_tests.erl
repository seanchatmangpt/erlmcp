...
telemetry_poc_test_() ->
    {setup,
     ...
     fun(Pid) -> erlmcp_telemetry_poc:stop(Pid) end,
     fun(Pid) ->
...
telemetry_event_fires(Pid) ->
    ...
    receive
        {telemetry_event, test_event, Metadata} ->
            ok
    after 1000 ->
        ?assert(false)
    end.
...
pubsub_poc_test_() ->
    {setup,
     ...
     fun(Pid) -> erlmcp_pubsub_poc:stop(Pid) end,
     fun(Pid) ->
...
pubsub_subscribe_and_publish(Pid) ->
    ...
    receive
        {pubsub_message, test_topic, Message} ->
            ok
    after 1000 ->
        ?assert(false)
    end.
...
distributed_registry_poc_test_() ->
    {setup,
     ...
     fun(Pid) -> erlmcp_distributed_registry_poc:stop(Pid) end,
     fun(Pid) ->
...
streaming_poc_test_() ->
    {setup,
     ...
     fun(Pid) -> erlmcp_streaming_poc:stop(Pid) end,
     fun(Pid) ->
...
streaming_chunk_delivery(Pid) ->
    ...
    ok =
        erlmcp_streaming_poc:start_stream(Pid,
                                          StreamId,
                                          #{data => Data,
                                            chunk_size => 5,
                                            subscriber => self()}),
...
circuit_breaker_poc_test_() ->
    {setup,
     fun() ->
        {ok, Pid} =
            erlmcp_circuit_breaker_poc:start_link([{failure_threshold, 3},
                                                   {success_threshold, 2},
                                                   {timeout, 1000}]),
        Pid
     end,
     fun(Pid) -> erlmcp_circuit_breaker_poc:stop(Pid) end,
     fun(Pid) ->
...
pool_poc_test_() ->
    {setup,
     ...
     fun(Pid) -> erlmcp_pool_poc:stop(Pid) end,
     fun(Pid) ->
