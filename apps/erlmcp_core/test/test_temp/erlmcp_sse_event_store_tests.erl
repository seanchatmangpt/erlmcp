...
test_concurrent_add_events(_Pid) ->
    ...
    Pids =
        lists:map(fun(ProcessNum) ->
                     spawn(fun() ->
                              StartNum = ProcessNum * EventsPerProcess + 1,
                              lists:foreach(fun(EventNum) ->
                                               erlmcp_sse_event_store:add_event(SessionId,
                                                                                EventNum,
                                                                                <<"event_",
                                                                                  (integer_to_binary(EventNum))/binary>>)
                                            end,
                                            lists:seq(StartNum, StartNum + EventsPerProcess - 1))
                           end)
                  end,
                  lists:seq(0, NumProcesses - 1)),
...
test_concurrent_session_isolation(_Pid) ->
    ...
    NumSessions = 5,
    SessionIds =
        [<<"session_concurrent_iso_", (integer_to_binary(N))/binary>>
    ...
    Readers =
        lists:map(fun(_) ->
                     spawn(fun() ->
                              lists:foreach(fun(_) ->
                                               {ok, _Events} =
                                                   erlmcp_sse_event_store:get_events_since(SessionId,
                                                                                           undefined),
                                               timer:sleep(20)
                                            end,
                                            lists:seq(1, 10))
...
termination_test_() ->
    {setup,
     ...
     fun cleanup/1,
     fun(Pid) ->
        [?_test(test_terminate_cleans_all_tables(Pid)), ?_test(test_terminate_cancels_timer(Pid))]
     end}.
...
test_terminate_cleans_all_tables(Pid) ->
    ...
    lists:foreach(fun(SessionId) -> ok = add_events(SessionId, [{1, <<"e1">>}]) end, SessionIds),

    %% Verify tables exist
    TableNames = [{erlmcp_sse_event_store, S} || S <- SessionIds],
    lists:foreach(fun(TableName) -> ?assertNotEqual(undefined, ets:whereis(TableName)) end,
                  TableNames),
    ...
    lists:foreach(fun(TableName) -> ?assertEqual(undefined, ets:whereis(TableName)) end,
                  TableNames).
