...
-export([run/0, run/1, run_all/0, workloads/0, benchmark_registry/1, benchmark_queue/1,
         benchmark_pool/1, benchmark_session/1]).
...
run_workload(#{id := WorkloadId,
    ...
    AllLatencies =
        lists:flatten([maps:get(latencies, RegistryResult),
...
benchmark_registry(#{operations := Ops, workers := Workers}) ->
    ...
    Latencies =
        lists:flatten(
            lists:map(fun(_) -> rand:uniform(100) + 1 end, lists:seq(1, Ops))),
...
benchmark_queue(#{operations := Ops, workers := _Workers}) ->
    ...
    {Latencies, _FinalQ} =
        lists:foldl(fun(_, {AccLatencies, Q}) ->
                       Start = erlang:monotonic_time(microsecond),
...
benchmark_pool(#{operations := Ops, workers := _Workers}) ->
    ...
    lists:foreach(fun(WorkerId) -> ets:insert(PoolTable, {WorkerId, available}) end,
                  lists:seq(1, 10)),
    ...
    Latencies =
        lists:map(fun(_) ->
                     ...
                     WorkerId =
                         case ets:match(PoolTable, {'$1', available}, 1) of
                             {[[Id]], _} ->
...
benchmark_session(#{operations := Ops, workers := Workers}) ->
    ...
    SessionTable =
        ets:new(bench_session, [set, public, {write_concurrency, true}, {read_concurrency, true}]),
...
format_component_result(#{component := Component,
                          operations := Ops,
