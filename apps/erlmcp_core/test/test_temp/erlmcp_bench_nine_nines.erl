...
-export([run/0, run/1, baseline/0, overload_profile/0, full_validation/0,
         measure_latency_under_load/1, measure_control_plane_isolation/0,
         measure_memory_efficiency/1, measure_gc_pauses/1, measure_sustained_throughput/1]).
...
full_validation() ->
    ...
    AllPassed =
        LatencyPass andalso ControlPass andalso ThroughputPass andalso MemoryPass andalso GCPass,
    ...
    print_slo_result("  p999",
                     maps:get(p999, LatencyResult),
                     ?SLO_LATENCY_P999_US,
                     "µs",
                     LatencyPass),

    io:format("~nControl plane (health checks during data flood):~n"),
    print_slo_result("  Latency p99",
                     maps:get(p99, ControlResult),
                     ?SLO_CONTROL_PLANE_P99_US,
                     "µs",
                     ControlPass),
    ...
    print_slo_result("  Heap per connection",
                     maps:get(heap_per_conn_mib, MemoryResult),
                     ?SLO_HEAP_PER_CONN_MIB,
                     "MiB",
                     MemoryPass),
    print_slo_result("  RSS per node",
                     maps:get(rss_gib, MemoryResult),
                     ?SLO_RSS_PER_NODE_GIB,
                     "GiB",
                     MemoryPass),

    io:format("~nGC pause times:~n"),
    print_slo_result("  Max pause",
                     maps:get(max_pause_ms, GCResult),
                     ?SLO_GC_MAX_PAUSE_MS,
                     "ms",
                     GCPass),
    print_slo_result("  Mean pause",
                     maps:get(mean_pause_ms, GCResult),
                     ?SLO_GC_MEAN_PAUSE_MS,
                     "ms",
                     GCPass),
...
measure_registry_throughput() ->
    Operations = 1000000,
    Table =
        ets:new(bench_registry, [set, public, {read_concurrency, true}, {write_concurrency, true}]),
...
measure_session_throughput() ->
    Operations = 1000000,
    Table =
        ets:new(bench_session, [set, public, {read_concurrency, true}, {write_concurrency, true}]),
...
measure_memory_efficiency(ConnectionCount) ->
    ...
    MemHeap =
        lists:sum([element(2, erlang:process_info(Pid, heap_size)) * erlang:system_info(wordsize)
...
measure_gc_pauses(LoadType) ->
    ...
    WorkloadFun =
        case LoadType of
            baseline ->
                fun() -> baseline_workload() end;
            under_load ->
                fun() -> loaded_workload() end
        end,
...
measure_sustained_throughput(TargetMsgPerSec) ->
    ...
    WorkTable =
        ets:new(work_table, [set, public, {write_concurrency, true}, {read_concurrency, true}]),
...
identify_bottleneck(_LoadResult, LatencyResult) ->
    ...
    if P999 > ?SLO_LATENCY_P999_US * 2 ->
           tail_latency;
       P99 > ?SLO_LATENCY_P99_US * 2 ->
           high_latency;
       true ->
           cpu_bound
    end.
...
validate_latency_slos(#{p50 := P50,
                        p95 := P95,
                        p99 := P99,
                        p999 := P999}) ->
    P50 < ?SLO_LATENCY_P50_US
    andalso P95 < ?SLO_LATENCY_P95_US
    andalso P99 < ?SLO_LATENCY_P99_US
    andalso P999 < ?SLO_LATENCY_P999_US.
...
validate_memory_slos(#{heap_per_conn_mib := HeapPerConn, rss_gib := RSS}) ->
    HeapPerConn < ?SLO_HEAP_PER_CONN_MIB andalso RSS < ?SLO_RSS_PER_NODE_GIB.

validate_gc_slos(#{max_pause_ms := MaxPause, mean_pause_ms := MeanPause}) ->
    MaxPause < ?SLO_GC_MAX_PAUSE_MS andalso MeanPause < ?SLO_GC_MEAN_PAUSE_MS.
