...
test_global_registration() ->
    ...
    %% Verify registration
    ?assertMatch({ok, {_Node, TestPid, _Config}},
                 erlmcp_registry_dist:whereis_global({server, test_server})),
...
test_whereis_global() ->
    ...
    {ok, {Node, FoundPid, FoundConfig}} =
        erlmcp_registry_dist:whereis_global({server, where_server}),
...
test_list_global() ->
    ...
    ?assertEqual(ok,
                 erlmcp_registry_dist:register_global(transport, list_transport1, Transport1, #{})),
...
cluster_nodes_test_() ->
    {setup, fun setup/0, fun cleanup/1, [{"Can query cluster nodes", fun test_cluster_nodes/0}]}.
