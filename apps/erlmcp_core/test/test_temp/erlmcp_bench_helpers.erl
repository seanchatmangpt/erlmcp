...
-export([format_result_json/2, format_metric/3, format_metric/4, save_result/2, validate_and_save/2,
         generate_workload_id/2, get_environment_info/0, format_timestamp/0, format_timestamp/1]).
...
format_metric(Value, Unit, Scope) when is_number(Value), is_binary(Unit), is_binary(Scope) ->
    ...
    #{<<"value">> => Value, <<"unit">> => CompositeUnit}.
