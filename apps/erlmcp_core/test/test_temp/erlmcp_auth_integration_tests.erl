...
setup() ->
    ...
    {ok, RateLimiterPid} =
        erlmcp_auth_rate_limiter:start_link(#{max_attempts_per_second => 10,
                                              window_ms => 1000,
                                              max_failures => 5,
...
test_full_auth_flow() ->
    ...
    {ok, SessionId} =
        erlmcp_auth:authenticate(api_key,
                                 #{api_key => <<"test_key_123">>, ip_address => {192, 168, 1, 1}}),
...
test_multiple_failed_attempts() ->
    ...
    lists:foreach(fun(_) ->
                     {error, invalid_api_key} =
                         erlmcp_auth:authenticate(api_key, #{api_key => <<"bad_key">>})
                  end,
                  lists:seq(1, 3)),
