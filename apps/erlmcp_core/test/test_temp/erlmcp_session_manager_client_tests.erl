...
test_multiple_clients_isolated(_Pid) ->
    fun() ->
       ...
       {ok, Id1} =
           erlmcp_session_manager:create_session(#{client_id => <<"client_1">>, user => alice}),
       {ok, Id2} =
           erlmcp_session_manager:create_session(#{client_id => <<"client_2">>, user => bob}),
...
test_client_lifecycle_integration(_Pid) ->
    fun() ->
       ...
       {ok, SessionId} =
           erlmcp_session_manager:create_session(#{client_id => <<"client_lifecycle">>,
                                                   state => connecting}),
