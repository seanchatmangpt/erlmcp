...
test_jsonrpc_error_response_structure() ->
    ...
    ErrorObj =
        #{<<"code">> => Code,
          <<"message">> => Message,
          <<"data">> => #{<<"http_status">> => Status, <<"remediation_hint">> => Hint}},
...
category_distribution_test() ->
    ...
    lists:foreach(fun(Code) -> {ok, _, _, _, _, _} = erlmcp_refusal:get_metadata(Code) end,
                  AllCodes),
...
edge_case_message_content_test() ->
    ...
    DefinedCodes =
        [1001, 1002, 1003, 1004, 1005, 1011, 1012, 1013, 1014, 1015, 1016, 1021, 1022, 1023, 1024,
         1025, 1026, 1027, 1028, 1029, 1036, 1037, 1038, 1039, 1040, 1046, 1047, 1048, 1049, 1050,
         1051, 1052],
    ValidCodes =
        [C
         || C <- DefinedCodes,
            case erlmcp_refusal:get_metadata(C) of
                {ok, _, _, Message, _, _} when is_binary(Message), byte_size(Message) > 0 ->
                    true;
                _ ->
                    false
            end],
...
edge_case_hint_content_test() ->
    ...
    ValidCodes =
        [C
         || C <- DefinedCodes,
            case erlmcp_refusal:get_metadata(C) of
                {ok, _, _, _, Hint, _} when is_binary(Hint), byte_size(Hint) > 0 ->
                    true;
                _ ->
                    false
            end],
