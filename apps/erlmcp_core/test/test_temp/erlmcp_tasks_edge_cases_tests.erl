    ...
    lists:foreach(fun({N, TaskId}) ->
                     spawn(fun() ->
                              case N rem 3 of
                                  0 ->
                                      %% Complete task
                                      erlmcp_tasks:complete(undefined,
                                                            TaskId,
                                                            #{<<"done">> => true});
                                  1 ->
                                      %% Update status
                                      erlmcp_tasks:update_status(undefined,
                                                                 TaskId,
                                                                 <<"processing">>);
                                  2 ->
                                      %% Cancel task
