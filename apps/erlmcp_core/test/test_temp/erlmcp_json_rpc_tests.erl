...
test_create_error_with_data_record() ->
    Error =
        erlmcp_json_rpc:create_error_with_data(-32602,
                                               <<"Invalid Params">>,
                                               details,
                                               <<"missing field">>),
    ?assert(is_record(Error, mcp_error)),
...
error_classification_test_() ->
    {setup,
     ...
     fun cleanup/1,
     fun(_) ->
        [?_test(test_error_severity_critical()), ?_test(test_error_severity_error()),
         ?_test(test_error_severity_warning()), ?_test(test_error_severity_info()),
         ?_test(test_error_category_jsonrpc()), ?_test(test_error_category_mcp_core()),
         ?_test(test_error_category_content()), ?_test(test_error_category_resource()),
         ?_test(test_error_category_tool()), ?_test(test_error_category_prompt()),
         ?_test(test_error_category_auth()), ?_test(test_error_category_protocol()),
         ?_test(test_error_category_pagination()), ?_test(test_error_category_task()),
         ?_test(test_error_category_progress()), ?_test(test_error_category_completion()),
         ?_test(test_is_jsonrpc_standard_error()), ?_test(test_is_mcp_core_error()),
         ?_test(test_is_mcp_content_error()), ?_test(test_is_mcp_resource_error()),
         ?_test(test_is_mcp_tool_error()), ?_test(test_is_mcp_prompt_error()),
         ?_test(test_is_mcp_auth_error()), ?_test(test_is_mcp_protocol_error()),
         ?_test(test_is_mcp_pagination_error()), ?_test(test_is_mcp_task_error()),
         ?_test(test_is_mcp_progress_error()), ?_test(test_is_mcp_completion_error())]
     end}.
...
test_validate_all_error_codes() ->
    ...
    lists:foreach(fun(Code) -> ?assert(erlmcp_json_rpc:validate_error_code(Code)) end, ValidCodes).
...
test_all_error_helpers_produce_valid_json() ->
    ...
    lists:foreach(fun(Response) ->
                     ...
                     ?assert(erlmcp_json_rpc:validate_error_code(
                                 maps:get(<<"code">>, Error)))
                  end,
                  Errors).
...
test_error_code_range_coverage() ->
    ...
    lists:foreach(fun({Min, Max}) when Min =< Max ->
                     Found =
                         lists:any(fun(Code) -> erlmcp_json_rpc:validate_error_code(Code) end,
                                   lists:seq(Min, Max)),
                     ?assert(Found)
                  end,
                  Ranges).
