...
cleanup(#{registry_pid := RegistryPid, server_pids := ServerPids}) ->
    ...
    lists:foreach(fun(ServerPid) -> erlmcp_test_helpers:stop_test_server(ServerPid) end,
                  ServerPids),
...
test_transport_auto_binding(#{}) ->
    ...
    erlmcp_test_helpers:with_test_server(ServerId,
                                         fun(_ServerPid) ->
                                            TransportPid = spawn_transport_process(),
                                            ...
                                            [?_test(begin
                                             ...
                                             ?_test(begin
                                                        ...
                                                        ?assertMatch({ok, ServerId},
                                                                     erlmcp_registry:get_server_for_transport(TransportId))
                                                    end)]
                                         end).
    ...
    erlmcp_test_helpers:with_test_server(ServerId,
                                         fun(_ServerPid) ->
                                            ...
                                            [?_test(begin
                                                        ...
                                                        ?assertEqual(ok,
                                                                     erlmcp_registry:bind_transport_to_server(TransportId,
                                                                                                              ServerId))
                                                    end),
                                             ?_test(begin
                                                        ?assertMatch({ok, ServerId},
                                         ...
                                         fun(_ServerPid) ->
                                            ...
                                            [?_test(begin
                                             ...
                                             ?_test(begin
                                                        ...
                                                        ?assertEqual(ok,
                                                                     erlmcp_registry:unbind_transport(nonexistent_transport))
                                                    end)]
                                         end).
...
test_broadcast_routing(#{}) ->
    ...
    erlmcp_test_helpers:with_test_server(ServerId,
                                         fun(_ServerPid) ->
                                            ...
                                            TransportConfig = #{type => stdio},
                                            ok =
                                                erlmcp_registry:register_transport(Transport1Id,
                                                                                   Transport1Pid,
                                                                                   TransportConfig),
                                            ok =
                                                erlmcp_registry:register_transport(Transport2Id,
                                                                                   Transport2Pid,
                                                                                   TransportConfig),
                                            ok =
                                                erlmcp_registry:register_transport(Transport3Id,
                                                                                   Transport3Pid,
                                                                                   TransportConfig),

                                            % Bind all transports to server
                                            ok =
                                                erlmcp_registry:bind_transport_to_server(Transport1Id,
                                                                                         ServerId),
                                            ok =
                                                erlmcp_registry:bind_transport_to_server(Transport2Id,
                                                                                         ServerId),
                                            ok =
                                                erlmcp_registry:bind_transport_to_server(Transport3Id,
                                                                                         ServerId),
                                            ...
                                            [?_test(begin
                                                        ?assertEqual(ok,
                                                                     erlmcp_registry:route_to_transport(broadcast,
                                                                                                        ServerId,
                                                                                                        TestMessage)),
                                                        % Allow message delivery
                                                        timer:sleep(100)                % If we got here without crash, broadcast succeeded
                                                    end)]
                                         end).
...
test_message_routing_to_server(#{}) ->
    ...
    erlmcp_test_helpers:with_test_server(ServerId,
                                         fun(ServerPid) ->
                                            ...
                                            % Register and bind
                                            ok =
                                                erlmcp_registry:register_server(ServerId,
                                                                                ServerPid,
                                                                                #{}),
                                            ok =
                                                erlmcp_registry:register_transport(TransportId,
                                            ...
                                            [?_test(begin
                                                        ...
                                                        ?assertEqual(ok,
                                                                     erlmcp_registry:route_to_server(ServerId,
                                                                                                     TransportId,
                                                                                                     TestMessage))
                                                    end)]
                                         end).
    ...
    erlmcp_test_helpers:with_test_server(ServerId,
                                         fun(_ServerPid) ->
                                            ...
                                            [?_test(begin
                                                        ...
                                                        ?assertEqual(ok,
                                                                     erlmcp_registry:route_to_transport(TransportId,
                                                                                                        ServerId,
                                                                                                        TestResponse))
                                                    end)]
                                         end).
    ...
    erlmcp_test_helpers:with_test_server(ServerId,
                                         fun(_ServerPid) ->
                                            ...
                                            [?_test(begin
                                                        ...
                                                        ?assertEqual(ok,
                                                                     erlmcp_registry:route_message({server,
                                                                                                    ServerId},
                                                                                                   TestMessage))
                                                    end)]
                                         end).
    ...
    erlmcp_test_helpers:with_test_server(ServerId,
                                         fun(_ServerPid) ->
                                            ...
                                            [?_test(begin
                                                        ...
                                                        ?assertEqual(ok,
                                                                     erlmcp_registry:route_message({transport,
                                                                                                    TransportId},
                                                                                                   TestMessage))
                                                    end)]
                                         end).
                                         ...
                                         fun(_ServerPid) ->
                                            ...
                                            {Transports, TransportIds} =
                                                lists:unzip(
                                                    lists:map(fun(N) ->
                                                                 TransportId =
                                                                     list_to_atom("concurrent_transport_"
                                                                                  ++ integer_to_list(N)),
                                                                 TransportPid =
...
spawn_message_receiving_transport() ->
    Parent = self(),
    spawn_link(fun() -> loop_message_receiving_transport(Parent) end).
