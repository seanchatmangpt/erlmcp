...
setup() ->
    ...
    Config =
        #{api_keys =>
          ...
          jwt_keys => #{<<"test_kid">> => <<"test_public_key">>},
          oauth2 =>
              #{enabled => true, introspection_url => <<"https://auth.example.com/introspect">>},
          mtls => #{enabled => true, ca_cert => <<"/path/to/ca.pem">>},
    ...
    ok =
        erlmcp_auth:add_permission(<<"/api/data">>,
                                   <<"read">>,
                                   [<<"admin">>, <<"user">>, <<"guest">>]),
...
test_jwt_algorithm_confusion_attack() ->
    ...
    Header =
        jsx:encode(#{<<"alg">> => <<"none">>,
                     ...
                     <<"kid">> => <<"none_attack_kid">>}),
    Payload =
        jsx:encode(#{<<"sub">> => <<"attacker">>, <<"exp">> => erlang:system_time(second) + 3600}),
...
create_jwt_signed(_PrivateKey, Claims, Options) ->
    ...
    % Build payload
    Payload =
        case lists:member({skip_exp}, Options) of
            true ->
                maps:remove(<<"exp">>, Claims);
            false ->
                Claims
        end,

    Payload2 =
        case lists:member({skip_sub}, Options) of
            true ->
                maps:remove(<<"sub">>, Payload);
            false ->
                Payload
        end,
...
test_jwt_missing_expiration() ->
    ...
    Header =
        base64:encode(
            jsx:encode(#{<<"alg">> => <<"HS256">>, <<"kid">> => <<"missing_exp_kid">>})),
    Payload =
        base64:encode(
            jsx:encode(#{<<"sub">> => <<"user_test">>, <<"iat">> => erlang:system_time(second)})),
...
test_jwt_invalid_base64() ->
    ...
    Header =
        base64:encode(
            jsx:encode(#{<<"alg">> => <<"HS256">>, <<"kid">> => <<"invalid_base64_kid">>})),
...
test_mtls_validation() ->
    ...
    CertInfo =
        #{subject => #{cn => <<"client.example.com">>, o => <<"Test Org">>},
          issuer => <<"Test CA">>},
...
test_session_management() ->
    ...
    {error, invalid_session} =
        erlmcp_auth:check_permission(SessionId, <<"/api/admin">>, <<"write">>),
...
test_permission_invalid_session() ->
    ...
    {error, invalid_session} =
        erlmcp_auth:check_permission(<<"invalid_session_id">>, <<"/api/tools">>, <<"execute">>),
...
test_token_rotation() ->
    ...
    {error, invalid_session} =
        erlmcp_auth:check_permission(OldSessionId, <<"/api/tools">>, <<"execute">>),
...
test_revoked_token_validation() ->
    ...
    Header =
        base64:encode(
            jsx:encode(#{<<"alg">> => <<"HS256">>, <<"kid">> => <<"test_revoked_kid">>})),
    FutureExp = erlang:system_time(second) + 3600,
    Payload =
        base64:encode(
            jsx:encode(#{<<"sub">> => <<"user_test">>, <<"exp">> => FutureExp})),
    Signature = base64:encode(<<"sig">>),
    Token = <<Header/binary, ".", Payload/binary, ".", Signature/binary>>,
...
test_authenticate_with_rate_limiting() ->
    ...
    {ok, SessionId} =
        erlmcp_auth:authenticate(api_key,
                                 #{api_key => <<"test_key_456">>, ip_address => {127, 0, 0, 1}}),
...
test_unsupported_auth_method() ->
    ...
    {error, unsupported_auth_method} =
        erlmcp_auth:authenticate(unknown_method, #{credentials => <<"test">>}),
...
session_isolation_test_() ->
    {setup,
     ...
     [fun() ->
         ...
         {error, invalid_session} =
             erlmcp_auth:check_permission(Session1, <<"/api/admin">>, <<"write">>),
         ok = erlmcp_auth:check_permission(Session2, <<"/api/admin">>, <<"write">>),
