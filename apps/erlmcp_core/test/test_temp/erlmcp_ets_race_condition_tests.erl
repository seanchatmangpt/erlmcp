...
atomic_update_counter_test_() ->
    [{"Buggy pattern loses updates under concurrency", fun test_buggy_pattern_data_loss/0},
     {"Atomic update_counter preserves all updates", fun test_atomic_pattern_no_data_loss/0}].
...
test_concurrent_violation_increments() ->
           ...
           lists:foreach(fun(_) ->
                            ...
                            case ets:lookup(Table, ClientId) of
                                [{_, {Count, Timestamp}}] ->
                                    ...
                                    if TimeNowMs - Timestamp > ViolationWindow ->
                         ...
                         lists:seq(1, IncrementsPerProcess))
        end,
    ...
    ?debugFmt("Expected: ~p, Actual: ~p, Data Loss: ~p%",
              [ExpectedTotal, FinalCount, DataLossPercent]),
...
test_concurrent_role_additions() ->
    % Start auth server
    {ok, Pid} = erlmcp_auth:start_link(#{rate_limiter_enabled => false}),
    ...
    Pids =
        [spawn_link(fun() ->
         ...
         || ProcessNum <- lists:seq(0, NumProcesses - 1)],
...
test_atomic_pattern_no_data_loss() ->
    ...
    AtomicIncrement =
        fun() ->
           ...
           lists:foreach(fun(_) -> ets:update_counter(Table, counter, {2, 1}) end,
                         lists:seq(1, IncrementsPerProcess)),
           Parent ! {done, self()}
        end,
...
setup_rate_limiter() ->
    ...
    case whereis(erlmcp_rate_limiter) of
        undefined ->
            ok;
        _ ->
            gen_server:stop(erlmcp_rate_limiter)
    end,
    {ok}.
