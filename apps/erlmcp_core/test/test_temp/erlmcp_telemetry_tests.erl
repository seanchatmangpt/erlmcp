...
emit_tool_call_success_test() ->
    ...
    ?assertMatch({[erlmcp, tool, call],
                  #{duration_us := 1500, count := 1},
                  #{tool_name := <<"test_tool">>,
                    status := ok,
                    status_atom := ok}},
                 Event).
...
emit_resource_read_success_test() ->
    ...
    Event = receive_event(Collector, 1000),
    ?assertMatch({[erlmcp, resource, read],
                  #{duration_us := 800},
                  #{resource_uri := <<"resource://test">>, status := ok}},
                 Event).
...
emit_prompt_render_success_test() ->
    ...
    %% Verify: Event received
    Event = receive_event(Collector, 1000),
    ?assertMatch({[erlmcp, prompt, render],
                  #{duration_us := 200},
                  #{prompt_name := <<"greeting">>, status := ok}},
                 Event).
...
emit_prompt_render_error_test() ->
    ...
    ?assertMatch({[erlmcp, prompt, render],
                  #{duration_us := 50},
                  #{prompt_name := <<"bad_prompt">>,
                    status := error,
                    error_reason := invalid_template}},
                 Event).
...
emit_subscription_add_test() ->
    ...
    %% Verify: Event received
    Event = receive_event(Collector, 1000),
    ?assertMatch({[erlmcp, subscription, add],
                  #{count := 1},
                  #{subscription_uri := <<"resource://weather">>, subscriber_pid := SubscriberPid}},
                 Event),
...
multiple_events_test() ->
    ...
    telemetry:attach_many(<<"test-multi-handler">>,
                          Events,
...
concurrent_events_test() ->
    ...
    AllToolCalls =
        lists:all(fun({EventName, _, _}) -> EventName =:= [erlmcp, tool, call] end, Events),
    ?assert(AllToolCalls).
...
attach_test_handler(EventName, Collector) ->
    HandlerId =
        iolist_to_binary(io_lib:format("test-handler-~p", [erlang:unique_integer([positive])])),
