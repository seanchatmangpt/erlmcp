...
test_ping_http() ->
    ...
    {ok, Client} = erlmcp_client:start_link({http, #{url => Url, test_mode => true}}),
...
test_ping_concurrent() ->
    ...
    try
                         ...
                         spawn_link(fun() ->
                                       case erlmcp_client:ping(Client) of
                                           {ok, Result} ->
                                               ?assertEqual(#{},
                                                            Result,
                                                            "Concurrent ping should return empty object");
                                           {error, _} ->
                                               %% Expected in test mode
                                               ok
                                       end
        ...
        lists:foreach(fun(Pid) -> ?assertNot(erlang:is_process_alive(Pid)) end, Pids)
    after
        erlmcp_client:stop(Client),
        erlmcp_server:stop(Server),
        timer:sleep(100)
                     ...
                     spawn(fun() ->
                              case erlmcp_client:ping(Client) of
                                  {ok, Result} ->
                                      ?assertEqual(#{},
                                                   Result,
                                                   "Ping during operation should return empty object");
                                  {error, _} ->
                                      ok
                              end
                           end)
