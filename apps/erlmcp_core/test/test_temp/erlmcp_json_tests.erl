...
json_roundtrip_test() ->
    ...
    ?assertEqual(42,
                 erlmcp_json:decode(
                     erlmcp_json:encode(42))),
    ?assertEqual(3.14,
                 erlmcp_json:decode(
                     erlmcp_json:encode(3.14))),
    ?assertEqual(<<"test">>,
                 erlmcp_json:decode(
                     erlmcp_json:encode(<<"test">>))),
    ?assertEqual(true,
                 erlmcp_json:decode(
                     erlmcp_json:encode(true))),
    ?assertEqual(null,
                 erlmcp_json:decode(
                     erlmcp_json:encode(null))),

    %% Test: Array round-trip
    Array = [1, 2, 3, <<"test">>, true, null],
    ?assertEqual(Array,
                 erlmcp_json:decode(
                     erlmcp_json:encode(Array))),
...
json_large_payload_test() ->
    ...
    LargeMap =
        maps:from_list([{list_to_binary("key" ++ integer_to_list(N)), N}
                        || N <- lists:seq(1, 1000)]),
    ...
    DeepNested =
        lists:foldl(fun(_, Acc) -> #{<<"level">> => Acc} end,
                    #{<<"value">> => <<"deep">>},
                    lists:seq(1, 100)),
...
json_special_floats_test() ->
    ...
    ?assertEqual(0.0,
                 erlmcp_json:decode(
                     erlmcp_json:encode(0.0))),
    ?assertEqual(0,
                 erlmcp_json:decode(
                     erlmcp_json:encode(0))).
...
json_binary_keys_test() ->
    ...
    NestedBinaryMap = #{<<"outer">> => #{<<"inner">> => #{<<"deep">> => <<"value">>}}},
...
json_escaped_characters_test() ->
    ...
    ?assertEqual(<<"\r">>,
                 erlmcp_json:decode(
                     erlmcp_json:encode(<<"\r">>))),

    %% Test: Combined escape sequences
    ComplexString = <<"Line1\nLine2\tTabbed\"Quoted\"\\Backslash">>,
    ?assertEqual(ComplexString,
                 erlmcp_json:decode(
                     erlmcp_json:encode(ComplexString))).
