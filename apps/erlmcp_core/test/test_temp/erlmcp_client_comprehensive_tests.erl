...
-record(test_transport_state,
        {client_pid :: pid(), sent_messages = [] :: [binary()], message_queue = [] :: [binary()]}).
...
test_initialized_notification(Client) ->
    ...
    Self = self(),
    Handler = fun(Method, _Params) -> Self ! {notification, Method} end,
...
test_sampling_request(Client) ->
    ...
    Self = self(),
    SamplingHandler = fun(Method, Params) -> Self ! {sampling, Method, Params} end,
...
test_multiple_subscriptions(Client) ->
    ...
    Uris = [<<"test://resource/", (integer_to_binary(N))/binary>> || N <- lists:seq(1, 10)],

    Results = [erlmcp_client:subscribe_to_resource(Client, Uri) || Uri <- Uris],

    % All should fail with not_initialized
    AllErrors =
        lists:all(fun ({error, {not_initialized, _, _}}) ->
                          true;
                      (_) ->
                          false
                  end,
...
test_subscription_notifications(Client) ->
    ...
    Self = self(),
    Handler = fun(Method, Params) -> Self ! {notification, Method, Params} end,

    ok = erlmcp_client:set_notification_handler(Client, <<"resources/updated">>, Handler),

    % Cleanup
    ok = erlmcp_client:remove_notification_handler(Client, <<"resources/updated">>).

test_resources_updated_notification(Client) ->
    % Test resources/updated notification handling
    Self = self(),
    Handler = fun(Method, Params) -> Self ! {resource_updated, Method, Params} end,
...
test_resources_list_changed_notification(Client) ->
    ...
    Self = self(),
    Handler = fun(Method, _Params) -> Self ! {list_changed, Method} end,

    ok = erlmcp_client:set_notification_handler(Client, <<"resources/list_changed">>, Handler),

    ok = erlmcp_client:remove_notification_handler(Client, <<"resources/list_changed">>).

test_subscription_filtering(Client) ->
    % Test that notifications are only sent to subscribed URIs
    Self = self(),
    Handler = fun(Method, Params) -> Self ! {filtered, Method, Params} end,
...
test_concurrent_subscriptions(Client) ->
    ...
    Uris = [<<"test://resource/", (integer_to_binary(N))/binary>> || N <- lists:seq(1, 20)],
...
test_capability_validation_errors(_Client) ->
    ...
    case erlmcp_client:start_link({stdio, #{test_mode => true}}, #{strict_mode => true}) of
        {ok, StrictClient} ->
...
test_notification_handler_errors(Client) ->
    ...
    ok = erlmcp_client:set_notification_handler(Client, <<"test/crash">>, CrashingHandler),
...
test_batch_error_handling(Client) ->
    ...
    try
        erlmcp_client:with_batch(Client, fun(_BatchId) -> error(intentional_error) end),
...
test_large_parameters(Client) ->
    ...
    LargeMap =
        maps:from_list([{<<"key_", (integer_to_binary(N))/binary>>,
...
test_rapid_start_stop(_Client) ->
    ...
    AllStopped = lists:all(fun(C) -> not erlang:is_process_alive(C) end, Clients),
...
test_zero_timeout(_Client) ->
    % Test zero timeout
    case erlmcp_client:start_link({stdio, #{test_mode => true}}, #{timeout => 0}) of
        {ok, ZeroClient} ->
...
test_negative_timeout(_Client) ->
    ...
    case erlmcp_client:start_link({stdio, #{test_mode => true}}, #{timeout => -1000}) of
        {ok, NegClient} ->
