...
test_server_registration() ->
    ...
    try
        [?_test(begin
                    ?assertEqual(ok,
                                 erlmcp_registry:register_server(<<"test_server_1">>,
                                                                 ServerPid,
                                                                 ServerConfig))
                end),
         ?_test(begin
...
test_server_deregistration() ->
    ...
    try
        [?_test(begin
                    ?assertEqual(ok,
                                 erlmcp_registry:register_server(<<"test_server_dereg">>,
                                                                 ServerPid,
                                                                 ServerConfig))
                end),
         ?_test(begin
                    ...
                    ?assertMatch({error, not_found},
                                 erlmcp_registry:find_server(<<"test_server_dereg">>))
                end),
         ?_test(begin
...
test_duplicate_registration_same_pid() ->
    ...
    try
        [?_test(begin
                    ?assertEqual(ok,
                                 erlmcp_registry:register_server(<<"dup_server_same">>,
                                                                 ServerPid,
                                                                 ServerConfig))
                end),
         ?_test(begin
...
test_list_servers() ->
    ...
    ServerCount = 5,
    ServerIds =
        [list_to_binary("list_server_" ++ integer_to_list(N)) || N <- lists:seq(1, ServerCount)],
    ...
    try
        ...
        [?_assertEqual(ServerCount, length(ServerList)),
         ?_test(begin
                    ...
                    lists:foreach(fun(ServerId) -> ?assert(lists:keymember(ServerId, 1, ServerList))
                                  end,
                                  ServerIds)
                end)]
    after
    ...
    Pids =
        lists:map(fun(_) ->
                     spawn_link(fun() ->
                                   ...
                                   TestServer = self(),
                                   Result =
                                       erlmcp_registry:register_server(ServerId, TestServer, #{}),
