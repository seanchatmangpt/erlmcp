...
test_add_resource_template() ->
    ...
    ?assertEqual(ok,
                 erlmcp_server:add_resource_template(Server, TemplateUri, TemplateName, Handler)),
    ...
    [begin
         ...
         case Result of
             ...
             {error, {_Code, _Msg, _Data}} ->
                 ok;
             {error, _Reason} ->
                 ok  % Simple error tuple from validator
         end
     end
     || Uri <- InvalidUris],
...
test_uri_fragments() ->
    ...
    case erlmcp_server:add_resource(Server, UriWithFragment, Handler) of
        ok ->
            ok;
        {error, _} ->
            ok  %% May be rejected by validator
    end,
...
start_server() ->
    ServerId =
        <<"resources_test_server_", (integer_to_binary(erlang:unique_integer([positive])))/binary>>,
    Capabilities =
