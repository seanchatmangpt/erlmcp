...
tools_list_pagination_test() ->
    %% Simulate tools list
    Tools = [#{<<"name">> => <<"tool_", (integer_to_binary(I))/binary>>} || I <- lists:seq(1, 75)],
    PageSize = 20,
...
resources_list_pagination_test() ->
    ...
    Resources =
        [#{<<"uri">> => <<"resource://", (integer_to_binary(I))/binary>>}
         || I <- lists:seq(1, 150)],
    PageSize = 50,
...
prompts_list_pagination_test() ->
    ...
    Prompts =
        [#{<<"name">> => <<"prompt_", (integer_to_binary(I))/binary>>} || I <- lists:seq(1, 30)],
    PageSize = 10,

    %% Navigate through all pages
    AllPages = paginate_all(Prompts, PageSize, 30),

    TotalItems = lists:foldl(fun(Page, Acc) -> Acc + length(Page) end, 0, AllPages),
