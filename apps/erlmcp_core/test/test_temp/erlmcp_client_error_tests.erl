    ...
    try
        ?assertMatch({error, _},
                     erlmcp_client:with_batch(Client,
                                              fun(Bc) ->
                                                 erlmcp_client:list_tools(Bc),
                                                 erlmcp_client:list_resources(Bc)
                                              end))
    catch
        error:badarg ->
            ok
...
test_empty_batch_execution(Client) ->
    try
        ?assertMatch({ok, 0}, erlmcp_client:with_batch(Client, fun(_) -> ok end))
    catch
        error:badarg ->
            ok
    end.
...
test_concurrent_timeout_requests(Client) ->
    case erlmcp_client:start_link({stdio, #{test_mode => true}}, #{timeout => 100}) of
        {ok, CWithTimeout} ->
            ...
            ?assert(lists:all(fun ({_, {error, _}}) ->
                                      true;
                                  (_) ->
                                      false
                              end,
...
test_large_arguments_map(Client) ->
    LargeArgs =
        maps:from_list([{<<"arg", (integer_to_binary(I))/binary>>, I} || I <- lists:seq(1, 1000)]),
...
collect_results(Count, Timeout) ->
    collect_results(Count, Timeout, []).

collect_results(0, _, Acc) ->
    lists:reverse(Acc);
collect_results(Count, Timeout, Acc) ->
    receive
        R ->
            collect_results(Count - 1, Timeout, [R | Acc])
    after Timeout ->
        lists:reverse(Acc)
    end.
