...
test_orphaned_cleanup() ->
    ...
    lists:foreach(fun(Pid) -> stop_real_connection(Pid) end, OrphanedPids),
...
test_force_cleanup() ->
    ...
    lists:foreach(fun(Pid) -> stop_real_connection(Pid) end, lists:sublist(ConnectionPids, 3)),
    ...
    lists:foreach(fun(Pid) ->
                  ...
                  lists:sublist(ConnectionPids, 4, 10)).
