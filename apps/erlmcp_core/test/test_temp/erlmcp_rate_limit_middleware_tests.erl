...
setup() ->
    ...
    application:set_env(erlmcp, rate_limiting, #{max_messages_per_sec => 10, enabled => true}),
...
test_high_priority_bypass() ->
    ...
    Results =
        [erlmcp_rate_limit_middleware:check_request(ClientId, Method, TimeMs, high)
         || _ <- lists:seq(1, 100)],
...
test_normal_priority_limited() ->
    ...
    Results =
        [erlmcp_rate_limit_middleware:check_request(ClientId, Method, TimeMs, normal)
         || _ <- lists:seq(1, 20)],
...
test_low_priority_limited() ->
    ...
    % Make requests until limited
    Results =
        [erlmcp_rate_limit_middleware:check_request(ClientId, Method, TimeMs, low)
         || _ <- lists:seq(1, 20)],
...
method_specific_limits_test_() ->
    {setup,
     fun setup/0,
     fun cleanup/1,
     [?_test(test_different_methods_isolated()), ?_test(test_method_limit_override())]}.
...
test_method_limit_override() ->
    ...
    Config = #{max_rate => 2, scope => per_client},
