...
server_config() ->
    proper_types:map(
        proper_types:atom(),
        proper_types:oneof([proper_types:int(),
                            proper_types:binary(),
                            proper_types:atom(),
                            proper_types:bool(),
                            proper_types:list(
...
prop_server_registration_idempotent() ->
    ?FORALL({ServerId, Config},
            ...
            begin
                ...
                %% Register twice
                Result1 =
                    gen_server:call(Registry, {register_server, ServerId, TestServer, Config}),
                Result2 =
                    gen_server:call(Registry, {register_server, ServerId, TestServer, Config}),
...
prop_server_reregistration_different_pid() ->
    ?FORALL({ServerId, Config},
            ...
            begin
                ...
                Result =
                    gen_server:call(Registry, {register_server, ServerId, TestServer2, Config}),
            ...
            begin
                ...
                Result1 =
                    gen_server:call(Registry,
                                    {register_transport, TransportId, TestTransport, Config}),
                Result2 =
                    gen_server:call(Registry,
                                    {register_transport, TransportId, TestTransport, Config}),
...
prop_list_servers_consistency() ->
    ?FORALL(Count,
            ...
            begin
                ...
                ServerIds =
                    [list_to_atom("server_" ++ integer_to_list(N)) || N <- lists:seq(1, Count)],
                TestServers =
                    [spawn(fun() ->
                              receive after 5000 ->
                                  ok
                ...
                lists:foreach(fun({Id, Pid}) ->
                              ...
                              lists:zip(ServerIds, TestServers)),
...
prop_list_transports_consistency() ->
    ?FORALL(Count,
            ...
            begin
                ...
                TransportIds =
                    [list_to_atom("transport_" ++ integer_to_list(N)) || N <- lists:seq(1, Count)],
                TestTransports =
                    [spawn(fun() ->
                              receive after 5000 ->
                                  ok
                ...
                lists:foreach(fun({Id, Pid}) ->
                                 gen_server:call(Registry,
                                                 {register_transport, Id, Pid, #{type => stdio}})
                              end,
                              lists:zip(TransportIds, TestTransports)),
        ...
        lists:foreach(fun(Key) ->
                         case gproc:where({n, l, {mcp, server, Key}}) of
                             undefined ->
                                 ok;
                             Pid ->
                                 gproc:unreg_other({n, l, {mcp, server, Key}}, Pid)
                         end
                      end,
                      ServerKeys),
        ...
        lists:foreach(fun(Key) ->
                         case gproc:where({n, l, {mcp, transport, Key}}) of
                             undefined ->
                                 ok;
                             Pid ->
                                 gproc:unreg_other({n, l, {mcp, transport, Key}}, Pid)
                         end
                      end,
                      TransportKeys)
    catch
...
proper_test_() ->
    {setup,
     fun setup_suite/0,
     fun cleanup_suite/1,
     [{"Server registration idempotent",
       ?_assertEqual(true, proper:quickcheck(prop_server_registration_idempotent(), 20))},
      {"Server lookup consistency",
       ?_assertEqual(true, proper:quickcheck(prop_server_lookup_consistency(), 20))},
      {"Server reregistration different PID",
       ?_assertEqual(true, proper:quickcheck(prop_server_reregistration_different_pid(), 20))},
      {"Server duplicate different PID fails",
       ?_assertEqual(true, proper:quickcheck(prop_server_duplicate_different_pid_fails(), 20))},
      ...
      {"Server death auto-unregisters",
       ?_assertEqual(true, proper:quickcheck(prop_server_death_auto_unregisters(), 20))},
      {"Transport death auto-unregisters",
       ?_assertEqual(true, proper:quickcheck(prop_transport_death_auto_unregisters(), 20))},
      {"Server death cleans up bindings",
       ?_assertEqual(true, proper:quickcheck(prop_server_death_cleans_up_bindings(), 20))},
      {"List servers consistency",
       ?_assertEqual(true, proper:quickcheck(prop_list_servers_consistency(), 10))},
