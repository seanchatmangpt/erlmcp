...
%% Generate client IDs
client_id() ->
    proper_types:oneof([proper_types:binary(), proper_types:int(), proper_types:atom()]).
...
prop_token_consumption_decreases() ->
    ?FORALL({Cap, Count},
            {capacity(), proper_types:range(1, 10)},
            begin
                Bucket = erlmcp_rate_limiter:create_token_bucket(Cap),
...
prop_token_consumption_limit() ->
    ?FORALL({Cap, Count},
            {capacity(), proper_types:range(1, 100)},
            ?IMPLIES(Count > Cap,
                     begin
                         Bucket = erlmcp_rate_limiter:create_token_bucket(Cap),
...
prop_sliding_window_max_requests() ->
    ?FORALL({MaxRequests, WindowMs},
            {proper_types:range(1, 100), proper_types:range(100, 1000)},
            begin
...
prop_sliding_window_exceeds_max() ->
    ?FORALL({MaxRequests, WindowMs},
            {proper_types:range(1, 10), proper_types:range(100, 500)},
            begin
                Window = erlmcp_rate_limiter:create_sliding_window(WindowMs),
...
prop_sliding_window_expiration() ->
    ?FORALL({MaxRequests, WindowMs},
            {proper_types:range(1, 10), proper_types:range(200, 500)},
            begin
                Window = erlmcp_rate_limiter:create_sliding_window(WindowMs),

                %% Fill window
                {ok, Window1, _} =
                    erlmcp_rate_limiter:check_sliding_window(Window,
                                                             MaxRequests,
                ...
                Result =
                    erlmcp_rate_limiter:check_sliding_window(Window1,
                                                             MaxRequests,
                                                             erlang:system_time(millisecond)),
...
prop_client_state_initialization() ->
    ?FORALL(_Ignored,
            ...
            begin
                ...
                is_map(State)
                andalso maps:size(State) > 0  % State should have multiple fields
            end).
...
try_consume_n_times(Bucket, Cap, N) when N > 0 ->
    case erlmcp_rate_limiter:consume_token(Bucket, Cap) of
        {ok, NewBucket, _} ->
            try_consume_n_times(NewBucket, Cap, N - 1);
        {error, exceeded} ->
            {error, exceeded}
    end.
...
consume_all_tokens(Bucket, Cap, N) when N > 0 ->
    case erlmcp_rate_limiter:consume_token(Bucket, Cap) of
        {ok, NewBucket, _} ->
            consume_all_tokens(NewBucket, Cap, N - 1);
        {error, exceeded} ->
            {error, Bucket}
    end.
...
try_sliding_window_requests(Window, Max, N) when N > 0 ->
    case erlmcp_rate_limiter:check_sliding_window(Window, Max, erlang:system_time(millisecond)) of
        {ok, NewWindow, _} ->
            try_sliding_window_requests(NewWindow, Max, N - 1);
        {error, exceeded} ->
            {error, exceeded}
    end.
...
try_leaky_bucket_add(Bucket, Capacity, N) when N > 0 ->
    case erlmcp_rate_limiter:check_leaky_bucket(Bucket, Capacity) of
        {ok, NewBucket, _} ->
            try_leaky_bucket_add(NewBucket, Capacity, N - 1);
        {error, exceeded} ->
            {error, exceeded}
    end.
...
proper_test_() ->
    [{"Token bucket initial capacity",
      ?_assertEqual(true, proper:quickcheck(prop_token_bucket_initial_capacity(), 100))},
     {"Token bucket initial time",
      ?_assertEqual(true, proper:quickcheck(prop_token_bucket_initial_time(), 100))},
     {"Token consumption decreases",
      ?_assertEqual(true, proper:quickcheck(prop_token_consumption_decreases(), 50))},
     {"Token consumption limit",
      ?_assertEqual(true, proper:quickcheck(prop_token_consumption_limit(), 50))},
     {"Token refill over time",
      ?_assertEqual(true, proper:quickcheck(prop_token_refill_over_time(), 10))},
     {"Token refill max capacity",
      ?_assertEqual(true, proper:quickcheck(prop_token_refill_max_capacity(), 10))},
     {"Sliding window max requests",
      ?_assertEqual(true, proper:quickcheck(prop_sliding_window_max_requests(), 20))},
     {"Sliding window exceeds max",
      ?_assertEqual(true, proper:quickcheck(prop_sliding_window_exceeds_max(), 20))},
     {"Sliding window expiration",
      ?_assertEqual(true, proper:quickcheck(prop_sliding_window_expiration(), 10))},
     {"Leaky bucket capacity",
      ?_assertEqual(true, proper:quickcheck(prop_leaky_bucket_capacity(), 50))},
     {"Leaky bucket exceeds capacity",
      ?_assertEqual(true, proper:quickcheck(prop_leaky_bucket_exceeds_capacity(), 50))},
     {"Leaky bucket leaks", ?_assertEqual(true, proper:quickcheck(prop_leaky_bucket_leaks(), 10))},
     {"Client state initialization",
      ?_assertEqual(true, proper:quickcheck(prop_client_state_initialization(), 50))},
     {"Bucket tokens query",
      ?_assertEqual(true, proper:quickcheck(prop_bucket_tokens_query(), 100))}].
