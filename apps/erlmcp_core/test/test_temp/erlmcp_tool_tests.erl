...
validate_input_schema_test_() ->
    [?_test(test_undefined_schema()), ?_test(test_valid_schema()), ?_test(test_invalid_schema())].
...
test_encode_basic_tool() ->
    ...
    ?assertMatch(#{<<"name">> := <<"calculator">>, <<"description">> := <<"Basic calculator">>},
                 Encoded),
...
test_encode_tool_with_schema() ->
    ...
    Tool =
        #mcp_tool{name = <<"math_tool">>,
                  description = <<"Math operations">>,
                  input_schema = Schema},
    Encoded = erlmcp_tool:encode_tool(Tool),
    ?assertMatch(#{<<"name">> := <<"math_tool">>,
                   <<"description">> := <<"Math operations">>,
                   <<"inputSchema">> := Schema},
                 Encoded).
...
test_decode_basic_tool() ->
    Map = #{<<"name">> => <<"test_tool">>, <<"description">> => <<"Test description">>},
    Tool = erlmcp_tool:decode_tool(Map),
    ?assertMatch(#mcp_tool{name = <<"test_tool">>,
...
test_decode_tool_with_schema() ->
    Schema = #{type => <<"object">>},
    Map = #{<<"name">> => <<"schema_tool">>,
            <<"description">> => <<"With schema">>,
            <<"inputSchema">> => Schema},
    Tool = erlmcp_tool:decode_tool(Map),
    ?assertMatch(#mcp_tool{name = <<"schema_tool">>,
                           description = <<"With schema">>,
                           input_schema = Schema},
                 Tool).
