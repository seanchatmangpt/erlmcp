...
validate_uri_test_() ->
    [?_test(test_valid_uri()), ?_test(test_empty_uri()), ?_test(test_non_binary_uri())].
...
test_valid_resource() ->
    Resource =
        #mcp_resource{uri = <<"file:///document.txt">>,
                      name = <<"Document">>,
...
test_invalid_resource_structure() ->
    ...
    ?assertEqual({error, invalid_resource},
                 erlmcp_resource:validate_resource(#mcp_resource{uri = <<"file:///test">>,
                                                                 name = atom})).
...
validate_resource_template_test_() ->
    [?_test(test_valid_template()), ?_test(test_invalid_template())].

test_valid_template() ->
    Template =
        #mcp_resource_template{uri_template = <<"file:///{path}">>,
                               name = <<"File Template">>,
...
test_invalid_template() ->
    ...
    ?assertEqual({error, invalid_resource_template},
                 erlmcp_resource:validate_resource_template(#mcp_resource_template{uri_template =
                                                                                       <<"file:///{id}">>,
                                                                                   name = 123})).
...
edge_cases_test_() ->
    [?_test(test_unicode_uri()), ?_test(test_long_uri())].
