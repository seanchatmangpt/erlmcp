...
validate_jsonrpc_version_test_() ->
    [?_test(test_valid_version()), ?_test(test_invalid_version()), ?_test(test_missing_version())].
...
test_parse_request_with_params() ->
    ...
    Result = erlmcp_message_parser:parse_request(Id, Method, Data),
    ?assertMatch({ok,
                  #json_rpc_request{id = <<"test-id">>,
                                    method = Method,
                                    params = Params}},
                 Result).

test_parse_request_with_array_params() ->
    Id = 1,
    Method = <<"test/method">>,
    Params = [1, 2, 3],
    Data =
...
test_parse_response_success() ->
    Id = 42,
    Result = #{<<"status">> => <<"ok">>},
    ParseResult = erlmcp_message_parser:parse_response(Id, Result, undefined),
    ?assertMatch({ok,
                  #json_rpc_response{id = 42,
                                     result = Result,
                                     error = undefined}},
                 ParseResult).
...
test_parse_response_error() ->
    ...
    ParseResult = erlmcp_message_parser:parse_response(Id, undefined, Error),
    ?assertMatch({ok,
                  #json_rpc_response{id = <<"req-123">>,
                                     result = undefined,
                                     error = Error}},
                 ParseResult).
...
test_detect_notification() ->
    Data = #{<<"jsonrpc">> => <<"2.0">>, <<"method">> => <<"notification/test">>},
    Result = erlmcp_message_parser:parse_by_type(Data),
    ?assertMatch({ok, #json_rpc_notification{}}, Result).

test_unknown_message_type() ->
    Data = #{<<"jsonrpc">> => <<"2.0">>, <<"unknown">> => <<"field">>},
    Result = erlmcp_message_parser:parse_by_type(Data),
...
test_full_notification_parsing() ->
    ...
    Message =
        #{<<"jsonrpc">> => <<"2.0">>,
          ...
          <<"params">> =>
              #{<<"uri">> => <<"file:///test.txt">>, <<"metadata">> => #{<<"modified">> => true}}},
    {ok, Parsed} = erlmcp_message_parser:parse_json_rpc(Message),
...
test_malformed_messages() ->
    ...
    ?assertMatch({error, {invalid_request, unknown_message_type}},
                 erlmcp_message_parser:parse_json_rpc(#{<<"jsonrpc">> => <<"2.0">>,
                                                        <<"data">> => <<"unknown">>})).
