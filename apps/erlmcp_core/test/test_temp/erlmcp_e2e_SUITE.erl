...
e2e_test_full_mcp_lifecycle(_Config) ->
    ...
    ResourceHandler = fun(_Uri) -> #{contents => [#{text => <<"Hello from resource!">>}]} end,
...
e2e_test_resource_operations(_Config) ->
    ...
    TemplateHandler = fun(_Params) -> #{contents => [#{text => <<"Template result">>}]} end,
    ok =
...
e2e_test_tool_operations(_Config) ->
    ...
    ok =
        erlmcp_server:add_tool_with_description(Server, <<"add">>, <<"Add two numbers">>, Handler2),
    ...
    ok =
        erlmcp_server:add_tool_full(Server,
                                    ...
                                    Handler4,
                                    #{<<"inputSchema">> => Schema, <<"deprecated">> => false}),
        ...
        spawn(fun() ->
                 receive
                     {mcp_notification, Method, Params} ->
                         ct:log("Handler received notification: ~p with params: ~p",
                                [Method, Params])
                 end
              end),

    ok = erlmcp_server:register_notification_handler(Server, <<"resources/updated">>, HandlerPid),
    ct:log("Registered notification handler"),

    %% Add a resource
    ResourceHandler = fun(_Uri) -> #{contents => [#{text => <<"Test resource">>}]} end,
