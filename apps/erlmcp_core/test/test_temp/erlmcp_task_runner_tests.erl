...
test_task_failure() ->
    ...
    TaskFun = fun() -> error(deliberate_error) end,
...
test_exception_handling() ->
    % Task that throws
    TaskFun = fun() -> throw(test_exception) end,
...
test_progress_reporting() ->
    ...
    {ok, TaskId, _Pid} = erlmcp_task_runner:start_task(TaskFun, #{progress_token => ProgressToken}),
...
test_erlmcp_tasks_integration() ->
    ...
    {ok, TaskId} =
        erlmcp_tasks:create(self(),
                            #{type => <<"test">>, operation => <<"integration_test">>},
                            #{}),
...
test_concurrent_tasks() ->
    ...
    lists:foreach(fun({N, Result}) -> ?assertEqual(N * 2, Result) end, Results).
...
test_long_running_task() ->
           ...
           lists:foreach(fun(N) ->
                            ...
                            erlmcp_progress:update(ProgressToken,
                                                   #{current => N,
                                                     total => 5,
                                                     message =>
                                                         iolist_to_binary(io_lib:format("Step ~p",
                                                                                        [N]))}),
    ...
    {ok, TaskId, _Pid} =
        erlmcp_task_runner:start_task(TaskFun,
                                      #{progress_token => ProgressToken, timeout => 10000}),
