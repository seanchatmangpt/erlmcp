...
extract_server_capabilities_minimal_test() ->
    ...
    ?assertEqual(false,
                 Caps#mcp_server_capabilities.resources#mcp_resources_capability.listChanged).
...
server_capabilities_to_map_minimal_test() ->
    Caps =
        #mcp_server_capabilities{resources =
                                     #mcp_resources_capability{subscribe = false,
                                                               listChanged = false},
...
map_to_server_capabilities_test() ->
    Map = #{<<"resources">> => #{<<"subscribe">> => true, <<"listChanged">> => true},
            <<"tools">> => #{<<"listChanged">> => true},
            <<"prompts">> => #{},
            <<"logging">> => #{}},
...
negotiate_capabilities_full_test() ->
    ...
    ?assertEqual(true,
                 Negotiated#mcp_server_capabilities.resources#mcp_resources_capability.subscribe),
...
negotiate_capabilities_sampling_test() ->
    ...
    ClientCaps =
        #mcp_client_capabilities{roots = #mcp_capability{enabled = false},
                                 sampling = #mcp_capability{enabled = true}},
    ServerCaps =
...
has_capability_resources_test() ->
    Caps =
        #mcp_server_capabilities{resources = #mcp_resources_capability{subscribe = true},
                                 tools = #mcp_tools_capability{},
                                 prompts = #mcp_prompts_capability{},
                                 logging = #mcp_logging_capability{},
                                 sampling = #mcp_sampling_capability{},
...
format_capability_error_unsupported_feature_test() ->
    Error =
        erlmcp_capabilities:format_capability_error({unsupported_feature, resources, subscribe}),
    ?assertMatch(<<"Feature 'subscribe' not supported for capability 'resources'">>, Error).

format_capability_error_protocol_version_test() ->
    Error =
        erlmcp_capabilities:format_capability_error({protocol_version_mismatch, <<"2020-01-01">>}),
...
graceful_degradation_tools_not_supported_test() ->
    ...
    ServerCaps =
        #mcp_server_capabilities{resources = undefined,  %% Not supported
