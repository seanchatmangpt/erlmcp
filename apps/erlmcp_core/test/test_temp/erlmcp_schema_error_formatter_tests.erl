...
test_format_data_invalid() ->
    ...
    Result = erlmcp_schema_validator:format_jesse_error({data_invalid, Schema, Error, Data, Path}),
...
test_format_schema_invalid() ->
    ...
    Result = erlmcp_schema_validator:format_jesse_error({schema_invalid, Schema, Error}),
...
test_format_parse_errors() ->
    Result1 =
        erlmcp_schema_validator:format_jesse_error({data_error, {parse_error, "unexpected token"}}),
    ...
    Result2 =
        erlmcp_schema_validator:format_jesse_error({schema_error, {parse_error, "invalid schema"}}),
...
test_format_unknown_errors() ->
    Result = erlmcp_schema_validator:format_jesse_error({unknown_error, some, random, stuff}),
...
test_atom_path() ->
    ?assertEqual([<<"user">>, <<"name">>], erlmcp_schema_validator:format_path([user, name])).
...
format_error_message_test_() ->
    [{"Missing required property", fun test_missing_required_property/0},
     {"Wrong type", fun test_wrong_type/0}, {"Not in enum", fun test_not_in_enum/0},
     {"Not unique", fun test_not_unique/0}, {"Wrong length", fun test_wrong_length/0},
     {"Wrong size", fun test_wrong_size/0}, {"Missing dependency", fun test_missing_dependency/0},
     {"No match", fun test_no_match/0}, {"No extra properties", fun test_no_extra_properties/0},
     {"No extra items", fun test_no_extra_items/0}, {"Not allowed", fun test_not_allowed/0},
     {"Not in range", fun test_not_in_range/0}, {"Not divisible", fun test_not_divisible/0},
     {"Not array", fun test_not_array/0}, {"Wrong format", fun test_wrong_format/0},
     ...
     {"External error", fun test_external/0}, {"Not found", fun test_not_found/0},
...
test_missing_required_property() ->
    ?assertEqual(<<"Missing required property: name">>,
                 erlmcp_schema_validator:format_error_message({missing_required_property,
                                                               <<"name">>})).

test_wrong_type() ->
    ?assertEqual(<<"Wrong type, expected: string">>,
                 erlmcp_schema_validator:format_error_message({wrong_type, <<"string">>})).

test_not_in_enum() ->
    ?assertEqual(<<"Value not in enum: [1,2,3]">>,
                 erlmcp_schema_validator:format_error_message({not_in_enum, [1, 2, 3]})).
...
test_missing_dependency() ->
    ?assertEqual(<<"Missing dependency: address">>,
                 erlmcp_schema_validator:format_error_message({missing_dependency, <<"address">>})).
...
test_not_found() ->
    ?assertEqual(<<"Resource not found">>, erlmcp_schema_validator:format_error_message(not_found)).

test_custom_tuple_error() ->
    ?assert(is_binary(erlmcp_schema_validator:format_error_message({custom_error, some_details}))).

test_custom_atom_error() ->
    ?assert(is_binary(erlmcp_schema_validator:format_error_message(some_atom_error))).
