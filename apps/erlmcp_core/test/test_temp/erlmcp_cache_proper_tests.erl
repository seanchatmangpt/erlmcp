...
cache_strategy() ->
    proper_types:oneof([?LET(TTL, ttl_value(), {ttl, TTL}),
...
prop_cache_put_get_roundtrip() ->
    ?FORALL({Key, Value, Strategy},
            {cache_key(), cache_value(), cache_strategy()},
            begin
...
prop_lru_max_size_limit() ->
    ?FORALL({MaxSize, Count},
            {lru_max_size(), proper_types:range(1, 200)},
            ?IMPLIES(Count >= MaxSize,
                     begin
                         ...
                         lists:foreach(fun(N) ->
                                          ...
                                          Value =
                                              list_to_binary("lru_value_" ++ integer_to_list(N)),
...
prop_cache_clear() ->
    ?FORALL(Count,
            ...
            begin
                ...
                %% Verify all gone
                Results =
                    [erlmcp_cache:get(list_to_atom("clear_key_" ++ integer_to_list(N)))
                     || N <- lists:seq(1, Count)],
...
is_valid_etag_format(ETag) ->
    is_binary(ETag)
    andalso byte_size(ETag) >= 2
    andalso binary:first(ETag) =:= $"
    andalso binary:last(ETag) =:= $"
    andalso is_valid_hex(binary:part(ETag, 1, byte_size(ETag) - 2)).
...
proper_test_() ->
    [{"Cache put/get roundtrip",
      ?_assertEqual(true, proper:quickcheck(prop_cache_put_get_roundtrip(), 20))},
     {"Cache get nonexistent",
      ?_assertEqual(true, proper:quickcheck(prop_cache_get_nonexistent(), 20))},
     {"Cache put overwrites",
      ?_assertEqual(true, proper:quickcheck(prop_cache_put_overwrites(), 20))},
     {"Cache delete", ?_assertEqual(true, proper:quickcheck(prop_cache_delete(), 20))},
     {"Cache delete nonexistent",
      ?_assertEqual(true, proper:quickcheck(prop_cache_delete_nonexistent(), 20))},
     ...
     {"LRU eviction increases counter",
      ?_assertEqual(true, proper:quickcheck(prop_lru_eviction_increases_counter(), 10))},
     ...
     {"ETag check no match",
      ?_assertEqual(true, proper:quickcheck(prop_etag_check_no_match(), 20))},
