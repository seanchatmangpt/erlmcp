...
lifecycle_test_() ->
    {setup,
     ...
     fun(_) -> [?_test(test_start_stop()), ?_test(test_start_idempotent())] end}.
...
get_replicas_test_() ->
    {setup,
     ...
     fun replicator_cleanup/1,
     fun(_) ->
        [?_test(test_get_replicas_not_found()), ?_test(test_get_replicas_after_sync_replicate())]
     end}.
...
replicate_alias_test_() ->
    {setup,
     fun replicator_setup/0,
     fun replicator_cleanup/1,
     fun(_) -> [?_test(test_replicate_alias())] end}.
...
bootstrap_node_test_() ->
    {setup,
     ...
     fun replicator_cleanup/1,
     fun(_) ->
        [?_test(test_bootstrap_local_node_fails()), ?_test(test_bootstrap_unreachable_node())]
     end}.

test_bootstrap_local_node_fails() ->
    %% Cannot bootstrap local node
    ?assertEqual({error, cannot_bootstrap_local}, erlmcp_session_replicator:bootstrap_node(node())).

test_bootstrap_unreachable_node() ->
    %% Non-existent node should fail
    FakeNode = fake_node@nonexistent,

    ?assertEqual({error, node_unreachable}, erlmcp_session_replicator:bootstrap_node(FakeNode)).
...
concurrent_replication_test_() ->
    {setup,
     ...
     fun replicator_cleanup/1,
     fun(_) ->
        [?_test(test_concurrent_async_replications()), ?_test(test_concurrent_sync_replications())]
     end}.
...
test_concurrent_sync_replications() ->
    ...
    lists:foreach(fun({_I, Result}) -> ?assertMatch({ok, _}, Result) end, Results).
...
queue_management_test_() ->
    {setup,
     fun replicator_setup/0,
     fun replicator_cleanup/1,
     fun(_) -> [?_test(test_queue_flush_on_batch_size()), ?_test(test_queue_flush_on_timer())] end}.
...
error_handling_test_() ->
    {setup,
     ...
     fun replicator_cleanup/1,
     fun(_) ->
        [?_test(test_replicate_invalid_session_id()), ?_test(test_replicate_malformed_session())]
     end}.
...
performance_test_() ->
    {setup,
     fun replicator_setup/0,
     fun replicator_cleanup/1,
     fun(_) -> [?_test(test_bulk_replication_performance())] end}.
...
property_replication_idempotent_test_() ->
    {setup,
     fun replicator_setup/0,
     fun replicator_cleanup/1,
     fun(_) -> [?_test(test_multiple_replications_idempotent())] end}.
...
test_multiple_replications_idempotent() ->
    ...
    lists:foreach(fun(_) -> {ok, _} = erlmcp_session_replicator:sync_replicate(SessionId, Session)
                  end,
                  lists:seq(1, 10)),
