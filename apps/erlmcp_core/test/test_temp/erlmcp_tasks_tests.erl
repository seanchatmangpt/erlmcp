    ...
    fun() ->
       ...
       WorkerPid =
           spawn(fun() ->
                    ...
                    receive
                        cancel ->
                            ok  % Will receive exit signal from cancellation
                    after 10000 ->
                        ok  % Long timeout - should be cancelled before this
                    end
                 end),
...
test_concurrent_task_limit({_Pid, _ServerName}) ->
    fun() ->
       ...
       lists:foreach(fun(Id) -> ok = erlmcp_tasks:complete(erlmcp_tasks, Id, #{<<"done">> => true})
                     end,
                     lists:sublist(TaskIds, 5))
    end.
...
test_progress_token_generation(_Pid) ->
    fun() ->
       ...
       ?assert(is_reference(ProgressToken)
               orelse is_binary(ProgressToken)
               orelse is_integer(ProgressToken)),
...
test_progress_update(_Pid) ->
    fun() ->
       ...
       lists:foreach(fun(Progress) ->
                        ok =
                            erlmcp_tasks:update_progress(erlmcp_tasks,
                                                         TaskId,
...
test_duplicate_task_id(_Pid) ->
    fun() ->
       ...
       TaskIds = [begin crypto:strong_rand_bytes(16) end || _ <- lists:seq(1, 1000)],
...
test_expired_task_cleanup(_Pid) ->
    fun() ->
       ...
       {ok, ShortTaskId} =
           erlmcp_tasks:create(erlmcp_tasks,
                               #{<<"type">> => <<"short_lived">>},
                               #{},
                               #{ttl_ms => ShortTTL}),
...
test_very_large_result(_Pid) ->
    fun() ->
       ...
       Result = #{<<"data">> => LargeBinary, <<"count">> => 100000},
...
prop_task_state_transition() ->
    ?FORALL({Status1, Status2},
            {proper_types:oneof([<<"pending">>, <<"processing">>]),
             proper_types:oneof([<<"processing">>,
                                 <<"completed">>,
                                 <<"failed">>,
                                 <<"cancelled">>])},
            begin
...
prop_task_metadata_preservation() ->
    ?FORALL(Metadata,
            proper_types:map(
                proper_types:binary(), proper_types:any()),
            begin
                {ok, Pid} = erlmcp_tasks:start_link(),
...
test_meta_propagation_through_states(_Pid) ->
    fun() ->
       ...
       Meta =
           #{<<"_meta">> =>
                 #{<<"traceId">> => <<"trace-state-test">>, <<"tag">> => <<"propagation-test">>}},
...
test_meta_in_task_list(_Pid) ->
    fun() ->
       ...
       lists:foreach(fun(Id) -> ?assert(lists:member(Id, ListedIds)) end, TaskIds)
    end.
...
test_meta_in_cancelled_task(_Pid) ->
    fun() ->
       ...
       receive
           {'DOWN', MonitorRef, process, WorkerPid, _} ->
               ok
       after 1000 ->
           ?assert(false, worker_should_have_exited)
       end,
    ...
    {setup,
     ...
     fun(_Pid) ->
        [?_test(begin
                    ...
                    {ok, TaskId} =
                        erlmcp_tasks:create(erlmcp_tasks, Action, #{<<"workflow">> => true}),
     ...
     fun(_Pid) ->
        [?_test(begin
                    ...
                    lists:foreach(fun(Id) ->
                                  ...
                                  lists:sublist(TaskIds, NumTasks div 2)),

                    %% Complete first quarter
                    lists:foreach(fun(Id) ->
                                     erlmcp_tasks:complete(erlmcp_tasks, Id, #{<<"done">> => true})
                                  end,
                                  lists:sublist(TaskIds, NumTasks div 4)),
    ...
    {setup,
     ...
     fun(_Pid) ->
        [?_test(begin
                    ...
                    {ok, TaskId} =
                        erlmcp_tasks:create(erlmcp_tasks, #{<<"type">> => <<"error_test">>}, #{}),
