...
prop_list_servers_consistency() ->
    ?FORALL(Count,
            ...
            begin
                ...
                ServerIds =
                    [list_to_binary("server_" ++ integer_to_list(N)) || N <- lists:seq(1, Count)],
                ServerPids =
                    [begin
                ...
                %% Cleanup
                lists:foreach(fun(Pid) -> erlmcp_test_helpers:stop_test_server(Pid) end,
                              ServerPids),
...
cleanup_suite(_) ->
    ...
    try
        ...
        lists:foreach(fun({ServerId, _Pid, _Caps}) -> erlmcp_registry:unregister_server(ServerId)
                      end,
                      ServerKeys)
    catch
        _:_ ->
