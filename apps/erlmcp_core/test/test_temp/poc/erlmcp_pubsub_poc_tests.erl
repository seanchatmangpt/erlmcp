    ...
    {setup,
     ...
     fun(_Pid) ->
        [?_test(begin
                    ...
                    Parent = self(),
                    Subs =
                        [spawn_link(fun() -> subscriber_helper(Parent) end)
                         || _ <- lists:seq(1, 3)],

                    %% Subscribe all
                    lists:foreach(fun(Sub) -> ok = erlmcp_pubsub_poc:subscribe(Topic, Sub) end,
                                  Subs),
    ...
    {setup,
     ...
     fun(_Pid) ->
        [?_test(begin
                    ...
                    lists:foreach(fun(_) ->
                                     erlmcp_pubsub_poc:broadcast(Topic,
                                                                 #{seq => 1},
                    ...
                    ?assertEqual(10,
                                 maps:get(total_messages_sent, Metrics)),  % 5 broadcasts * 2 subs
    ...
    {setup,
     ...
     fun(_Pid) ->
        [?_test(begin
                    ...
                    lists:foreach(fun(Chunk) -> ok = erlmcp_pubsub_poc:broadcast(ToolTopic, Chunk)
                                  end,
                                  Chunks),
    ...
    {setup,
     ...
     fun(_Pid) ->
        [?_test(begin
                    ...
                    lists:foreach(fun(Agent) -> ok = erlmcp_pubsub_poc:subscribe(Topic, Agent) end,
                                  Agents),
     ...
     {setup,
      ...
      fun(_Pid) ->
         [?_test(begin
                     ...
                     lists:foreach(fun(N) -> ok = erlmcp_pubsub_poc:broadcast(Topic, #{seq => N})
                                   end,
                                   lists:seq(1, 1000)),
