...
prop_lru_max_size_limit() ->
    ?FORALL({MaxSize, Count},
            ...
            ?IMPLIES(Count >= MaxSize,
                     begin
                         {ok, _Cache} =
                             erlmcp_test_helpers:start_test_cache(#{max_l1_size => MaxSize}),
...
prop_lru_evicts_least_recently_used() ->
    ?FORALL(MaxSize,
            ...
            begin
                ...
                FirstResult =:= {ok, list_to_binary("lru_value_1")}
                andalso SecondResult =:= {error, not_found}
                andalso NewResult =:= {ok, NewValue}
            end).

%% Property: Cache with LRU strategy maintains size limit
prop_lru_strategy_enforces_limit() ->
    ?FORALL({MaxSize, OverCount},
            {lru_max_size(), proper_types:range(1, 20)},
            begin
                {ok, _Cache} = erlmcp_test_helpers:start_test_cache(#{max_l1_size => MaxSize}),
...
prop_lru_multiple_operations() ->
    ?FORALL(MaxSize,
            ...
            begin
                ...
                lists:foreach(fun(N) ->
                                 Key = list_to_atom("multi_key_"
                                                    ++ integer_to_list(N rem MaxSize + 1)),
...
proper_test_() ->
    [{"LRU max size limit", ?_assertEqual(true, proper:quickcheck(prop_lru_max_size_limit(), 10))},
     ...
     {"LRU evicts least recently used",
      ?_assertEqual(true, proper:quickcheck(prop_lru_evicts_least_recently_used(), 10))},
     {"LRU strategy enforces limit",
      ?_assertEqual(true, proper:quickcheck(prop_lru_strategy_enforces_limit(), 10))},
     {"LRU multiple operations",
      ?_assertEqual(true, proper:quickcheck(prop_lru_multiple_operations(), 5))}].
