...
benchmark_cache_hits(TotalOps, NumWorkers) ->
    ...
    Latencies =
        run_parallel_workers(NumWorkers,
                             fun(_WorkerId) ->
                                lists:map(fun(_) ->
                                             KeyNum = rand:uniform(NumKeys),
                                             Key = iolist_to_binary(io_lib:format("key_~p",
                                                                                  [KeyNum])),
                                ...
                                lists:map(fun(_) ->
                                             ...
                                             case rand:uniform(100) of
                                                 ...
                                                 _ ->
                                                     %% Write operation
                                                     Value =
                                                         iolist_to_binary(io_lib:format("value_~p",
                                                                                        [rand:uniform(10000)])),
                                                     erlmcp_cache:put(Key, Value)
                                             end,
                             ...
                             fun(_WorkerId) ->
                                lists:map(fun(N) ->
                                             ...
                                             Key = iolist_to_binary(io_lib:format("key_~p",
                                                                                  [KeyNum])),
                                             Value =
                                                 iolist_to_binary(io_lib:format("value_~p", [N])),
...
-spec calculate_percentiles([non_neg_integer()]) -> {float(), float(), float()}.
...
write_results(WorkloadId, Results) ->
    ...
    Filename =
        lists:flatten(
            io_lib:format("~s/cache_~s_~s.json", [ResultsDir, WorkloadId, Timestamp])),
...
convert_to_json(List) when is_list(List) ->
    case io_lib:printable_unicode_list(List) of
        true ->
            list_to_binary(List);
        false ->
            [convert_to_json(Item) || Item <- List]
    end;
