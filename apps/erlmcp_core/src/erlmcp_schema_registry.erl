...
-spec register(schema_name(), version(), schema_type(), schema_definition()) -> ok | {error, term()}.
register(Name, Version, Type, Definition) when is_binary(Name), is_tuple(Version), is_map(Definition) ->
...
init([]) ->
    ...
    SchemasTable = ets:new(erlmcp_schemas, [
        named_table,
        public,
    ...
    VersionsTable = ets:new(erlmcp_schema_versions, [
        named_table,
        public,
        {read_concurrency, true}
    ]),
...
handle_call({register, Name, Version, Type, Definition}, _From, State) ->
    ...
    case validate_schema_definition(Type, Definition) of
        ok ->
            ...
            Versions = case ets:lookup(State#state.versions_table, Name) of
                [{Name, Vs}] -> lists:usort([Version | Vs]);
                [] -> [Version]
            end,
...
handle_call({get_latest, Name}, _From, State) ->
    case ets:lookup(State#state.versions_table, Name) of
        [{Name, Versions}] ->
            LatestVersion = lists:last(lists:sort(Versions)),
...
handle_call({check_compatibility, Name, FromVersion, ToVersion}, _From, State) ->
    ...
    case {ets:lookup(State#state.schemas_table, KeyFrom),
          ets:lookup(State#state.schemas_table, KeyTo)} of
        {[{KeyFrom, SchemaFrom}], [{KeyTo, SchemaTo}]} ->
            Result = check_backward_compatibility(SchemaFrom#schema.definition,
            ...
            case ets:lookup(State#state.versions_table, Name) of
                [{Name, Versions}] ->
                    ...
                    case NewVersions of
                        ...
                        _ -> ets:insert(State#state.versions_table, {Name, NewVersions})
                    end;
                [] -> ok
            end,
...
validate_schema_definition(_Type, Definition) when is_map(Definition) ->
    ...
    case maps:is_key(<<"type">>, Definition) of
        true -> ok;
        false -> {error, missing_type_field}
    end;
