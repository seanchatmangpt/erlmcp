...
-export([start_link/0, start_link/1, create_message/2,
         set_backend/1, get_backend/0, set_base_url/1, get_base_url/0,
         set_model/1, get_model/0]).
...
set_backend(Backend) when Backend =:= ollama; Backend =:= lm_studio; Backend =:= openai_compatible ->
    gen_server:call(?MODULE, {set_backend, Backend}).

get_backend() -> gen_server:call(?MODULE, get_backend).

set_base_url(Url) when is_binary(Url) ->
    gen_server:call(?MODULE, {set_base_url, Url}).

get_base_url() -> gen_server:call(?MODULE, get_base_url).
...
init([Config]) ->
    ...
    BaseUrl = case Backend of
        ollama -> maps_get(base_url, Config, <<"http://localhost:11434">>);
        lm_studio -> maps_get(base_url, Config, <<"http://localhost:1234">>);
        openai_compatible -> maps_get(base_url, Config, <<"http://localhost:8000">>)
    end,
    ...
    {ok, #state{backend = Backend, base_url = BaseUrl, model = Model, timeout = Timeout}}.
...
do_create_message(Messages, Params, #state{backend = Backend} = State) ->
    case Backend of
        ollama -> do_ollama_request(Messages, Params, State);
        lm_studio -> do_openai_compatible_request(Messages, Params, State);
        openai_compatible -> do_openai_compatible_request(Messages, Params, State)
    end.
