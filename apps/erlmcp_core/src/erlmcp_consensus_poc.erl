...
handle_info({vote_request, Term, CandidateId}, State = #state{term = CurrentTerm, voted_for = VotedFor}) ->
    if
        Term > CurrentTerm ->
...
handle_info({vote_response, granted, Term}, State = #state{term = Term, votes_received = Votes, cluster_members = Members}) ->
    %% Add vote
...
start_election(State = #state{node_id = NodeId, term = Term, cluster_members = Members}) ->
    %% Increment term
    NewTerm = Term + 1,
    %% Vote for self
    NewState = State#state{
