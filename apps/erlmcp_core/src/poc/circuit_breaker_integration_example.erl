...
-export([
    example_with_telemetry/0,
    example_per_tool_protection/0,
    example_dashboard_integration/0,
...
example_with_telemetry() ->
    ...
    ok = erlmcp_otel:init(#{
        service_name => <<"circuit_breaker_demo">>,
        exporters => [console],
        sampling => always_on
    }),

    %% Start circuit breaker for database tool
    {ok, DbBreaker} = erlmcp_circuit_breaker_poc:start_link(
        <<"database_query">>,
        #{failure_threshold => 3, timeout_ms => 5000}
    ),

    %% Start a trace span
    SpanCtx = erlmcp_otel:start_span(<<"tool.execute">>, #{
    ...
    Results = lists:map(fun(N) ->
        Result = erlmcp_circuit_breaker_poc:call_tool(DbBreaker, fun() ->
        ...
        case Result of
            {ok, _} ->
                erlmcp_otel:add_event(SpanCtx, <<"call.success">>, #{
                    <<"call_number">> => N
                });
            {error, Reason} ->
                erlmcp_otel:add_event(SpanCtx, <<"call.failure">>, #{
                    <<"call_number">> => N,
...
example_per_tool_protection() ->
    ...
    Breakers = lists:map(fun({ToolName, Config}) ->
    ...
    lists:foreach(fun({ToolName, Breaker}) ->
        ...
        io:format("  ~s: ~p (trips: ~p)~n", [
            ToolName,
            State,
            maps:get(trip_count, Data)
        ])
    end, Breakers),

    %% Cleanup
    lists:foreach(fun({_Name, Breaker}) ->
...
example_failure_recovery() ->
    ...
    {ok, Breaker} = erlmcp_circuit_breaker_poc:start_link(
        <<"recovery_test">>,
        #{failure_threshold => 2, success_threshold => 2, timeout_ms => 1000}
    ),
...
simulate_tool_call(ToolName, N) ->
    ...
    case {ToolName, N rem 3} of
        {<<"database">>, 0} -> {error, connection_lost};
        {<<"api_service">>, 0} -> timer:sleep(50), {error, timeout};
        {<<"cache">>, 0} -> {error, cache_miss};
        _ -> {ok, {ToolName, N}}
    end.
        ...
        [
            erlmcp_circuit_breaker_poc:call_tool(Breaker, fun() ->
                case rand:uniform(10) of
                    N when N > 7 -> {error, random_failure};
                    _ -> {ok, ToolName}
                end
            end)
            || _ <- lists:seq(1, 10)
...
show_final_stats(Breaker) ->
    ...
    io:format("  Successes: ~p (~.1f%%)~n", [
        maps:get(total_successes, Stats),
        maps:get(success_rate, Stats) * 100
    ]),
    io:format("  Failures: ~p (~.1f%%)~n", [
        maps:get(total_failures, Stats),
        maps:get(failure_rate, Stats) * 100
    ]),
