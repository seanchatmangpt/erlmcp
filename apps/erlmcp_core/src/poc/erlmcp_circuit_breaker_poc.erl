...
-type breaker_config() :: #{
    failure_threshold => pos_integer(),      % Failures to trip (default: 3)
    success_threshold => pos_integer(),      % Successes in half_open to close (default: 2)
    timeout_ms => pos_integer(),             % Time before half_open attempt (default: 5000)
    half_open_max_calls => pos_integer()     % Max calls in half_open (default: 3)
}.
...
init({ToolName, Config}) ->
    ...
    emit_telemetry(breaker_initialized, ToolName, #{
        config => MergedConfig,
...
closed({call, From}, {call_tool, Fun}, Data) ->
    ...
    case Result of
        {ok, _Value} ->
            ...
            emit_telemetry(call_succeeded, Data#data.tool_name, #{
                state => closed,
                latency_us => Latency,
        ...
        {error, Reason} ->
            ...
            emit_telemetry(call_failed, Data#data.tool_name, #{
                state => closed,
                error => Reason,
                consecutive_failures => NewFailures
            }),
...
open({call, From}, {call_tool, _Fun}, Data) ->
    ...
    emit_telemetry(call_rejected, Data#data.tool_name, #{
        state => open,
        total_rejected => UpdatedData#data.total_rejected
    }),
...
open({call, From}, reset, Data) ->
    ...
    emit_telemetry(state_transition, Data#data.tool_name, #{
        from => open,
        to => closed,
        reason => manual_reset
    }),
...
half_open({call, From}, {call_tool, Fun}, Data) ->
    ...
    case Data#data.half_open_calls >= MaxCalls of
        ...
        false ->
            ...
            NewData = TempData#data{
                half_open_calls = Data#data.half_open_calls  % Reset to original
            },
...
terminate(_Reason, _State, Data) ->
    emit_telemetry(breaker_terminated, Data#data.tool_name, #{
        final_state => _State,
...
-spec trip_to_open(gen_statem:from(), call_result(), #data{}) ->
    {next_state, open, #data{}, [{reply, gen_statem:from(), call_result()} | {state_timeout, pos_integer(), atom()}]}.
trip_to_open(From, Result, Data) ->
    ...
    {next_state, open, NewData, [
        {reply, From, Result},
        {state_timeout, Timeout, transition_to_half_open}
    ]}.
...
transition_to_half_open(Data) ->
    ...
    ?LOG_INFO("Circuit breaker transitioning to HALF_OPEN for tool: ~p",
             [Data#data.tool_name]),
...
build_stats(State, Data) ->
    #{
        ...
        %% Metrics
        failure_rate => calculate_failure_rate(Data),
        success_rate => case Data#data.total_calls of
            0 -> 0.0;
            N -> Data#data.total_successes / N
        end,
        ...
        true ->
            try
                ...
                case erlang:get(erlmcp_otel_current_context) of
                    undefined -> ok;
                    SpanCtx -> erlmcp_otel:add_event(SpanCtx, EventName, Attributes)
                end
            catch
                _:_ -> ok  % Silently ignore telemetry errors
            end;
        false ->
            ok
    end,
...
convert_metadata(Metadata) ->
    maps:fold(fun(K, V, Acc) ->
        ...
        BinValue = case V of
            ...
            Val when is_integer(Val); is_float(Val); is_binary(Val) -> Val;
