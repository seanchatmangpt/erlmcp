...
handle_cast(start_agents, State) ->
    ...
    Agents = lists:map(
        fun(Id) ->
            Agent = start_agent(Id, State#state.server_pid, State#state.circuit_breaker_pid),
...
handle_info({tool_call_completed, AgentId, Latency}, State) ->
    % Update agent stats
    Agents = lists:map(
        fun(Agent) when Agent#agent_state.id =:= AgentId ->
                Agent#agent_state{
                    tool_calls = Agent#agent_state.tool_calls + 1,
    ...
    {noreply, State#state{
        agents = Agents,
        total_tool_calls = State#state.total_tool_calls + 1
    }};
...
handle_info({subscription_created, AgentId}, State) ->
    Agents = lists:map(
        fun(Agent) when Agent#agent_state.id =:= AgentId ->
                Agent#agent_state{subscriptions = [<<"resource://data">> | Agent#agent_state.subscriptions]};
           (Agent) -> Agent
        end,
        State#state.agents
    ),
    {noreply, State#state{
        agents = Agents,
        total_subscriptions = State#state.total_subscriptions + 1
    }};
...
add_demo_tools(ServerPid) ->
    ...
    FastTool = fun(Args) ->
        timer:sleep(rand:uniform(50)),  % 0-50ms latency
        {ok, #{
            content => [#{
                type => <<"text">>,
    ...
    erlmcp_server:add_tool_with_description(ServerPid, <<"fast_tool">>, FastTool,
    ...
    erlmcp_server:add_tool_with_description(ServerPid, <<"slow_stream_tool">>, SlowTool,
        ...
        case rand:uniform(10) of
            ...
            _ ->
                ...
                {ok, #{
                    content => [#{
                        type => <<"text">>,
                        text => <<"Flaky tool succeeded">>
                    }]
                }}
        end
    end,
    erlmcp_server:add_tool_with_description(ServerPid, <<"flaky_tool">>, FlakyTool,
        <<"A flaky tool that fails 30% of the time">>),

    % Coordinated tool (simulates leader election requirement)
    CoordinatedTool = fun(Args) ->
    ...
    erlmcp_server:add_tool_with_description(ServerPid, <<"coordinated_tool">>, CoordinatedTool,
    ...
    DynamicResource = fun(_Uri) ->
        ...
        {ok, #{
            ...
            contents => [#{
                uri => <<"resource://dynamic/data">>,
                mimeType => <<"application/json">>,
                text => iolist_to_binary(io_lib:format("{\"type\": \"dynamic\", \"value\": ~p}", [Value]))
            }]
        }}
    end,
...
add_demo_prompts(ServerPid) ->
    ...
    erlmcp_server:add_prompt_with_args(ServerPid, <<"topic_prompt">>, ParamPrompt,
...
simulate_notifications(Agent, _DemoState) ->
    ...
    lists:foreach(
        fun(_) ->
            ...
            case DemoPid of
                Pid when is_pid(Pid) ->
                    Pid ! {resource_notification, Agent#agent_state.id, <<"resource://dynamic/data">>};
                _ ->
                    ok
            end
        end,
...
print_current_metrics(State) ->
    ...
    AvgLatency = case AllLatencies of
        [] -> 0;
        _ -> lists:sum(AllLatencies) div length(AllLatencies)
    end,
...
print_final_summary(State) ->
    ...
    lists:foreach(
        fun(Agent) ->
            AvgAgentLatency = case Agent#agent_state.latencies of
                [] -> 0;
    ...
    AvgFanout = case {State#state.pubsub_notifications, SubscribedAgents} of
        {0, _} -> 0;
        {N, 0} -> 0;
        {N, S} -> N / S
    end,
    ...
    % Success rate
    SuccessRate = case State#state.total_tool_calls of
        0 -> 100.0;
        Total ->
