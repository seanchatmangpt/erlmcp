...
%% API
-export([start_link/0, start_link/1, check_rate_limit/1, check_rate_limit/2, record_failure/1,
         record_failure/2, record_success/1, record_success/2, is_blocked/1, get_client_stats/1,
         reset_client/1, clear_all_blocks/0, get_blocked_clients/0, stop/0]).
...
-record(rate_limit_state, {count :: attempt_count(), window_start :: window_start()}).
...
init([Config]) ->
    ...
    State =
        #state{rate_limits = ets:new(auth_rate_limits, [set, protected]),
...
check_rate_limit_window(ClientId, Now, State) ->
    MaxAttempts =
        maps_get(max_attempts_per_second, State#state.config, ?DEFAULT_MAX_ATTEMPTS_PER_SECOND),
        ...
        [{_, #rate_limit_state{count = Count, window_start = Start}}] ->
            ...
            if TimeSinceStart >= WindowMs ->
               ...
               true ->
                   ...
                   ets:insert(State#state.rate_limits,
                              {ClientId,
                               #rate_limit_state{count = NewCount, window_start = Start}}),
                   ok
            end;
        [] ->
...
do_record_failure(ClientId, IpAddress, State) ->
    ...
    Stats =
        case ets:lookup(State#state.client_stats, ClientId) of
            [{_, ExistingStats}] ->
                ExistingStats#client_stats{failed_auths =
                                               ExistingStats#client_stats.failed_auths + 1,
                                           total_attempts =
                                               ExistingStats#client_stats.total_attempts + 1,
                                           last_attempt_at = Now};
            [] ->
    ...
    if Failures >= MaxFailures andalso CurrentBackoff < length(BackoffLevels) - 1 ->
           ...
           ets:insert(State#state.blocks,
                      {ClientId,
                       #block_state{reason = too_many_failures,
           ...
           % Update stats
           NewStats =
               Stats#client_stats{blocked_count = Stats#client_stats.blocked_count + 1,
           ...
           case IpAddress of
               undefined ->
                   ok;
               Ip ->
                   ets:insert(State#state.ip_blocks,
                              {Ip,
                               #block_state{reason = too_many_failures,
                                            blocked_until = BlockedUntil,
                                            failure_count = Failures,
       ...
       Failures >= MaxFailures * 2 ->
           ...
           ets:insert(State#state.blocks,
                      {ClientId,
                       #block_state{reason = excessive_failures,
                                    blocked_until = BlockedUntil,
                                    failure_count = Failures,
                                    backoff_level = 5}}),
...
do_record_success(ClientId, IpAddress, State) ->
    ...
    Stats =
        case ets:lookup(State#state.client_stats, ClientId) of
            [{_, ExistingStats}] ->
                ExistingStats#client_stats{successful_auths =
                                               ExistingStats#client_stats.successful_auths + 1,
                                           total_attempts =
                                               ExistingStats#client_stats.total_attempts + 1,
    ...
    case IpAddress of
        undefined ->
            ok;
        Ip ->
            ets:delete(State#state.ip_blocks, Ip)
    end,
