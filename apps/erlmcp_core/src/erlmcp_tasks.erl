...
-type task() :: #mcp_task{}.
-type task_list_result() :: #{tasks := [map()], cursor => binary()}.
-type update_function() :: fun((map()) -> map()).

-record(state, {task_count = 0 :: non_neg_integer(), pending_cleanup = [] :: [task_id()]}).
...
-spec create_task(pid() | undefined, map(), map() | undefined) -> {ok, task_id()} | {error, term()}.
create_task(ClientPid, Action, Metadata) when is_map(Action) ->
    MetadataMap =
        case Metadata of
            undefined ->
                #{};
            _ when is_map(Metadata) ->
                Metadata
        end,
...
-spec get_task(pid() | undefined, task_id()) -> {ok, task()} | {error, term()}.
...
-spec cancel_task(pid() | undefined, task_id(), binary()) -> {ok, cancelled} | {error, term()}.
...
-spec get_task_result(pid() | undefined, task_id()) -> {ok, term()} | {error, term()}.
...
-spec create(pid() | undefined, map(), map()) -> {ok, task_id()} | {error, term()}.
...
-spec create(pid() | undefined, map(), map(), map()) -> {ok, task_id()} | {error, term()}.
create(ClientPid, Action, Metadata, Options)
    when is_map(Action), is_map(Metadata), is_map(Options) ->
...
-spec create_task(pid() | undefined, map(), map(), map()) -> {ok, task_id()} | {error, term()}.
create_task(ClientPid, Action, Metadata, Options) ->
    create(ClientPid, Action, Metadata, Options).

%% @doc List tasks without cursor (defaults to limit 100).
-spec list_tasks(pid() | undefined) -> {ok, task_list_result()} | {error, term()}.
list_tasks(ClientPid) ->
    list_tasks(ClientPid, undefined, 100).

%% @doc List tasks with limit only (no cursor).
-spec list_tasks(pid() | undefined, pos_integer()) -> {ok, task_list_result()} | {error, term()}.
...
-spec cancel(pid() | undefined, task_id(), binary()) -> {ok, cancelled} | {error, term()}.
...
-spec update_task(pid() | undefined, task_id(), fun((map()) -> map())) -> ok | {error, term()}.
...
-spec update_status(pid() | undefined, task_id(), binary()) -> ok | {error, term()}.
...
-spec set_task_progress(task_id(), number() | {number(), number()}) -> ok | {error, term()}.
...
init([]) ->
    ...
    {ok, #state{task_count = 0, pending_cleanup = []}}.
...
do_list_tasks(ClientPid, Cursor, Limit) ->
    ...
    AllTasks =
        ets:foldl(fun(#mcp_task{client_pid = TaskClientPid} = Task, Acc) ->
                     % Match if:
    ...
    SortedTasks =
        lists:sort(fun(A, B) -> A#mcp_task.created_at >= B#mcp_task.created_at end, AllTasks),
        ...
        case Cursor of
            ...
            _ ->
                case lists:keyfind(Cursor, #mcp_task.id, SortedTasks) of
                    ...
                    _CursorTask ->
                        {_Before, After} =
                            lists:splitwith(fun(T) -> T#mcp_task.id =/= Cursor end, SortedTasks),
                        case After of
                            [] ->
                                [];
                            [_ | Rest] ->
        ...
        [#mcp_task{client_pid = TaskPid, status = Status} = Task] ->
            ...
            case match_client_pids(NormalizedPid, TaskNormalized) of
                true ->
                    case Status of
                        completed ->
                            {{error, {task_already_completed, ?MCP_ERROR_TASK_ALREADY_COMPLETED}},
                             State};
                        cancelled ->
                        ...
                        _ ->
                            ...
                            UpdatedTask =
                                Task#mcp_task{status = cancelled,
                                              updated_at = erlang:system_time(millisecond),
                                              error =
                                                  #mcp_error{code = ?MCP_ERROR_TASK_CANCELLED,
                ...
                true ->
                    case Status of
                        completed ->
                            {ok, Result};
                        failed ->
                            {error, ?MCP_ERROR_TASK_FAILED};
                        pending ->
                            {error, ?MCP_ERROR_TASK_RESULT_NOT_READY};
                        processing ->
                            {error, ?MCP_ERROR_TASK_RESULT_NOT_READY};
                        cancelled ->
                            {error, ?MCP_ERROR_TASK_CANCELLED}
                    end;
                false ->
                    {error, ?MCP_MSG_TASK_NOT_FOUND}
            end;
...
do_start_task_execution(TaskId, WorkerPid, State) ->
    case ets:lookup(?TASKS_TABLE, TaskId) of
        [#mcp_task{status = pending} = Task] ->
            ...
            UpdatedTask =
                Task#mcp_task{status = processing,
...
do_complete_task(TaskId, Result, State) ->
    case ets:lookup(?TASKS_TABLE, TaskId) of
        [#mcp_task{status = Status,
            ...
            UpdatedTask =
                Task#mcp_task{status = completed,
                              result = Result,
                              updated_at = erlang:system_time(millisecond),
                              timer_ref = undefined},
            ets:insert(?TASKS_TABLE, UpdatedTask),
            ...
            ErrorRec =
                case Error of
                    ...
                    _ when is_binary(Error) ->
                        #mcp_error{code = ?MCP_ERROR_TASK_FAILED,
                                   message = Error,
                                   data = undefined};
                    _ ->
                        #mcp_error{code = ?MCP_ERROR_TASK_FAILED,
            ...
            {Current, Total} =
                case Progress of
                    ...
                    Cur when is_number(Cur) ->
                        {Cur, Task#mcp_task.total}
                end,

            UpdatedTask =
                Task#mcp_task{progress = Current,
                              total = Total,
                              updated_at = erlang:system_time(millisecond)},
            ets:insert(?TASKS_TABLE, UpdatedTask),
    ...
    case ets:lookup(?TASKS_TABLE, TaskId) of
        [#mcp_task{status = Status} = Task] when Status =:= pending; Status =:= processing ->
            UpdatedTask =
                Task#mcp_task{status = failed,
                              error =
                                  #mcp_error{code = ?MCP_ERROR_TASK_TIMEOUT,
...
do_handle_worker_down(MonitorRef, State) ->
    case ets:foldl(fun (#mcp_task{worker_monitor = Mon, status = processing} = Task, Acc)
                           when Mon =:= MonitorRef ->
                           [Task | Acc];
                       (_, Acc) ->
                           Acc
...
%% @private
format_task_for_api(#mcp_task{id = Id,
                              status = Status,
                              action = Action,
                              metadata = Metadata,
                              result = Result,
    ...
    Base1 =
        case Progress of
            undefined ->
                Base;
            _ ->
                Base#{?MCP_PARAM_PROGRESS => Progress}
        end,
    ...
    Base3 =
        case ExpiresAt of
            undefined ->
                Base2;
            _ ->
                Base2#{<<"expiresAt">> => ExpiresAt}
        end,

    Base4 =
        case ProgressToken of
            undefined ->
                Base3#{<<"progressToken">> => null};
            _ ->
                Base3#{<<"progressToken">> => ProgressToken}
        end,
    ...
    Base6 =
        case Error of
            ...
            #mcp_error{code = Code,
                ...
                DataBin =
                    maps:map(fun(_K, V) -> V end,
                             maps:fold(fun(K, V, Acc) -> Acc#{atom_to_binary(K) => V} end,
                                       #{},
                                       Data)),
...
%% @private
status_to_binary(pending) ->
    <<"pending">>;
status_to_binary(processing) ->
    <<"processing">>;
status_to_binary(completed) ->
    <<"completed">>;
status_to_binary(failed) ->
    <<"failed">>;
status_to_binary(cancelled) ->
    <<"cancelled">>.

%% @private
cancel_task_timer(#mcp_task{timer_ref = undefined}) ->
    ok;
...
%% @private
send_task_notification(undefined, _TaskId) ->
    ok;
...
send_task_notification(ClientPidName, TaskId) when is_atom(ClientPidName) ->
    ...
    case whereis(ClientPidName) of
        undefined ->
            ok;
        Pid when is_pid(Pid) ->
            send_notification_to_pid(Pid, TaskId)
    end.
        ...
        [#mcp_task{} = Task] ->
            Notification =
                #{?JSONRPC_FIELD_JSONRPC => ?JSONRPC_VERSION,
                  ...
                  ?JSONRPC_FIELD_PARAMS => #{?MCP_PARAM_TASK => format_task_for_api(Task)}},
...
resolve_client_pid(undefined) ->
    undefined;
resolve_client_pid(Pid) when is_pid(Pid) ->
    Pid;
...
match_client_pids(undefined, _TaskPid) ->
    true;
match_client_pids(_QueryPid, undefined) ->
    true;
match_client_pids(QueryPid, TaskPid) when QueryPid =:= TaskPid ->
    true;
match_client_pids(_, _) ->
    false.
...
do_create_task_with_options(ClientPid, Action, Metadata, Options, State) ->
    ...
    TimeoutMs =
        case maps:get(<<"timeout">>, Metadata, undefined) of
            undefined ->
            ...
            case match_client_pids(NormalizedPid, TaskNormalized) of
                true ->
                    try
                        ...
                        NewMetadata =
                            maps:get(<<"metadata">>, UpdatedTaskMap, Task#mcp_task.metadata),

                        UpdatedTask =
                            Task#mcp_task{metadata = NewMetadata,
                                          updated_at = erlang:system_time(millisecond)},
                        ets:insert(?TASKS_TABLE, UpdatedTask),
                        ok
                    catch
