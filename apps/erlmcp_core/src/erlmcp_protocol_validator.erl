...
validate_field_type(_FieldName, Value, any) ->
...
format_validation_error(#{reason := Reason, details := Details}) ->
    ...
    <<(atom_to_binary(Reason, utf8))/binary, ": ", DetailsBin/binary>>.
...
validate_error_object(ErrorObj) when is_map(ErrorObj) ->
    ...
    case {maps:get(<<"code">>, ErrorObj, undefined), maps:get(<<"message">>, ErrorObj, undefined)} of
        {undefined, _} ->
        ...
        {Code, _} ->
            {error, #{reason => invalid_error_code_type, details => #{code => Code}}};
        {_, Message} ->
            {error, #{reason => invalid_error_message_type, details => #{message => Message}}}
    end;
validate_error_object(ErrorObj) ->
...
validate_result_or_error(ResultOrError) when is_map(ResultOrError) ->
    %% Assume this is an error object
