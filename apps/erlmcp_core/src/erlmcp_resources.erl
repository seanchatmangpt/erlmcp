...
%% State record
-record(state, {roots = #{} :: #{root_uri() => root_entry()}}).
...
init([]) ->
    ...
    {ok, Cwd} = file:get_cwd(),
    DefaultRoot =
        #{uri => list_to_binary(["file://", Cwd]), name => <<"Current Working Directory">>},
