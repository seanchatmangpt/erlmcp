...
%% API exports
-export([start_link/0, register_component/2, unregister_component/1, send_priority/2,
         send_health_check/2, send_drain_session/2, send_cancel_task/2, send_circuit_breaker/2,
         get_stats/0, reset_stats/0]).
...
-spec send_health_check(component_id(), health_check_type()) ->
...
-spec send_drain_session(component_id(), binary() | atom()) ->
                            {ok, non_neg_integer()} | {error, term()}.
...
send_cancel_task(ComponentId, TaskId) ->
    Message = {control, cancel_task, #{task_id => TaskId, timeout_ms => 1000}},
    send_priority(ComponentId, Message).

%% @doc Convenience function for circuit breaker control
-spec send_circuit_breaker(component_id(), circuit_breaker_action()) ->
                              {ok, non_neg_integer()} | {error, term()}.
...
handle_info({'DOWN', MonitorRef, process, _Pid, _Reason}, State) ->
    ...
    NewComponents =
        maps:filter(fun(_Id, #{monitor_ref := Ref}) -> Ref =/= MonitorRef end,
                    State#state.components),
...
terminate(_Reason, #state{components = Components}) ->
    ...
    maps:foreach(fun(_Id, #{pid := Pid, monitor_ref := MonitorRef}) ->
                    demonitor(MonitorRef, [flush]),
                    exit(Pid, normal)
                 end,
                 Components),
...
compute_stats(#{total_delivered := Total,
                by_type := ByType,
                latencies := Latencies,
                max_latency_us := MaxLatency,
                slo_violations := Violations}) ->
    ...
    {P50, P95, P99} =
        case length(SortedLatencies) of
            0 ->
                {0, 0, 0};
            Len ->
