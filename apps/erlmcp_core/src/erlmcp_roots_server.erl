...
-export([
    ...
    get_root/1,
    remove_root/1,
    subscribe_root_changes/1,
    unsubscribe_root_changes/1
]).
...
handle_call(list_roots, _From, State) ->
    Roots = ets:foldl(fun({_Id, Root}, Acc) ->
        [Root | Acc]
    end, [], State#state.roots_ets),
...
handle_call({remove_root, RootId}, _From, State) ->
    case ets:lookup(State#state.roots_ets, RootId) of
        [{_, Root}] ->
            ...
            %% Update root list
            NewRootList = lists:filter(fun({Id, _Uri, _Meta}) ->
                Id =/= RootId
            end, State#state.root_list),
            NewState = State#state{root_list = NewRootList},
...
handle_info({'DOWN', MonitorRef, process, Pid, _Reason}, State) ->
    ...
    NewSubscribers = maps:filter(fun(SubPid, {MRef, _SubId}) ->
        not (SubPid =:= Pid andalso MRef =:= MonitorRef)
...
notify_subscribers(EventType, Root, State) ->
    maps:foreach(fun(Pid, {_MonitorRef, SubId}) ->
        Pid ! {root_change, SubId, EventType, Root}
    end, State#state.subscribers),
    ok.
