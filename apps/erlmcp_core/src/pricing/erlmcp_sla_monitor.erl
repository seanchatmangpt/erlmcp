...
%% API
-export([start_link/0, start_link/1, monitor_envelope/2, check_throughput/1, check_latency/1,
         check_failover/1, alert_sla_violation/2, export_sla_metrics/2, get_sla_status/1,
         get_sla_dashboard/1, stop/0]).
...
handle_call({check_throughput, Plan}, _From, State) ->
    ...
    Reply =
        case CurrentThroughput >= MinThroughput of
            true ->
...
handle_call({check_failover, Plan}, _From, State) ->
    ...
    Reply =
        case CurrentFailover =< MaxFailover of
            true ->
                {ok, CurrentFailover};
            false ->
...
handle_cast({start_monitoring, Plan, Version}, State) ->
    ...
    NewMonitored =
        maps:put(Plan, {Version, erlang:system_time(millisecond)}, State#state.monitored_plans),
    {noreply, State#state{monitored_plans = NewMonitored}};
handle_cast({record_violation, Plan, Type, Expected, Actual}, State) ->
    ?LOG_WARNING("SLA Violation for ~w: ~w (expected ~w, got ~w)", [Plan, Type, Expected, Actual]),

    Violation =
        #sla_violation{timestamp = erlang:system_time(millisecond),
                       plan = Plan,
                       violation_type = Type,
                       expected = Expected,
                       actual = Actual,
...
get_plan_status(Plan, State) ->
    ...
    ThroughputStatus =
        case Throughput >= MinThroughput of
            true ->
                'PASS';
            false ->
                'FAIL'
    ...
    OverallStatus =
        case {ThroughputStatus, LatencyStatus, FailoverStatus} of
            {'PASS', 'PASS', 'PASS'} ->
                'PASS';
            _ ->
                'FAIL'
...
get_plan_dashboard(Plan, State) ->
    ...
    Status#{description => maps:get(description, Envelope, ""),
...
count_recent_violations(Violations, WindowMinutes) ->
    ...
    length(lists:filter(fun(V) -> V#sla_violation.timestamp >= CutoffTime end, Violations)).
...
prepare_export_metrics(Plan, State) ->
    ...
    Status#{violations_detail => ExportViolations,
...
log_to_receipt_chain(Violation) ->
    ...
    Message =
        io_lib:format("SLA Violation: Plan=~w, Type=~w, Expected=~w, Actual=~w, Severity=~w",
