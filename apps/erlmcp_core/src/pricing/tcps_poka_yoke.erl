...
-export([validate_plan/1, validate_plan_file/1, validate_all_plans/1]).
%% API - Individual validators (for unit testing)
-export([validate_plan_schema/1, validate_envelope_consistency/1, validate_refusal_codes_exist/1,
         validate_evidence_requirements/1]).
%% API - Utility functions
-export([format_error/1, format_errors/1]).
...
validate_plan(_Data) ->
    {error,
     [{schema, root, <<"Plan must be a JSON object">>, <<"Ensure input is valid JSON map">>, 0}]}.
                ...
                error:_ ->
                    {error,
                     [{parse,
                       ...
                       <<"Check JSON syntax - must be valid RFC 7158">>,
                       1}]}
            end;
        {error, enoent} ->
...
-spec validate_all_plans(file:filename()) -> {ok, map()} | {error, map()}.
validate_all_plans(PlansDir) ->
    case filelib:wildcard(
             filename:join(PlansDir, "*.plan.json"))
    of
        [] ->
            {error,
             #{no_plans => <<"No .plan.json files found">>, directory => list_to_binary(PlansDir)}};
        PlanFiles ->
            Results =
                maps:from_list([{filename:basename(F), validate_plan_file(F)} || F <- PlanFiles]),

            case lists:partition(fun({_, R}) -> R == {ok, []} end, maps:to_list(Results)) of
                {Passed, []} ->
        ...
        #{} ->
            ...
            case lists:filter(fun(E) -> E =/= ok end, Errors) of
                ...
                _Errs ->
                    {schema,
                     pricing,
                     <<"pricing has missing fields">>,
                     <<"Check pricing.model, pricing.description, pricing.cost">>,
                     Line}
            end;
        _ ->
            {schema,
...
check_envelope_fields(Envelope, Line) ->
    ...
    MissingFields = lists:filter(fun(F) -> not maps:is_key(F, Envelope) end, RequiredFields),
    ...
    case MissingFields of
        ...
        Missing ->
            {schema,
             envelope,
             iolist_to_binary(io_lib:format("Missing envelope fields: ~w", [Missing])),
...
check_limits_fields(Limits, Line) ->
    ...
    MissingFields = lists:filter(fun(F) -> not maps:is_key(F, Limits) end, RequiredFields),
    ...
    case MissingFields of
        ...
        Missing ->
            {schema,
             limits,
             iolist_to_binary(io_lib:format("Missing limits fields: ~w", [Missing])),
...
check_evidence_fields_schema(Evidence, Line) ->
    ...
    MissingFields = lists:filter(fun(F) -> not maps:is_key(F, Evidence) end, RequiredFields),
    ...
    case MissingFields of
        ...
        Missing ->
            {schema,
             evidence,
             iolist_to_binary(io_lib:format("Missing evidence artifacts: ~w", [Missing])),
...
validate_envelope_consistency(Plan) ->
    ...
    Errors =
        [check_concurrent_connections_limit(maps:get(concurrent_connections, Envelope, 0)),
         check_throughput_variance(maps:get(throughput_req_s, Envelope, 0),
                                   maps:get(concurrent_connections, Envelope, 0)),
         check_queue_depth(maps:get(queue_depth_messages, Envelope, 0),
         ...
         check_timeout_values(maps:get(failover_sla_seconds, Envelope, 0),
...
check_throughput_variance(Throughput, Concurrent) when Throughput > 0, Concurrent > 0 ->
    ...
    case Throughput > MaxThroughput of
        true ->
            {envelope,
             throughput_req_s,
             iolist_to_binary(io_lib:format("Throughput ~w exceeds 2x baseline (~w) for ~w connections",
...
check_queue_depth(QueueDepth, MaxMsgSize) when QueueDepth > 0, MaxMsgSize > 0 ->
...
check_timeout_values(FailoverSLA, ConnectionTimeout) when FailoverSLA > ConnectionTimeout ->
    {envelope,
     connection_timeout_seconds,
...
validate_refusal_codes_exist(Plan) ->
    ...
    Errors =
        maps:fold(fun(Key, RefusalEntry, Acc) ->
                     case RefusalEntry of
                         ...
                         _ ->
                             [{refusal,
                               ...
                               iolist_to_binary(io_lib:format("Add error_code to refusal_behavior.~w",
                                                              [Key])),
                               20}
                              | Acc]
                     end
    ...
    try
        ...
        case erlmcp_refusal:is_valid_code(Code) of
            ...
            false ->
                {refusal,
                 error_code,
                 iolist_to_binary(io_lib:format("Error code ~w does not exist in erlmcp_refusal.erl",
                                                [Code])),
...
check_evidence_path(Evidence, ArtifactName, Line) ->
    case maps:find(ArtifactName, Evidence) of
        ...
        {ok, Path} ->
            {evidence,
             ArtifactName,
             iolist_to_binary(io_lib:format("Evidence path must be non-empty string, got: ~w",
                                            [Path])),
        ...
        error ->
            {evidence,
             ArtifactName,
             iolist_to_binary(io_lib:format("Missing evidence artifact: ~w", [ArtifactName])),
             iolist_to_binary(io_lib:format("Add ~w path to evidence section", [ArtifactName])),
             Line}
    end.
