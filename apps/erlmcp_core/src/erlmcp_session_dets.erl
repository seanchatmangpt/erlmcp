...
store(SessionId, Session, State) ->
    case dets:insert(
             maps:get(table_name, State), {SessionId, Session})
    of
        ok ->
            {ok, State};
        {error, Reason} ->
...
fetch(SessionId, State) ->
    case dets:lookup(
             maps:get(table_name, State), SessionId)
    of
        [{SessionId, Session}] ->
            %% Update last accessed time
            UpdatedSession = Session#{last_accessed => erlang:system_time(millisecond)},
            case dets:insert(
                     maps:get(table_name, State), {SessionId, UpdatedSession})
            of
                ok ->
...
list(State) ->
    case dets:foldl(fun({SessionId, _Session}, Acc) -> [SessionId | Acc] end,
                    [],
...
cleanup_expired(State) ->
    ...
    case dets:foldl(fun({_SessionId, Session}, Acc) ->
                       case is_expired(Session, Now) of
                           true ->
                               [maps:get(id, Session) | Acc];
                           false ->
                               Acc
                       end
                    end,
