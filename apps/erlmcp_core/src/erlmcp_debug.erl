...
-export([get_state/1, get_status/1, trace/1, trace/2, untrace/1, suspend/1, resume/1,
         replace_state/2, statistics/1, log_to_file/2, install_all/1, remove_all/1,
         debug_all_servers/0, debug_all_clients/0, debug_registry/0]).
...
debug_all_clients() ->
    ...
    Clients =
        [Pid
         || {_Name, Pid, _Type, [erlmcp_client]} <- supervisor:which_children(erlmcp_client_sup)],
    lists:map(fun(Pid) ->
                 State = get_state(Pid),
                 Stats = statistics(Pid),
                 {Pid, #{state => State, stats => Stats}}
              end,
              Clients).
