...
-export([start_link/0, get/1, get/2, get_all/0, set/2, update/1, delete/1, reload/0,
         clear_cache/0]).
...
%% State record
-record(state, {defaults :: #{atom() => term()}, loaded_at :: erlang:timestamp()}).
...
get_all() ->
    ...
    ConfigTerms = [{K, V} || {{erlmcp_config, K}, V} <- AllTerms],
...
handle_call({update, Updates}, _From, State) ->
    maps:foreach(fun(K, V) -> persistent_term:put(?KEY(K), V) end, Updates),
...
handle_call(clear_cache, _From, State) ->
    ...
    AllTerms = persistent_term:get(),
    lists:foreach(fun ({Key, _}) when element(1, Key) =:= erlmcp_config ->
...
load_config(Defaults) ->
    maps:foreach(fun(Key, DefaultValue) ->
