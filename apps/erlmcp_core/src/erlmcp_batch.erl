...
%% API exports
-export([start_link/1, start_link/2, add_request/3, add_request/4, flush/1, get_stats/1,
         update_strategy/2, stop/1]).
...
-type batch_strategy() ::
    {size, N :: pos_integer()} |
    {time, Ms :: pos_integer()} |
    {adaptive, #{min => pos_integer(), max => pos_integer()}}.

                                                                         % Batch every N requests

                                           % Batch every Ms milliseconds
...
-export_type([batch_strategy/0, batch_opts/0, executor_fun/0, request/0, result/0]).
    ...
    AdaptiveState =
        case Strategy of
            {adaptive, AdaptiveOpts} ->
                ...
                #adaptive_state{current_size = Min,
                                min_size = Min,
                                max_size = Max};
            _ ->
                undefined
...
handle_cast({add_request, RequestId, Method, Params, CallerPid, Ref}, State) ->
    ...
    NewState =
        State#state{pending_requests = NewPending, batch_count = State#state.batch_count + 1},
...
should_execute_batch(#state{strategy = {time, _Ms}}) ->
    ...
    false;
should_execute_batch(#state{strategy = {adaptive, _},
                            adaptive_state = AdaptiveState,
                            batch_count = Count}) ->
...
execute_batch(#state{pending_requests = PendingRequests} = State) ->
    ...
    NewAvgBatchSize =
        (Stats#batch_stats.avg_batch_size * Stats#batch_stats.total_batches + BatchSize)
        / TotalBatches,
    NewAvgLatency =
        (Stats#batch_stats.avg_latency_us * Stats#batch_stats.total_batches + LatencyUs)
        / TotalBatches,

    NewStats =
        Stats#batch_stats{total_requests = TotalRequests,
    ...
    NewAdaptiveState =
        case State#state.adaptive_state of
            undefined ->
                undefined;
            AdaptiveState ->
             ...
             spawn_link(fun() ->
                           ChunkResults =
                               try
                                   ...
                                   Class:Reason:Stack ->
                                       logger:error("Batch executor crashed: ~p:~p~n~p",
                                                    [Class, Reason, Stack]),
...
update_adaptive_state(AdaptiveState, BatchSize, LatencyUs, Failures) ->
    ...
    NewSize =
        if % High failure rate, decrease batch size
           FailureRate > 0.1 ->
    ...
    AdaptiveState#adaptive_state{current_size = NewSize,
