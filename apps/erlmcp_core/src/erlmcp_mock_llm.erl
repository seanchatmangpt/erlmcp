...
-export([
    create_message/2,
    create_message_with_delay/2,
    set_response_mode/1,
    create_echo_response/2,
...
create_message(Messages, Params) when is_list(Messages), is_map(Params) ->
    ...
    ResponseMode = case get(response_mode) of
        undefined -> echo;
        Val -> Val
    end,
...
create_echo_response(Messages, _Params) ->
    ...
    LastUserMessage = case lists:reverse(Messages) of
    ...
    ResponseContent = case is_binary(LastUserMessage) of
        true ->
    ...
    {ok, #{
        ...
        <<"usage">> => #{
            ...
            <<"completionTokens">> => estimate_tokens([#{<<"role">> => <<"assistant">>, <<"content">> => ResponseContent}]),
            <<"totalTokens">> => 0
        }
    }}.
...
create_template_response(Messages, Params) ->
    ...
    ResponseText = case length(Messages) of
        1 ->
        ...
        N when N > 2 ->
            <<"This is a mock response acknowledging the conversation history of ",
              (integer_to_binary(N))/binary, " messages.">>;
        _ ->
...
estimate_tokens(Messages) when is_list(Messages) ->
    lists:foldl(fun(#{<<"content">> := Content}, Acc) ->
