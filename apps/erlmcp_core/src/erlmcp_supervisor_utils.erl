...
%% API exports
-export([
    get_children_status/1,
    get_supervision_tree/1,
    get_supervision_tree_flat/1,
    count_processes/1,
    calculate_health_score/1,
    export_to_json/1,
    export_to_json_pretty/1,
    get_process_metrics/1,
    get_tree_metrics/1,
    find_unhealthy_processes/1,
    get_restart_statistics/1,
    validate_supervision_tree/1
]).
...
-type child_process_status() ::
    not_started | restarting | dead |
    #{status => atom(), queue_len => non_neg_integer(), memory => non_neg_integer()} |
...
calculate_health_score(SupRef) ->
    try
        ...
        if
            TotalChildren =:= 0 -> 1.0; % Empty supervisor is healthy
            true ->
                Penalties = lists:foldl(fun(Child, Acc) ->
                    Acc + calculate_child_penalty(Child)
                end, 0.0, Children),
                ...
                ChildScores = [
                    ...
                    || #{type := supervisor, pid := Pid} <- Children,
                       is_pid(Pid)
                ],

                case ChildScores of
                    [] -> Score;
                    _ -> (Score + lists:sum(ChildScores)) / (1 + length(ChildScores))
                end
        end
    catch
        _:_ -> 0.0
    ...
    {SupervisorCount, WorkerCount, TotalMemory} =
        lists:foldl(fun(#{pid := Pid}, {Sups, Workers, Mem}) ->
            case get_process_metrics(Pid) of
                {error, _} -> {Sups, Workers, Mem};
                Metrics ->
                    IsSupervisor = is_supervisor(Pid),
                    NewSups = if IsSupervisor -> Sups + 1; true -> Sups end,
                    NewWorkers = if IsSupervisor -> Workers; true -> Workers + 1 end,
...
find_unhealthy_processes(SupRef) ->
    ...
    Unhealthy = lists:filtermap(fun(Child) ->
        case maps:get(status, Child) of
            dead -> {true, #{pid => maps:get(pid, Child), reason => dead}};
            restarting -> {true, #{pid => maps:get(pid, Child), reason => restarting}};
            not_started -> {true, #{pid => maps:get(pid, Child), reason => not_started}};
            Status when is_map(Status) ->
                QueueLen = maps:get(queue_len, Status, 0),
                Memory = maps:get(memory, Status, 0),
                if
                    QueueLen > 1000 ->
                        {true, #{pid => maps:get(pid, Child), reason => high_queue, queue_len => QueueLen}};
                    Memory > 100000000 ->
                        {true, #{pid => maps:get(pid, Child), reason => high_memory, memory => Memory}};
                    true ->
                        false
                end;
            _ ->
    ...
    % Recursively check child supervisors
    ChildUnhealthy = lists:flatmap(fun(Child) ->
...
-spec validate_supervision_tree(sup_ref()) ->
    ...
    {error, #{valid := false, violations := [#{check := atom(), reason := term()}]}}.
validate_supervision_tree(SupRef) ->
    ...
    Violations = lists:filtermap(fun({CheckName, CheckFun}) ->
        try
            case CheckFun() of
                true -> false;
                false -> {true, #{check => CheckName, reason => failed}};
                {false, Reason} -> {true, #{check => CheckName, reason => Reason}}
            end
        catch
            _:Error ->
...
count_processes_internal(SupRef, Acc) ->
    try
        ...
        Children = supervisor:which_children(Pid),
        lists:foldl(fun
            ({_, ChildPid, supervisor, _}, A) when is_pid(ChildPid) ->
...
calculate_child_penalty(#{status := Status}) when is_map(Status) ->
    ...
    MemoryPenalty = if
        Memory > 100000000 -> 0.2; % >100MB
        Memory > 50000000 -> 0.1;  % >50MB
        true -> 0.0
    end,
...
flatten_tree(#{supervisor := SupRef, pid := Pid, children := Children}, Acc) when is_pid(Pid) ->
    Entry = #{pid => Pid, supervisor => SupRef},

    ChildEntries = lists:flatmap(fun(Child) ->
        case Child of
...
serialize_tree(#{supervisor := SupRef, pid := Pid, health_score := Score, children := Children} = Tree) ->
    #{
...
serialize_child(#{id := Id, type := Type, pid := Pid, modules := Mods, tree := Tree}) ->
    Base = #{
        <<"id">> => serialize_id(Id),
    ...
    case Tree of
        undefined -> Base;
        T when is_map(T) -> Base#{<<"tree">> => serialize_tree(T)}
    end.
...
calculate_max_depth(#{children := Children}, CurrentDepth) ->
    ChildDepths = [
        calculate_max_depth(Tree, CurrentDepth + 1)
        || #{tree := Tree} <- Children,
           is_map(Tree)
    ],
...
resolve_supervisor_pid(Name) when is_atom(Name) ->
    case whereis(Name) of
        undefined -> error({supervisor_not_running, Name});
        Pid -> Pid
    end.
...
check_no_dead_children(SupRef) ->
    Children = get_children_status(SupRef),
    DeadChildren = [
        Id || #{id := Id, status := dead} <- Children
    ],

    case DeadChildren of
        [] -> true;
        _ -> {false, {dead_children, DeadChildren}}
    end.
...
check_no_high_queues(SupRef) ->
    ...
    case HighQueues of
        [] -> true;
        _ -> {false, {high_queues, HighQueues}}
    end.
