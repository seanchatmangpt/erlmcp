...
%% API
-export([emit_tool_call/3, emit_resource_read/3, emit_prompt_render/3, emit_subscription_add/2,
         attach_default_handler/0, detach_default_handler/0]).
...
emit_resource_read(ResourceUri, DurationUs, Status) ->
    ...
    telemetry:execute([erlmcp, resource, read],
                      #{duration_us => DurationUs, count => 1},
                      Metadata#{status_atom => StatusAtom}).
...
emit_prompt_render(PromptName, DurationUs, Status) ->
    ...
    telemetry:execute([erlmcp, prompt, render],
                      #{duration_us => DurationUs, count => 1},
                      Metadata#{status_atom => StatusAtom}).
...
emit_subscription_add(Uri, SubscriberPid) ->
    telemetry:execute([erlmcp, subscription, add],
                      #{count => 1},
...
attach_default_handler() ->
    ...
    telemetry:attach_many(<<"erlmcp-default-handler">>,
                          Events,
...
-spec handle_telemetry_event([atom()], #{atom() => number()}, #{atom() => term()}, term()) -> ok.
