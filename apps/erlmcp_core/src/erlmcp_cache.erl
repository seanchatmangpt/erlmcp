...
-type cache_strategy() ::
    {ttl, Seconds :: pos_integer()} |
    {lru, MaxSize :: pos_integer()} |
    write_through |
    write_back.
...
check_etag(Key, ETag) ->
    case get(Key) of
        {ok, Value} -> generate_etag(Value) =:= ETag;
...
init(Config) ->
    ...
    L1Table = ets:new(erlmcp_cache_l1, [
        set,
        public,
        named_table,
...
terminate(_Reason, State) ->
    ...
    case State#state.l3_conn of
        undefined -> ok;
        Conn -> close_external_cache(State#state.l3_module, Conn)
    end,
...
perform_cleanup(State) ->
    ...
    NewState = lists:foldl(fun(_, S) ->
        update_stats(S, expiration)
    end, State, ExpiredKeys),
