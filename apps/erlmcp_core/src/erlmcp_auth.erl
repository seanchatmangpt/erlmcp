...
verify_jwt_signature(Token, State) ->
    try
        ...
        Protected = erlmcp_json_native:decode(ProtectedBin, [return_maps]),
...
verify_jwt_with_key(Token, PublicKeyPem, State) ->
    try
        ...
        case jose_jws:verify(JWK, Token) of
            {true, Payload, _JWS} ->
                Claims = erlmcp_json_native:decode(Payload, [return_maps]),
...
handle_introspect_response(Status, _Headers, Body, Token, State) ->
    case Status of
        200 ->
            try
                TokenInfo = erlmcp_json_native:decode(Body, [return_maps]),
