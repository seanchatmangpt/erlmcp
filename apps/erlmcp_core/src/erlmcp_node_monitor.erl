...
-spec get_node_health(node()) -> {ok, #{status => up | down, last_seen => integer()}} | {error, not_found}.
...
init([]) ->
    ...
    MonitoredNodes = maps:from_list([{Node, #{status => down, last_seen => Now}} || Node <- ClusterNodes]),
    ...
    end, MonitoredNodes, ClusterNodes),

    State = #node_monitor_state{
        monitored_nodes = UpdatedNodes,
...
handle_info({nodeup, Node, _InfoList}, State) ->
    ...
    Now = erlang:system_time(second),
    UpdatedNodes = maps:put(Node, #{status => up, last_seen => Now}, State#node_monitor_state.monitored_nodes),
...
handle_info({nodedown, Node, InfoList}, State) ->
    ...
    Now = erlang:system_time(second),
    UpdatedNodes = maps:update_with(Node,
...
perform_node_check(State) ->
    ...
    UpdatedNodes = maps:map(fun(Node, Health) ->
        Status = case lists:member(Node, ConnectedNodes) of
            true -> up;
            false -> down
        end,
        Health#{status => Status, last_seen => Now}
    end, State#node_monitor_state.monitored_nodes),
