...
format_error({ErrorCode, ErrorMsg}) when is_atom(ErrorCode), is_binary(ErrorMsg) ->
    #{<<"code">> => error_code(ErrorCode),
      <<"message">> => ErrorMsg,
      <<"data">> => #{}};
format_error({ErrorCode, ErrorMsg, Data})
    when is_atom(ErrorCode), is_binary(ErrorMsg), is_map(Data) ->
    #{<<"code">> => error_code(ErrorCode),
      <<"message">> => ErrorMsg,
      <<"data">> => Data}.
...
refusal_reason(1010) ->
    <<"Custom refusal">>;
refusal_reason(Code) when is_integer(Code), Code >= 1001, Code =< 1089 ->
    <<"Tool refused">>;
refusal_reason(_) ->
    undefined.
