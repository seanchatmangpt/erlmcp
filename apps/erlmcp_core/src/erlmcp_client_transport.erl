...
handle_call({connect, NewOpts}, _From, #state{transport_pid = TransportPid} = State)
  when is_pid(TransportPid) ->
    ...
    Result = case State#state.transport_type of
        tcp -> erlmcp_transport_tcp:connect(TransportPid, NewOpts);
...
terminate(_Reason, #state{transport_type = Type, transport_pid = Pid,
...
do_send(Data, #state{transport_type = stdio, transport_state = Pid})
  when is_pid(Pid) ->
    erlmcp_transport_stdio:send(Pid, Data);

do_send(Data, #state{transport_type = tcp, transport_state = Pid})
  when is_pid(Pid) ->
    erlmcp_transport_tcp:send(Pid, Data);

do_send(Data, #state{transport_type = http, transport_state = Pid})
  when is_pid(Pid) ->
...
ensure_map(Opts) when is_map(Opts) -> Opts;
ensure_map(Opts) when is_list(Opts) -> maps:from_list(Opts).
