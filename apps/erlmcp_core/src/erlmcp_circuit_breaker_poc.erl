...
%% API
-export([start_link/0, start_link/1, stop/1, call/2, get_state/1, reset/1, get_stats/1]).
...
init(Opts) ->
    ...
    {ok,
     #state{failure_threshold = FailureThreshold,
...
handle_call(reset, _From, State) ->
    ...
    {reply,
     ok,
     #state{failure_threshold = State#state.failure_threshold,
