...
%% Public API
-export([start_server/2, stop_server/1, list_servers/0, get_server_config/1, update_server_config/2,
         add_tool/3, remove_tool/2, add_resource/3, remove_resource/2, add_prompt/3,
         remove_prompt/2, start_transport/3, stop_transport/1, list_transports/0,
         bind_transport_to_server/2, get_transport_bindings/0, get_transport_config/1,
         update_transport_config/2, validate_transport_config/2, find_server/1, status/0, session/1,
         streams/1, tasks/0, queues/0, health/0, sessions/0, servers/0, transports/0, help/0]).
...
%% @end
%%--------------------------------------------------------------------
-spec start_server(ServerId :: atom(), Config :: map()) -> {ok, pid()} | {error, term()}.
...
%% @end
%%--------------------------------------------------------------------
-spec get_server_config(ServerId :: atom()) -> {ok, map()} | {error, term()}.
...
%% @end
%%--------------------------------------------------------------------
-spec update_server_config(ServerId :: atom(), Config :: map()) -> ok | {error, term()}.
...
%% @end
%%--------------------------------------------------------------------
-spec add_tool(ServerId :: atom(), ToolName :: binary(), Handler :: function()) ->
                  ok | {error, term()}.
...
%% @end
%%--------------------------------------------------------------------
-spec remove_tool(ServerId :: atom(), ToolName :: binary()) -> ok | {error, term()}.
...
%% @end
%%--------------------------------------------------------------------
-spec add_resource(ServerId :: atom(), Uri :: binary(), Handler :: function()) ->
                      ok | {error, term()}.
...
%% @end
%%--------------------------------------------------------------------
-spec remove_resource(ServerId :: atom(), Uri :: binary()) -> ok | {error, term()}.
...
%% @end
%%--------------------------------------------------------------------
-spec add_prompt(ServerId :: atom(), PromptName :: binary(), Handler :: function()) ->
                    ok | {error, term()}.
...
%% @end
%%--------------------------------------------------------------------
-spec remove_prompt(ServerId :: atom(), PromptName :: binary()) -> ok | {error, term()}.
...
%% @end
%%--------------------------------------------------------------------
-spec start_transport(TransportId :: atom(), Type :: atom(), Config :: map()) ->
                         {ok, pid()} | {error, term()}.
...
%% @end
%%--------------------------------------------------------------------
-spec bind_transport_to_server(TransportId :: atom(), ServerId :: atom()) -> ok | {error, term()}.
...
%% @end
%%--------------------------------------------------------------------
-spec get_transport_bindings() -> [{atom(), atom()}] | {error, term()}.
...
%% @end
%%--------------------------------------------------------------------
-spec get_transport_config(TransportId :: atom()) -> {ok, map()} | {error, term()}.
...
%% @end
%%--------------------------------------------------------------------
-spec update_transport_config(TransportId :: atom(), Config :: map()) -> ok | {error, term()}.
...
%% @end
%%--------------------------------------------------------------------
-spec validate_transport_config(Type :: atom(), Config :: map()) -> ok | {error, term()}.
...
%% @end
%%--------------------------------------------------------------------
-spec find_server(ServerId :: atom()) -> {ok, {pid(), map()}} | {error, not_found}.
...
servers() ->
    case whereis(erlmcp_registry) of
        ...
        _Pid ->
            ...
            io:format("~s~n", [lists:duplicate(60, $=)]),
            lists:foreach(fun({ServerId, {Pid, Config}}) ->
                             QueueLen =
...
transports() ->
    case whereis(erlmcp_registry) of
        ...
        _Pid ->
            ...
            io:format("~s~n", [lists:duplicate(60, $=)]),
            lists:foreach(fun({TransportId, {Pid, Config}}) ->
