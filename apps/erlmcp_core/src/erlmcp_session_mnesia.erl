...
-record(erlmcp_session,
        {session_id :: session_id(), session_data :: session(), last_accessed :: integer()}).
        ...
        true ->
            ...
            NeedsRecreate =
                case CurrentStorage of
                    disc_copies when not UseDiscCopies ->
                        true;
                    ram_copies when UseDiscCopies ->
                        true;
                    disc_only_copies when not UseDiscCopies ->
                        true;
                    _ ->
                        false
                end,
...
create_mnesia_table(TableName, Nodes, UseDiscCopies) ->
    ...
    of
        {atomic, ok} ->
            logger:info("Created Mnesia table: ~p (storage: ~p)",
                        [TableName,
                         case UseDiscCopies of
                             true ->
                                 disc_copies;
...
-spec store(session_id(), session(), state()) -> {ok, state()} | {error, term()}.
store(SessionId, Session, State) ->
    ...
    Record = {TableName, SessionId, Session, erlang:system_time(millisecond)},
...
-spec delete(session_id(), state()) -> {ok, state()} | {error, term(), state()}.
delete(SessionId, State) ->
    Transaction =
        fun() ->
           mnesia:delete(
               maps:get(table_name, State), SessionId, write)
        end,
...
-spec list(state()) -> {ok, [session_id()], state()}.
list(State) ->
    Transaction =
        fun() ->
           mnesia:all_keys(
...
-spec cleanup_expired(state()) -> {ok, non_neg_integer(), state()}.
cleanup_expired(State) ->
    ...
    Transaction =
        fun() ->
           ...
           lists:foreach(fun(SessionId) -> mnesia:delete(TableName, SessionId, write) end,
                         ExpiredSessions),
