...
%% API exports
-export([start_link/0, get/3, put/5, invalidate/2, clear/0, stats/0, memory_usage/0]).
...
-spec get(icon_type(), icon_uri(), icon_size()) -> {ok, icon_data()} | {error, not_found | expired}.
get(Type, Uri, Size) ->
...
init([]) ->
    ...
    State = #state{cache_table = CacheTable, cleanup_timer = TimerRef},
...
handle_call({put, Type, Uri, Size, Data, TTLMs}, _From, State) ->
    ...
    NewState =
        case State#state.current_memory_bytes + DataSize > State#state.max_memory_bytes of
            true ->
...
handle_call({invalidate, Type, Uri}, _From, State) ->
    ...
    KeysToDelete =
        ets:foldl(fun({{T, U, _S} = Key, _Entry}, Acc) ->
                  ...
                  [],
                  State#state.cache_table),

    %% Delete entries and update memory usage
    {DeletedBytes, NewState} =
        lists:foldl(fun(Key, {Bytes, AccState}) ->
    ...
    FinalState =
        NewState#state{current_memory_bytes = NewState#state.current_memory_bytes - DeletedBytes},

    ?LOG_INFO("Icon cache invalidated: ~p entries (~p bytes)",
              [length(KeysToDelete), DeletedBytes]),
    {reply, ok, FinalState};
%% Clear entire cache
handle_call(clear, _From, State) ->
    ...
    {reply, ok, State#state{current_memory_bytes = 0, stats = NewStats}};
%% Get statistics
handle_call(stats, _From, State) ->
    ...
    DetailedStats =
        Stats#{cache_size => CacheSize,
               ...
               memory_utilization =>
                   case State#state.max_memory_bytes of
                       0 ->
                           0.0;
                       Max ->
                           State#state.current_memory_bytes / Max
                   end,
               hit_rate => HitRate,
...
handle_info(cleanup, State) ->
    ...
    ExpiredKeys =
        ets:foldl(fun({Key, #icon_entry{last_access = LastAccess, size = Size}}, Acc) ->
    ...
    {DeletedBytes, NewState} =
        lists:foldl(fun({Key, Size}, {Bytes, AccState}) ->
                       ets:delete(AccState#state.cache_table, Key),
...
evict_lru_entries(State, RequiredBytes) ->
    ...
    NeededBytes =
        case RequiredBytes > FreeMemory of
            true ->
                RequiredBytes - FreeMemory;
            false ->
                0
        end,

    {EvictedBytes, NewState} = evict_until_enough_space(SortedEntries, NeededBytes, 0, State),
...
evict_until_enough_space([], _NeededBytes, FreedBytes, State) ->
    ...
    {FreedBytes, State};
evict_until_enough_space(_Entries, NeededBytes, FreedBytes, State) when FreedBytes >= NeededBytes ->
    %% Freed enough space
    {FreedBytes, State};
evict_until_enough_space([{_LastAccess, Key, Size} | Rest], NeededBytes, FreedBytes, State) ->
    ...
    NewState = State#state{current_memory_bytes = State#state.current_memory_bytes - Size},
