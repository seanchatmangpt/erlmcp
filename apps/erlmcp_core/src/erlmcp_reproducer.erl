...
%% API exports
-export([start_link/0, capture/1, capture_protocol_failure/4, capture_transport_failure/4,
         capture_sse_failure/4, list_all/0, list_unfixed/0, get_reproducer/1, replay/1,
         mark_fixed/1, generate_reproducer_module/1, audit_report/0]).
...
%% State record
-record(state,
        {reproducers = #{} :: #{binary() => reproducer()}, counter = 0 :: non_neg_integer()}).
...
-spec capture_sse_failure(binary(), term(), term(), term()) -> {ok, binary()} | {error, term()}.
...
replay(Scenario) ->
    ...
    Result =
        case maps:get(category, Scenario, unknown) of
            protocol ->
    ...
    case Result of
        Expected ->
            {ok, Result};
        _ ->
            {error, {mismatch, #{expected => Expected, actual => Result}}}
    end.
...
handle_call({capture, Scenario}, _From, State) ->
    ...
    NewState = State#state{reproducers = NewReproducers, counter = Counter},
...
handle_call(list_unfixed, _From, State) ->
    Unfixed = [R || R <- maps:values(State#state.reproducers), R#reproducer.fixed =:= false],
...
handle_call({mark_fixed, Id}, _From, State) ->
    case maps:find(Id, State#state.reproducers) of
        {ok, Reproducer} ->
            UpdatedReproducer =
                Reproducer#reproducer{fixed = true, fixed_at = erlang:system_time(millisecond)},
...
handle_call(audit_report, _From, State) ->
    ...
    Unfixed =
        length([R || R <- maps:values(State#state.reproducers), R#reproducer.fixed =:= false]),
    Fixed = Total - Unfixed,
    FixRate =
        case Total of
            0 ->
...
load_reproducers_from_disk() ->
    case filelib:wildcard(
             filename:join([?REPRODUCER_DIR, "reproducer_*.erl"]))
    of
        [] ->
            #{};
        Files ->
...
-spec load_reproducer_from_file(file:filename()) -> {ok, binary(), reproducer()} | {error, term()}.
...
replay_sse([{sse_message, Data}]) when is_map(Data) ->
    ...
    case maps:get(resume_id, Data, undefined) of
        undefined ->
            {ok, no_resume};
        ResumeId when is_binary(ResumeId) ->
            {ok, ResumeId};
        _ ->
