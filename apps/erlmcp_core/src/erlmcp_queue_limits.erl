...
-type limit_config() :: #{
    ...
    warning_threshold => float(),  % 0.0-1.0, default 0.8
    alert_threshold => float()     % 0.0-1.0, default 0.9
}.
...
init([]) ->
    ...
    Stats = maps:from_list([
        {Role, init_role_stats(Role)} || Role <- maps:keys(Limits)
    ]),
...
handle_call({check_capacity, Role, ProcId}, _From, State) ->
    ...
    case CurrentDepth >= Limit of
        ...
        false ->
            ...
            if
                CurrentDepth >= AlertThreshold ->
                    logger:warning("Queue depth at ~p% for ~p:~p (~p/~p)",
                                  [round(CurrentDepth / Limit * 100), Role, ProcId,
                                   CurrentDepth, Limit]);
...
handle_call({get_queue_depth, ProcId}, _From, State) ->
    ...
    Result = maps:fold(
        fun({_Role, Pid}, Depth, Acc) when Pid =:= ProcId ->
                case Acc of
                    {error, not_found} -> Depth;
...
handle_cast({record_enqueue, Role, ProcId}, State) ->
    ...
    NewMonitors = case is_pid(ProcId) of
        true ->
    ...
    {noreply, State#state{queue_depths = NewQueueDepths, stats = NewStats, monitors = NewMonitors}};
...
update_dequeue_stats(Role, Stats) ->
    RoleStats = maps:get(Role, Stats, init_role_stats(Role)),
    UpdatedStats = RoleStats#{
        total_dequeued => maps:get(total_dequeued, RoleStats) + 1,
...
record_rejection(Role, Stats) ->
    RoleStats = maps:get(Role, Stats, init_role_stats(Role)),
    UpdatedStats = RoleStats#{
        total_rejected => maps:get(total_rejected, RoleStats) + 1,
