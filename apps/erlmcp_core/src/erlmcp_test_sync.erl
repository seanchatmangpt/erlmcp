...
%% Exported functions
-export([poll_until/4, poll_until/5, wait_for_process_death/2, wait_for_process_death/3,
         wait_for_message/2, wait_for_message/3, wait_for_state/3, wait_for_state/4,
         wait_for_condition/3, wait_for_condition/4, wait_for_ets_insert/2, wait_for_ets_insert/3,
         wait_for_ets_delete/2, wait_for_ets_delete/3, wait_for_ets_match/3, assert_process_dead/1,
         assert_process_alive/1, synchronize_with_monitor/1, flush_mailbox/0, flush_messages/1]).
...
-spec poll_until(condition_fun(),
                 any(),
                 poll_options() | non_neg_integer(),
                 non_neg_integer(),
                 map()) ->
                    {ok, any()} | {error, timeout}.
poll_until(ConditionFun, TimeoutMsg, OptionsOrTimeout, DefaultInterval, DefaultOptions) ->
    Options =
        case is_map(OptionsOrTimeout) of
            true ->
                OptionsOrTimeout;
            false ->
                DefaultOptions#{timeout => OptionsOrTimeout}
        end,
...
wait_for_process_death(Pid, Timeout, Options) ->
    ...
    ConditionFun = fun() -> not is_process_alive(Pid) end,
...
assert_process_dead(Pid) ->
    case is_process_alive(Pid) of
        false ->
            ok;
        true ->
            error({process_should_be_dead, Pid})
    end.
...
assert_process_alive(Pid) ->
    case is_process_alive(Pid) of
        true ->
            ok;
        false ->
            error({process_should_be_alive, Pid})
    end.
...
-spec wait_for_ets_insert(ets:tid(), any()) -> {ok, any()} | {error, timeout}.
wait_for_ets_insert(Table, Key) ->
    wait_for_ets_insert(Table, Key, 1000).

%% @doc Wait for an ETS insert with timeout
-spec wait_for_ets_insert(ets:tid(), any(), non_neg_integer()) -> {ok, any()} | {error, timeout}.
...
flush_messages(PatternFun, Count, Acc) ->
    receive
        Msg ->
            case PatternFun(Msg) of
                true ->
                    flush_messages(PatternFun, Count + 1, Acc);
                false ->
                    flush_messages(PatternFun, Count, [Msg | Acc])
            end
    after 0 ->
