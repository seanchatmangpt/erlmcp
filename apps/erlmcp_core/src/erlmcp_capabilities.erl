...
%% API exports
-export([start_link/0, extract_client_capabilities/1, extract_server_capabilities/1,
         capability_to_map/1, map_to_capability/1, validate_protocol_version/1,
         validate_capability_structures/2, negotiate_capabilities/2, merge_capability/3,
         ...
         get_optional_capabilities/0, is_capability_required/1, is_capability_optional/1,
         get_capability_dependencies/1, validate_capability_dependencies/1,
         get_negotiated_capabilities/0, set_negotiated_capabilities/1,
...
extract_client_capabilities(Params) when is_map(Params) ->
    ...
    #mcp_client_capabilities{roots = extract_roots_client_capability(CapsMap),
...
extract_tools_client_capability(CapsMap) ->
    case maps:get(<<"tools">>, CapsMap, undefined) of
        ...
        CapMap when is_map(CapMap) ->
            #mcp_tools_capability{listChanged = maps:get(<<"listChanged">>, CapMap, false)};
        _ ->
            #mcp_tools_capability{listChanged = false}
    end.
...
extract_server_capabilities(Response) when is_map(Response) ->
    ...
    #mcp_server_capabilities{resources = extract_resources_capability(CapsMap),
...
map_to_capability(Map) when is_map(Map) ->
    ...
    case maps:is_key(<<"resources">>, Map)
         orelse maps:is_key(<<"tools">>, Map)
         orelse maps:is_key(<<"prompts">>, Map)
    of
        true ->
            %% Server capabilities
            #mcp_server_capabilities{resources = extract_resources_capability(Map),
        ...
        false ->
            %% Client capabilities
            #mcp_client_capabilities{roots = extract_capability(Map, <<"roots">>),
        ...
        false ->
            {error,
             <<"Unsupported protocol version: ",
               Version/binary,
               ". Supported: ",
               (join_versions(SupportedVersions))/binary>>}
    end;
...
negotiate_capabilities(ClientCaps, ServerCaps) ->
    ...
    NegotiatedCaps =
        ServerCaps#mcp_server_capabilities{resources =
                                               negotiate_capability(resources,
                                                                    ClientCaps,
                                                                    ServerCaps),
                                           tools =
                                               negotiate_capability(tools, ClientCaps, ServerCaps),
                                           prompts =
                                               negotiate_capability(prompts,
                                                                    ClientCaps,
                                                                    ServerCaps),
                                           logging =
                                               negotiate_capability(logging,
                                                                    ClientCaps,
                                                                    ServerCaps),
                                           sampling =
                                               negotiate_capability(sampling,
                                                                    ClientCaps,
                                                                    ServerCaps),
                                           roots =
                ...
                maps:fold(fun(Key, ServerVal, Acc) ->
                             case maps:get(Key, ClientMap, undefined) of
                                 ...
                                 _ClientVal ->
                                     ...
                                     Acc#{Key =>
                                              negotiate_experimental_feature(Key,
                                                                             ServerVal,
                                                                             maps:get(Key,
                                                                                      ClientMap))}
                             end
...
apply_graceful_degradation(ClientCaps, ServerCaps) ->
    ...
    case ClientCaps#mcp_client_capabilities.roots of
        #mcp_capability{enabled = false} ->
            ...
            ServerCaps#mcp_server_capabilities{resources =
                                                   ResCaps#mcp_resources_capability{listChanged =
                                                                                        false}};
        #mcp_capability{enabled = true} ->
...
format_capability_error({unsupported_feature, Cap, Feature}) ->
    <<"Feature '",
      (atom_to_binary(Feature))/binary,
      "' not supported for capability '",
...
init([]) ->
    {ok,
     #state{server_capabilities = undefined,
...
extract_resources_capability(Map) ->
    case maps:get(<<"resources">>, Map, undefined) of
        ...
        CapMap when is_map(CapMap) ->
            #mcp_resources_capability{subscribe = maps:get(<<"subscribe">>, CapMap, false),
...
extract_prompts_capability(Map) ->
    case maps:get(<<"prompts">>, Map, undefined) of
        ...
        CapMap when is_map(CapMap) ->
            #mcp_prompts_capability{listChanged = maps:get(<<"listChanged">>, CapMap, false)}
    end.
    ...
    case maps:get(<<"sampling">>, Map, undefined) of
        ...
        CapMap when is_map(CapMap) ->
            #mcp_sampling_capability{modelPreferences =
                                         maps:get(<<"modelPreferences">>, CapMap, undefined)}
    end.
...
negotiate_capability(resources, ClientCaps, ServerCaps) ->
    ...
    case ClientCaps#mcp_client_capabilities.roots of
        ...
        #mcp_capability{enabled = false} ->
            ...
            NegotiatedSub = negotiate_subscribe_flag(ClientCaps, ResCap),
            ResCap#mcp_resources_capability{subscribe = NegotiatedSub, listChanged = false}
    end;
...
negotiate_capability(sampling, ClientCaps, ServerCaps) ->
    ...
    case ClientCaps#mcp_client_capabilities.sampling of
        #mcp_capability{enabled = true} ->
            ...
            merge_capability(sampling,
                             ClientCaps#mcp_client_capabilities.sampling,
...
-spec negotiate_subscribe_flag(#mcp_client_capabilities{}, #mcp_resources_capability{}) ->
                                  boolean().
...
maybe_add_capability_map(Map, Key, #mcp_sampling_capability{modelPreferences = Prefs}) ->
    case Prefs of
        undefined ->
            Map#{Key => #{}};
        _ ->
            Map#{Key => #{<<"modelPreferences">> => Prefs}}
    end;
...
validate_client_capability_record(#mcp_client_capabilities{roots = Roots,
                                                           sampling = Sampling,
                                                           tools = Tools}) ->
    %% Validate roots capability
    case Roots of
        #mcp_capability{enabled = IsEnabled} when is_boolean(IsEnabled) ->
            ok;
        _ ->
            error({invalid_capability_structure, roots})
    end,
    %% Validate sampling capability
    case Sampling of
        #mcp_capability{enabled = IsEnabled2} when is_boolean(IsEnabled2) ->
            ok;
        _ ->
    ...
    case Tools of
        #mcp_tools_capability{listChanged = ListChanged} when is_boolean(ListChanged) ->
            ok;
        _ ->
            error({invalid_capability_structure, tools})
    end,
    ok.

%% @doc Validate server capability record structure
-spec validate_server_capability_record(#mcp_server_capabilities{}) -> ok.
validate_server_capability_record(#mcp_server_capabilities{resources = Resources,
                                                           tools = Tools,
                                                           prompts = Prompts,
                                                           logging = Logging,
                                                           sampling = Sampling,
    ...
    case Resources of
        #mcp_resources_capability{subscribe = S, listChanged = L}
            when is_boolean(S), is_boolean(L) ->
            ok;
        _ ->
            error({invalid_capability_structure, resources})
    end,
    %% Validate tools capability
    case Tools of
        #mcp_tools_capability{listChanged = L1} when is_boolean(L1) ->
            ok;
        _ ->
            error({invalid_capability_structure, tools})
    end,
    ...
    case Prompts of
        #mcp_prompts_capability{listChanged = L2} when is_boolean(L2) ->
            ok;
        _ ->
            error({invalid_capability_structure, prompts})
    end,
    ...
    case Logging of
        #mcp_logging_capability{} ->
            ok;
        _ ->
            error({invalid_capability_structure, logging})
    end,
    %% Validate sampling capability
    case Sampling of
        #mcp_sampling_capability{modelPreferences = Prefs} ->
            case Prefs of
                undefined ->
                    ok;
                Map when is_map(Map) ->
                    validate_model_preferences(Map);
                _ ->
                    error({invalid_capability_structure, sampling_model_preferences})
            end;
        _ ->
            error({invalid_capability_structure, sampling})
    end,
...
validate_model_preferences(Prefs) ->
    ...
    case maps:get(<<"costPriority">>, Prefs, undefined) of
        undefined ->
            ok;
        P when is_float(P); is_integer(P) ->
            ok;
        _ ->
            error({invalid_model_preferences, costPriority})
    end,
    case maps:get(<<"speedPriority">>, Prefs, undefined) of
        undefined ->
            ok;
        P2 when is_float(P2); is_integer(P2) ->
            ok;
        _ ->
            error({invalid_model_preferences, speedPriority})
    end,
    case maps:get(<<"intelligencePriority">>, Prefs, undefined) of
        undefined ->
            ok;
        P3 when is_float(P3); is_integer(P3) ->
            ok;
        _ ->
            error({invalid_model_preferences, intelligencePriority})
    end,
    case maps:get(<<"temperature">>, Prefs, undefined) of
        undefined ->
            ok;
        T when is_float(T), T >= 0.0, T =< 2.0 ->
            ok;
        _ ->
            error({invalid_model_preferences, temperature})
    end,
    case maps:get(<<"maxTokens">>, Prefs, undefined) of
        undefined ->
            ok;
        MT when is_integer(MT), MT > 0 ->
            ok;
        _ ->
            error({invalid_model_preferences, maxTokens})
    end,
    case maps:get(<<"stopSequences">>, Prefs, undefined) of
        ...
        SS when is_list(SS) ->
            ok;
        _ ->
            error({invalid_model_preferences, stopSequences})
    end,
    ok.

%% @doc Enhanced negotiate_capability with proper flag handling for all capabilities
-spec negotiate_capability_enhanced(atom(),
...
negotiate_capability_enhanced(resources, ClientCaps, ServerCaps) ->
    ...
    case ServerRes of
        #mcp_resources_capability{subscribe = SrvSub, listChanged = SrvLC} ->
            ...
            #mcp_resources_capability{subscribe = SrvSub, listChanged = NegotiatedLC};
        _ ->
            ServerRes
    end;
...
supports_flag(resources, subscribe) ->
    true;
supports_flag(resources, listChanged) ->
    true;
supports_flag(tools, listChanged) ->
    true;
supports_flag(prompts, listChanged) ->
    true;
supports_flag(logging, _) ->
    false;
supports_flag(sampling, _) ->
    false;
supports_flag(roots, _) ->
    false;
supports_flag(_, _) ->
    false.
...
validate_flag(Capability, Flag, Value) when is_boolean(Value) ->
    case supports_flag(Capability, Flag) of
        true ->
            ok;
        false ->
            {error, {unsupported_flag, Capability, Flag}}
    end;
...
apply_graceful_degradation_enhanced(ClientCaps, ServerCaps) ->
    ...
    ServerCaps#mcp_server_capabilities{resources = NegotiatedRes, sampling = NegotiatedSamp}.
...
build_capability_map(resources, Flags) ->
    Base = #{},
    Base1 =
        case maps:get(subscribe, Flags, false) of
            true ->
                Base#{<<"subscribe">> => true};
            false ->
                Base
...
get_optional_capabilities() ->
    [resources, tools, prompts, logging, sampling, roots].
                       ...
                       ok ->
                           ...
                           case has_capability(ServerCaps, Capability) of
                               true ->
                                   ...
                                   MissingDeps =
                                       lists:filter(fun(Dep) -> not has_capability(ServerCaps, Dep)
                                                    end,
                                                    Dependencies),
                                   case MissingDeps of
                                       [] ->
...
set_capability_flag(Caps, resources, subscribe, Value) when is_boolean(Value) ->
    ResCaps = Caps#mcp_server_capabilities.resources,
    Caps#mcp_server_capabilities{resources = ResCaps#mcp_resources_capability{subscribe = Value}};
set_capability_flag(Caps, resources, listChanged, Value) when is_boolean(Value) ->
    ResCaps = Caps#mcp_server_capabilities.resources,
    Caps#mcp_server_capabilities{resources = ResCaps#mcp_resources_capability{listChanged = Value}};
set_capability_flag(Caps, tools, listChanged, Value) when is_boolean(Value) ->
    ToolsCaps = Caps#mcp_server_capabilities.tools,
    Caps#mcp_server_capabilities{tools = ToolsCaps#mcp_tools_capability{listChanged = Value}};
set_capability_flag(Caps, prompts, listChanged, Value) when is_boolean(Value) ->
    PromptsCaps = Caps#mcp_server_capabilities.prompts,
    Caps#mcp_server_capabilities{prompts = PromptsCaps#mcp_prompts_capability{listChanged = Value}};
...
build_client_init_params(#mcp_client_capabilities{} = ClientCaps) ->
    ...
    #{<<"protocolVersion">> => ?MCP_VERSION,
      <<"capabilities">> => CapMap,
      <<"clientInfo">> => #{<<"name">> => ?APP_NAME, <<"version">> => <<"1.0.0">>}};
...
build_server_init_response(ServerCaps, ServerInfo) ->
    DefaultServerInfo = #{<<"name">> => ?APP_NAME, <<"version">> => <<"1.0.0">>},
...
-spec get_experimental_features(#mcp_client_capabilities{} | #mcp_server_capabilities{}) ->
                                   [binary()].
