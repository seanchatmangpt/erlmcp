...
multiple_plugins_test(Config) ->
    ...
    [spawn(fun() -> {ok, _} = erlmcp_plugin_manager:execute(Manager, Id, "test", #{}) end)
     || Id <- Plugins],
...
plugin_configuration_test(Config) ->
    ...
    PluginConfig = #{setting1 => <<"value1">>, setting2 => 42},
...
plugin_discovery_and_load_test(Config) ->
    ...
    [ok =
         create_plugin(PluginDir ++ io_lib:format("disco_~p.erl", [N]),
...
create_versioned_plugin(Path, Version) ->
    Code =
        io_lib:format("-module(versioned).
