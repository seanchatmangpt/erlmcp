...
owasp_vulnerability_scan_test_() ->
    {setup,
     fun setup_owasp_scan/0,
     fun cleanup_owasp_scan/1,
     fun owasp_scan_tests/1}.
...
check_a01_broken_access_control() ->
    ...
    AuthResult = erlmcp_auth:validate_request(
        #{resource => <<"admin">>},
        #{api_key => <<"admin_key">>}
    ),
...
check_a03_injection() ->
    ...
    Result = case erlmcp_auth:validate_request(
        #{resource => <<"resources/read">>, uri => SQLPayload},
        #{api_key => <<"test_valid_key">>}
    ) of
        {error, invalid_input} -> safe;
        {ok, _} -> unsafe
    end,
...
check_a04_insecure_design() ->
    ...
    {ok, _} = erlmcp_auth:validate_request(
        #{resource => <<"tools/list">>},
        #{api_key => <<"test_valid_key">>}
    ),
...
check_a07_authentication_failures() ->
    ...
    InvalidResult = erlmcp_auth:validate_request(
        #{resource => <<"admin">>},
        #{api_key => <<"invalid_key">>}
    ),
    ...
    ValidResult = erlmcp_auth:validate_request(
        #{resource => <<"tools/list">>},
        #{api_key => <<"test_valid_key">>}
    ),
...
check_a09_logging_failures() ->
    ...
    _ = erlmcp_auth:validate_request(
        #{resource => <<"admin">>},
        #{api_key => <<"invalid_key">>}
    ),
...
check_a10_ssrf() ->
    ...
    Result = erlmcp_auth:validate_request(
        #{resource => <<"resources/read">>, uri => SSRFURI},
        #{api_key => <<"test_valid_key">>}
    ),
    ...
    case Result of
        {error, invalid_uri} -> ok;
        {error, unauthorized} -> ok;
        _ -> ?assert(false, "SSRF attempt not blocked")
    end.
...
detect_insecure_configurations() ->
    ...
    DefaultCredResult = erlmcp_auth:validate_request(
        #{resource => <<"admin">>},
...
identify_information_disclosure() ->
    ...
    ErrorResult = erlmcp_auth:validate_request(
        #{resource => <<"tools/invalid_nonexistent_tool">>},
        #{api_key => <<"test_valid_key">>}
    ),
...
scan_for_xss_vectors() ->
    ...
    lists:foreach(fun(Payload) ->
        ...
        case Result of
            {error, _} -> ok;
            {ok, _} -> ok  % May be sanitized instead of blocked
        end
    end, XSSPayloads).
