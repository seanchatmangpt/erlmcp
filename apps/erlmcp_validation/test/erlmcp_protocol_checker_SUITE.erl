...
all() ->
    [jsonrpc_version_test, request_validation_test, response_validation_test,
     notification_validation_test, error_code_validation_test,
     initialize_request_test, phase_transition_test,
     tools_call_test, resources_read_test, prompts_get_test,
     set_level_test, progress_test, cancelled_test,
     full_initialize_sequence_test, error_recovery_test,
...
jsonrpc_version_test(_Config) ->
    ...
    {error, _} = erlmcp_protocol_checker:validate_jsonrpc_version(#{?JSONRPC_FIELD_JSONRPC => <<"1.0">>}),
    ok.

request_validation_test(_Config) ->
    Valid = #{?JSONRPC_FIELD_JSONRPC => ?JSONRPC_VERSION, ?JSONRPC_FIELD_ID => 1, ?JSONRPC_FIELD_METHOD => <<"test">>},
    ok = erlmcp_protocol_checker:validate_request(Valid),
    {error, _} = erlmcp_protocol_checker:validate_request(#{?JSONRPC_FIELD_JSONRPC => ?JSONRPC_VERSION, ?JSONRPC_FIELD_ID => 1}),
    ok.

response_validation_test(_Config) ->
    Valid = #{?JSONRPC_FIELD_JSONRPC => ?JSONRPC_VERSION, ?JSONRPC_FIELD_ID => 1, ?JSONRPC_FIELD_RESULT => #{<<"data">> => <<"value">>}},
    ok = erlmcp_protocol_checker:validate_response(Valid),
    {error, _} = erlmcp_protocol_checker:validate_response(#{?JSONRPC_FIELD_JSONRPC => ?JSONRPC_VERSION, ?JSONRPC_FIELD_ID => 1}),
    ok.
...
notification_validation_test(_Config) ->
    ...
    {error, _} = erlmcp_protocol_checker:validate_notification(#{?JSONRPC_FIELD_JSONRPC => ?JSONRPC_VERSION, ?JSONRPC_FIELD_ID => 1, ?JSONRPC_FIELD_METHOD => <<"test">>}),
    ok.
...
initialize_request_test(_Config) ->
    Valid = #{
        ...
        ?JSONRPC_FIELD_PARAMS => #{
            ...
            ?MCP_FIELD_CLIENT_INFO => #{?MCP_INFO_NAME => <<"test">>, ?MCP_INFO_VERSION => <<"1.0">>}
        }
    },
...
phase_transition_test(_Config) ->
    ...
    {error, _} = erlmcp_protocol_checker:validate_phase_transition(initialized, initialization, initialize),
    ok.
...
tools_call_test(_Config) ->
    ...
    {error, _} = erlmcp_protocol_checker:validate_tools_call(#{?JSONRPC_FIELD_JSONRPC => ?JSONRPC_VERSION, ?JSONRPC_FIELD_ID => 1, ?JSONRPC_FIELD_METHOD => ?MCP_METHOD_TOOLS_CALL, ?JSONRPC_FIELD_PARAMS => #{}}),
    ok.
...
resources_read_test(_Config) ->
    ...
    {error, _} = erlmcp_protocol_checker:validate_resources_read(#{?JSONRPC_FIELD_JSONRPC => ?JSONRPC_VERSION, ?JSONRPC_FIELD_ID => 1, ?JSONRPC_FIELD_METHOD => ?MCP_METHOD_RESOURCES_READ, ?JSONRPC_FIELD_PARAMS => #{}}),
    ok.
...
prompts_get_test(_Config) ->
    ...
    {error, _} = erlmcp_protocol_checker:validate_prompts_get(#{?JSONRPC_FIELD_JSONRPC => ?JSONRPC_VERSION, ?JSONRPC_FIELD_ID => 1, ?JSONRPC_FIELD_METHOD => ?MCP_METHOD_PROMPTS_GET, ?JSONRPC_FIELD_PARAMS => #{}}),
    ok.
...
set_level_test(_Config) ->
    ...
    {error, _} = erlmcp_protocol_checker:validate_setLevel(#{?JSONRPC_FIELD_JSONRPC => ?JSONRPC_VERSION, ?JSONRPC_FIELD_ID => 1, ?JSONRPC_FIELD_METHOD => ?MCP_METHOD_LOGGING_SET_LEVEL, ?JSONRPC_FIELD_PARAMS => #{<<"level">> => invalid}}),
    ok.
...
progress_test(_Config) ->
    Valid = #{
        ...
        ?JSONRPC_FIELD_PARAMS => #{?MCP_PARAM_PROGRESS_TOKEN => 1, ?MCP_PARAM_PROGRESS => 50, ?MCP_PARAM_TOTAL => 100}
    },
    ok = erlmcp_protocol_checker:validate_progress(Valid),
    {error, _} = erlmcp_protocol_checker:validate_progress(#{?JSONRPC_FIELD_JSONRPC => ?JSONRPC_VERSION, ?JSONRPC_FIELD_METHOD => ?MCP_METHOD_NOTIFICATIONS_PROGRESS, ?JSONRPC_FIELD_PARAMS => #{}}),
    ok.
...
cancelled_test(_Config) ->
    Valid = #{
        ...
        ?JSONRPC_FIELD_PARAMS => #{?MCP_PARAM_REQUEST_ID => <<"req-1">>, ?MCP_PARAM_REASON => <<"cancelled">>}
    },
    ok = erlmcp_protocol_checker:validate_cancelled(Valid),
    {error, _} = erlmcp_protocol_checker:validate_cancelled(#{?JSONRPC_FIELD_JSONRPC => ?JSONRPC_VERSION, ?JSONRPC_FIELD_METHOD => ?MCP_METHOD_NOTIFICATIONS_CANCELLED, ?JSONRPC_FIELD_PARAMS => #{}}),
    ok.
...
error_recovery_test(_Config) ->
    Invalid = #{?JSONRPC_FIELD_JSONRPC => ?JSONRPC_VERSION, ?JSONRPC_FIELD_ID => 1, ?JSONRPC_FIELD_METHOD => <<"invalid">>},
    {error, _} = erlmcp_protocol_checker:validate_request(Invalid),
    Valid = #{?JSONRPC_FIELD_JSONRPC => ?JSONRPC_VERSION, ?JSONRPC_FIELD_ID => 2, ?JSONRPC_FIELD_METHOD => ?MCP_METHOD_PING},
    ok = erlmcp_protocol_checker:validate_request(Valid),
    ok.

concurrent_requests_test(_Config) ->
    Requests = [#{?JSONRPC_FIELD_JSONRPC => ?JSONRPC_VERSION, ?JSONRPC_FIELD_ID => I, ?JSONRPC_FIELD_METHOD => ?MCP_METHOD_TOOLS_LIST} || I <- lists:seq(1, 10)],
