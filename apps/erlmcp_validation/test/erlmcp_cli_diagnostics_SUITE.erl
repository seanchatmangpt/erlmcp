...
network_diagnostics_test(_Config) ->
    ...
    #{connectivity := _Connectivity, latency := _Latency} = NetDiag,
...
continuous_monitoring_test(_Config) ->
    ...
    {ok, Monitor} =
        erlmcp_cli_diagnostics:start_monitoring(#{interval => 100,
...
alert_threshold_test(_Config) ->
    ...
    {ok, Monitor} =
        erlmcp_cli_diagnostics:start_monitoring(#{interval => 100,
                                                  checks => [memory],
                                                  thresholds => #{memory_usage_percent => 95}}),
...
diagnostic_report_generation_test(_Config) ->
    ...
    {ok, Report} =
        erlmcp_cli_diagnostics:generate_report(#{format => html,
