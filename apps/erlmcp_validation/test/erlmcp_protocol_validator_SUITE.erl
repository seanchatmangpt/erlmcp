...
all() ->
    [
        ...
        test_valid_request,
        test_valid_response_with_result,
        test_valid_response_with_error,
        test_valid_notification,
        test_invalid_json,
        test_missing_jsonrpc_field,
        test_wrong_jsonrpc_version,
        test_invalid_request_id_negative,
        test_invalid_request_id_float,
        test_result_and_error_exclusive,

        %% Request ID validation tests (5 tests)
        test_request_id_null,
        test_request_id_integer,
        test_request_id_string,
        test_request_id_invalid_type,
        test_request_id_negative_integer,

        %% Error code validation tests (8 tests)
        test_error_code_parse_error,
        test_error_code_invalid_request,
        test_error_code_method_not_found,
        test_error_code_invalid_params,
        test_error_code_internal_error,
        test_error_code_mcp_resource_not_found,
        test_error_code_experimental_range,
        test_error_code_invalid,

        %% Initialize method tests (5 tests)
        test_initialize_valid_params,
        test_initialize_missing_protocol_version,
        test_initialize_missing_capabilities,
        test_initialize_missing_client_info,
        test_initialize_invalid_client_info,

        %% Resources methods tests (8 tests)
        test_resources_list_no_params,
        test_resources_list_with_cursor,
        test_resources_list_invalid_cursor,
        test_resources_read_valid,
        test_resources_read_missing_uri,
        test_resources_subscribe_valid,
        test_resources_unsubscribe_valid,
        test_resources_unsubscribe_missing_uri,

        %% Tools methods tests (6 tests)
        test_tools_list_no_params,
        test_tools_list_with_cursor,
        test_tools_call_valid,
        test_tools_call_missing_name,
        test_tools_call_with_arguments,
        test_tools_call_invalid_arguments,

        %% Prompts methods tests (5 tests)
        test_prompts_list_no_params,
        test_prompts_get_valid,
        test_prompts_get_missing_name,
        ...
        test_sampling_create_message_missing_messages,
        test_sampling_create_message_invalid_messages,

        %% Logging method tests (3 tests)
        test_logging_set_level_valid,
        test_logging_set_level_invalid,
        test_logging_set_level_missing,
        ...
        test_mcp_message_response_valid,
        test_mcp_message_error_response_valid,
        test_mcp_message_error_invalid_code,
        test_mcp_message_missing_fields,
...
test_invalid_request_id_float(_Config) ->
    ...
    {error, invalid_request_id} =
        erlmcp_protocol_validator:validate_request_id(1.5),
    ok.
...
test_request_id_invalid_type(_Config) ->
    {error, invalid_request_id} =
        erlmcp_protocol_validator:validate_request_id([1, 2, 3]),
    {error, invalid_request_id} =
        erlmcp_protocol_validator:validate_request_id(#{id => 1}),
    ok.

test_request_id_negative_integer(_Config) ->
    {error, invalid_request_id} =
        erlmcp_protocol_validator:validate_request_id(-1),
    ok.
...
test_error_code_invalid(_Config) ->
    {error, invalid_error_code} =
        erlmcp_protocol_validator:validate_error_code(999),
    {error, invalid_error_code} =
        erlmcp_protocol_validator:validate_error_code(-99999),
    ok.
...
test_sampling_create_message_missing_messages(_Config) ->
    Params = #{},
    {error, _} = erlmcp_protocol_validator:validate_params(?MCP_METHOD_SAMPLING_CREATE_MESSAGE, Params),
    ok.
...
test_mcp_message_unknown_method(_Config) ->
    Message = #{
        ?JSONRPC_FIELD_METHOD => <<"unknown_method">>,
        ?JSONRPC_FIELD_PARAMS => #{}
    },
    {error, ?JSONRPC_METHOD_NOT_FOUND, _} =
        erlmcp_protocol_validator:validate_mcp_message(Message),
    ok.
...
test_mcp_message_error_invalid_code(_Config) ->
    ...
    {error, ?JSONRPC_INVALID_REQUEST, _} =
        erlmcp_protocol_validator:validate_mcp_message(Message),
    ok.
...
test_validate_capabilities(_Config) ->
    %% Valid capabilities
    ok = erlmcp_protocol_validator:validate_capabilities(#{
        <<"tools">> => #{},
...
test_validate_resource(_Config) ->
    %% Valid resource
    ok = erlmcp_protocol_validator:validate_resource(#{
        ?MCP_PARAM_URI => <<"file:///test.txt">>,
        ?MCP_PARAM_NAME => <<"Test Resource">>
    }),
    %% Missing name
    {error, _} = erlmcp_protocol_validator:validate_resource(#{
        ?MCP_PARAM_URI => <<"file:///test.txt">>
    }),
    ok.
...
test_validate_tool(_Config) ->
    %% Valid tool
    ok = erlmcp_protocol_validator:validate_tool(#{
        ?MCP_PARAM_NAME => <<"test_tool">>,
    ...
    %% Missing inputSchema
    {error, _} = erlmcp_protocol_validator:validate_tool(#{
        ?MCP_PARAM_NAME => <<"test_tool">>
    }),
    ok.
...
test_validate_prompt(_Config) ->
    %% Valid prompt
    ok = erlmcp_protocol_validator:validate_prompt(#{
        ?MCP_PARAM_NAME => <<"test_prompt">>
    }),
    %% Missing name
    {error, _} = erlmcp_protocol_validator:validate_prompt(#{
        ?MCP_PARAM_DESCRIPTION => <<"Test">>
    }),
    ok.

test_validate_content(_Config) ->
    %% Valid text content
    ok = erlmcp_protocol_validator:validate_content(#{
        ?MCP_PARAM_TYPE => ?MCP_CONTENT_TYPE_TEXT,
        ?MCP_PARAM_TEXT => <<"Hello">>
    }),
    %% Valid image content
    ok = erlmcp_protocol_validator:validate_content(#{
        ?MCP_PARAM_TYPE => ?MCP_CONTENT_TYPE_IMAGE,
        ?MCP_PARAM_DATA => <<"base64data">>,
        ?MCP_PARAM_MIME_TYPE => <<"image/png">>
    }),
    %% Missing type
    {error, <<"Missing required field: content.type">>} =
        erlmcp_protocol_validator:validate_content(#{
            ?MCP_PARAM_TEXT => <<"Hello">>
        }),
    ok.
