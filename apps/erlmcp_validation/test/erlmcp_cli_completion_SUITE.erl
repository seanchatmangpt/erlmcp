...
command_completion_workflow_test(_Config) ->
    {ok, Completer} = erlmcp_cli_completer:start_link(),
    ok = erlmcp_cli_completer:add_entries(Completer, [
        {command, <<"help">>},
        {command, <<"version">>},
        {command, <<"validate">>}
    ]),
...
subcommand_completion_workflow_test(_Config) ->
    {ok, Completer} = erlmcp_cli_completer:start_link(),
    ok = erlmcp_cli_completer:register_command(Completer, "validate", [
        {subcommand, <<"spec">>},
        {subcommand, <<"protocol">>}
    ]),
    
    %% Complete subcommands
    {ok, Subs} = erlmcp_cli_completer:complete_with_context(Completer, <<"s">>, 
                                                             #{context => [<<"validate">>]}),
...
dynamic_completion_test(_Config) ->
    ...
    Provider = fun(Prefix) ->
        case Prefix of
            <<"dynamic_">> -> [<<"dynamic_cmd1">>, <<"dynamic_cmd2">>];
            _ -> []
        end
    end,
...
context_aware_completion_test(_Config) ->
    ...
    ok = erlmcp_cli_completer:register_command(Completer, "git", [
        {subcommand, <<"commit">>},
        {subcommand, <<"push">>}
    ]),
    
    %% Complete in git context
    {ok, GitSubs} = erlmcp_cli_completer:complete_with_context(
        Completer, <<"c">>, #{context => [<<"git">>]}),
