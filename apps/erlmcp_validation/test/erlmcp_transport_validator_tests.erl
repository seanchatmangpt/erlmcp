...
test_http_callback_validation() ->
    ...
    ?assertMatch(#{module := erlmcp_transport_http, category := callbacks}, Result),
...
test_websocket_callback_validation() ->
    ...
    ?assertMatch(#{module := erlmcp_transport_ws, category := callbacks}, Result),
    Status = maps:get(status, Result, warning),
    ?assert(lists:member(Status, [passed, warning])).
...
test_stdio_framing() ->
    Result = erlmcp_transport_validator:validate_framing(erlmcp_transport_stdio, stdio),
    ?assertMatch(#{module := erlmcp_transport_stdio,
                   category := framing,
    ...
    ?assertMatch({stdio_newline_delimited, #{status := _Status}}, NewlineCheck),
...
test_tcp_framing() ->
    Result = erlmcp_transport_validator:validate_framing(erlmcp_transport_tcp, tcp),
    ?assertMatch(#{module := erlmcp_transport_tcp,
                   category := framing,
                   transport_type := tcp},
                 Result),
...
test_http_framing() ->
    Result = erlmcp_transport_validator:validate_framing(erlmcp_transport_http, http),
...
test_websocket_framing() ->
    Result = erlmcp_transport_validator:validate_framing(erlmcp_transport_ws, websocket),
...
test_stdio_registry() ->
    ...
    ?assertMatch(#{module := erlmcp_transport_stdio, category := registry}, Result),
    Status = maps:get(status, Result, warning),
    ?assert(lists:member(Status, [passed, warning])).

test_tcp_registry() ->
    Result = erlmcp_transport_validator:validate_registry(erlmcp_transport_tcp),
    ?assertMatch(#{module := erlmcp_transport_tcp, category := registry}, Result),
    Status = maps:get(status, Result, warning),
    ?assert(lists:member(Status, [passed, warning])).
...
test_stdio_lifecycle() ->
    ...
    ?assertMatch(#{module := erlmcp_transport_stdio, category := lifecycle}, Result),
    ?assert(maps:get(passed, Result) >= 1).

test_tcp_lifecycle() ->
    Result = erlmcp_transport_validator:validate_lifecycle(erlmcp_transport_tcp),
    ?assertMatch(#{module := erlmcp_transport_tcp, category := lifecycle}, Result),
    ?assert(maps:get(passed, Result) >= 1).
...
report_generation_test_() ->
    {foreach,
     fun setup_validator/0,
     fun cleanup_validator/1,
     [?_test(test_get_results()), ?_test(test_generate_report())]}.
...
test_generate_report() ->
    ...
    ?assertMatch({ok, #{timestamp := _, transports_validated := _}}, Report),
    {ok, ReportMap} = Report,
...
code_inspection_test_() ->
    [?_test(test_check_module_usage()), ?_test(test_abstract_code_analysis())].
...
test_abstract_code_analysis() ->
    ...
    case beam_lib:chunks(
             code:which(erlmcp_transport_stdio), [abstract_code])
    of
...
test_invalid_framing_type() ->
    ...
    Result = erlmcp_transport_validator:validate_framing(erlmcp_transport_stdio, unknown_type),
...
test_non_existent_module() ->
    ...
    ?assertException(error,
                     _,
                     erlmcp_transport_validator:validate_callbacks(non_existent_module_xyz)).
...
performance_test_() ->
    {foreach,
     fun setup_validator/0,
     fun cleanup_validator/1,
     [?_test(test_validation_performance()), ?_test(test_multiple_transports())]}.
...
test_multiple_transports() ->
    ...
    Transports = [erlmcp_transport_stdio, erlmcp_transport_tcp, erlmcp_transport_http],
    Start = erlang:monotonic_time(millisecond),
