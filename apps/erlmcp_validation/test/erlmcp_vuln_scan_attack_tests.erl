...
attack_detection_tests(_State) ->
    [{"Enumeration Attack Detection",
     ...
     {"Timing Attack Detection",
      [?_test(detect_variable_length_timing()), ?_test(detect_deep_nesting_timing())]},
...
detect_sequential_id_enumeration() ->
    ...
    Results =
        lists:map(fun(ID) ->
                     erlmcp_auth:validate_request(#{resource => <<"tools/get">>, id => ID},
                                                  #{api_key => <<"test_key">>})
                  end,
                  SequentialIDs),
...
detect_dictionary_word_enumeration() ->
    ...
    Results =
        lists:map(fun(Word) ->
                     erlmcp_auth:validate_request(#{resource => Word}, #{api_key => <<"test_key">>})
                  end,
                  DictionaryWords),
...
detect_common_name_enumeration() ->
    ...
    Results =
        lists:map(fun(Name) ->
                     erlmcp_auth:validate_request(#{resource => Name}, #{api_key => <<"test_key">>})
                  end,
                  CommonNames),
...
detect_random_binary_fuzzing() ->
    ...
    Result =
        erlmcp_auth:validate_request(#{resource => <<"tools/call">>, args => RandomBinary},
                                     #{api_key => <<"test_key">>}),
...
detect_null_byte_fuzzing() ->
    ...
    Result =
        erlmcp_auth:validate_request(#{resource => <<"tools/call">>, name => NullBytes},
                                     #{api_key => <<"test_key">>}),
...
detect_overflow_fuzzing() ->
    ...
    Result =
        erlmcp_auth:validate_request(#{resource => <<"tools/call">>, name => OverflowPayload},
                                     #{api_key => <<"test_key">>}),
...
detect_special_chars_fuzzing() ->
    ...
    Result =
        erlmcp_auth:validate_request(#{resource => <<"tools/call">>, args => SpecialChars},
                                     #{api_key => <<"test_key">>}),
...
detect_large_payload_dos() ->
    ...
    Result =
        erlmcp_auth:validate_request(#{resource => <<"tools/call">>, data => LargePayload},
                                     #{api_key => <<"test_key">>}),
...
detect_deep_nesting_timing() ->
    ...
    Result =
        erlmcp_auth:validate_request(#{resource => <<"tools/call">>, args => DeepNesting},
                                     #{api_key => <<"test_key">>}),
...
detect_invalid_json_abuse() ->
    ...
    Result =
        erlmcp_auth:validate_request(#{resource => <<"tools/call">>, payload => InvalidJSON},
                                     #{api_key => <<"test_key">>}),
...
detect_version_bypass_abuse() ->
    ...
    Result = erlmcp_auth:validate_request(VersionBypass, #{api_key => <<"test_key">>}),
...
detect_sequential_api_key_brute_force() ->
    ...
    Results =
        lists:map(fun(Key) ->
                     erlmcp_auth:validate_request(#{resource => <<"admin">>}, #{api_key => Key})
                  end,
                  SequentialKeys),

    %% All should fail (invalid keys)
    ?assert(lists:all(fun ({error, unauthorized}) ->
                              true;
                          (_) ->
                              false
                      end,
...
detect_session_reuse() ->
    ...
    Result1 =
        erlmcp_auth:validate_request(#{resource => <<"tools/list">>, session => SessionID},
                                     #{api_key => <<"test_key">>}),
...
detect_csrf_vectors() ->
    ...
    Results =
        lists:map(fun(Vector) -> erlmcp_auth:validate_request(Vector, #{api_key => <<"test_key">>})
                  end,
                  CSRFVectors),
...
detect_weak_token_bypass() ->
    ...
    Result = erlmcp_auth:validate_request(#{resource => <<"admin">>}, #{api_key => <<"weak">>}),
...
detect_forged_token_bypass() ->
    ...
    Result = erlmcp_auth:validate_request(#{resource => <<"admin">>}, #{jwt => ForgedToken}),
...
detect_role_manipulation_escalation() ->
    ...
    Result = erlmcp_auth:validate_request(#{resource => <<"admin/config">>}, RoleManipulation),
...
direct_access_escalation() ->
    ...
    AdminResources = [<<"admin/users">>, <<"admin/config">>, <<"admin/logs">>],

    Results =
        lists:map(fun(Resource) ->
                     erlmcp_auth:validate_request(#{resource => Resource},
