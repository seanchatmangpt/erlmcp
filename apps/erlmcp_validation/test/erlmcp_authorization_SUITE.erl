...
init_per_suite(Config) ->
    ...
    {ok, AuthPid} =
        erlmcp_auth:start_link(#{rate_limiter_enabled => false,
                                 api_keys =>
...
init_per_testcase(_TestCase, Config) ->
    ...
    [{admin_session, AdminSession}, {user_session, UserSession}, {guest_session, GuestSession}
     | Config].
...
end_per_testcase(_TestCase, Config) ->
    ...
    lists:foreach(fun(SessionId) ->
                  ...
                  [AdminSession, UserSession, GuestSession]),
...
user_role_limited_access_test(Config) ->
    ...
    ?assertEqual({error, forbidden},
                 erlmcp_auth:check_permission(UserSession, <<"user_resource">>, <<"execute">>)),
    ?assertEqual({error, forbidden},
                 erlmcp_auth:check_permission(UserSession, <<"user_resource">>, <<"delete">>)),
...
guest_role_read_only_access_test(Config) ->
    ...
    ?assertEqual({error, forbidden},
                 erlmcp_auth:check_permission(GuestSession, <<"public_resource">>, <<"write">>)),
    ?assertEqual({error, forbidden},
                 erlmcp_auth:check_permission(GuestSession, <<"public_resource">>, <<"execute">>)),
    ?assertEqual({error, forbidden},
                 erlmcp_auth:check_permission(GuestSession, <<"public_resource">>, <<"delete">>)),
...
role_escalation_prevention_test(Config) ->
    ...
    ?assertEqual({error, forbidden},
                 erlmcp_auth:check_permission(UserSession, <<"admin_management">>, <<"write">>)),
...
resource_access_permissions_test(Config) ->
    ...
    ok =
        erlmcp_auth:add_permission(<<"resource_1">>,
                                   <<"read">>,
                                   [<<"admin">>, <<"user">>, <<"guest">>]),
    ...
    ?assertEqual({error, forbidden},
                 erlmcp_auth:check_permission(GuestSession, <<"resource_1">>, <<"write">>)),

    %% Test resource_2 admin-only access
    ?assertEqual(ok, erlmcp_auth:check_permission(AdminSession, <<"resource_2">>, <<"read">>)),
    ?assertEqual({error, forbidden},
                 erlmcp_auth:check_permission(UserSession, <<"resource_2">>, <<"read">>)),
    ?assertEqual({error, forbidden},
                 erlmcp_auth:check_permission(GuestSession, <<"resource_2">>, <<"read">>)),
...
tool_execution_permissions_test(Config) ->
    ...
    ok =
        erlmcp_auth:add_permission(<<"tool_safe">>,
                                   <<"execute">>,
                                   [<<"admin">>, <<"user">>, <<"guest">>]),
    ...
    ?assertEqual({error, forbidden},
                 erlmcp_auth:check_permission(UserSession, <<"tool_admin">>, <<"execute">>)),
    ?assertEqual({error, forbidden},
                 erlmcp_auth:check_permission(GuestSession, <<"tool_admin">>, <<"execute">>)),

    %% Test dangerous tool access
    ?assertEqual(ok,
                 erlmcp_auth:check_permission(AdminSession, <<"tool_dangerous">>, <<"execute">>)),
    ?assertEqual({error, forbidden},
                 erlmcp_auth:check_permission(UserSession, <<"tool_dangerous">>, <<"execute">>)),
    ?assertEqual({error, forbidden},
                 erlmcp_auth:check_permission(GuestSession, <<"tool_dangerous">>, <<"execute">>)),
...
prompt_template_permissions_test(Config) ->
    ...
    ok =
        erlmcp_auth:add_permission(<<"prompt_public">>,
                                   <<"read">>,
                                   [<<"admin">>, <<"user">>, <<"guest">>]),
    ...
    ?assertEqual({error, forbidden},
                 erlmcp_auth:check_permission(GuestSession, <<"prompt_premium">>, <<"read">>)),

    %% Test internal prompt access
    ?assertEqual(ok, erlmcp_auth:check_permission(AdminSession, <<"prompt_internal">>, <<"read">>)),
    ?assertEqual({error, forbidden},
                 erlmcp_auth:check_permission(UserSession, <<"prompt_internal">>, <<"read">>)),
    ?assertEqual({error, forbidden},
                 erlmcp_auth:check_permission(GuestSession, <<"prompt_internal">>, <<"read">>)),
...
administrative_operation_permissions_test(Config) ->
    ...
    ?assertEqual({error, forbidden},
                 erlmcp_auth:check_permission(UserSession, <<"admin_users">>, <<"write">>)),
    ?assertEqual({error, forbidden},
                 erlmcp_auth:check_permission(GuestSession, <<"admin_users">>, <<"write">>)),

    %% Test configuration management (admin only)
    ?assertEqual(ok, erlmcp_auth:check_permission(AdminSession, <<"admin_config">>, <<"write">>)),
    ?assertEqual({error, forbidden},
                 erlmcp_auth:check_permission(UserSession, <<"admin_config">>, <<"write">>)),
    ?assertEqual({error, forbidden},
                 erlmcp_auth:check_permission(GuestSession, <<"admin_config">>, <<"write">>)),

    %% Test log deletion (admin only)
    ?assertEqual(ok, erlmcp_auth:check_permission(AdminSession, <<"admin_logs">>, <<"delete">>)),
    ?assertEqual({error, forbidden},
                 erlmcp_auth:check_permission(UserSession, <<"admin_logs">>, <<"delete">>)),
    ?assertEqual({error, forbidden},
                 erlmcp_auth:check_permission(GuestSession, <<"admin_logs">>, <<"delete">>)),
...
resource_uri_access_control_test(Config) ->
    ...
    ok =
        erlmcp_auth:add_permission(<<"file://public/data.txt">>,
                                   <<"read">>,
                                   [<<"admin">>, <<"user">>, <<"guest">>]),
    ok =
        erlmcp_auth:add_permission(<<"file://private/data.txt">>,
                                   <<"read">>,
                                   [<<"admin">>, <<"user">>]),
    ...
    ?assertEqual(ok,
                 erlmcp_auth:check_permission(AdminSession,
                                              <<"file://public/data.txt">>,
                                              <<"read">>)),
    ?assertEqual(ok,
                 erlmcp_auth:check_permission(UserSession,
                                              <<"file://public/data.txt">>,
                                              <<"read">>)),
    ?assertEqual(ok,
                 erlmcp_auth:check_permission(GuestSession,
                                              <<"file://public/data.txt">>,
                                              <<"read">>)),
...
resource_subscription_authorization_test(Config) ->
    ...
    ok =
        erlmcp_auth:add_permission(<<"resource_events">>, <<"execute">>, [<<"admin">>, <<"user">>]),
    ok = erlmcp_auth:add_permission(<<"admin_events">>, <<"execute">>, [<<"admin">>]),

    %% Test public resource subscription
    ?assertEqual(ok,
                 erlmcp_auth:check_permission(AdminSession, <<"resource_events">>, <<"execute">>)),
    ?assertEqual(ok,
                 erlmcp_auth:check_permission(UserSession, <<"resource_events">>, <<"execute">>)),
    ?assertEqual({error, forbidden},
                 erlmcp_auth:check_permission(GuestSession, <<"resource_events">>, <<"execute">>)),

    %% Test admin resource subscription
    ?assertEqual(ok, erlmcp_auth:check_permission(AdminSession, <<"admin_events">>, <<"execute">>)),
    ?assertEqual({error, forbidden},
                 erlmcp_auth:check_permission(UserSession, <<"admin_events">>, <<"execute">>)),
    ?assertEqual({error, forbidden},
                 erlmcp_auth:check_permission(GuestSession, <<"admin_events">>, <<"execute">>)),
...
resource_update_permissions_test(Config) ->
    ...
    ok =
        erlmcp_auth:add_permission(<<"user_profile">>,
                                   <<"read">>,
                                   [<<"admin">>, <<"user">>, <<"guest">>]),
    ...
    ?assertEqual({error, forbidden},
                 erlmcp_auth:check_permission(GuestSession, <<"user_profile">>, <<"write">>)),

    %% Test system config write (admin only)
    ?assertEqual(ok, erlmcp_auth:check_permission(AdminSession, <<"system_config">>, <<"write">>)),
    ?assertEqual({error, forbidden},
                 erlmcp_auth:check_permission(UserSession, <<"system_config">>, <<"write">>)),
    ?assertEqual({error, forbidden},
                 erlmcp_auth:check_permission(GuestSession, <<"system_config">>, <<"write">>)),

    %% Test system config delete (admin only)
    ?assertEqual(ok, erlmcp_auth:check_permission(AdminSession, <<"system_config">>, <<"delete">>)),
    ?assertEqual({error, forbidden},
                 erlmcp_auth:check_permission(UserSession, <<"system_config">>, <<"delete">>)),
    ?assertEqual({error, forbidden},
                 erlmcp_auth:check_permission(GuestSession, <<"system_config">>, <<"delete">>)),
...
tool_call_authorization_test(Config) ->
    ...
    %% Setup tool permissions
    ok =
        erlmcp_auth:add_permission(<<"tool_calculator">>,
                                   <<"execute">>,
                                   [<<"admin">>, <<"user">>, <<"guest">>]),
    ...
    ?assertEqual(ok,
                 erlmcp_auth:check_permission(AdminSession, <<"tool_calculator">>, <<"execute">>)),
    ?assertEqual(ok,
                 erlmcp_auth:check_permission(UserSession, <<"tool_calculator">>, <<"execute">>)),
    ?assertEqual(ok,
                 erlmcp_auth:check_permission(GuestSession, <<"tool_calculator">>, <<"execute">>)),

    %% Test database tool (admin and user)
    ?assertEqual(ok,
                 erlmcp_auth:check_permission(AdminSession, <<"tool_database">>, <<"execute">>)),
    ?assertEqual(ok, erlmcp_auth:check_permission(UserSession, <<"tool_database">>, <<"execute">>)),
    ?assertEqual({error, forbidden},
                 erlmcp_auth:check_permission(GuestSession, <<"tool_database">>, <<"execute">>)),

    %% Test system tool (admin only)
    ?assertEqual(ok, erlmcp_auth:check_permission(AdminSession, <<"tool_system">>, <<"execute">>)),
    ?assertEqual({error, forbidden},
                 erlmcp_auth:check_permission(UserSession, <<"tool_system">>, <<"execute">>)),
    ?assertEqual({error, forbidden},
                 erlmcp_auth:check_permission(GuestSession, <<"tool_system">>, <<"execute">>)),
...
tool_parameter_validation_test(Config) ->
    ...
    ?assertEqual(ok,
                 erlmcp_auth:check_permission(AdminSession, <<"tool_file_read">>, <<"execute">>)),
    ?assertEqual(ok,
                 erlmcp_auth:check_permission(UserSession, <<"tool_file_read">>, <<"execute">>)),

    %% Test file write tool (admin only)
    ?assertEqual(ok,
                 erlmcp_auth:check_permission(AdminSession, <<"tool_file_write">>, <<"execute">>)),
    ?assertEqual({error, forbidden},
                 erlmcp_auth:check_permission(UserSession, <<"tool_file_write">>, <<"execute">>)),

    %% Verify no access to unauthorized tools
    ?assertEqual({error, forbidden},
                 erlmcp_auth:check_permission(UserSession, <<"tool_unauthorized">>, <<"execute">>)),
    ?assertEqual({error, forbidden},
                 erlmcp_auth:check_permission(UserSession, <<"tool_shell">>, <<"execute">>)),
...
dangerous_tool_restrictions_test(Config) ->
    ...
    lists:foreach(fun(Tool) -> ok = erlmcp_auth:add_permission(Tool, <<"execute">>, [<<"admin">>])
                  end,
                  DangerousTools),
    ...
    lists:foreach(fun(Tool) ->
                     ?assertEqual(ok,
                                  erlmcp_auth:check_permission(AdminSession, Tool, <<"execute">>))
                  end,
                  DangerousTools),
    ...
    lists:foreach(fun(Tool) ->
                     ?assertEqual({error, forbidden},
                                  erlmcp_auth:check_permission(UserSession, Tool, <<"execute">>)),
                     ?assertEqual({error, forbidden},
                                  erlmcp_auth:check_permission(GuestSession, Tool, <<"execute">>))
                  end,
                  DangerousTools),
...
session_termination_rights_test(Config) ->
    ...
    ?assertEqual({error, invalid_session},
                 erlmcp_auth:check_permission(Session1, <<"any">>, <<"read">>)),
    ?assertEqual({error, invalid_session},
                 erlmcp_auth:check_permission(Session2, <<"any">>, <<"read">>)),
...
cross_session_access_prevention_test(Config) ->
    ...
    ?assertEqual({error, forbidden},
                 erlmcp_auth:check_permission(AdminSession, <<"user_regular_data">>, <<"read">>)),
    ?assertEqual({error, forbidden},
                 erlmcp_auth:check_permission(UserSession, <<"user_admin_data">>, <<"read">>)),

    %% But admin can access their own resources
    ?assertEqual(ok, erlmcp_auth:check_permission(AdminSession, <<"user_admin_data">>, <<"read">>)),
    ?assertEqual(ok,
                 erlmcp_auth:check_permission(UserSession, <<"user_regular_data">>, <<"read">>)),
...
invalid_permission_denied_test(Config) ->
    ...
    ?assertEqual({error, forbidden},
                 erlmcp_auth:check_permission(AdminSession,
                                              ...
                                              <<"invalid_perm">>)),
    ?assertEqual({error, forbidden},
                 erlmcp_auth:check_permission(AdminSession, <<"any_resource">>, <<>>)),

    %% Try with invalid session
    ?assertEqual({error, invalid_session},
                 erlmcp_auth:check_permission(<<"invalid_session">>,
                                              <<"any_resource">>,
                                              <<"read">>)),
...
expired_session_authorization_test(Config) ->
    ...
    ?assertEqual({error, invalid_session},
                 erlmcp_auth:check_permission(SessionId, <<"any_resource">>, <<"read">>)),
    ?assertEqual({error, invalid_session},
                 erlmcp_auth:check_permission(SessionId, <<"any_resource">>, <<"write">>)),
...
concurrent_permission_changes_test(Config) ->
    ...
    ?assertEqual(ok,
                 erlmcp_auth:check_permission(AdminSession, <<"concurrent_resource">>, <<"read">>)),
    ?assertEqual({error, forbidden},
                 erlmcp_auth:check_permission(UserSession, <<"concurrent_resource">>, <<"read">>)),
...
setup_default_permissions() ->
    ...
    ok =
        erlmcp_auth:add_permission(<<"public_resource">>,
                                   <<"read">>,
                                   [<<"admin">>, <<"user">>, <<"guest">>]),
...
create_test_session(UserId, Roles) ->
    %% Add roles to user
    lists:foreach(fun(Role) -> erlmcp_auth:add_role(UserId, Role) end, Roles),

    %% Create session
    {ok, SessionId} =
        erlmcp_auth:create_session(UserId,
                                   #{test => true, created_at => erlang:system_time(second)}),
