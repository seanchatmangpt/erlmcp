...
diagnose(Opts) ->
    try
        ...
        ProcessInfo =
            case maps:get(include_processes, Opts, false) of
                true ->
                    process_info(Opts);
                false ->
...
memory_usage(Opts) ->
    ...
    case Verbose >= 4 of
        true ->
            BaseInfo#{detailed_breakdown => erlang:memory(),
...
check_health(_Opts) ->
    ...
    #{overall_status => OverallStatus,
      ...
      memory => #{usage_percentage => MemoryUsage, status => MemoryStatus},
      processes =>
          #{count => ProcessCount,
            limit => ProcessLimit,
...
get_top_memory_processes(N) ->
    ...
    Sorted =
        lists:sort(fun(#{memory_bytes := M1}, #{memory_bytes := M2}) -> M1 > M2 end,
                   ProcessesWithMem),
...
get_top_reduction_processes(N) ->
    ...
    Sorted =
        lists:sort(fun(#{reductions := R1}, #{reductions := R2}) -> R1 > R2 end, ProcessesWithReds),
...
get_top_message_queue_processes(N) ->
    ...
    Sorted =
        lists:sort(fun(#{message_queue_len := Q1}, #{message_queue_len := Q2}) -> Q1 > Q2 end,
                   ProcessesWithQueue),
...
get_large_binaries(N) ->
    ...
    BinaryMaps =
        lists:map(fun({Pid, RegName, {_BinId, Size, RefCount}}) ->
                     #{pid => pid_to_list(Pid),
                       registered_name => RegName,
                       size_bytes => Size,
    ...
    Sorted = lists:sort(fun(#{size_bytes := S1}, #{size_bytes := S2}) -> S1 > S2 end, BinaryMaps),
...
determine_overall_status(Statuses) ->
    HasCritical =
        lists:any(fun(S) -> maps:get(status, S, ok) =:= critical orelse S =:= critical end,
                  Statuses),

    HasWarning =
        lists:any(fun(S) -> maps:get(status, S, ok) =:= warning orelse S =:= warning end, Statuses),
...
generate_recommendations(HealthStatus, MemoryInfo, ProcessInfo) ->
    ...
    HealthRecs =
        case maps:get(health_monitor, HealthStatus, not_running) of
            not_running ->
