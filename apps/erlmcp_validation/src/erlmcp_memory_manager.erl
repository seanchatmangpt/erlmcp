...
%% API
-export([start_link/0, start_link/1, stop/1, start_memory_monitor/0, get_memory_usage/0,
         cache_spec/1, cache_spec/2, get_cached_spec/1, purge_cache/0, purge_cache/1,
         optimize_memory_usage/1, force_garbage_collection/0, set_memory_limit/1,
         get_memory_limit/0, check_memory_pressure/0, get_stats/0, reset_stats/0]).
...
start_memory_monitor() ->
    gen_server:call(?MODULE, start_memory_monitor).

get_memory_usage() ->
    gen_server:call(?MODULE, get_memory_usage).
...
purge_cache() ->
    gen_server:call(?MODULE, purge_cache).

purge_cache(SpecId) ->
    gen_server:call(?MODULE, {purge_cache, SpecId}).

optimize_memory_usage(Pressure) ->
    gen_server:call(?MODULE, {optimize_memory, Pressure}).

force_garbage_collection() ->
    gen_server:call(?MODULE, force_garbage_collection).
...
init(Config) ->
    ...
    State =
        #state{max_cache_size = MaxCacheSize,
...
handle_call({cache_spec, SpecId, Spec}, _From, State) ->
    ...
    case SpecSize > MaxSpecMemory of
        ...
        false ->
            ...
            State2 =
                case CacheSize >= State#state.max_cache_size of
                    true ->
...
handle_call({get_cached_spec, SpecId}, _From, State) ->
    case maps:get(SpecId, State#state.spec_cache, undefined) of
        ...
        CacheEntry ->
            ...
            {reply,
             {ok, CacheEntry#{spec => CacheEntry#{spec => CacheEntry}}},
...
terminate(_Reason, State) ->
    case State#state.memory_monitor_ref of
        undefined ->
            ok;
        MemRef ->
            erlang:cancel_timer(MemRef)
    end,
    case State#state.gc_timer_ref of
        undefined ->
            ok;
        GcRef ->
            erlang:cancel_timer(GcRef)
    end,
    ok.
...
calculate_memory_stats(State) ->
    ...
    CacheMemory =
        lists:foldl(fun({_SpecId, Entry}, Acc) -> Acc + maps:get(size_bytes, Entry, 0) end,
                    0,
                    maps:to_list(State#state.spec_cache)),
    UsedPercent =
...
determine_pressure_level(UsedPercent) when UsedPercent >= 95.0 ->
    critical;
determine_pressure_level(UsedPercent) when UsedPercent >= 85.0 ->
    high;
determine_pressure_level(UsedPercent) when UsedPercent >= 70.0 ->
    medium;
determine_pressure_level(_UsedPercent) ->
    low.
...
monitor_memory_pressure(State) ->
    ...
    case Pressure of
        critical ->
            apply_optimization(aggressive, State);
        high ->
            apply_optimization(balanced, State);
        medium ->
            apply_optimization(conservative, State);
        low ->
            State
    end.
...
apply_optimization(_Level, State) ->
    State.

purge_cache_entries(State, Percentage) ->
    CacheList =
        lists:sort(fun(_SpecIdA, EntryA, _SpecIdB, EntryB) ->
    ...
    NewCache =
        lists:foldl(fun({SpecId, _Entry}, Acc) -> maps:remove(SpecId, Acc) end,
                    State#state.spec_cache,
                    ToPurge),
    NewStats =
        maps:update_with(cache_evictions, fun(V) -> V + EntriesToPurge end, State#state.stats),
    State#state{spec_cache = NewCache, stats = NewStats}.
...
evict_lru_entry(State) ->
    ...
    case CacheList of
        ...
        _ ->
            {SpecId, _Entry} =
                lists:foldl(fun({_Id, Entry}, {AccId, AccEntry}) ->
                               case maps:get(last_accessed, Entry)
                                    < maps:get(last_accessed, AccEntry)
                               of
                                   true ->
                                       {_Id, Entry};
                                   false ->
...
memory_monitor_loop(Parent) ->
    ...
    after 10000 ->
        case erlmcp_memory_manager:check_memory_pressure() of
            critical ->
                Parent ! {memory_pressure, critical};
            high ->
                Parent ! {memory_pressure, high};
            _ ->
                ok
        end,
        memory_monitor_loop(Parent)
