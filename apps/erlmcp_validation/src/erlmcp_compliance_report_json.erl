...
format_validator_result(ValidatorKey, ValidationResults) ->
    case maps:get(ValidatorKey, ValidationResults, undefined) of
        ...
        Result when is_map(Result) ->
            ...
            StatusBin = case maps:get(status, Result, unknown) of
                passed -> <<"passed">>;
                failed -> <<"failed">>;
                warning -> <<"warning">>;
            ...
            case maps:get(checks, Result, undefined) of
                undefined -> Base;
                Checks -> Base#{checks => format_checks(Checks)}
            end
    end.
...
format_check(Check) ->
    StatusBin = case maps:get(status, Check, unknown) of
        passed -> <<"passed">>;
        failed -> <<"failed">>;
        warning -> <<"warning">>;
        Other -> atom_to_binary(Other)
    ...
    Base = #{
        name => maps:get(name, Check, <<"unknown">>),
        status => StatusBin
    },
    %% Add optional fields if present
    Maybe1 = case maps:get(message, Check, undefined) of
        undefined -> Base;
        Msg -> Base#{message => Msg}
    end,
    Maybe2 = case maps:get(details, Check, undefined) of
        undefined -> Maybe1;
        Details -> Maybe1#{details => Details}
    end,
    Maybe2.
...
compute_status(ValidationResults) ->
    ...
    if
        Total =:= 0 -> <<"skipped">>;
        TotalFailed =:= 0 -> <<"pass">>;
        TotalPassed > TotalFailed -> <<"partial">>;
        true -> <<"fail">>
    end.
...
count_passed(ValidationResults) ->
    ...
    lists:foldl(fun(Key, Acc) ->
        case maps:get(Key, ValidationResults, undefined) of
            undefined -> Acc;
            Result -> Acc + maps:get(passed, Result, 0)
        end
    end, 0, Validators).
...
count_failed(ValidationResults) ->
    ...
    lists:foldl(fun(Key, Acc) ->
        case maps:get(Key, ValidationResults, undefined) of
            undefined -> Acc;
            Result -> Acc + maps:get(failed, Result, 0)
        end
    end, 0, Validators).
    ...
    lists:foldl(fun(Key, Acc) ->
        case maps:get(Key, ValidationResults, undefined) of
            ...
            Result ->
                ...
                FailedChecks = lists:filter(fun(C) ->
                    maps:get(status, C, passed) =:= failed
                end, Checks),
                %% Add validator context to each failure
    FailedWithCtx = lists:map(fun(C) ->
                    C#{validator => ValidatorName}
                end, FailedChecks),
                Acc ++ FailedWithCtx
        end
    end, [], Validators).
    ...
    lists:foldl(fun(Key, Acc) ->
        case maps:get(Key, ValidationResults, undefined) of
            ...
            Result ->
                ValidatorName = atom_to_binary(Key),
                Checks = maps:get(checks, Result, []),
                WarningChecks = lists:filter(fun(C) ->
                    maps:get(status, C, passed) =:= warning
                end, Checks),
                %% Add validator context to each warning
                WarningWithCtx = lists:map(fun(C) ->
                    C#{validator => ValidatorName}
                end, WarningChecks),
                Acc ++ WarningWithCtx
        end
...
validate_report_structure(Report) ->
    ...
    HasValidators = maps:is_key(validators, Report) andalso
        lists:all(fun(K) -> maps:is_key(K, maps:get(validators, Report)) end,
                  [protocol, transport, security, performance]),
    HasSummary = maps:is_key(summary, Report) andalso
        lists:all(fun(K) -> maps:is_key(K, maps:get(summary, Report)) end,
