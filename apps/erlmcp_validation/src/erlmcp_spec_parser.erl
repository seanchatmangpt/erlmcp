...
%% API exports
-export([start_link/0, stop/0, get_spec/0, parse_spec/0, get_method_requirements/0,
         get_method_requirements/1, list_methods/0, get_error_requirements/0,
         ...
         is_valid_notification_type/1, get_required_capabilities/1, get_all_error_codes/0,
         get_all_request_types/0, get_all_notification_types/0, is_valid_uri/1,
         is_valid_request_id/1, get_method_params/1, get_method_result/1, get_capability_features/1,
         get_version_compatibility/1, is_deprecated_method/1]).
...
check_capability_support(CapabilityName, Features)
    when is_binary(CapabilityName), is_list(Features) ->
...
build_spec() ->
    #mcp_spec{version = ?MCP_2025_11_25_VERSION,
...
build_methods() ->
    [#method_req{name = <<"initialize">>,
     ...
     #method_req{name = <<"tools/list">>,
                 method_type = request,
                 direction = client_to_server,
                 required = false,
                 params_spec = #{cursor => #{type => <<"string">>, required => false}},
                 result_spec = #{tools => #{type => <<"array">>, items => #{type => <<"object">>}}},
                 capability_required = <<"tools">>,
                 deprecation_status = stable,
                 documentation = <<"List available tools">>},
     #method_req{name = <<"tools/call">>,
                 method_type = request,
                 direction = client_to_server,
                 required = false,
                 params_spec =
     ...
     #method_req{name = <<"resources/read">>,
                 method_type = request,
                 direction = client_to_server,
                 required = false,
                 params_spec = #{uri => #{type => <<"string">>, required => true}},
                 result_spec = #{contents => #{type => <<"array">>}},
                 capability_required = <<"resources">>,
                 deprecation_status = stable,
                 documentation = <<"Read a resource">>},
     #method_req{name = <<"resources/list">>,
                 method_type = request,
                 direction = client_to_server,
                 required = false,
                 params_spec = #{cursor => #{type => <<"string">>, required => false}},
                 result_spec = #{resources => #{type => <<"array">>}},
                 capability_required = <<"resources">>,
                 deprecation_status = stable,
                 documentation = <<"List available resources">>},
     #method_req{name = <<"prompts/list">>,
                 method_type = request,
                 direction = client_to_server,
                 required = false,
                 params_spec = #{cursor => #{type => <<"string">>, required => false}},
                 result_spec = #{prompts => #{type => <<"array">>}},
                 capability_required = <<"prompts">>,
                 deprecation_status = stable,
                 documentation = <<"List available prompts">>},
     #method_req{name = <<"prompts/get">>,
                 method_type = request,
                 direction = client_to_server,
                 required = false,
                 params_spec =
                 ...
                 result_spec = #{messages => #{type => <<"array">>}},
                 capability_required = <<"prompts">>,
                 deprecation_status = stable,
                 documentation = <<"Get a specific prompt">>},
     %% PING - Liveness check
     #method_req{name = <<"ping">>,
                 method_type = request,
                 direction = client_to_server,
                 required = false,
                 params_spec = #{},
     ...
     #method_req{name = <<"notifications/initialized">>,
                 method_type = notification,
     ...
     #method_req{name = <<"notifications/message">>,
                 method_type = notification,
                 direction = server_to_client,
                 required = false,
                 params_spec =
     ...
     #method_req{name = <<"tasks/cancel">>,
                 method_type = request,
                 direction = client_to_server,
                 required = false,
                 params_spec = #{<<"taskId">> => #{type => <<"string">>, required => true}},
                 result_spec =
     ...
     #method_req{name = <<"requests/cancel">>,
                 method_type = request,
                 direction = client_to_server,
                 required = false,
                 params_spec =
                 ...
                 result_spec = #{<<"cancelled">> => #{type => <<"boolean">>, required => true}},
                 capability_required = undefined,
                 deprecation_status = stable,
                 documentation = <<"Cancel a pending request">>},
     %% COMPLETION/COMPLETE - Complete request
     #method_req{name = <<"completion/complete">>,
                 method_type = request,
                 direction = client_to_server,
                 required = false,
                 params_spec =
...
build_transports() ->
    [#transport_req{name = <<"stdio">>,
                    ...
                    optional_features = []},
     #transport_req{name = <<"stdio-transport">>,
                    transport_type = stream_based,
                    framing = json_delimiter,
                    connection_oriented = true,
                    multiplexing_support = false,
...
build_capabilities() ->
    [#capability_req{name = <<"resources">>,
     ...
     #capability_req{name = <<"logging">>,
                     category = server,
                     required = false,
                     dependencies = [],
                     features = [<<"level">>],
                     validation_rules = [<<"logging/set must accept level parameter">>]},
...
validate_params_against_spec(Params, Spec) when is_map(Spec) ->
    RequiredParams =
        [K
         || {K, V} <- maps:to_list(Spec), is_map(V) andalso maps:get(required, V, false) =:= true],
    ...
    case lists:keyfind(CapabilityName, #capability_req.name, Capabilities) of
        ...
        CapabilityReq ->
            ...
            case lists:sort(RequiredFeatures) =< lists:sort(Features) of
                true ->
                    {ok, capability_supported};
                false ->
                    {error, {missing_features, RequiredFeatures -- Features}}
            end
    end.
...
is_valid_request_id(RequestId) when is_binary(RequestId) ->
    true;
is_valid_request_id(RequestId) when is_integer(RequestId) ->
    true;
is_valid_request_id(RequestId) when is_float(RequestId) ->
    false; % JSON-RPC doesn't allow floats
is_valid_request_id(null) ->
    true;
is_valid_request_id(_) ->
    false.
...
get_method_params(MethodName) ->
    ...
    case lists:keyfind(MethodName, #method_req.name, Methods) of
        false ->
            undefined;
        MethodReq ->
            MethodReq#method_req.params_spec
    end.
...
get_method_result(MethodName) ->
    ...
    case lists:keyfind(MethodName, #method_req.name, Methods) of
        false ->
            undefined;
        MethodReq ->
            MethodReq#method_req.result_spec
    end.
...
get_capability_features(CapabilityName) ->
    ...
    case lists:keyfind(CapabilityName, #capability_req.name, Capabilities) of
        false ->
            undefined;
        CapabilityReq ->
            CapabilityReq#capability_req.features
    end.
...
get_version_compatibility(Version) ->
    case Version of
        ...
        <<"2024-11-05">> ->
            deprecated;
        <<"2025-01-23">> ->
            compatible; % Future version example
        _ ->
            incompatible
    end.
