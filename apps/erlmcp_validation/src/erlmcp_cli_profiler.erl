...
profile_cpu(Opts) ->
    ...
    try
        case Backend of
            fprof -> profile_with_fprof(Duration, Opts);
            eprof -> profile_with_eprof(Duration, Opts);
            cprof -> profile_with_cprof(Duration, Opts)
        end
    catch
        Class:Error:Stack ->
            {error, #{
...
profile_command_cpu(Command, Opts) ->
    ...
    %% Start profiler
    case Backend of
        fprof -> fprof:trace([start, {procs, [self()]}]);
        eprof ->
            eprof:start(),
...
calculate_memory_deltas(Initial, Final) ->
    ...
    maps:from_list([{Key, #{
        ...
        delta_mb => (maps:get(Key, FinalMap, 0) - maps:get(Key, InitialMap, 0)) / (1024 * 1024)
    }} || Key <- Keys]).
...
get_process_memory_profile() ->
    ...
    ProcessInfo = lists:filtermap(fun(Pid) ->
        case process_info(Pid, [memory, registered_name, current_function, heap_size]) of
            undefined -> false;
            Info ->
...
get_eprof_results() ->
    try
        #{
            status => completed,
            note => <<"Analysis output printed to console">>
        }
    catch
        _:_ -> {error, no_results}
    end.
...
generate_html_profile(ProfileData) ->
    [
        ...
        "  <pre>", io_lib:format("~p", [ProfileData]), "</pre>\n",
        "</body>\n",
        "</html>\n"
    ].
