...
test_concurrent_health_checks() ->
        ...
        [begin
             {ok, Pid} =
                 gen_server:start_link(fun() ->
                                          {ok,
                                           #{transport_id =>
                                                 list_to_atom("test_health_" ++ integer_to_list(N)),
                                             type => tcp}}
                                       end,
                                       [],
                                       []),
    ...
    Results = [erlmcp_transport_health:check_health(Pid, 1000) || Pid <- Transports],

    %% All health checks should return valid results
    ?assert(lists:all(fun ({ok, _Status}) ->
                              true;
                          ({error, _Reason}) ->
                              true
                      end,
...
test_status_change_up_to_degraded() ->
    ...
    case erlmcp_transport_health:update_status(TransportPid, degraded) of
        ok ->
            %% Verify status changed (API call - observable)
            {ok, NewStatus} = erlmcp_transport_health:get_status(TransportPid),
...
test_status_change_degraded_to_down() ->
    ...
    %% Update health status (API call - observable)
    case erlmcp_transport_health:update_status(TransportPid, down) of
        ok ->
            %% Verify status changed (API call - observable)
            {ok, NewStatus} = erlmcp_transport_health:get_status(TransportPid),
...
test_status_change_down_to_up() ->
    ...
    %% Update health status (API call - observable)
    case erlmcp_transport_health:update_status(TransportPid, up) of
        ok ->
            %% Verify status changed (API call - observable)
            {ok, NewStatus} = erlmcp_transport_health:get_status(TransportPid),
