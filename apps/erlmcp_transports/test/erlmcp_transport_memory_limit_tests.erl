     ...
     fun(_) ->
        [?_test(begin
                    ...
                    ?assertEqual({error, payload_too_large},
                                 erlmcp_memory_guard:check_allocation(OversizeMessage)),
    ...
    {setup,
     ...
     fun(_) ->
        [?_test(begin
                    ...
                    ?assertEqual({error, payload_too_large},
                                 erlmcp_memory_guard:check_allocation(byte_size(OversizePayload)))
                end)]
     end}.
        ...
        [?_test(begin
                    ...
                    case erlmcp_memory_guard:check_allocation(OversizeMessage) of
                        {error, payload_too_large} ->
                            ?assert(true);
                        Other ->
                            ?assert(false, {expected_payload_too_large, Other})
                    end
                end)]
     end}.
    ...
    {setup,
     ...
     fun(_) ->
        [?_test(begin
                    ...
                    lists:foreach(fun(Size) -> erlmcp_memory_guard:check_allocation(Size) end,
