...
test_register_unregister(_Pid) ->
    % Spawn a dummy transport process
...
test_get_transport(_Pid) ->
    ...
    erlmcp_transport_registry:register_transport(test_get, TransportPid, Config),

    % Get transport info
    {ok, Info} = erlmcp_transport_registry:get_transport(test_get),
    ...
    [
     ?_assertEqual(test_get, maps:get(transport_id, Info)),
     ?_assertEqual(tcp, maps:get(type, Info)),
     ?_assertEqual(up, maps:get(health_status, Info))
    ].
...
test_record_success_failure(_Pid) ->
    ...
    timer:sleep(100),  % Give time for async processing
...
test_process_monitoring(_Pid) ->
    ...
    TransportPid ! die,
    timer:sleep(200),  % Give time for monitor to trigger
...
get_all_transports_test() ->
    {ok, _Pid} = erlmcp_transport_registry:start_link(),
