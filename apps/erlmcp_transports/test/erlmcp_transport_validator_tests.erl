...
state_management_stdio_test_( ) -> { setup , fun setup/ 0 , fun teardown/ 1 , [ { "stdio init returns {ok, Pid}" , fun ( ) -> Owner = self( ) , Result = erlmcp_transport_stdio : start_link( Owner ) , ?assertMatch( { ok , Pid } when is_pid( Pid ) , Result ) , { ok , Pid } = Result , gen_server : stop( Pid , normal , 1000 ) end } , { "stdio transport alive after init" , fun ( ) -> Owner = self( ) , { ok , Pid } = erlmcp_transport_stdio : start_link( Owner ) , ?assert( is_process_alive( Pid ) ) , gen_server : stop( Pid , normal , 1000 ) end } , { "stdio transport dead after close" , fun ( ) -> Owner = self( ) , { ok , Pid } = erlmcp_transport_stdio : start_link( Owner ) , ?assert( is_process_alive( Pid ) ) , ok = erlmcp_transport_stdio : close( Pid ) , timer : sleep( 100 ) , ?assertNot( is_process_alive( Pid ) ) end } ] } .
     ...
     {timeout,
      ...
      fun() ->
         ...
         try
             ...
             NumMessages = 50,
             Pids =
                 [spawn(fun() -> erlmcp_transport_stdio:send(Transport, integer_to_binary(N)) end)
