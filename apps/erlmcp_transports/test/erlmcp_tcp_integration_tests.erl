    ...
    {setup,
     ...
     fun() ->
         ...
         ServerHandler = receive
             {transport_connected, Handler} when is_pid(Handler) -> Handler
         after 3000 ->
...
bidirectional_communication_test_() ->
    {setup,
     ...
     fun() ->
         ...
         % Client -> Server
         ?assertEqual(ok, erlmcp_test_helpers:send_transport_data(ClientPid, <<"client msg">>, 1000)),
         receive {transport_message, <<"client msg">>} -> ok after 2000 -> ?assert(false) end,

         % Server -> Client (by sending through handler)
         ?assertEqual(ok, erlmcp_test_helpers:send_transport_data(ServerHandler, <<"server msg">>, 1000)),
    ...
    {setup,
     ...
     fun() ->
         ...
         lists:foreach(fun(Client) ->
             ?assertEqual(ok, erlmcp_test_helpers:send_transport_data(Client, <<"test">>, 1000))
         end, Clients),

         % Receive all messages
         lists:foreach(fun(_) ->
             receive
                 {transport_message, <<"test">>} -> ok
             after 2000 ->
...
client_reconnects_after_server_restart_test_() ->
    {setup,
     ...
     fun() ->
         ...
         {ok, _} = erlmcp_test_helpers:wait_for_transport_connected(5000),
         ServerHandler = receive {transport_connected, H} -> H after 3000 -> error("No handler") end,
         ...
         receive {transport_disconnected, ClientPid, _} -> ok after 3000 -> ?assert(false) end,
         ...
         NewHandler = receive {transport_connected, H2} -> H2 after 3000 -> error("No new handler") end,
...
large_message_handling_test_() ->
    {setup,
     ...
     fun() ->
         ...
         {ok, _} = erlmcp_test_helpers:wait_for_transport_connected(5000),
         receive {transport_connected, _Handler} -> ok after 3000 -> ok end,
...
message_too_large_test_() ->
    {setup,
     ...
     fun() ->
         ...
         ?assertEqual(ok, erlmcp_test_helpers:send_transport_data(ClientPid, TooLargeMessage, 10000)),

         % Connection should be closed
         receive {transport_disconnected, ClientPid, _} -> ok after 5000 -> ok end,
...
line_based_protocol_test_() ->
    {setup,
     ...
     fun() ->
         ...
         receive {transport_message, <<"msg1">>} -> ok after 2000 -> ?assert(false) end,
         receive {transport_message, <<"msg2">>} -> ok after 2000 -> ?assert(false) end,
         receive {transport_message, <<"msg3">>} -> ok after 2000 -> ?assert(false) end,
...
buffer_overflow_test_() ->
    {setup,
     ...
     fun() ->
         ...
         receive {transport_message, <<"partial message">>} -> ok after 2000 -> ok end,
         receive {transport_message, <<" rest">>} -> ok after 2000 -> ok end,
