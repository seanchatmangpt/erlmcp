...
-export([
    ...
    http_transport_init_success/1,
    http_transport_init_invalid_url/1,
    http_transport_init_missing_owner/1,
    http_transport_init_with_custom_headers/1,
    http_transport_init_with_ssl_options/1,

    %% Connection tests
    http_connection_establish/1,
    http_connection_refused/1,
    http_connection_timeout/1,
    http_connection_reconnect/1,

    %% Request handling tests
    http_send_post_request/1,
    http_send_get_request/1,
    http_send_multiple_requests/1,
    http_send_large_request/1,

    %% Response handling tests
    http_receive_success_response/1,
    http_receive_error_response/1,
    http_receive_empty_response/1,

    %% Retry logic tests
    http_retry_on_connection_failure/1,
    http_retry_max_attempts_exceeded/1,

    %% Error handling tests
    http_error_timeout/1,
    http_error_message_too_large/1,

    %% Pool management tests
    http_pool_size_respected/1,
    http_pool_request_completion/1,
...
groups() ->
    [
        ...
        {responses, [parallel], [
            http_receive_success_response,
            http_receive_error_response,
            http_receive_empty_response
        ]},
        {retry_logic, [sequential], [
            http_retry_on_connection_failure,
            http_retry_max_attempts_exceeded
        ]},
        {error_handling, [parallel], [
            http_error_timeout,
            http_error_message_too_large
        ]},
        {pool_management, [parallel], [
            http_pool_size_respected,
            http_pool_request_completion
        ]},
        {cleanup, [parallel], [
            http_close_connection,
            http_close_pending_requests
        ]}
    ].
...
http_transport_init_success(Config) ->
    ...
    Opts = #{
        url => lists:flatten(io_lib:format("http://localhost:~p/mcp", [MockPort])),
        owner => self()
    },
...
http_send_multiple_requests(Config) ->
    ...
    Requests = [
        jsx:encode(#{<<"id">> => N, <<"data">> => <<"request">>}) ||
        N <- lists:seq(1, 10)
    ],

    Results = [
        erlmcp_transport_http:send(Pid, Req) || Req <- Requests
    ],
...
http_receive_error_response(Config) ->
    ...
    MockPort = ?config(mock_port, Config),
    Opts = #{
        url => lists:flatten(io_lib:format("http://localhost:~p/mcp/error", [MockPort])),
        owner => self(),
        timeout => 5000,
        max_retries => 0
    },
...
http_receive_empty_response(Config) ->
    ...
    MockPort = ?config(mock_port, Config),
    Opts = #{
        url => lists:flatten(io_lib:format("http://localhost:~p/mcp/empty", [MockPort])),
        owner => self(),
        timeout => 5000,
        max_retries => 0
    },
...
http_retry_max_attempts_exceeded(Config) ->
    ...
    MockPort = ?config(mock_port, Config),
    Opts = #{
        url => lists:flatten(io_lib:format("http://localhost:~p/mcp/fail", [MockPort])),
        owner => self(),
        timeout => 1000,
        max_retries => 2,
        retry_delay => 50
...
http_error_timeout(Config) ->
    ...
    MockPort = ?config(mock_port, Config),
    Opts = #{
        url => lists:flatten(io_lib:format("http://localhost:~p/mcp/slow", [MockPort])),
        owner => self(),
        timeout => 100
    },
    {ok, Pid} = erlmcp_transport_http:init(Opts),
...
http_pool_size_respected(Config) ->
    ...
    Requests = [
        jsx:encode(#{<<"id">> => N}) || N <- lists:seq(1, 10)
    ],
...
http_pool_request_completion(Config) ->
    ...
    Requests = [
        jsx:encode(#{<<"id">> => N}) || N <- lists:seq(1, 5)
    ],
...
http_close_pending_requests(Config) ->
    ...
    lists:foreach(
        fun(Req) -> erlmcp_transport_http:send(Pid, Req) end,
        Requests
    ),
