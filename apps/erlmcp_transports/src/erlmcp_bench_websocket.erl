...
run_all() ->
    ...
    Results =
        lists:map(fun({WorkloadId, MessageSize, FrameType}) ->
...
run(#{workload_id := WorkloadId,
    ...
    {MessageCount, LatencySamples} =
        benchmark_loop(EncodedMessage, FrameType, StartTime, Duration * 1000, 0, []),
...
print_summary(Results) ->
    ...
    lists:foreach(fun(#{workload_id := Id,
                        ...
                        latency_p50_us := P50,
                        latency_p99_us := P99}) ->
                     ct:pal("~-20s ~-10s ~12w ~12w ~12w~n", [Id, Type, Throughput, P50, P99])
                  end,
                  Results),
