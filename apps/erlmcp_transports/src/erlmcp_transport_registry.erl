...
%% API
-export([start_link/0, start_link/1, register_transport/3, register_transport/4,
         unregister_transport/1, unregister_transport/2, get_transport/1, get_transport/2,
         list_transports/0, list_transports/1, list_transports_by_type/1,
         list_transports_by_capability/1, find_transport_by_capability/1, update_transport_config/2,
         get_transport_capabilities/1, get_transport_stats/1, get_all_stats/0, health_check/1,
         health_check_all/0, discover_transports/0, discover_transports/1]).
...
-spec register_transport(transport_id(), pid() | undefined, map()) -> ok | {error, term()}.
...
-spec get_transport(transport_id()) -> {ok, map()} | {error, not_found}.
get_transport(TransportId) ->
    get_transport(local, TransportId).

%% @doc Get transport information with explicit scope
-spec get_transport(transport_scope(), transport_id()) -> {ok, map()} | {error, not_found}.
...
-spec update_transport_config(transport_id(), map()) -> ok | {error, not_found}.
...
-spec get_transport_stats(transport_id()) -> {ok, map()} | {error, not_found}.
...
-spec health_check(transport_id()) -> {ok, healthy | degraded | unhealthy} | {error, not_found}.
...
%% @private
handle_call({register_transport, Scope, _TransportId, _Pid, _Config},
            _From,
            #state{scope = RegistryScope} = State)
    when Scope =/= RegistryScope ->
...
handle_call({register_transport, _Scope, TransportId, Pid, Config}, _From, State) ->
    case maps:is_key(TransportId, State#state.transports) of
        ...
        false ->
            ...
            {reply,
             ok,
             NewState#state{transports =
                                maps:put(TransportId, TransportInfo, NewState#state.transports)}}
    end;
handle_call({unregister_transport, Scope, _TransportId},
            _From,
            #state{scope = RegistryScope} = State)
    when Scope =/= RegistryScope ->
    {reply, {error, scope_mismatch}, State};
handle_call({unregister_transport, _Scope, TransportId}, _From, State) ->
    case maps:get(TransportId, State#state.transports, undefined) of
        ...
        TransportInfo ->
            ...
            {reply,
             ok,
             NewState#state{transports = maps:remove(TransportId, NewState#state.transports)}}
    end;
handle_call({get_transport, Scope, _TransportId}, _From, #state{scope = RegistryScope} = State)
    when Scope =/= RegistryScope ->
    {reply, {error, scope_mismatch}, State};
...
handle_call({list_transports, Scope}, _From, #state{scope = RegistryScope} = State)
    when Scope =/= RegistryScope ->
    {reply, [], State};
...
handle_call({list_transports_by_type, Type}, _From, State) ->
    ...
    TransportList =
        lists:filtermap(fun(TransportId) ->
                           case maps:get(TransportId, State#state.transports, undefined) of
                               undefined ->
                                   false;
                               TransportInfo ->
                                   {true, {TransportId, transport_info_to_map(TransportInfo)}}
                           end
                        end,
                        TransportIds),
...
handle_call({update_transport_config, TransportId, ConfigUpdate}, _From, State) ->
    case maps:get(TransportId, State#state.transports, undefined) of
        ...
        TransportInfo ->
            ...
            NewCapabilities =
                case maps:get(capabilities, ConfigUpdate, undefined) of
                    undefined ->
                        TransportInfo#transport_info.capabilities;
                    Caps ->
                        normalize_capabilities(Caps)
                end,

            UpdatedInfo =
                TransportInfo#transport_info{config = NewConfig, capabilities = NewCapabilities},

            ?LOG_DEBUG("Updated config for transport: ~p", [TransportId]),
            {reply,
             ok,
             State#state{transports = maps:put(TransportId, UpdatedInfo, State#state.transports)}}
    end;
...
handle_call(get_all_stats, _From, State) ->
    AllStats =
        maps:map(fun(_TransportId, TransportInfo) -> TransportInfo#transport_info.stats end,
                 State#state.transports),
...
handle_call(health_check_all, _From, State) ->
    HealthResults =
        maps:map(fun(_TransportId, TransportInfo) -> check_transport_health(TransportInfo) end,
                 State#state.transports),
...
handle_info({'DOWN', _MonitorRef, process, Pid, Reason}, State) ->
    ...
    NewTransports =
        maps:map(fun(TransportId, TransportInfo) ->
                    case TransportInfo#transport_info.pid of
                        Pid ->
                            ?LOG_WARNING("Transport process ~p died: ~p", [TransportId, Reason]),
                            TransportInfo#transport_info{status = error, pid = undefined};
                        _ ->
                            TransportInfo
                    end
                 end,
...
-spec add_to_indices(transport_id(),
                     transport_type(),
...
add_to_indices(TransportId, Type, Capabilities, _TransportInfo, State) ->
    ...
    State#state{by_type = NewByType, by_capability = NewByCapability}.
        ...
        case maps:get(Type, ByType, []) of
            ...
            TypeList ->
                ...
                case FilteredList of
                    ...
                    _ ->
                        maps:put(Type, FilteredList, ByType)
                end
        end,
    ...
    NewByCapability =
        lists:foldl(fun(Capability, Acc) ->
                       case maps:get(Capability, Acc, []) of
                           ...
                           CapList ->
                               CapFilteredList =
                                   lists:filter(fun(Id) -> Id =/= TransportId end, CapList),
                               case CapFilteredList of
                                   [] ->
                                       maps:remove(Capability, Acc);
                                   _ ->
                                       maps:put(Capability, CapFilteredList, Acc)
                               end
                       end
                    end,
                    ByCapability,
...
-spec discover_transports_internal(state()) -> {ok, [transport_id()], state()} | {error, term()}.
discover_transports_internal(State) ->
    try
        % Get all loaded modules that match transport pattern
        AllModules = [M || M <- erlang:loaded_modules(), is_transport_module(M)],

        % Register discovered transports
        {DiscoveredIds, NewState} =
            lists:foldl(fun(Module, {Ids, AccState}) ->
        ...
        _ ->
            ...
            ExcludePatterns =
                ["behavior",
                 ...
                 "adapter"],
            not
                lists:any(fun(Pattern) -> string:find(ModuleStr, Pattern) =/= nomatch end,
                          ExcludePatterns)
    end.
