...
run_all() ->
    ...
    Results =
        lists:map(fun(Transport) ->
                     run(#{workload_id => list_to_atom(atom_to_list(Transport) ++ "_integration"),
                           transport => Transport})
                  end,
                  Transports),
...
run(#{workload_id := WorkloadId, transport := Transport} = _Config) ->
    ...
    CapabilityResults =
        lists:map(fun(Capability) -> test_capability(Transport, Capability) end, Capabilities),

    % Calculate average negotiation latency
    LatencySamples = [L || #{latency_us := L} <- CapabilityResults],
    AvgLatency =
        case LatencySamples of
            [] ->
                0;
            _ ->
                lists:sum(LatencySamples) div length(LatencySamples)
        end,
...
test_capability(Transport, Capability) ->
    ...
    Status =
        case {Transport, Capability} of
            ...
            % HTTP transport
            {http, _} ->
                skip;  % Requires HTTP server
            % WebSocket transport
            {websocket, _} ->
                skip;  % Requires WS server
            % SSE transport
            {sse, resources} ->
                skip;  % Requires SSE server
            {sse, tools} ->
                fail;  % Not supported (unidirectional)
            {sse, prompts} ->
                skip;
            {sse, roots} ->
                skip;
...
test_stdio_tools() ->
    case erlmcp_transport_stdio:start_link(self(),
                                           #{transport_id => stdio_tools_test, test_mode => true})
    of
        {ok, Pid} ->
            Request =
                #{<<"jsonrpc">> => <<"2.0">>,
...
test_stdio_prompts() ->
    case erlmcp_transport_stdio:start_link(self(),
                                           #{transport_id => stdio_prompts_test, test_mode => true})
    of
        {ok, Pid} ->
            Request =
                #{<<"jsonrpc">> => <<"2.0">>,
...
test_stdio_roots() ->
    case erlmcp_transport_stdio:start_link(self(),
                                           #{transport_id => stdio_roots_test, test_mode => true})
    of
        {ok, Pid} ->
            Request =
                #{<<"jsonrpc">> => <<"2.0">>,
...
test_tcp_resources() ->
    ...
    case erlmcp_transport_tcp:start_server(#{transport_id => tcp_res_server,
                                             server_id => tcp_res_server,
                                             owner => self(),
                                             port => Port})
    of
        {ok, ServerPid} ->
            case erlmcp_transport_tcp:start_client(#{transport_id => tcp_res_client,
                                                     owner => self(),
                                                     host => "localhost",
                                                     port => Port})
            of
...
test_tcp_tools() ->
    ...
    case erlmcp_transport_tcp:start_server(#{transport_id => tcp_tools_server,
                                             server_id => tcp_tools_server,
                                             owner => self(),
                                             port => Port})
    of
        {ok, ServerPid} ->
            case erlmcp_transport_tcp:start_client(#{transport_id => tcp_tools_client,
                                                     owner => self(),
                                                     host => "localhost",
                                                     port => Port})
            of
...
test_tcp_prompts() ->
    ...
    case erlmcp_transport_tcp:start_server(#{transport_id => tcp_prompts_server,
                                             server_id => tcp_prompts_server,
                                             owner => self(),
                                             port => Port})
    of
        {ok, ServerPid} ->
            case erlmcp_transport_tcp:start_client(#{transport_id => tcp_prompts_client,
                                                     owner => self(),
                                                     host => "localhost",
                                                     port => Port})
            of
...
test_tcp_roots() ->
    ...
    case erlmcp_transport_tcp:start_server(#{transport_id => tcp_roots_server,
                                             server_id => tcp_roots_server,
                                             owner => self(),
                                             port => Port})
    of
        {ok, ServerPid} ->
            case erlmcp_transport_tcp:start_client(#{transport_id => tcp_roots_client,
                                                     owner => self(),
                                                     host => "localhost",
                                                     port => Port})
            of
...
print_result(#{workload_id := WorkloadId,
    ...
    lists:foreach(fun(#{capability := Cap,
                        status := Status,
                        latency_us := Lat}) ->
                     StatusStr =
                         case Status of
...
print_summary(Results) ->
    ...
    ct:pal("~-15s ~-18s ~-15s~n", ["Transport", "Avg Latency (μs)", "All Supported"]),
    ct:pal("~s~n", [lists:duplicate(50, $-)]),

    lists:foreach(fun(#{transport := Transport,
                        avg_negotiation_latency_us := AvgLat,
                        all_capabilities_supported := AllSupp}) ->
                     ct:pal("~-15s ~18w ~15w~n", [Transport, AvgLat, AllSupp])
                  end,
                  Results),
    ok.
