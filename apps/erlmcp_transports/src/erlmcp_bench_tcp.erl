...
run(#{workload_id := WorkloadId, message_size := MessageSize} = Config) ->
    ...
    ct:pal("~nRunning TCP benchmark: ~p (message size: ~p bytes)~n",
           [WorkloadId, MessageSize]),

    % Start TCP server
    Port = 19000 + rand:uniform(1000),
    {ok, ServerPid} = erlmcp_transport_tcp:start_server(#{
        transport_id => list_to_atom("tcp_bench_server_" ++ atom_to_list(WorkloadId)),
        server_id => list_to_atom("tcp_bench_server_" ++ atom_to_list(WorkloadId)),
        owner => self(),
        port => Port,
    ...
    {ok, ClientPid} = erlmcp_transport_tcp:start_client(#{
        transport_id => list_to_atom("tcp_bench_client_" ++ atom_to_list(WorkloadId)),
        owner => self(),
        host => "localhost",
        port => Port
    }),
...
print_summary(Results) ->
    ...
    Result4KB = lists:filter(fun(#{message_size_bytes := Size}) ->
        Size >= 4000 andalso Size =< 5000
    end, Results),
