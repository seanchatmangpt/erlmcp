...
%% Connection info
-record(conn_info, {session_id :: binary(), req :: term(), ping_timer :: reference()}).
...
init([TransportId, Config]) ->
    ...
    ?LOG_INFO("Starting SSE transport manager: ~p on port ~p path ~p", [TransportId, Port, Path]),

    % Start Cowboy listener
    Dispatch =
        cowboy_router:compile([{'_',
                                [{Path, erlmcp_transport_sse, [TransportId, Config]},
                                 {<<Path/binary, "/subscribe">>,
                                  erlmcp_transport_sse,
                                  [TransportId, Config]}]}]),

    case cowboy:start_clear(erlmcp_sse_listener, [{port, Port}], #{env => #{dispatch => Dispatch}})
    of
        {ok, ListenerRef} ->
...
handle_call({send_event, Data}, _From, State) ->
    ...
    lists:foreach(fun({SessionId, _ConnInfo}) ->
...
handle_cast({register_connection, SessionId, Req}, State) ->
    ...
    NewConnections =
        maps:put(SessionId,
                 #conn_info{session_id = SessionId,
                            req = Req,
