...
%% API
-export([start_link/0, build_tls_options/2, validate_tls_config/1, get_default_tls_opts/0,
         get_client_tls_opts/0, get_server_tls_opts/0, reload_config/0]).
...
-export_type([tls_role/0, tls_config/0, tls_options/0, tls_version/0, verify_mode/0,
              validation_error/0]).
...
-spec build_tls_options(tls_role(), tls_config()) ->
                           {ok, tls_options()} | {error, validation_error()}.
        ...
        ok ->
            try
                ...
                Type:Error:Stacktrace ->
                    ?LOG_ERROR("Failed to build TLS options: ~p:~p~n~p", [Type, Error, Stacktrace]),
...
validate_tls_config(Config) when is_map(Config) ->
    Required = [role],
    checks([fun() -> validate_required_keys(Config, Required) end,
...
handle_call(reload_config, _From, State) ->
    {reply,
     ok,
     State#state{cached_client_opts = undefined,
...
build_cert_options(Config) ->
    Opts = [],
    Opts1 =
        case maps:get(certfile, Config, undefined) of
            undefined ->
                Opts;
            CertFile ->
    ...
    Opts2 =
        case maps:get(keyfile, Config, undefined) of
            undefined ->
                Opts1;
            KeyFile ->
                [{keyfile, KeyFile} | Opts1]
        end,
    Opts3 =
        case maps:get(cacertfile, Config, undefined) of
            undefined ->
                Opts2;
            CaCertFile ->
                [{cacertfile, CaCertFile} | Opts2]
        end,
    Opts4 =
        case maps:get(dhfile, Config, undefined) of
            undefined ->
                Opts3;
            DHFile ->
                [{dhfile, DHFile} | Opts3]
    ...
    Opts5 =
        case maps:get(password, Config, undefined) of
            undefined ->
                Opts4;
            Password ->
                [{password, Password} | Opts4]
        end,
    lists:reverse(Opts5).
...
build_server_verify_options(Config) ->
    Opts = [],
    Opts1 =
        case maps:get(fail_if_no_peer_cert, Config, false) of
            false ->
                Opts;
            true ->
    ...
    Opts2 =
        case maps:get(verify_fun, Config, undefined) of
            undefined ->
                Opts1;
            VerifyFun ->
                [{verify_fun, VerifyFun} | Opts1]
        end,
    Opts3 =
        case maps:get(depth, Config, undefined) of
            undefined ->
                Opts2;
            Depth ->
                [{depth, Depth} | Opts2]
...
build_session_options(Config) ->
    Opts = [],
    Opts1 =
        case maps:get(session_tickets, Config, true) of
            true ->
                Opts;
            false ->
                [{session_tickets, disabled} | Opts]
        end,
    Opts2 =
        case maps:get(reuse_session, Config, true) of
            true ->
                [{reuse_sessions, true} | Opts1];
            false ->
                [{reuse_sessions, false} | Opts1]
        end,
    lists:reverse(Opts2).
...
merge_options(OptionLists) ->
    ...
    MergedMap =
        lists:foldl(fun(Opts, AccMap) ->
                       lists:foldl(fun ({K, _V} = Pair, Map) when is_atom(K) ->
                                           maps:put(K, Pair, Map);
...
validate_required_keys(Config, RequiredKeys) ->
    MissingKeys = lists:filter(fun(Key) -> not maps:is_key(Key, Config) end, RequiredKeys),
...
validate_versions(Config) ->
    case maps:get(versions, Config, undefined) of
        ...
        Versions when is_list(Versions) ->
            ...
            InvalidVersions =
                lists:filter(fun(V) -> not lists:member(V, ValidVersions) end, Versions),

            case InvalidVersions of
                [] ->
                    ok;
                _ ->
                    {error, {invalid_config, {invalid_versions, InvalidVersions}}}
            end;
        _ ->
...
validate_ciphers(Config) ->
    case maps:get(ciphers, Config, undefined) of
        ...
        Ciphers when is_list(Ciphers) ->
            ...
            InvalidCiphers =
                lists:filter(fun (Cipher) when is_binary(Cipher) ->
            ...
            case InvalidCiphers of
                [] ->
                    ok;
                _ ->
                    {error, {invalid_config, {invalid_ciphers, InvalidCiphers}}}
            end;
        _ ->
