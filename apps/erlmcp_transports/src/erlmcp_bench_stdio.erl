...
run_all() ->
    ...
    Workloads = [{stdio_1kb, 1024}, {stdio_10kb, 10240}, {stdio_100kb, 102400}],
...
run(#{workload_id := WorkloadId, message_size := MessageSize} = Config) ->
    ...
    ct:pal("~nRunning STDIO benchmark: ~p (message size: ~p bytes)~n", [WorkloadId, MessageSize]),

    % Start STDIO transport in test mode
    {ok, TransportPid} =
        erlmcp_transport_stdio:start_link(self(),
                                          #{transport_id =>
                                                list_to_atom("stdio_bench_"
    ...
    {MessageCount, LatencySamples} =
        benchmark_loop(TransportPid, EncodedMessage, StartTime, Duration * 1000, 0, []),
