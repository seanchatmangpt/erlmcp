...
run_all() ->
    ...
    Workloads = [{sse_1kb, 1024}, {sse_10kb, 10240}, {sse_100kb, 102400}],
...
run(#{workload_id := WorkloadId, message_size := MessageSize} = Config) ->
    ...
    ct:pal("~nRunning SSE benchmark: ~p (message size: ~p bytes)~n", [WorkloadId, MessageSize]),
    ...
    {EventCount, LatencySamples} =
        benchmark_loop(EncodedMessage, StartTime, Duration * 1000, 0, []),
...
print_summary(Results) ->
    ...
    ct:pal("~-15s ~-12s ~-12s ~-12s~n", ["Workload", "Throughput", "p50 (μs)", "p99 (μs)"]),
    ...
    lists:foreach(fun(#{workload_id := Id,
                        ...
                        latency_p50_us := P50,
                        latency_p99_us := P99}) ->
                     ct:pal("~-15s ~12w ~12w ~12w~n", [Id, Throughput, P50, P99])
                  end,
                  Results),
