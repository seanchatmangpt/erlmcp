...
test_analyze() ->
    {ok, Analysis} =
        erlmcp_memory_analyzer:analyze(#{top => 5,
                                         include_ets => true,
                                         include_binaries => true}),

    ?assertMatch(#{timestamp := _,
                   system_memory := _,
                   top_processes := _,
                   total_processes := _,
                   ets_tables := _,
...
test_ets_tables() ->
    ...
    TestTable = lists:filter(fun(#{name := Name}) -> Name == test_table end, AllTables),
...
test_heap_analysis() ->
    {ok, HeapAnalysis} = erlmcp_memory_analyzer:heap_analysis(#{threshold => 20.0}),

    ?assertMatch(#{threshold_pct := 20.0,
