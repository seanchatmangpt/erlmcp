...
test_trace_context() ->
    %% Create a span
    SpanCtx = erlmcp_otel:start_span(<<"test.operation">>, #{<<"test.attribute">> => <<"value">>}),

    %% Verify span context structure
    ?assertMatch(#{trace_id := _,
                   span_id := _,
                   baggage := _},
                 SpanCtx),
    ...
    ?assertMatch(#trace_ctx{trace_id = _, span_id = _}, TraceCtx),
...
test_middleware_transport() ->
    ...
    Result =
        erlmcp_otel_middleware:trace_transport(tcp,
                                               <<"send">>,
                                               fun() -> {ok, sent} end,
...
test_middleware_handler() ->
    ...
    Result =
        erlmcp_otel_middleware:trace_handler(Method, RequestId, fun() -> {ok, <<"result">>} end),
...
test_request_response_annotation() ->
    ...
    %% Annotate request
    Request = #{<<"method">> => <<"tools/call">>, <<"params">> => #{<<"tool_name">> => <<"test">>}},
...
test_e2e_trace() ->
    ...
    Result = erlmcp_otel_middleware:trace_handler(Method, RequestId, fun() -> {ok, 42} end),

    ?assertEqual({ok, 42}, Result),

    %% Client side: wrap response
    ok = erlmcp_otel_middleware:wrap_rpc_response(#{span_context => ClientSpan}, Result, RequestId),
