...
test_prometheus_format() ->
    ?_test(begin
               ...
               ?assertMatch({match, _},
                            re:run(PrometheusText, <<"erlmcp_requests_success_total 1">>)),
               ...
               ?assertMatch({match, _},
                            re:run(PrometheusText, <<"# TYPE erlmcp_requests_total counter">>)),
               ?assertMatch({match, _},
                            re:run(PrometheusText, <<"# TYPE erlmcp_connections_active gauge">>))
           end).
...
test_concurrent_increments() ->
    ?_test(begin
               ...
               Pids =
                   [spawn_link(fun() ->
                                  [erlmcp_counters:inc_requests()
                                   || _ <- lists:seq(1, IncrementsPerProcess)]
                               end)
