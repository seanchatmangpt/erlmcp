...
test_nested_spans() ->
    ...
    InnerCtx = erlmcp_otel:start_span(
        <<"inner.span">>,
        #{<<"parent">> => <<"outer">>},
        OuterCtx
    ),
    ...
    ?assertEqual(
        maps:get(trace_id, OuterCtx),
        maps:get(trace_id, InnerCtx)
    ),

    % Verify parent span ID is set
    ?assertEqual(
        maps:get(span_id, OuterCtx),
        maps:get(parent_span_id, InnerCtx)
    ),
...
test_add_events() ->
    ...
    ?assertEqual(ok, erlmcp_otel:add_event(SpanCtx, <<"test.event2">>, #{
...
test_context_propagation() ->
    SpanCtx = erlmcp_otel:start_span(<<"propagation.span">>, #{
...
test_restore_context() ->
    ...
    ?assertEqual(
        maps:get(trace_id, OriginalCtx),
        maps:get(trace_id, RestoredCtx)
    ),
...
with_span_multiple_errors_test() ->
    ...
    ?assertThrow(
        {test_error, reason},
        erlmcp_otel:with_span(<<"error.span">>, #{}, fun() ->
            erlang:error({test_error, reason})
        end)
    ),
     ...
     fun(_Config) ->
         [
             ?_test(begin
                 ...
                 RequestCtx = erlmcp_otel:start_span(
                     <<"mcp.request">>,
                     #{<<"request.id">> => <<"req-001">>}
                 ),
                 ...
                 % Simulate tool call
                 ToolCtx = erlmcp_otel:start_span(
                     <<"mcp.tool.call">>,
                     #{<<"tool.name">> => <<"test_tool">>},
                     RequestCtx
                 ),
    ...
    {setup,
     ...
     fun(_Config) ->
         [
             ?_test(begin
                 % Create RPC span
                 RpcCtx = erlmcp_otel:inject_rpc_span(
                     <<"tools.call">>,
                     <<"req-002">>,
                     #{<<"name">> => <<"echo">>, <<"arg">> => <<"hello">>}
                 ),
