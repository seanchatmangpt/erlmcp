...
%% API
-export([start_link/0, start_link/1, record_latency/3, record_throughput/3, record_memory/2,
         record_gc_pause/1, record_queue_depth/2, get_latency_percentiles/1,
         get_throughput_summary/0, get_memory_summary/0, get_all_metrics/0, reset_metrics/0,
         export_to_file/1]).
...
-record(throughput_sample,
        {role :: role(), timestamp :: integer(), msg_count :: non_neg_integer()}).
-record(memory_sample,
        {pid :: pid(), timestamp :: integer(), heap_mib :: float(), rss_mib :: float()}).
-record(gc_sample, {pid :: pid(), timestamp :: integer(), pause_us :: non_neg_integer()}).
-record(queue_sample, {pid :: pid(), timestamp :: integer(), depth :: non_neg_integer()}).
-record(state,
...
handle_call({get_latency_percentiles, Role}, _From, State) ->
    Samples =
        [S#latency_sample.duration_us
         || S <- State#state.latencies, S#latency_sample.role =:= Role],
...
handle_cast({record_latency, Role, Operation, DurationUs}, State) ->
    Sample =
        #latency_sample{role = Role,
                        operation = Operation,
                        timestamp = erlang:monotonic_time(millisecond),
                        duration_us = DurationUs},
    {noreply,
     State#state{latencies = [Sample | State#state.latencies],
                 sample_count = State#state.sample_count + 1}};
handle_cast({record_throughput, Role, MsgCount, Timestamp}, State) ->
    Sample =
        #throughput_sample{role = Role,
                           timestamp = Timestamp,
                           msg_count = MsgCount},
    {noreply,
     State#state{throughput = [Sample | State#state.throughput],
                 sample_count = State#state.sample_count + 1}};
handle_cast({record_memory, Pid, HeapMiB, RssMiB}, State) ->
    Sample =
        #memory_sample{pid = Pid,
                       timestamp = erlang:monotonic_time(millisecond),
                       heap_mib = HeapMiB,
                       rss_mib = RssMiB},
    {noreply,
     State#state{memory = [Sample | State#state.memory],
                 sample_count = State#state.sample_count + 1}};
handle_cast({record_gc_pause, Pid, PauseUs}, State) ->
    Sample =
        #gc_sample{pid = Pid,
                   ...
                   pause_us = PauseUs},
    {noreply,
     State#state{gc_pauses = [Sample | State#state.gc_pauses],
                 sample_count = State#state.sample_count + 1}};
handle_cast({record_queue_depth, Pid, Depth}, State) ->
    Sample =
        #queue_sample{pid = Pid,
                      timestamp = erlang:monotonic_time(millisecond),
                      depth = Depth},
    {noreply,
     State#state{queue_depths = [Sample | State#state.queue_depths],
                 sample_count = State#state.sample_count + 1}};
...
group_latencies_by_role(Latencies) ->
    Grouped =
        lists:foldl(fun(S, Acc) ->
                    ...
                    #{},
                    Latencies),
    maps:map(fun(_Role, Durations) -> calculate_percentiles(Durations) end, Grouped).
