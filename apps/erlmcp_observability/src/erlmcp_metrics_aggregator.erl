...
%% API
-export([start_link/0, record_metric/3, get_current_metrics/0, get_historical_metrics/2,
         get_percentiles/1, export_metrics/1]).
...
init([]) ->
    ...
    {ok,
     #state{current_bucket = #bucket{timestamp = erlang:system_time(millisecond)},
...
rotate_buckets(State) ->
    ...
    State#state{current_bucket = #bucket{timestamp = Now},
...
maybe_aggregate_5min(Now, Buckets60s, Buckets5min) ->
    case length(Buckets60s) >= 300  % 5 minutes = 300 seconds
    of
        true ->
            Aggregated = aggregate_buckets(lists:sublist(Buckets60s, 300)),
            TrimmedBuckets5min =
                lists:sublist([Aggregated#bucket{timestamp = Now} | Buckets5min],
                              ?MAX_BUCKETS_5MIN),
            TrimmedBuckets5min;
        false ->
            Buckets5min
...
maybe_aggregate_1hr(Now, Buckets5min, Buckets1hr) ->
    case length(Buckets5min) >= 12  % 1 hour = 12 x 5-minute buckets
    of
        true ->
            Aggregated = aggregate_buckets(lists:sublist(Buckets5min, 12)),
            TrimmedBuckets1hr =
                lists:sublist([Aggregated#bucket{timestamp = Now} | Buckets1hr], ?MAX_BUCKETS_1HR),
            TrimmedBuckets1hr;
        false ->
            Buckets1hr
    end.
...
aggregate_buckets(Buckets) ->
    ...
    #bucket{timestamp = erlang:system_time(millisecond),
...
get_buckets_in_range(State, StartTime, EndTime) ->
    ...
    FilteredBuckets =
        lists:filter(fun(B) -> B#bucket.timestamp >= StartTime andalso B#bucket.timestamp =< EndTime
                     end,
                     AllBuckets),
...
export_metrics_internal(State, json) ->
    AllBuckets =
        [State#state.current_bucket | State#state.buckets_60s]
...
check_alerts(State) ->
    ...
    case maps:get(latency_p99_us, Metrics, 0) > maps:get(latency_p99_ms, Thresholds, 100) * 1000 of
        true ->
            ?LOG_WARNING("ALERT: Latency p99 exceeded threshold: ~p us",
                         [maps:get(latency_p99_us, Metrics)]);
        false ->
            ok
    end,
    ...
    case maps:get(memory_mb, Metrics, 0) > maps:get(memory_mb, Thresholds, 1024) of
        true ->
            ?LOG_WARNING("ALERT: Memory usage exceeded threshold: ~p MB",
