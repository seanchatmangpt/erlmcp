...
%% Public API
-export([init/1, start_span/2, start_span/3, end_span/1, with_span/3, with_span/4, record_error/2,
...
%% Internal utilities
-export([create_span_name/2, extract_error_attributes/1, get_current_context/0, make_trace_id/0,
         make_span_id/0]).
...
-type otel_config() ::
    #{service_name => binary(),
      ...
      sampling_rate => float(),
      tail_sampling_latency_threshold_us => pos_integer(),
      tail_sampling_error_rate => float(),
  ...
  % Error rate threshold for tail sampling
...
propagate_context(#{trace_id := TraceId,
                    span_id := SpanId,
                    trace_flags := Flags,
                    baggage := Baggage}) ->
        ...
        TraceParent ->
            case parse_traceparent(TraceParent) of
                {ok, TraceId, ParentSpanId, Flags} ->
                    Baggage =
                        case maps:get(<<"baggage">>, Headers, undefined) of
                            undefined ->
                                #{};
                            BaggageHeader ->
...
setup_exporters(Config) ->
    ...
    lists:foreach(fun(Exporter) -> ok = configure_exporter(Exporter, Config) end, Exporters),
    ok.
...
parse_traceparent(TraceParent) ->
    case binary:split(TraceParent, <<"-">>, [global]) of
        [<<"00">>, TraceId, SpanId, Flags]
            when byte_size(TraceId) == 32, byte_size(SpanId) == 16, byte_size(Flags) == 2 ->
            try
...
setup_otlp_exporter(Config) ->
    ...
    HeaderList =
        maps:fold(fun(K, V, Acc) -> [{binary_to_list(K), binary_to_list(V)} | Acc] end,
                  [],
                  Headers),
...
create_trace_ctx(#{trace_id := TraceId,
                   span_id := SpanId,
                   parent_span_id := ParentSpanId,
                   baggage := Baggage}) ->
    ...
    #trace_ctx{trace_id = TraceId,
               span_id = SpanId,
               parent_span_id = ParentSpanId,
               baggage = AtomBaggage};
create_trace_ctx(_) ->
    #trace_ctx{trace_id = make_trace_id(),
...
restore_trace_ctx(#trace_ctx{trace_id = TraceId,
                             span_id = SpanId,
                             parent_span_id = ParentSpanId,
                             baggage = Baggage}) ->
...
tail_sample_decision(#{start_time := StartTime,
                       status := Status,
                       attributes := Attributes}) ->
    EndTime = erlang:system_time(nanosecond),
    Duration = EndTime - StartTime,
    ...
    LatencyThreshold =
        maps:get(tail_sampling_latency_threshold_us, Config, 100000),  % 100ms default
...
format_attribute_value(V) when is_binary(V) ->
    V;
format_attribute_value(V) when is_integer(V) ->
    V;
format_attribute_value(V) when is_float(V) ->
    V;
format_attribute_value(V) when is_boolean(V) ->
    V;
format_attribute_value(V) when is_atom(V) ->
    atom_to_binary(V, utf8);
