...
-export([start_link/0,
         start_link/1,
         validate_benchmark_output/1,
         validate_metric_units/1,
         validate_required_fields/1,
         detect_ambiguous_units/1,
         validate_scope/1,
         generate_violations_report/1,
...
-type violation() :: #{type := binary(), field := binary(), reason => binary()}.
...
validate_benchmark_output(Output) when is_map(Output) ->
    Validations = [
        ...
        fun validate_metric_units/1,
        fun validate_scope/1
    ],
...
validate_required_fields(Output) when is_map(Output) ->
    ...
    Missing = lists:filtermap(fun({Field, Desc}) ->
        case maps:get(Field, Output, undefined) of
            undefined -> {true, {Field, Desc}};
            _ -> false
        end
    end, RequiredFields),
    ...
    case Missing of
        ...
        _ ->
            ErrorMsgs = [io_lib:format("Missing required field '~s': ~s", [Field, Desc])
                         || {Field, Desc} <- Missing],
...
detect_ambiguous_units(Output) when is_map(Output) ->
    AmbiguousPatterns = [
        ...
        {<<"throughput">>, <<"throughput_msg_per_s">>, <<"Throughput metric must specify msg_per_s unit">>},
        {<<"latency">>, <<"latency_p50_us">>, <<"Latency metric must specify percentile and _us unit">>},
        {<<"latency_ms">>, <<"latency_p50_us">>, <<"Latency must use microseconds (_us), not milliseconds (_ms)">>},
        ...
        {<<"memory">>, <<"memory_heap_mib_per_conn">>, <<"Memory metric must specify scope (heap/rss) and unit (mib/gib)">>},
        {<<"mem">>, <<"memory_heap_mib_per_conn">>, <<"Memory metric must specify scope and unit">>}
    ],
    ...
    lists:filtermap(fun(Key) ->
        case maps:get(Key, Output, undefined) of
            undefined ->
                false;
            _Value ->
                {Ambiguous, Canonical, Reason} = lists:keyfind(Key, 1, AmbiguousPatterns),
                Violation = #{
                    type => <<"ambiguous_unit">>,
                    field => Ambiguous,
                    expected => Canonical,
    ...
    end, [Key || {Key, _, _} <- AmbiguousPatterns]).
        ...
        Scope when is_binary(Scope) ->
            case lists:member(Scope, ValidScopes) of
                true ->
                    {ok, #{scope => valid}};
                false ->
                    ErrorMsg = io_lib:format("Invalid scope '~s'. Valid scopes: ~p",
                                           [Scope, ValidScopes]),
...
generate_violations_report(Output) when is_map(Output) ->
    ...
    RequiredViolations = case validate_required_fields(Output) of
        {error, Msgs} ->
    ...
    ScopeViolations = case validate_scope(Output) of
        {error, ScopeMsgs} ->
