...
%% API
-export([analyze/0, analyze/1, top_processes/1, ets_tables/0, ets_tables/1, detect_leaks/0,
         heap_analysis/0, heap_analysis/1, memory_trends/1]).
...
analyze(Opts) ->
    ...
    Result2 =
        case IncludeEts of
            true ->
                Result#{ets_tables => ets_tables(Top)};
            false ->
                Result
        end,

    Result3 =
        case IncludeBinaries of
            true ->
                Result2#{binary_leaks => detect_leaks()};
            false ->
                Result2
        end,
...
top_processes(N) ->
    Processes =
        [{Pid,
          process_info(Pid,
                       [memory, message_queue_len, reductions, registered_name, current_function])}
...
ets_tables(N) ->
                                   ...
                                   _ ->
                                       {true,
                                        #{table => Tab,
                                          ...
                                          memory_bytes =>
                                              proplists:get_value(memory, Info, 0)
    ...
    Sorted = lists:sort(fun(#{memory_bytes := A}, #{memory_bytes := B}) -> A >= B end, InfoList),
...
detect_leaks() ->
                           ...
                           case process_info(Pid, [message_queue_len, registered_name]) of
                               ...
                               Info ->
                                   ...
                                   case QLen > 1000 of
                                       true ->
                                           RegName =
                                               proplists:get_value(registered_name,
                                                                   Info,
                                                                   undefined),
                                           {true,
                                            #{pid => Pid,
    ...
    LargeTables = lists:filter(fun(#{size := Size}) -> Size > 10000 end, ets_tables(all)),
...
heap_analysis(Opts) ->
    ...
    Sorted =
        lists:sort(fun(#{fragmentation_pct := A}, #{fragmentation_pct := B}) -> A >= B end,
                   Fragmented),
...
queue_severity(QLen) when QLen > 100000 ->
    critical;
queue_severity(QLen) when QLen > 10000 ->
    high;
queue_severity(QLen) when QLen > 1000 ->
    medium;
queue_severity(_) ->
    low.
