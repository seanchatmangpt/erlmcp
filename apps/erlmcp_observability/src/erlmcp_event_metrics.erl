...
handle_event({tool_executed, ToolName, Duration, _Result}, State) ->
    ...
    telemetry:execute([erlmcp, event, tool_executed],
...
handle_event({resource_updated, Uri, _Metadata}, State) ->
    %% Emit telemetry event
    telemetry:execute([erlmcp, event, resource_updated], #{count => 1}, #{uri => Uri}),
...
handle_event({connection_state, ConnectionState, Info}, State) ->
    %% Emit telemetry event
    telemetry:execute([erlmcp, event, connection_state],
                      #{count => 1},
...
handle_event({error, Category, Reason}, State) ->
    %% Emit telemetry event
    telemetry:execute([erlmcp, event, error],
                      #{count => 1},
...
handle_event({request_received, Method, _RequestId}, State) ->
    %% Emit telemetry event
    telemetry:execute([erlmcp, event, request_received], #{count => 1}, #{method => Method}),
...
handle_event({response_sent, Method, _RequestId, Duration}, State) ->
    %% Emit telemetry event
    telemetry:execute([erlmcp, event, response_sent],
                      #{duration => Duration, count => 1},
                      #{method => Method}),
...
handle_event({notification_sent, Method, _Params}, State) ->
    %% Emit telemetry event
    telemetry:execute([erlmcp, event, notification_sent], #{count => 1}, #{method => Method}),
...
handle_event({session_created, SessionId, Metadata}, State) ->
    %% Emit telemetry event
    telemetry:execute([erlmcp, event, session_created],
                      #{count => 1},
...
handle_event({session_terminated, SessionId, Reason}, State) ->
    %% Emit telemetry event
    telemetry:execute([erlmcp, event, session_terminated],
                      #{count => 1},
...
handle_call(reset_metrics, State) ->
    NewState = #state{start_time = erlang:system_time(millisecond)},
    {ok, ok, NewState};
