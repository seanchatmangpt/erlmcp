...
-export([attach/1, detach/1, inspect_state/1, list_attached/0]).
%% API - Message Tracing
-export([trace_calls/3, trace_messages/2, stop_trace/1]).
%% API - Breakpoints
-export([breakpoint/3, breakpoint/4, clear_breakpoint/2, list_breakpoints/0]).
%% API - Call Graph
-export([call_graph/2, visualize_call_graph/2]).
%% Internal exports (for testing)
-export([process_info_to_map/1, call_graph_to_dot/1, timestamp/0]).
...
attach(Pid) when is_pid(Pid) ->
    case process_info(Pid, [registered_name, current_function, initial_call]) of
        ...
        Info ->
            ...
            ?LOG_INFO("Debugger attached to ~p (~p)",
                      [Pid, proplists:get_value(registered_name, Info)]),
            {ok, Pid}
    end.
...
get_attached_state(Pid) ->
    case persistent_term:get({?MODULE, attached}, undefined) of
        undefined ->
            undefined;
        Attached ->
            maps:get(Pid, Attached, undefined)
    end.
...
message_tracer(Ref, TargetPid, Acc) ->
    receive
        {trace, TargetPid, 'receive', Msg} ->
            Entry = #{timestamp => erlang:system_time(microsecond), message => Msg},
...
call_graph_to_dot(ProcInfo) ->
    ...
    Name =
        case maps:get(registered_name, ProcInfo, undefined) of
            undefined ->
                pid_to_list(Pid);
    ...
    CurrentFunc =
        case maps:get(current_function, ProcInfo, undefined) of
            {Mod, Func, Arity} ->
                io_lib:format("~s:~s/~p", [Mod, Func, Arity]);
            _ ->
                "unknown"
        end,
...
timestamp() ->
    ...
    io_lib:format("~4..0B-~2..0B-~2..0B ~2..0B:~2..0B:~2..0B UTC", [Y, Month, D, H, Min, S]).
