...
%% API
-export([start_link/0, start_link/1, register_component/3, unregister_component/1,
         trigger_recovery/2, trigger_recovery/3, get_recovery_status/1, get_all_recovery_status/0,
         reset_metrics/0, get_metrics/0, get_circuit_status/0, get_circuit_status/1,
...
init(Opts) ->
    ...
    State =
        #state{default_policy = DefaultPolicy,
...
handle_call({register_component, ComponentId, Pid, Policy}, _From, State) ->
    ...
    NewState = State#state{components = NewComponents, metrics = NewMetrics},
...
handle_call(get_circuit_status, _From, State) ->
    CircuitStatus =
        maps:map(fun(_Id, Component) -> Component#component.circuit_state end,
                 State#state.components),
    ...
    case maps:find(ComponentId, State#state.restart_cooldowns) of
            ...
            case TimeSinceLastRestart < ?RESTART_COOLDOWN of
                ...
                false ->
                    ...
                    case do_restart_component(SupPid, ComponentId) of
                        {ok, NewPid} ->
                            NewCooldowns =
                                maps:put(ComponentId,
                                         erlang:timestamp(),
                                         State#state.restart_cooldowns),
                            ...
                            {reply,
                             {ok, NewPid},
                             State#state{restart_cooldowns = NewCooldowns, metrics = NewMetrics2}};
                        {error, Reason} ->
        ...
        error ->
            ...
            case do_restart_component(SupPid, ComponentId) of
                {ok, NewPid} ->
                    NewCooldowns =
                        maps:put(ComponentId, erlang:timestamp(), State#state.restart_cooldowns),
                    NewMetrics = update_metric(total_recoveries, 1, State#state.metrics),
                    NewMetrics2 = update_metric(successful_recoveries, 1, NewMetrics),
                    {reply,
                     {ok, NewPid},
...
handle_component_failure(ComponentId, Reason, Options, State) ->
    case maps:find(ComponentId, State#state.components) of
        {ok, Component} ->
            ...
            UpdatedComponent =
                Component#component{failures = NewFailures, last_failure = erlang:timestamp()},
            ...
            State#state{components = NewComponents, metrics = NewMetrics};
        error ->
...
execute_recovery_action({restart, Options}, Component, Reason) ->
    ...
    case attempt_component_restart(Component, Reason, Options) of
        {ok, NewPid} ->
            ...
            NewCircuitState =
                case maps:get(circuit_state, Options, closed) of
                    half_open ->
                        closed; % Successful recovery from half-open closes circuit
                    _ ->
                        Component#component.circuit_state
                end,

            Component#component{pid = NewPid,
        ...
        {error, RecoveryReason} ->
            ...
            Component#component{recovery_attempts = Component#component.recovery_attempts + 1,
                                recovery_start = RecoveryStart,
...
-spec attempt_component_restart(#component{}, term(), map()) -> {ok, pid()} | {error, term()}.
        ...
        {ok, ParentPid} ->
            case is_supervisor(ParentPid) of
                true ->
                    {ok, ParentPid};
                false ->
                    find_supervisor_for_pid(ParentPid) % Walk up the tree
            end;
        {error, Reason} ->
            {error, Reason}
    end.
...
find_matching_supervisor([{SupId, SupPid, supervisor, _} | _Rest], _ChildPid) when is_pid(SupPid) ->
    ...
    case is_child_under_supervisor(SupPid, _ChildPid) of
        true ->
            {ok, SupPid};
        false ->
            {error, not_under_this_supervisor}
    end;
    ...
    try
        ...
        case proplists:get_value("$initial_call", Dict) of
            {Module, _, _} ->
                ...
                lists:keymember(init, 1, Exports)
                andalso lists:keymember(handle_call, 1, Exports) =:= false;
            _ ->
                false
        end
    catch
...
find_component_by_pid(Pid, Components) ->
    case maps:to_list(
             maps:filter(fun(_Id, Component) -> Component#component.pid =:= Pid end, Components))
    of
...
check_and_update_recoveries(State) ->
    UpdatedComponents =
        maps:map(fun(ComponentId, Component) -> check_component_recovery(ComponentId, Component)
                 end,
                 State#state.components),
    ...
    case Component#component.circuit_state of
        ...
        half_open ->
            ...
            case is_component_healthy(Component) of
                ...
                false ->
                    ?LOG_WARNING("Component ~p failed health check, opening circuit breaker",
                                 [ComponentId]),
                    Component#component{circuit_state = open}
            end;
        closed ->
            ...
            {ok, ChildSpec} ->
                ...
                case RestartType of
                    ...
                    permanent ->
                        ...
                        ?LOG_INFO("Terminating permanent component ~p for auto-restart",
                                  [ComponentId]),
                        case supervisor:terminate_child(SupPid, ComponentId) of
                            ok ->
                                    ...
                                    Children ->
                                        case lists:keyfind(ComponentId, 1, Children) of
                                            ...
                                            {_, undefined, _, _} ->
                                                ...
                                                case supervisor:restart_child(SupPid, ComponentId)
                                                of
                                                    {ok, Pid} ->
                                                        {ok, Pid};
                                                    {error, Reason} ->
