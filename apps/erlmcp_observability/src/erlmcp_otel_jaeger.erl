...
export_spans(Spans, #{queue := Queue, config := Config} = State) ->
    %% Add spans to queue
    NewQueue = lists:foldl(fun(Span, Q) -> queue:in(Span, Q) end, Queue, Spans),
...
send_batch(Spans,
    ...
    AllHeaders =
        [{"Content-Type", ContentType}, {"Content-Length", integer_to_list(byte_size(Payload))}
         | Headers],

    %% Send HTTP POST
    case httpc:request(post, {binary_to_list(Endpoint), AllHeaders, ContentType, Payload}, [], [])
    of
        {ok, {{_, 200, _}, _, _}} ->
            ok;
        {ok, {{_, Code, _}, _, Body}} ->
...
format_attributes(Attributes) ->
    maps:fold(fun(Key, Value, Acc) ->
                 [#{<<"key">> => Key, <<"value">> => format_attribute_value(Value)} | Acc]
              end,
              [],
              Attributes).
