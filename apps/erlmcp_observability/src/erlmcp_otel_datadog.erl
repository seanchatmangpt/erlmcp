...
%% Internal API
-export([format_span/1, add_datadog_tags/2, encode_batch/1]).
...
export_spans(Spans, #{queue := Queue, config := Config} = State) ->
    ...
    %% Add to queue
    NewQueue = lists:foldl(fun(Span, Q) -> queue:in(Span, Q) end, Queue, TaggedSpans),
...
add_datadog_tags(#{attributes := Attributes} = Span, Config) ->
    ...
    AllTags =
        maps:merge(
            maps:merge(Attributes, DDTags), CustomTags),
...
encode_batch(Spans) ->
    ...
    Batch =
        #{<<"resourceSpans">> =>
              [#{<<"resource">> => #{<<"attributes">> => []},
                 <<"scopeSpans">> =>
                     [#{<<"scope">> => #{<<"name">> => <<"erlmcp">>, <<"version">> => <<"2.0.0">>},
...
map_to_datadog_key(<<"span.kind">>) ->
    <<"span.kind">>;
map_to_datadog_key(<<"service.name">>) ->
    <<"service">>;
map_to_datadog_key(<<"service.version">>) ->
    <<"version">>;
map_to_datadog_key(Key) ->
    Key.
...
send_batch(Spans, #{endpoint := Endpoint} = Config) ->
    ...
    Headers =
        case maps:get(api_key, Config, undefined) of
            undefined ->
                [];
            ApiKey ->
                [{"DD-API-KEY", binary_to_list(ApiKey)}]
        end,
    ...
    case httpc:request(post,
                       {binary_to_list(Endpoint), AllHeaders, "application/json", Payload},
                       [],
                       [])
    of
        {ok, {{_, 200, _}, _, _}} ->
