...
-record(state,
        {enabled = true :: boolean(),
         log_all_events = false ::
             boolean(),  % If true, log all events; if false, only critical ones
...
handle_event({request_received, Method, RequestId},
             #state{enabled = true, log_all_events = true} = State) ->
...
handle_event({response_sent, Method, RequestId, Duration},
             #state{enabled = true, log_all_events = true} = State) ->
...
handle_event({notification_sent, Method, Params},
             #state{enabled = true, log_all_events = true} = State) ->
...
format_reason(Reason) when is_binary(Reason) ->
    case byte_size(Reason) > 1000 of
        true ->
            <<(binary_part(Reason, 0, 1000))/binary, "...">>;
        false ->
            Reason
    end;
