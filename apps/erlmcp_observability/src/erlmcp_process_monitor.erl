...
%% API
-export([start_link/0, start_link/1, get_process_metrics/0, get_capacity_estimate/0,
         check_process_limit/0, set_alert_thresholds/2, get_alert_thresholds/0,
         enable_auto_scaling/0, disable_auto_scaling/0, enumerate_processes/0,
...
-record(state,
        {check_interval :: pos_integer(),
         ...
         metrics_history :: [process_metrics()]}).
...
set_alert_thresholds(Warning, Critical)
    when is_float(Warning), is_float(Critical), Warning > 0, Warning < 1.0, Critical > 0,
         Critical < 1.0, Critical > Warning ->
...
init(Opts) ->
    ...
    CapacityConfig =
        #{per_connection_overhead_bytes =>
          ...
          target_memory_bytes => proplists:get_value(target_memory, Opts, ?TARGET_MEMORY),
          safety_margin_percent => proplists:get_value(safety_margin, Opts, ?SAFETY_MARGIN)},
...
handle_call({set_alert_thresholds, Warning, Critical}, _From, State) ->
    ?LOG_INFO("Updating alert thresholds: warning=~.2f, critical=~.2f", [Warning, Critical]),
...
-spec generate_recommendations(#state{}, non_neg_integer(), non_neg_integer()) -> [binary()].
generate_recommendations(State, ProcessCount, ProcessLimit) ->
    ...
    Recs1 =
        case UsagePercent >= WarningThreshold of
            true ->
    ...
    Recs2 =
        case State#state.auto_scaling_enabled of
    ...
    Recs3 =
        case ProcessCount >= CapacityLimit of
            true ->
...
perform_metrics_check(State) ->
    ...
    case Status of
        critical ->
            AlertMsg =
                io_lib:format("CRITICAL: Process usage at ~.2f% (~p/~p processes). "
            ...
            case whereis(erlmcp_recovery_manager) of
                undefined ->
                    ok;
                _Pid ->
                    erlmcp_recovery_manager:trigger_recovery(process_limit_critical,
        ...
        warning ->
            AlertMsg =
                io_lib:format("WARNING: Process usage at ~.2f% (~p/~p processes). "
