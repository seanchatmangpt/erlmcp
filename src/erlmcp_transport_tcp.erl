...
internal_init(TransportId, Config) ->
    ...
    case validate_config(Config) of
        ok ->
            ...
            %% Register with registry (if not in test mode)
            case maps:get(test_mode, Config, false) of
                ...
                false ->
                    case register_with_registry(State) of
                        ok ->
                            ?LOG_INFO("TCP transport ~p initialized and registered", [TransportId]),
                            {ok, State};
                        {error, Reason} ->
                            ?LOG_ERROR("Failed to register TCP transport ~p: ~p", 
                                      [TransportId, Reason]),
                            {error, {registry_registration_failed, Reason}}
                    end
            end;
        {error, Reason} ->
