...
-type refusal_reason() :: queue_overflow | rate_limit_exceeded | circuit_breaker_open
                        | concurrent_limit | throughput_limit | latency_p99_violated
...
%% @end
-spec get_receipt(ReceiptId :: receipt_id()) ->
    {ok, Receipt :: receipt()} | {error, term()}.
...
%% @end
-spec list_receipts(PlanId :: plan_id()) ->
    {ok, [Receipt :: receipt()]} | {error, term()}.
...
%% @end
-spec add_refusal(ReceiptId :: receipt_id(), Code :: refusal_code(), Reason :: refusal_reason(), AttemptedAction :: binary()) ->
    {ok, Receipt :: receipt()} | {error, term()}.
add_refusal(ReceiptId, Code, Reason, AttemptedAction)
  when is_binary(ReceiptId), is_integer(Code), Code >= 1001, Code =< 1089, is_atom(Reason), is_binary(AttemptedAction) ->
    try
        % Load existing receipt
        {ok, Receipt} = get_receipt(ReceiptId),

        % Verify current hash matches stored (detect tampering)
        CurrentHash = maps:get(current_hash, maps:get(hash_chain, Receipt)),
        ComputedHash = compute_hash(Receipt#{hash_chain => #{previous_receipt_hash => null, current_hash => <<"verify">>}}),
        ...
        UpdatedReceipt = Receipt#{
            refusal_trigger => RefusalEvent
        },

        % Recompute hash with refusal added
        NewHash = compute_hash(UpdatedReceipt#{
            hash_chain => #{
...
%% @end
-spec verify_receipt_chain(PlanId :: plan_id()) ->
...
%% @end
-spec verify_receipt(ReceiptId :: receipt_id()) ->
...
verify_receipt(ReceiptId) when is_binary(ReceiptId) ->
    try
        ...
        ComputedHash = compute_hash(Receipt#{
            hash_chain => #{
...
verify_conformance(ReceiptId, Metrics) when is_binary(ReceiptId), is_map(Metrics) ->
    try
        ...
        % Check each metric
        ThroughputOk = maps:get(throughput_actual, Metrics, 0) =< maps:get(throughput_req_s, Envelope),
        ...
        Status = case {ThroughputOk, ConcurrentOk, QueueOk, LatencyOk, FailoverOk} of
            {true, true, true, true, true} -> pass;
            _ -> fail
        end,
...
-spec export_receipts(PlanId :: plan_id(), Version :: binary(), Format :: format_type()) ->
    {ok, iodata()} | {error, term()}.
export_receipts(PlanId, Version, Format) when is_atom(PlanId), is_binary(Version), is_atom(Format) ->
    try
...
receipt_filename(Timestamp) ->
    ...
    [Y1, Y2, Y3, Y4, $-, M1, M2, $-, D1, D2, $T, H1, H2, $:, Mi1, Mi2, $:, S1, S2 | _] = binary_to_list(Timestamp),
    DateStr = [Y1, Y2, Y3, Y4, M1, M2, D1, D2],
    TimeStr = [H1, H2, Mi1, Mi2, S1, S2],
    lists:flatten(io_lib:format("~s-~s.receipt.json", [DateStr, TimeStr])).
...
find_violations(Envelope, Metrics) ->
    ...
    Violations1 = case maps:get(throughput_actual, Metrics, 0) > maps:get(throughput_req_s, Envelope) of
        true -> [throughput_exceeded | Violations];
        false -> Violations
    end,

    Violations2 = case maps:get(concurrent_actual, Metrics, 0) > maps:get(concurrent, Envelope) of
        true -> [concurrent_exceeded | Violations1];
        false -> Violations1
    end,

    Violations3 = case maps:get(queue_depth_actual, Metrics, 0) > maps:get(queue_depth, Envelope) of
        true -> [queue_depth_exceeded | Violations2];
        false -> Violations2
    end,

    Violations4 = case maps:get(latency_p99_actual, Metrics, 0) > maps:get(latency_p99_ms, Envelope) of
        true -> [latency_exceeded | Violations3];
        false -> Violations3
    end,

    case maps:get(failover_actual, Metrics, 0) > maps:get(failover_s, Envelope) of
        true -> [failover_exceeded | Violations4];
        false -> Violations4
    end.
...
receipt_to_csv(Receipt) ->
    Headers = "receipt_id,plan_id,version,timestamp,throughput_req_s,concurrent,queue_depth,latency_p99_ms,failover_s,current_hash\n",
    ...
    Row = io_lib:format("~s,~w,~s,~s,~f,~w,~w,~f,~f,~s\n",
        [ReceiptId, PlanId, Version, Timestamp,
