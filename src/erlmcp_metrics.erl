...
record_transport_operation(TransportId, TransportType, Operation, Duration) ->
    ...
    gen_server:cast(?MODULE, {record_metric, <<"transport_operation_duration_ms">>, Duration, Labels}).
...
handle_call({get_metrics, MetricName}, _From, #state{metrics = Metrics} = State) ->
    Filtered = lists:filter(fun(#metric{name = Name}) ->
        Name =:= MetricName
    end, Metrics),
...
record_metric_internal(Name, Value, Labels, #state{metrics = Metrics, counters = Counters, 
                                                   histograms = Histograms, gauges = Gauges} = State) ->
...
calculate_performance_summary(#state{metrics = Metrics, counters = Counters, 
    ...
    try
        case erlang:statistics(scheduler_wall_time) of
            ...
            SchedulerTimes ->
                ...
                case TotalTotal of
                    0 -> #{<<"utilization_percent">> => 0};
                    _ -> #{<<"utilization_percent">> => (TotalActive / TotalTotal) * 100}
                end
        end
    catch
        _:_ -> #{}
