...
%% Define records locally since escript can't find include_lib
...
-record(mcp_client_capabilities, {
    ...
    experimental :: #mcp_capability{} | undefined
}).
...
main(_) ->
    ...
    %% Spawn clients
    AllClients = lists:flatmap(fun({Port, _ServerPid, ServerId}) ->
        lists:map(fun(N) ->
    ...
    Results = lists:map(fun({Port, ClientPid, ClientId}) ->
    ...
    lists:foreach(fun({Port, ServerPid, _ServerId}) ->
        %% Stop server
