...
prop_cache_ttl_zero_expires() ->
    ?FORALL({Key, Value}, {cache_key(), cache_value()},
        begin
            {ok, _Cache} = erlmcp_test_helpers:start_test_cache(#{max_l1_size => 100, default_ttl_seconds => 1}),
...
prop_cache_ttl_positive_persists() ->
    ?FORALL({Key, Value}, {cache_key(), cache_value()},
        begin
            {ok, _Cache} = erlmcp_test_helpers:start_test_cache(#{max_l1_size => 100, default_ttl_seconds => 10}),
