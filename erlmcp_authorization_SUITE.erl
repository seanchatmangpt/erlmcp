...
admin_role_full_access_test(Config) ->
    ...
    %% Admin should have all permissions on admin_resource
    ?assertEqual(ok, erlmcp_auth:check_permission(AdminSession, <<"admin_resource">>, <<"read">>)),
    ?assertEqual(ok, erlmcp_auth:check_permission(AdminSession, <<"admin_resource">>, <<"write">>)),
    ?assertEqual(ok, erlmcp_auth:check_permission(AdminSession, <<"admin_resource">>, <<"execute">>)),
    ?assertEqual(ok, erlmcp_auth:check_permission(AdminSession, <<"admin_resource">>, <<"delete">>)),
...
expired_session_authorization_test(Config) ->
    ...
    ?assertEqual(ok, erlmcp_auth:check_permission(SessionId, <<"admin_resource">>, <<"read">>)),

    %% Destroy the session (simulating expiration)
    erlmcp_auth:destroy_session(SessionId),

    %% Verify expired session is rejected
    ?assertEqual({error, invalid_session}, erlmcp_auth:check_permission(SessionId, <<"admin_resource">>, <<"read">>)),
    ?assertEqual({error, invalid_session}, erlmcp_auth:check_permission(SessionId, <<"admin_resource">>, <<"write">>)),
