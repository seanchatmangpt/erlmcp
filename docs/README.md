...
```mermaid
graph TD
    A[Who are you?] --> B[Developer/Integrator]
    A --> C[Architect/CTO]
    A --> D[Contributor/Maintainer]

    B --> E[Read Concepts → Follow How-to]
    C --> F[Review Architecture → Check Reference]
    D --> G[Examples → Contributing Guidelines]
